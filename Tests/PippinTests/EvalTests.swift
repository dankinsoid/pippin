	...
	@Suite struct EvalTests {
		@Test func specialForms() throws {
			...
			do {
				...
				#expect(evalError("(nope 1)") == "#error {:message \"Unable to resolve symbol: nope in this context\", :data {:column 1, :line 1}}")
				...
				#expect(evalError("(loop [x 1] (+ 1 (recur 2)))") == "#error {:message \"Can only recur from tail position\", :data {:column 18, :line 1}}")
				#expect(evalError("(recur 1)") == "#error {:message \"Can only recur from tail position\", :data {:column 1, :line 1}}")
				#expect(evalError("(loop [x 1] (recur 1 2))") == "#error {:message \"Mismatched argument count to recur, expected: 1 args, got: 2\", :data {:column 13, :line 1}}")
				#expect(evalError("(let [x] x)") == "#error {:message \"let requires an even number of forms in binding vector\", :data {:column 1, :line 1}, :cause #error {:message \"let requires an even number of forms in binding vector\", :data nil}}")
				...
				#expect(evalError("(def ev-x 1 2 3)") == "#error {:message \"Too many arguments to def\", :data {:column 1, :line 1}}")
				#expect(evalError("(let [x 1 y (nope)] x)") == "#error {:message \"Unable to resolve symbol: nope in this context\", :data {:column 13, :line 1}}")
				#expect(evalError("(if 1 (nope) 2)") == "#error {:message \"Unable to resolve symbol: nope in this context\", :data {:column 7, :line 1}}")
				#expect(evalError("(def ev-f (fn [x] (nope)))") == "#error {:message \"Unable to resolve symbol: nope in this context\", :data {:column 19, :line 1}}")

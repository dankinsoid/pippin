	...
	@Suite struct BuiltinsTests {
		@Test func arithmetic() throws {
			...
			do {
				...
				#expect(try eval("(pr-str (/ 1 2))") == "1/2")
				#expect(try eval("(pr-str (/ 2))") == "1/2")
		...
		@Test func collections() throws {
			...
			do {
				...
				#expect(try eval("[(list? (rest [1 2])) (seq? (rest [1 2])) (list? (rest '(1 2)))]") == [false, true, true])
				...
				#expect(message("(assoc 1 :a 2)") == "assoc not supported on this type: long")
				#expect(message("(conj {} 1)") == "Vector arg to map conj must be a pair")
				#expect(message("(conj 1 2)") == "conj not supported on this type: long")
				#expect(message("(count 1)") == "count not supported on this type: long")
				#expect(message("(first 1)") == "Don't know how to create ISeq from: long")
				#expect(try eval("(rest {})") == Value(list: []))
				#expect(message("(rest 1)") == "Don't know how to create ISeq from: long")
				#expect(message("(cons 1 2)") == "Don't know how to create ISeq from: long")
				#expect(message("(hash-map :a)") == "No value supplied for key: :a")
				#expect(message("(apply + 1)") == "Don't know how to create ISeq from: long")
				#expect(message("(contains? 1 2)") == "contains? not supported on type: long")
			}
			#expect(clj_debug_live_objects() == before)
		}

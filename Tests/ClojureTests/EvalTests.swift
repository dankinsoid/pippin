	...
	@Suite struct EvalTests {
		@Test func specialForms() throws {
			...
			do {
				...
				#expect(evalError("(nope 1)") == "#error {:message \"Unable to resolve symbol: nope in this context\", :data {:line 1, :column 1}}")
				...
				#expect(evalError("(loop [x 1] (+ 1 (recur 2)))") == "#error {:message \"Can only recur from tail position\", :data {:line 1, :column 18}}")
				#expect(evalError("(recur 1)") == "#error {:message \"Can only recur from tail position\", :data {:line 1, :column 1}}")
				#expect(evalError("(loop [x 1] (recur 1 2))") == "#error {:message \"Mismatched argument count to recur, expected: 1 args, got: 2\", :data {:line 1, :column 13}}")
				#expect(evalError("(let [x] x)") == "#error {:message \"let requires an even number of forms in binding vector\", :data {:line 1, :column 1}, :cause #error {:message \"let requires an even number of forms in binding vector\", :data nil}}")
				#expect(evalError("(fn [x] (fn [x] 1) (fn [x x] 2) (fn [x] 3))") == nil)
				#expect(evalError("(fn ([x] 1) ([y] 2))") == "#error {:message \"Can't have 2 overloads with same arity\", :data {:line 1, :column 1}}")
				#expect(evalError("(fn ([x & r] 1) ([a b] 2))") == "#error {:message \"Can't have fixed arity function with more params than variadic function\", :data {:line 1, :column 1}}")
				#expect(evalError("([1 2] 5)") == "#error {:message \"Index 5 out of bounds for length 2\", :data nil}")
				#expect(evalError("(def ev-x 1 2 3)") == "#error {:message \"Too many arguments to def\", :data {:line 1, :column 1}}")

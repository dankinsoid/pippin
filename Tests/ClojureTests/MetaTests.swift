	...
	@Suite struct MetaTests {
		...
		@Test func metaOnEverySupportedType() throws {
			...
			do {
				...
				#expect(message(rt, "(with-meta 1 {})") == "with-meta: fixnum does not support metadata")
				...
				#expect(message(rt, "(with-meta [] 1)") == "with-meta: metadata must be a map, got: fixnum")
		...
		@Test func varMetaAltersAtomically() throws {
			...
			do {
				...
				#expect(try rt.eval("(meta #'mt-v)") == nil)
				#expect(message(rt, "(alter-meta! #'mt-v (fn [m] 1))") == "alter-meta! fn must return a map, got: fixnum")
				#expect(message(rt, "(alter-meta! #'mt-v (fn [m] (throw (ex-info \"boom\" {}))))") == "boom")
				#expect(message(rt, "(alter-meta! [] assoc :a 1)") == "alter-meta! expects a var, got: vector")
				#expect(message(rt, "(reset-meta! [] {})") == "reset-meta! expects a var, got: vector")
				#expect(message(rt, "(reset-meta! #'mt-v 1)") == "reset-meta! expects a map, got: fixnum")
				#expect(try rt.eval("(meta #'mt-v)") == nil)
		...
		@Test func defnAndDefmacroVarMeta() throws {
			...
			do {
				...
				#expect(try rt.eval("(:arglists (meta #'map))") == Value(reading: "([f coll] [f c1 c2] [f c1 c2 c3] [f c1 c2 c3 & colls])"))

...
extern const clj_type clj_nil_dispatch_type, clj_fixnum_dispatch_type, clj_boolean_dispatch_type, clj_char_dispatch_type;

static inline const clj_type *clj_dispatch_type_inline(clj_value v) {
	if (__builtin_expect(clj_is_ptr(v), 1)) return clj_type_of(v);
	if (clj_is_fixnum(v)) return &clj_fixnum_dispatch_type;

// Generated by clj-compile from <embedded>/clojure/test.clj; do not edit.
#include "compiled_internal.h"

#define FILE_STR (K[0])
#ifdef CLJ_COMPILED_CORE
static clj_value K[393];
static clj_value V[116];
static clj_value B[21];
static const clj_intrinsic *OP[21];
static const clj_fusion_var *F[1];
static clj_node S[56];
static void unit_pools(void);

static clj_value top_0(void);
static clj_value top_1(void);
static clj_value top_2(void);
static clj_value top_3(void);
static clj_value top_4(void);
static clj_value top_5(void);
static clj_value top_6(void);
static clj_value top_7(void);
static clj_value top_8(void);
static clj_value top_9(void);
static clj_value top_10(void);
static clj_value top_11(void);
clj_value clojure_test_with_test_out_v2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_with_test_out(void *ctx, const clj_value *args, size_t n);
static clj_value top_12(void);
clj_value clojure_test_testing_vars_str_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_testing_vars_str__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_testing_vars_str__0(void *ctx, const clj_value *args, size_t n);
static clj_value clojure_test_testing_vars_str(void *ctx, const clj_value *args, size_t n);
static clj_value top_13(void);
clj_value clojure_test_testing_contexts_str_a0(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_testing_contexts_str(void *ctx, const clj_value *args, size_t n);
static clj_value top_14(void);
clj_value clojure_test_inc_report_counter_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_inc_report_counter(void *ctx, const clj_value *args, size_t n);
static clj_value top_15(void);
static clj_value top_16(void);
clj_value clojure_test_print_trace_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_print_trace(void *ctx, const clj_value *args, size_t n);
static clj_value top_17(void);
static clj_value clojure_test_form17__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form17__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_18(void);
static clj_value top_19(void);
static clj_value clojure_test_form19__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form19__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_20(void);
static clj_value top_21(void);
static clj_value clojure_test_form21__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form21__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_22(void);
static clj_value top_23(void);
static clj_value clojure_test_form23__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form23__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_24(void);
static clj_value top_25(void);
static clj_value clojure_test_form25__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form25__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_26(void);
static clj_value top_27(void);
static clj_value clojure_test_form27__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form27__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_28(void);
static clj_value top_29(void);
static clj_value clojure_test_form29__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form29__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_30(void);
static clj_value top_31(void);
static clj_value clojure_test_form31__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form31__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_32(void);
static clj_value top_33(void);
static clj_value clojure_test_form33__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form33__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_34(void);
static clj_value top_35(void);
clj_value clojure_test_do_report_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_do_report(void *ctx, const clj_value *args, size_t n);
static clj_value top_36(void);
clj_value clojure_test_function_QMARK__a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_function_QMARK_(void *ctx, const clj_value *args, size_t n);
static clj_value top_37(void);
clj_value clojure_test_assert_predicate_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_assert_predicate(void *ctx, const clj_value *args, size_t n);
static clj_value top_38(void);
clj_value clojure_test_assert_any_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_assert_any(void *ctx, const clj_value *args, size_t n);
static clj_value top_39(void);
static clj_value clojure_test_form39__0_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form39__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_40(void);
static clj_value clojure_test_form40__0_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form40__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_41(void);
static clj_value top_42(void);
static clj_value clojure_test_form42__0_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form42__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_43(void);
static clj_value top_44(void);
static clj_value clojure_test_form44__0_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form44__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_45(void);
static clj_value top_46(void);
static clj_value clojure_test_form46__0_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form46__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_47(void);
static clj_value top_48(void);
static clj_value clojure_test_form48__0_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form48__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_49(void);
static clj_value top_50(void);
clj_value clojure_test_try_expr_a4(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_try_expr(void *ctx, const clj_value *args, size_t n);
static clj_value top_51(void);
clj_value clojure_test_is_a3(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
clj_value clojure_test_is_a4(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_is(void *ctx, const clj_value *args, size_t n);
static clj_value top_52(void);
clj_value clojure_test_are_v4(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_are(void *ctx, const clj_value *args, size_t n);
static clj_value top_53(void);
clj_value clojure_test_testing_v3(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_testing(void *ctx, const clj_value *args, size_t n);
static clj_value top_54(void);
clj_value clojure_test_with_test_v3(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_with_test(void *ctx, const clj_value *args, size_t n);
static clj_value top_55(void);
clj_value clojure_test_deftest_v3(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_deftest(void *ctx, const clj_value *args, size_t n);
static clj_value top_56(void);
clj_value clojure_test_deftest__v3(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_deftest_(void *ctx, const clj_value *args, size_t n);
static clj_value top_57(void);
clj_value clojure_test_set_test_v3(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_set_test(void *ctx, const clj_value *args, size_t n);
static clj_value top_58(void);
static clj_value top_59(void);
static clj_value clojure_test_form59__0_v1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form59__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_60(void);
static clj_value clojure_test_form60__0_v1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form60__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_61(void);
static clj_value top_62(void);
static clj_value clojure_test_form62__0_v1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_form62__0(void *ctx, const clj_value *args, size_t n);
static clj_value top_63(void);
static clj_value top_64(void);
clj_value clojure_test_default_fixture_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_default_fixture(void *ctx, const clj_value *args, size_t n);
static clj_value top_65(void);
clj_value clojure_test_compose_fixtures_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_compose_fixtures__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_compose_fixtures__0__1_a0(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_compose_fixtures__0__1(void *ctx, const clj_value *args, size_t n);
static clj_value clojure_test_compose_fixtures__0(void *ctx, const clj_value *args, size_t n);
static clj_value clojure_test_compose_fixtures(void *ctx, const clj_value *args, size_t n);
static clj_value top_66(void);
clj_value clojure_test_join_fixtures_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_join_fixtures(void *ctx, const clj_value *args, size_t n);
static clj_value top_67(void);
clj_value clojure_test_ns_key_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_ns_key(void *ctx, const clj_value *args, size_t n);
static clj_value top_68(void);
clj_value clojure_test_test_var_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_test_var(void *ctx, const clj_value *args, size_t n);
static clj_value top_69(void);
clj_value clojure_test_test_vars_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_test_vars__0_a0(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_test_vars__0__1_a0(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_test_vars__0__1(void *ctx, const clj_value *args, size_t n);
static clj_value clojure_test_test_vars__0(void *ctx, const clj_value *args, size_t n);
static clj_value clojure_test_test_vars(void *ctx, const clj_value *args, size_t n);
static clj_value top_70(void);
clj_value clojure_test_test_all_vars_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_test_all_vars__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_test_all_vars__0(void *ctx, const clj_value *args, size_t n);
static clj_value clojure_test_test_all_vars(void *ctx, const clj_value *args, size_t n);
static clj_value top_71(void);
clj_value clojure_test_test_ns_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_test_ns(void *ctx, const clj_value *args, size_t n);
static clj_value top_72(void);
clj_value clojure_test_run_tests_a0(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
clj_value clojure_test_run_tests_v0(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_run_tests(void *ctx, const clj_value *args, size_t n);
static clj_value top_73(void);
clj_value clojure_test_run_all_tests_a0(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
clj_value clojure_test_run_all_tests_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_run_all_tests__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_run_all_tests__0(void *ctx, const clj_value *args, size_t n);
static clj_value clojure_test_run_all_tests__1_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_run_all_tests__1(void *ctx, const clj_value *args, size_t n);
static clj_value clojure_test_run_all_tests(void *ctx, const clj_value *args, size_t n);
static clj_value top_74(void);
clj_value clojure_test_run_test_var_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_run_test_var(void *ctx, const clj_value *args, size_t n);
static clj_value top_75(void);
clj_value clojure_test_run_test_a3(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_run_test(void *ctx, const clj_value *args, size_t n);
static clj_value top_76(void);
clj_value clojure_test_successful_QMARK__a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs);
static clj_value clojure_test_successful_QMARK_(void *ctx, const clj_value *args, size_t n);

#ifdef CLJ_CLOSED
#define CLJC_DIRECT_clojure_core_concat_a2 1
static clj_compiled_fn CLJC_FN_clojure_core_concat_a2;
#define CLJC_DIRECT_clojure_core_map_a2 1
static clj_compiled_fn CLJC_FN_clojure_core_map_a2;
#define CLJC_DIRECT_clojure_core_interpose_a2 1
static clj_compiled_fn CLJC_FN_clojure_core_interpose_a2;
#define CLJC_DIRECT_clojure_core_fnil_a2 1
static clj_compiled_fn CLJC_FN_clojure_core_fnil_a2;
#define CLJC_DIRECT_clojure_core___GT_MultiFn_a7 1
static clj_compiled_fn CLJC_FN_clojure_core___GT_MultiFn_a7;
#define CLJC_DIRECT_clojure_core_take_a2 1
static clj_compiled_fn CLJC_FN_clojure_core_take_a2;
#define CLJC_DIRECT_clojure_test_inc_report_counter_a1 1
static clj_compiled_fn CLJC_FN_clojure_test_inc_report_counter_a1 = clojure_test_inc_report_counter_a1;
#define CLJC_DIRECT_clojure_test_testing_vars_str_a1 1
static clj_compiled_fn CLJC_FN_clojure_test_testing_vars_str_a1 = clojure_test_testing_vars_str_a1;
#define CLJC_DIRECT_clojure_test_testing_contexts_str_a0 1
static clj_compiled_fn CLJC_FN_clojure_test_testing_contexts_str_a0 = clojure_test_testing_contexts_str_a0;
#define CLJC_DIRECT_clojure_test_print_trace_a1 1
static clj_compiled_fn CLJC_FN_clojure_test_print_trace_a1 = clojure_test_print_trace_a1;
#define CLJC_DIRECT_clojure_test_function_QMARK__a1 1
static clj_compiled_fn CLJC_FN_clojure_test_function_QMARK__a1 = clojure_test_function_QMARK__a1;
#define CLJC_DIRECT_clojure_test_assert_predicate_a2 1
static clj_compiled_fn CLJC_FN_clojure_test_assert_predicate_a2 = clojure_test_assert_predicate_a2;
#define CLJC_DIRECT_clojure_test_assert_any_a2 1
static clj_compiled_fn CLJC_FN_clojure_test_assert_any_a2 = clojure_test_assert_any_a2;
#define CLJC_DIRECT_clojure_core_nthnext_a2 1
static clj_compiled_fn CLJC_FN_clojure_core_nthnext_a2;
#define CLJC_DIRECT_clojure_core_mod_a2 1
static clj_compiled_fn CLJC_FN_clojure_core_mod_a2;
#define CLJC_DIRECT_clojure_core_concat_a0 1
static clj_compiled_fn CLJC_FN_clojure_core_concat_a0;
#define CLJC_DIRECT_clojure_test_do_report_a1 1
static clj_compiled_fn CLJC_FN_clojure_test_do_report_a1 = clojure_test_do_report_a1;
#define CLJC_DIRECT_clojure_core_comp_a2 1
static clj_compiled_fn CLJC_FN_clojure_core_comp_a2;
#define CLJC_DIRECT_clojure_core_group_by_a2 1
static clj_compiled_fn CLJC_FN_clojure_core_group_by_a2;
#define CLJC_DIRECT_clojure_test_join_fixtures_a1 1
static clj_compiled_fn CLJC_FN_clojure_test_join_fixtures_a1 = clojure_test_join_fixtures_a1;
#define CLJC_DIRECT_clojure_test_test_var_a1 1
static clj_compiled_fn CLJC_FN_clojure_test_test_var_a1 = clojure_test_test_var_a1;
#define CLJC_DIRECT_clojure_core_vals_a1 1
static clj_compiled_fn CLJC_FN_clojure_core_vals_a1;
#define CLJC_DIRECT_clojure_core_sort_by_a2 1
static clj_compiled_fn CLJC_FN_clojure_core_sort_by_a2;
#define CLJC_DIRECT_clojure_test_test_vars_a1 1
static clj_compiled_fn CLJC_FN_clojure_test_test_vars_a1 = clojure_test_test_vars_a1;
#define CLJC_DIRECT_clojure_test_test_all_vars_a1 1
static clj_compiled_fn CLJC_FN_clojure_test_test_all_vars_a1 = clojure_test_test_all_vars_a1;
#define CLJC_DIRECT_clojure_core_filter_a2 1
static clj_compiled_fn CLJC_FN_clojure_core_filter_a2;
#endif

static clj_value top_0(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 3 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[0], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_value t1 = K[1];
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2 = clj_c_invoke(t0, a0, 1);
	(void)t1;
	if (o0) clj_release(t0);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_1(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 3 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[1], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	const clj_value *a0 = NULL;
	clj_value t1 = clj_c_invoke(t0, a0, 0);
	if (o0) clj_release(t0);
	if (t1 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t1;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_2(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 3 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[2], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_value t1 = K[2];
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2 = clj_c_invoke(t0, a0, 1);
	(void)t1;
	if (o0) clj_release(t0);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_3(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 3 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_retain(K[3]);
	clj_eval_top_leave();
	return t0;
}

static clj_value top_4(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 6 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_retain(K[5]);
	clj_var_bind_root(V[3], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[6]);
	clj_value t2 = clj_c_def(V[3], t1, false, true);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_5(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 10 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_retain(K[3]);
	clj_var_bind_root(V[4], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[8]);
	clj_value t2 = clj_c_def(V[4], t1, false, true);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_6(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 12 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_retain(K[3]);
	clj_var_bind_root(V[5], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[10]);
	clj_value t2 = clj_c_def(V[5], t1, false, true);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_7(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 14 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_retain(K[12]);
	clj_var_bind_root(V[6], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[13]);
	clj_value t2 = clj_c_def(V[6], t1, false, true);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_8(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 16 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[8], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	const clj_value *a0 = NULL;
	clj_value t1 = clj_c_invoke(t0, a0, 0);
	if (o0) clj_release(t0);
	if (t1 == CLJ_THROWN) {
	goto L1;
	}
	clj_var_bind_root(V[7], t1);
	clj_release(t1);
	clj_value t2 = clj_retain(K[15]);
	clj_value t3 = clj_c_def(V[7], t2, false, true);
	clj_release(t2);
	if (t3 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t3;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_9(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 18 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[8], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	const clj_value *a0 = NULL;
	clj_value t1 = clj_c_invoke(t0, a0, 0);
	if (o0) clj_release(t0);
	if (t1 == CLJ_THROWN) {
	goto L1;
	}
	clj_var_bind_root(V[9], t1);
	clj_release(t1);
	clj_value t2 = clj_retain(K[17]);
	clj_value t3 = clj_c_def(V[9], t2, false, true);
	clj_release(t2);
	if (t3 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t3;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_10(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 20 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_retain(K[3]);
	clj_var_bind_root(V[10], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[19]);
	clj_value t2 = clj_c_def(V[10], t1, false, true);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_with_test_out_v2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_value l2 = CLJ_NIL;
	l2 = nargs > 2 ? clj_list_from_array(args + 2, nargs - 2) : CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[0], &cc)) goto L1;
#line 24 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[13], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[8], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t2 = K[22];
	clj_value a2[1] = {t2};
	(void)a2;
	clj_value t3 = clj_c_invoke(t1, a2, 1);
	(void)t2;
	if (o1) clj_release(t1);
	if (t3 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t4 = l2;
	clj_value a1[2] = {t3, t4};
	(void)a1;
	clj_value t5;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t5 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a1, 2) : clj_c_invoke(t0, a1, 2);
#else
	t5 = clj_c_invoke(t0, a1, 2);
#endif
	(void)t4;
	clj_release(t3);
	if (o0) clj_release(t0);
	if (t5 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t5};
	(void)a0;
	clj_value t6;
	if (CLJC_GUARD(V[12], B[0])) {
	t6 = clj_seq(t5);
	} else {
	t6 = clj_c_intrinsic_fallback(V[12], a0, 1);
	}
	clj_release(t5);
	if (t6 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[0], &cc);
	clj_release(l2);
	return t6;
L2: ;
	clj_c_leave(&S[0], &cc);
L1: ;
	clj_release(l2);
	return CLJ_THROWN;
}

static clj_value clojure_test_with_test_out(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	default: break;
	}
	if (n >= 2) return clojure_test_with_test_out_v2(clj_from_ptr((void *)f), f->env, args, n);
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_11(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 24 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[21], clojure_test_with_test_out, NULL, 0, 0x0, 2, CLJ_ARITY_ANY);
	clj_var_bind_root(V[11], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[23]);
	clj_value t2 = clj_c_def(V[11], t1, true, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_testing_vars_str__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_ccall cc;
	if (!clj_c_enter(&S[2], &cc)) goto L1;
#line 33 "<embedded>/clojure/test.clj"
	clj_value t0 = K[29];
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[24], &o1);
	if (t1 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t2 = l0;
	clj_value a1[1] = {t2};
	(void)a1;
	clj_value t3 = clj_c_invoke(t1, a1, 1);
	(void)t2;
	if (o1) clj_release(t1);
	if (t3 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 1);
	clj_release(t3);
	(void)t0;
	if (t4 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[2], &cc);
	return t4;
L2: ;
	clj_c_leave(&S[2], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_testing_vars_str__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_testing_vars_str__0_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

clj_value clojure_test_testing_vars_str_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = CLJ_NIL;
	clj_value l2 = CLJ_NIL;
	clj_value l3 = CLJ_NIL;
	clj_value l4 = CLJ_NIL;
	clj_value l5 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[1], &cc)) goto L1;
#line 32 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_retain(l0);
	clj_c_rebind(&l1, t0);
	clj_value t1 = l1;
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2;
	if (CLJC_GUARD(V[15], B[1])) {
	t2 = clj_seq_p(t1);
	} else {
	t2 = clj_c_intrinsic_fallback(V[15], a0, 1);
	}
	(void)t1;
	if (t2 == CLJ_THROWN) {
	goto L2;
	}
	bool t3 = clj_truthy(t2);
	clj_release(t2);
	clj_value t4;
	if (t3) {
	clj_value t5 = l1;
	clj_value a1[1] = {t5};
	(void)a1;
	clj_value t6;
	if (CLJC_GUARD(V[16], B[2])) {
	t6 = clj_next(t5);
	} else {
	t6 = clj_c_intrinsic_fallback(V[16], a1, 1);
	}
	(void)t5;
	if (t6 == CLJ_THROWN) {
	goto L2;
	}
	bool t7 = clj_truthy(t6);
	clj_release(t6);
	clj_value t8;
	if (t7) {
	bool o9;
	clj_value t9 = clj_c_var_borrow(V[17], &o9);
	if (t9 == CLJ_THROWN) {
	goto L2;
	}
	bool o10;
	clj_value t10 = clj_c_var_borrow(V[18], &o10);
	if (t10 == CLJ_THROWN) {
	if (o9) clj_release(t9);
	goto L2;
	}
	clj_value t11 = l1;
	clj_value a2[2] = {t10, t11};
	(void)a2;
	clj_value t12 = clj_c_invoke(t9, a2, 2);
	(void)t11;
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (t12 == CLJ_THROWN) {
	goto L2;
	}
	t8 = t12;
	} else {
	clj_value t13 = l1;
	clj_value a3[1] = {t13};
	(void)a3;
	clj_value t14;
	if (CLJC_GUARD(V[12], B[0])) {
	t14 = clj_seq(t13);
	} else {
	t14 = clj_c_intrinsic_fallback(V[12], a3, 1);
	}
	(void)t13;
	if (t14 == CLJ_THROWN) {
	goto L2;
	}
	bool t15 = clj_truthy(t14);
	clj_release(t14);
	clj_value t16;
	if (t15) {
	clj_value t17 = l1;
	clj_value a4[1] = {t17};
	(void)a4;
	clj_value t18;
	if (CLJC_GUARD(V[19], B[3])) {
	t18 = clj_first(t17);
	} else {
	t18 = clj_c_intrinsic_fallback(V[19], a4, 1);
	}
	(void)t17;
	if (t18 == CLJ_THROWN) {
	goto L2;
	}
	t16 = t18;
	} else {
	clj_value t19 = clj_retain(K[26]);
	t16 = t19;
	}
	t8 = t16;
	}
	t4 = t8;
	} else {
	clj_value t20 = l1;
	l1 = CLJ_NIL;
	t4 = t20;
	}
	clj_c_rebind(&l2, t4);
	clj_value t21 = l2;
	clj_value t22 = K[27];
	clj_value a5[2] = {t21, t22};
	(void)a5;
	clj_value t23;
	if (CLJC_GUARD(V[20], B[4])) {
	t23 = clj_get2(t21, t22);
	} else {
	t23 = clj_c_intrinsic_fallback(V[20], a5, 2);
	}
	(void)t22;
	(void)t21;
	if (t23 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l3, t23);
	clj_value t24 = l2;
	clj_value t25 = K[28];
	clj_value a6[2] = {t24, t25};
	(void)a6;
	clj_value t26;
	if (CLJC_GUARD(V[20], B[4])) {
	t26 = clj_get2(t24, t25);
	} else {
	t26 = clj_c_intrinsic_fallback(V[20], a6, 2);
	}
	(void)t25;
	(void)t24;
	if (t26 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l4, t26);
#line 33 "<embedded>/clojure/test.clj"
	bool o27;
	clj_value t27 = clj_c_var_borrow(V[21], &o27);
	if (t27 == CLJ_THROWN) {
	goto L2;
	}
	bool o28;
	clj_value t28 = clj_c_var_borrow(V[22], &o28);
	if (t28 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	goto L2;
	}
	bool o29;
	clj_value t29 = clj_c_var_borrow(V[23], &o29);
	if (t29 == CLJ_THROWN) {
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	goto L2;
	}
	clj_value t30 = clj_c_closure(CLJ_NIL, clojure_test_testing_vars_str__0, NULL, 0, 0x2, 1, 1);
	bool o31;
	clj_value t31 = clj_c_var_borrow(V[7], &o31);
	if (t31 == CLJ_THROWN) {
	clj_release(t30);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	goto L2;
	}
	clj_value a9[2] = {t30, t31};
	(void)a9;
	clj_value t32;
#ifdef CLJC_DIRECT_clojure_core_map_a2
	if (!CLJC_FN_clojure_core_map_a2) CLJC_FN_clojure_core_map_a2 = clj_compiled_symbol("clojure_core_map_a2");
	t32 = CLJC_FN_clojure_core_map_a2 ? CLJC_FN_clojure_core_map_a2(clj_var_root_relaxed(V[23]), NULL, a9, 2) : clj_c_invoke(t29, a9, 2);
#else
	t32 = clj_c_invoke(t29, a9, 2);
#endif
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o29) clj_release(t29);
	if (t32 == CLJ_THROWN) {
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	goto L2;
	}
	clj_value a8[1] = {t32};
	(void)a8;
	clj_value t33 = clj_c_invoke(t28, a8, 1);
	clj_release(t32);
	if (o28) clj_release(t28);
	if (t33 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	goto L2;
	}
	clj_value t34 = K[30];
#line 34 "<embedded>/clojure/test.clj"
	clj_value t35 = l3;
	l3 = CLJ_NIL;
	clj_c_rebind(&l5, t35);
	clj_value t36 = l5;
	bool t37 = clj_truthy(t36);
	(void)t36;
	clj_value t38;
	if (t37) {
	clj_value t39 = clj_retain(l5);
	t38 = t39;
	} else {
	clj_value t40 = clj_retain(K[31]);
	t38 = t40;
	}
#line 33 "<embedded>/clojure/test.clj"
	clj_value t41 = K[32];
	clj_value t42 = l4;
	clj_value t43 = K[33];
	clj_value a7[6] = {t33, t34, t38, t41, t42, t43};
	(void)a7;
	clj_value t44 = clj_c_invoke(t27, a7, 6);
	(void)t43;
	(void)t42;
	(void)t41;
	clj_release(t38);
	(void)t34;
	clj_release(t33);
	if (o27) clj_release(t27);
	if (t44 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[1], &cc);
	clj_release(l1);
	clj_release(l2);
	clj_release(l3);
	clj_release(l4);
	clj_release(l5);
	return t44;
L2: ;
	clj_c_leave(&S[1], &cc);
L1: ;
	clj_release(l1);
	clj_release(l2);
	clj_release(l3);
	clj_release(l4);
	clj_release(l5);
	return CLJ_THROWN;
}

static clj_value clojure_test_testing_vars_str(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_testing_vars_str_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_12(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 29 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[25], clojure_test_testing_vars_str, NULL, 0, 0x2, 1, 1);
	clj_var_bind_root(V[14], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[34]);
	clj_value t2 = clj_c_def(V[14], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_testing_contexts_str_a0(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_ccall cc;
	if (!clj_c_enter(&S[3], &cc)) goto L1;
#line 39 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[17], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[21], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o2;
	clj_value t2 = clj_c_var_borrow(V[26], &o2);
	if (t2 == CLJ_THROWN) {
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t3 = K[37];
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[22], &o4);
	if (t4 == CLJ_THROWN) {
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o5;
	clj_value t5 = clj_c_var_borrow(V[9], &o5);
	if (t5 == CLJ_THROWN) {
	if (o4) clj_release(t4);
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a2[1] = {t5};
	(void)a2;
	clj_value t6 = clj_c_invoke(t4, a2, 1);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	if (t6 == CLJ_THROWN) {
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a1[2] = {t3, t6};
	(void)a1;
	clj_value t7;
#ifdef CLJC_DIRECT_clojure_core_interpose_a2
	if (!CLJC_FN_clojure_core_interpose_a2) CLJC_FN_clojure_core_interpose_a2 = clj_compiled_symbol("clojure_core_interpose_a2");
	t7 = CLJC_FN_clojure_core_interpose_a2 ? CLJC_FN_clojure_core_interpose_a2(clj_var_root_relaxed(V[26]), NULL, a1, 2) : clj_c_invoke(t2, a1, 2);
#else
	t7 = clj_c_invoke(t2, a1, 2);
#endif
	clj_release(t6);
	(void)t3;
	if (o2) clj_release(t2);
	if (t7 == CLJ_THROWN) {
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a0[2] = {t1, t7};
	(void)a0;
	clj_value t8 = clj_c_invoke(t0, a0, 2);
	clj_release(t7);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t8 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[3], &cc);
	return t8;
L2: ;
	clj_c_leave(&S[3], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_testing_contexts_str(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 0: return clojure_test_testing_contexts_str_a0(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_13(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 36 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[36], clojure_test_testing_contexts_str, NULL, 0, 0x1, 0, 0);
	clj_var_bind_root(V[25], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[38]);
	clj_value t2 = clj_c_def(V[25], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_inc_report_counter_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_ccall cc;
	if (!clj_c_enter(&S[4], &cc)) goto L1;
#line 44 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[5], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool t1 = clj_truthy(t0);
	if (o0) clj_release(t0);
	clj_value t2;
	if (t1) {
#line 45 "<embedded>/clojure/test.clj"
	bool o3;
	clj_value t3 = clj_c_var_borrow(V[28], &o3);
	if (t3 == CLJ_THROWN) {
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[5], &o4);
	if (t4 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o5;
	clj_value t5 = clj_c_var_borrow(V[29], &o5);
	if (t5 == CLJ_THROWN) {
	if (o4) clj_release(t4);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t6 = l0;
	bool o7;
	clj_value t7 = clj_c_var_borrow(V[30], &o7);
	if (t7 == CLJ_THROWN) {
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[31], &o8);
	if (t8 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t9 = K[41];
	clj_value a1[2] = {t8, t9};
	(void)a1;
	clj_value t10;
#ifdef CLJC_DIRECT_clojure_core_fnil_a2
	if (!CLJC_FN_clojure_core_fnil_a2) CLJC_FN_clojure_core_fnil_a2 = clj_compiled_symbol("clojure_core_fnil_a2");
	t10 = CLJC_FN_clojure_core_fnil_a2 ? CLJC_FN_clojure_core_fnil_a2(clj_var_root_relaxed(V[30]), NULL, a1, 2) : clj_c_invoke(t7, a1, 2);
#else
	t10 = clj_c_invoke(t7, a1, 2);
#endif
	(void)t9;
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	if (t10 == CLJ_THROWN) {
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a0[4] = {t4, t5, t6, t10};
	(void)a0;
	clj_value t11 = clj_c_invoke(t3, a0, 4);
	clj_release(t10);
	(void)t6;
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	if (o3) clj_release(t3);
	if (t11 == CLJ_THROWN) {
	goto L2;
	}
	t2 = t11;
	} else {
	t2 = CLJ_NIL;
	}
	clj_c_leave(&S[4], &cc);
	return t2;
L2: ;
	clj_c_leave(&S[4], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_inc_report_counter(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_inc_report_counter_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_14(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 41 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[40], clojure_test_inc_report_counter, NULL, 0, 0x2, 1, 1);
	clj_var_bind_root(V[27], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[42]);
	clj_value t2 = clj_c_def(V[27], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_15(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_value l0 = CLJ_NIL;
	clj_eval_top_enter();
#line 47 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_retain(K[44]);
	clj_value t1 = clj_c_def(V[32], t0, false, true);
	clj_release(t0);
	if (t1 == CLJ_THROWN) {
	goto L1;
	}
	clj_c_rebind(&l0, t1);
	bool o2;
	clj_value t2 = clj_c_var_borrow(V[33], &o2);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_value t3 = l0;
	clj_value a0[1] = {t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t2, a0, 1);
	(void)t3;
	if (o2) clj_release(t2);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	bool t5 = clj_truthy(t4);
	clj_release(t4);
	clj_value t6;
	if (t5) {
	clj_value t7 = clj_retain(K[3]);
	t6 = t7;
	} else {
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[34], &o8);
	if (t8 == CLJ_THROWN) {
	goto L1;
	}
	clj_value t9 = K[43];
	clj_value t10 = K[45];
	clj_value t11 = K[46];
	clj_value t12 = V[35];
	bool o13;
	clj_value t13 = clj_c_var_borrow(V[36], &o13);
	if (t13 == CLJ_THROWN) {
	if (o8) clj_release(t8);
	goto L1;
	}
	clj_value t14 = K[47];
	clj_value a2[1] = {t14};
	(void)a2;
	clj_value t15 = clj_c_invoke(t13, a2, 1);
	(void)t14;
	if (o13) clj_release(t13);
	if (t15 == CLJ_THROWN) {
	if (o8) clj_release(t8);
	goto L1;
	}
	bool o16;
	clj_value t16 = clj_c_var_borrow(V[36], &o16);
	if (t16 == CLJ_THROWN) {
	clj_release(t15);
	if (o8) clj_release(t8);
	goto L1;
	}
	clj_value t17 = K[47];
	clj_value a3[1] = {t17};
	(void)a3;
	clj_value t18 = clj_c_invoke(t16, a3, 1);
	(void)t17;
	if (o16) clj_release(t16);
	if (t18 == CLJ_THROWN) {
	clj_release(t15);
	if (o8) clj_release(t8);
	goto L1;
	}
	bool o19;
	clj_value t19 = clj_c_var_borrow(V[36], &o19);
	if (t19 == CLJ_THROWN) {
	clj_release(t18);
	clj_release(t15);
	if (o8) clj_release(t8);
	goto L1;
	}
	clj_value t20 = K[48];
	clj_value a4[1] = {t20};
	(void)a4;
	clj_value t21 = clj_c_invoke(t19, a4, 1);
	(void)t20;
	if (o19) clj_release(t19);
	if (t21 == CLJ_THROWN) {
	clj_release(t18);
	clj_release(t15);
	if (o8) clj_release(t8);
	goto L1;
	}
	clj_value a1[7] = {t9, t10, t11, t12, t15, t18, t21};
	(void)a1;
	clj_value t22;
#ifdef CLJC_DIRECT_clojure_core___GT_MultiFn_a7
	if (!CLJC_FN_clojure_core___GT_MultiFn_a7) CLJC_FN_clojure_core___GT_MultiFn_a7 = clj_compiled_symbol("clojure_core___GT_MultiFn_a7");
	t22 = CLJC_FN_clojure_core___GT_MultiFn_a7 ? CLJC_FN_clojure_core___GT_MultiFn_a7(clj_var_root_relaxed(V[34]), NULL, a1, 7) : clj_c_invoke(t8, a1, 7);
#else
	t22 = clj_c_invoke(t8, a1, 7);
#endif
	clj_release(t21);
	clj_release(t18);
	clj_release(t15);
	(void)t12;
	(void)t11;
	(void)t10;
	(void)t9;
	if (o8) clj_release(t8);
	if (t22 == CLJ_THROWN) {
	goto L1;
	}
	clj_var_bind_root(V[32], t22);
	clj_release(t22);
	clj_value t23 = clj_retain(K[49]);
	clj_value t24 = clj_c_def(V[32], t23, false, true);
	clj_release(t23);
	if (t24 == CLJ_THROWN) {
	goto L1;
	}
	t6 = t24;
	}
	clj_release(l0);
	clj_eval_top_leave();
	return t6;
L1: ;
	clj_release(l0);
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_print_trace_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = CLJ_NIL;
	clj_value l2 = CLJ_NIL;
	clj_value l3 = CLJ_NIL;
	clj_value l4 = CLJ_NIL;
	clj_value l5 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[5], &cc)) goto L1;
#line 52 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[38], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t1 = l0;
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2 = clj_c_invoke(t0, a0, 1);
	(void)t1;
	if (o0) clj_release(t0);
	if (t2 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l1, t2);
	clj_value t3 = l1;
	bool t4 = clj_truthy(t3);
	(void)t3;
	clj_value t5;
	if (t4) {
	clj_value t6 = l1;
	l1 = CLJ_NIL;
	clj_c_rebind(&l2, t6);
#line 53 "<embedded>/clojure/test.clj"
	bool o7;
	clj_value t7 = clj_c_var_borrow(V[4], &o7);
	if (t7 == CLJ_THROWN) {
	goto L2;
	}
	bool t8 = clj_truthy(t7);
	if (o7) clj_release(t7);
	clj_value t9;
	if (t8) {
	bool o10;
	clj_value t10 = clj_c_var_borrow(V[39], &o10);
	if (t10 == CLJ_THROWN) {
	goto L2;
	}
	bool o11;
	clj_value t11 = clj_c_var_borrow(V[4], &o11);
	if (t11 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t12 = l2;
	clj_value a2[2] = {t11, t12};
	(void)a2;
	clj_value t13;
#ifdef CLJC_DIRECT_clojure_core_take_a2
	if (!CLJC_FN_clojure_core_take_a2) CLJC_FN_clojure_core_take_a2 = clj_compiled_symbol("clojure_core_take_a2");
	t13 = CLJC_FN_clojure_core_take_a2 ? CLJC_FN_clojure_core_take_a2(clj_var_root_relaxed(V[39]), NULL, a2, 2) : clj_c_invoke(t10, a2, 2);
#else
	t13 = clj_c_invoke(t10, a2, 2);
#endif
	(void)t12;
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (t13 == CLJ_THROWN) {
	goto L2;
	}
	t9 = t13;
	} else {
	clj_value t14 = clj_retain(l2);
	t9 = t14;
	}
	clj_value a1[1] = {t9};
	(void)a1;
	clj_value t15;
	if (CLJC_GUARD(V[12], B[0])) {
	t15 = clj_seq(t9);
	} else {
	t15 = clj_c_intrinsic_fallback(V[12], a1, 1);
	}
	clj_release(t9);
	if (t15 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l3, t15);
L4: ;
	clj_value t16 = l3;
	bool t17 = clj_truthy(t16);
	(void)t16;
	clj_value t18;
	if (t17) {
	clj_value t19 = l3;
	clj_value a3[1] = {t19};
	(void)a3;
	clj_value t20;
	if (CLJC_GUARD(V[19], B[3])) {
	t20 = clj_first(t19);
	} else {
	t20 = clj_c_intrinsic_fallback(V[19], a3, 1);
	}
	(void)t19;
	if (t20 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l4, t20);
#line 54 "<embedded>/clojure/test.clj"
	bool o21;
	clj_value t21 = clj_c_var_borrow(V[40], &o21);
	if (t21 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t22 = K[52];
	clj_value t23 = K[53];
	clj_value t24 = l4;
	clj_value a5[1] = {t24};
	(void)a5;
	clj_value t25 = clj_c_invoke(t23, a5, 1);
	(void)t24;
	(void)t23;
	if (t25 == CLJ_THROWN) {
	if (o21) clj_release(t21);
	goto L2;
	}
	clj_c_rebind(&l5, t25);
	clj_value t26 = l5;
	bool t27 = clj_truthy(t26);
	(void)t26;
	clj_value t28;
	if (t27) {
	clj_value t29 = clj_retain(l5);
	t28 = t29;
	} else {
	clj_value t30 = clj_retain(K[54]);
	t28 = t30;
	}
	bool o31;
	clj_value t31 = clj_c_var_borrow(V[21], &o31);
	if (t31 == CLJ_THROWN) {
	clj_release(t28);
	if (o21) clj_release(t21);
	goto L2;
	}
	clj_value t32 = K[55];
	clj_value t33 = K[56];
	clj_value t34 = l4;
	clj_value a7[1] = {t34};
	(void)a7;
	clj_value t35 = clj_c_invoke(t33, a7, 1);
	(void)t34;
	(void)t33;
	if (t35 == CLJ_THROWN) {
	if (o31) clj_release(t31);
	clj_release(t28);
	if (o21) clj_release(t21);
	goto L2;
	}
	clj_value t36 = K[57];
	clj_value t37 = K[58];
	clj_value t38 = l4;
	clj_value a8[1] = {t38};
	(void)a8;
	clj_value t39 = clj_c_invoke(t37, a8, 1);
	(void)t38;
	(void)t37;
	if (t39 == CLJ_THROWN) {
	clj_release(t35);
	if (o31) clj_release(t31);
	clj_release(t28);
	if (o21) clj_release(t21);
	goto L2;
	}
	clj_value t40 = K[59];
	clj_value a6[5] = {t32, t35, t36, t39, t40};
	(void)a6;
	clj_value t41 = clj_c_invoke(t31, a6, 5);
	(void)t40;
	clj_release(t39);
	(void)t36;
	clj_release(t35);
	(void)t32;
	if (o31) clj_release(t31);
	if (t41 == CLJ_THROWN) {
	clj_release(t28);
	if (o21) clj_release(t21);
	goto L2;
	}
	clj_value a4[3] = {t22, t28, t41};
	(void)a4;
	clj_value t42 = clj_c_invoke(t21, a4, 3);
	clj_release(t41);
	clj_release(t28);
	(void)t22;
	if (o21) clj_release(t21);
	if (t42 == CLJ_THROWN) {
	goto L2;
	}
	clj_release(t42);
#line 53 "<embedded>/clojure/test.clj"
	clj_value t43 = l3;
	clj_value a9[1] = {t43};
	(void)a9;
	clj_value t44;
	if (CLJC_GUARD(V[16], B[2])) {
	t44 = clj_next(t43);
	} else {
	t44 = clj_c_intrinsic_fallback(V[16], a9, 1);
	}
	(void)t43;
	if (t44 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l3, t44);
	if (clj_c_loop_tick()) {
	goto L2;
	}
	goto L4;
	clj_value t45 = CLJ_NIL;
	t18 = t45;
	} else {
	t18 = CLJ_NIL;
	}
	t5 = t18;
	} else {
	t5 = CLJ_NIL;
	}
	clj_c_leave(&S[5], &cc);
	clj_release(l1);
	clj_release(l2);
	clj_release(l3);
	clj_release(l4);
	clj_release(l5);
	return t5;
L2: ;
	clj_c_leave(&S[5], &cc);
L1: ;
	clj_release(l1);
	clj_release(l2);
	clj_release(l3);
	clj_release(l4);
	clj_release(l5);
	return CLJ_THROWN;
}

static clj_value clojure_test_print_trace(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_print_trace_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_16(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 51 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[51], clojure_test_print_trace, NULL, 0, 0x2, 1, 1);
	clj_var_bind_root(V[37], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[60]);
	clj_value t2 = clj_c_def(V[37], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form17__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_ccall cc;
	if (!clj_c_enter(&S[6], &cc)) goto L1;
#line 57 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[42], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t1 = l0;
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2 = clj_c_invoke(t0, a0, 1);
	(void)t1;
	if (o0) clj_release(t0);
	if (t2 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[6], &cc);
	return t2;
L2: ;
	clj_c_leave(&S[6], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_form17__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_form17__0_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_17(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 56 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[32], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[62];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form17__0, NULL, 0, 0x2, 1, 1);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_18(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 56 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[32]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form19__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_ccall cc;
	if (!clj_c_enter(&S[7], &cc)) goto L1;
#line 60 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[27], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t1 = K[64];
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2;
#ifdef CLJC_DIRECT_clojure_test_inc_report_counter_a1
	if (!CLJC_FN_clojure_test_inc_report_counter_a1) CLJC_FN_clojure_test_inc_report_counter_a1 = clj_compiled_symbol("clojure_test_inc_report_counter_a1");
	t2 = CLJC_FN_clojure_test_inc_report_counter_a1 ? CLJC_FN_clojure_test_inc_report_counter_a1(clj_var_root_relaxed(V[27]), NULL, a0, 1) : clj_c_invoke(t0, a0, 1);
#else
	t2 = clj_c_invoke(t0, a0, 1);
#endif
	(void)t1;
	if (o0) clj_release(t0);
	if (t2 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[7], &cc);
	return t2;
L2: ;
	clj_c_leave(&S[7], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_form19__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_form19__0_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_19(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 59 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[32], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[64];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form19__0, NULL, 0, 0x2, 1, 1);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_20(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 59 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[32]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form21__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = CLJ_NIL;
	clj_value l2 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[8], &cc)) goto L1;
#line 62 "<embedded>/clojure/test.clj"
#line 63 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[27], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t1 = K[66];
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2;
#ifdef CLJC_DIRECT_clojure_test_inc_report_counter_a1
	if (!CLJC_FN_clojure_test_inc_report_counter_a1) CLJC_FN_clojure_test_inc_report_counter_a1 = clj_compiled_symbol("clojure_test_inc_report_counter_a1");
	t2 = CLJC_FN_clojure_test_inc_report_counter_a1 ? CLJC_FN_clojure_test_inc_report_counter_a1(clj_var_root_relaxed(V[27]), NULL, a0, 1) : clj_c_invoke(t0, a0, 1);
#else
	t2 = clj_c_invoke(t0, a0, 1);
#endif
	(void)t1;
	if (o0) clj_release(t0);
	if (t2 == CLJ_THROWN) {
	goto L2;
	}
	clj_release(t2);
#line 64 "<embedded>/clojure/test.clj"
	bool o3;
	clj_value t3 = clj_c_var_borrow(V[40], &o3);
	if (t3 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t4 = K[67];
	bool o5;
	clj_value t5 = clj_c_var_borrow(V[14], &o5);
	if (t5 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t6 = l0;
	clj_value a2[1] = {t6};
	(void)a2;
	clj_value t7;
#ifdef CLJC_DIRECT_clojure_test_testing_vars_str_a1
	if (!CLJC_FN_clojure_test_testing_vars_str_a1) CLJC_FN_clojure_test_testing_vars_str_a1 = clj_compiled_symbol("clojure_test_testing_vars_str_a1");
	t7 = CLJC_FN_clojure_test_testing_vars_str_a1 ? CLJC_FN_clojure_test_testing_vars_str_a1(clj_var_root_relaxed(V[14]), NULL, a2, 1) : clj_c_invoke(t5, a2, 1);
#else
	t7 = clj_c_invoke(t5, a2, 1);
#endif
	(void)t6;
	if (o5) clj_release(t5);
	if (t7 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a1[2] = {t4, t7};
	(void)a1;
	clj_value t8 = clj_c_invoke(t3, a1, 2);
	clj_release(t7);
	(void)t4;
	if (o3) clj_release(t3);
	if (t8 == CLJ_THROWN) {
	goto L2;
	}
	clj_release(t8);
#line 65 "<embedded>/clojure/test.clj"
	bool o9;
	clj_value t9 = clj_c_var_borrow(V[9], &o9);
	if (t9 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a3[1] = {t9};
	(void)a3;
	clj_value t10;
	if (CLJC_GUARD(V[12], B[0])) {
	t10 = clj_seq(t9);
	} else {
	t10 = clj_c_intrinsic_fallback(V[12], a3, 1);
	}
	if (o9) clj_release(t9);
	if (t10 == CLJ_THROWN) {
	goto L2;
	}
	bool t11 = clj_truthy(t10);
	clj_release(t10);
	clj_value t12;
	if (t11) {
	bool o13;
	clj_value t13 = clj_c_var_borrow(V[40], &o13);
	if (t13 == CLJ_THROWN) {
	goto L2;
	}
	bool o14;
	clj_value t14 = clj_c_var_borrow(V[25], &o14);
	if (t14 == CLJ_THROWN) {
	if (o13) clj_release(t13);
	goto L2;
	}
	const clj_value *a5 = NULL;
	clj_value t15;
#ifdef CLJC_DIRECT_clojure_test_testing_contexts_str_a0
	if (!CLJC_FN_clojure_test_testing_contexts_str_a0) CLJC_FN_clojure_test_testing_contexts_str_a0 = clj_compiled_symbol("clojure_test_testing_contexts_str_a0");
	t15 = CLJC_FN_clojure_test_testing_contexts_str_a0 ? CLJC_FN_clojure_test_testing_contexts_str_a0(clj_var_root_relaxed(V[25]), NULL, a5, 0) : clj_c_invoke(t14, a5, 0);
#else
	t15 = clj_c_invoke(t14, a5, 0);
#endif
	if (o14) clj_release(t14);
	if (t15 == CLJ_THROWN) {
	if (o13) clj_release(t13);
	goto L2;
	}
	clj_value a4[1] = {t15};
	(void)a4;
	clj_value t16 = clj_c_invoke(t13, a4, 1);
	clj_release(t15);
	if (o13) clj_release(t13);
	if (t16 == CLJ_THROWN) {
	goto L2;
	}
	t12 = t16;
	} else {
	t12 = CLJ_NIL;
	}
	clj_release(t12);
#line 66 "<embedded>/clojure/test.clj"
	clj_value t17 = K[68];
	clj_value t18 = l0;
	clj_value a6[1] = {t18};
	(void)a6;
	clj_value t19 = clj_c_invoke(t17, a6, 1);
	(void)t18;
	(void)t17;
	if (t19 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l1, t19);
	clj_value t20 = l1;
	bool t21 = clj_truthy(t20);
	(void)t20;
	clj_value t22;
	if (t21) {
	clj_value t23 = l1;
	l1 = CLJ_NIL;
	clj_c_rebind(&l2, t23);
	bool o24;
	clj_value t24 = clj_c_var_borrow(V[40], &o24);
	if (t24 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t25 = l2;
	clj_value a7[1] = {t25};
	(void)a7;
	clj_value t26 = clj_c_invoke(t24, a7, 1);
	(void)t25;
	if (o24) clj_release(t24);
	if (t26 == CLJ_THROWN) {
	goto L2;
	}
	t22 = t26;
	} else {
	t22 = CLJ_NIL;
	}
	clj_release(t22);
#line 67 "<embedded>/clojure/test.clj"
	bool o27;
	clj_value t27 = clj_c_var_borrow(V[40], &o27);
	if (t27 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t28 = K[69];
	bool o29;
	clj_value t29 = clj_c_var_borrow(V[43], &o29);
	if (t29 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	goto L2;
	}
	clj_value t30 = K[70];
	clj_value t31 = l0;
	clj_value a10[1] = {t31};
	(void)a10;
	clj_value t32 = clj_c_invoke(t30, a10, 1);
	(void)t31;
	(void)t30;
	if (t32 == CLJ_THROWN) {
	if (o29) clj_release(t29);
	if (o27) clj_release(t27);
	goto L2;
	}
	clj_value a9[1] = {t32};
	(void)a9;
	clj_value t33 = clj_c_invoke(t29, a9, 1);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (t33 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	goto L2;
	}
	clj_value a8[2] = {t28, t33};
	(void)a8;
	clj_value t34 = clj_c_invoke(t27, a8, 2);
	clj_release(t33);
	(void)t28;
	if (o27) clj_release(t27);
	if (t34 == CLJ_THROWN) {
	goto L2;
	}
	clj_release(t34);
#line 68 "<embedded>/clojure/test.clj"
	bool o35;
	clj_value t35 = clj_c_var_borrow(V[40], &o35);
	if (t35 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t36 = K[71];
	bool o37;
	clj_value t37 = clj_c_var_borrow(V[43], &o37);
	if (t37 == CLJ_THROWN) {
	if (o35) clj_release(t35);
	goto L2;
	}
	clj_value t38 = K[72];
	clj_value t39 = l0;
	clj_value a13[1] = {t39};
	(void)a13;
	clj_value t40 = clj_c_invoke(t38, a13, 1);
	(void)t39;
	(void)t38;
	if (t40 == CLJ_THROWN) {
	if (o37) clj_release(t37);
	if (o35) clj_release(t35);
	goto L2;
	}
	clj_value a12[1] = {t40};
	(void)a12;
	clj_value t41 = clj_c_invoke(t37, a12, 1);
	clj_release(t40);
	if (o37) clj_release(t37);
	if (t41 == CLJ_THROWN) {
	if (o35) clj_release(t35);
	goto L2;
	}
	clj_value a11[2] = {t36, t41};
	(void)a11;
	clj_value t42 = clj_c_invoke(t35, a11, 2);
	clj_release(t41);
	(void)t36;
	if (o35) clj_release(t35);
	if (t42 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[8], &cc);
	clj_release(l1);
	clj_release(l2);
	return t42;
L2: ;
	clj_c_leave(&S[8], &cc);
L1: ;
	clj_release(l1);
	clj_release(l2);
	return CLJ_THROWN;
}

static clj_value clojure_test_form21__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_form21__0_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_21(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 62 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[32], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[66];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form21__0, NULL, 0, 0x2, 1, 1);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_22(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 62 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[32]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form23__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = CLJ_NIL;
	clj_value l2 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[9], &cc)) goto L1;
#line 70 "<embedded>/clojure/test.clj"
#line 71 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[27], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t1 = K[74];
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2;
#ifdef CLJC_DIRECT_clojure_test_inc_report_counter_a1
	if (!CLJC_FN_clojure_test_inc_report_counter_a1) CLJC_FN_clojure_test_inc_report_counter_a1 = clj_compiled_symbol("clojure_test_inc_report_counter_a1");
	t2 = CLJC_FN_clojure_test_inc_report_counter_a1 ? CLJC_FN_clojure_test_inc_report_counter_a1(clj_var_root_relaxed(V[27]), NULL, a0, 1) : clj_c_invoke(t0, a0, 1);
#else
	t2 = clj_c_invoke(t0, a0, 1);
#endif
	(void)t1;
	if (o0) clj_release(t0);
	if (t2 == CLJ_THROWN) {
	goto L2;
	}
	clj_release(t2);
#line 72 "<embedded>/clojure/test.clj"
	bool o3;
	clj_value t3 = clj_c_var_borrow(V[40], &o3);
	if (t3 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t4 = K[75];
	bool o5;
	clj_value t5 = clj_c_var_borrow(V[14], &o5);
	if (t5 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t6 = l0;
	clj_value a2[1] = {t6};
	(void)a2;
	clj_value t7;
#ifdef CLJC_DIRECT_clojure_test_testing_vars_str_a1
	if (!CLJC_FN_clojure_test_testing_vars_str_a1) CLJC_FN_clojure_test_testing_vars_str_a1 = clj_compiled_symbol("clojure_test_testing_vars_str_a1");
	t7 = CLJC_FN_clojure_test_testing_vars_str_a1 ? CLJC_FN_clojure_test_testing_vars_str_a1(clj_var_root_relaxed(V[14]), NULL, a2, 1) : clj_c_invoke(t5, a2, 1);
#else
	t7 = clj_c_invoke(t5, a2, 1);
#endif
	(void)t6;
	if (o5) clj_release(t5);
	if (t7 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a1[2] = {t4, t7};
	(void)a1;
	clj_value t8 = clj_c_invoke(t3, a1, 2);
	clj_release(t7);
	(void)t4;
	if (o3) clj_release(t3);
	if (t8 == CLJ_THROWN) {
	goto L2;
	}
	clj_release(t8);
#line 73 "<embedded>/clojure/test.clj"
	bool o9;
	clj_value t9 = clj_c_var_borrow(V[9], &o9);
	if (t9 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a3[1] = {t9};
	(void)a3;
	clj_value t10;
	if (CLJC_GUARD(V[12], B[0])) {
	t10 = clj_seq(t9);
	} else {
	t10 = clj_c_intrinsic_fallback(V[12], a3, 1);
	}
	if (o9) clj_release(t9);
	if (t10 == CLJ_THROWN) {
	goto L2;
	}
	bool t11 = clj_truthy(t10);
	clj_release(t10);
	clj_value t12;
	if (t11) {
	bool o13;
	clj_value t13 = clj_c_var_borrow(V[40], &o13);
	if (t13 == CLJ_THROWN) {
	goto L2;
	}
	bool o14;
	clj_value t14 = clj_c_var_borrow(V[25], &o14);
	if (t14 == CLJ_THROWN) {
	if (o13) clj_release(t13);
	goto L2;
	}
	const clj_value *a5 = NULL;
	clj_value t15;
#ifdef CLJC_DIRECT_clojure_test_testing_contexts_str_a0
	if (!CLJC_FN_clojure_test_testing_contexts_str_a0) CLJC_FN_clojure_test_testing_contexts_str_a0 = clj_compiled_symbol("clojure_test_testing_contexts_str_a0");
	t15 = CLJC_FN_clojure_test_testing_contexts_str_a0 ? CLJC_FN_clojure_test_testing_contexts_str_a0(clj_var_root_relaxed(V[25]), NULL, a5, 0) : clj_c_invoke(t14, a5, 0);
#else
	t15 = clj_c_invoke(t14, a5, 0);
#endif
	if (o14) clj_release(t14);
	if (t15 == CLJ_THROWN) {
	if (o13) clj_release(t13);
	goto L2;
	}
	clj_value a4[1] = {t15};
	(void)a4;
	clj_value t16 = clj_c_invoke(t13, a4, 1);
	clj_release(t15);
	if (o13) clj_release(t13);
	if (t16 == CLJ_THROWN) {
	goto L2;
	}
	t12 = t16;
	} else {
	t12 = CLJ_NIL;
	}
	clj_release(t12);
#line 74 "<embedded>/clojure/test.clj"
	clj_value t17 = K[76];
	clj_value t18 = l0;
	clj_value a6[1] = {t18};
	(void)a6;
	clj_value t19 = clj_c_invoke(t17, a6, 1);
	(void)t18;
	(void)t17;
	if (t19 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l1, t19);
	clj_value t20 = l1;
	bool t21 = clj_truthy(t20);
	(void)t20;
	clj_value t22;
	if (t21) {
	clj_value t23 = l1;
	l1 = CLJ_NIL;
	clj_c_rebind(&l2, t23);
	bool o24;
	clj_value t24 = clj_c_var_borrow(V[40], &o24);
	if (t24 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t25 = l2;
	clj_value a7[1] = {t25};
	(void)a7;
	clj_value t26 = clj_c_invoke(t24, a7, 1);
	(void)t25;
	if (o24) clj_release(t24);
	if (t26 == CLJ_THROWN) {
	goto L2;
	}
	t22 = t26;
	} else {
	t22 = CLJ_NIL;
	}
	clj_release(t22);
#line 75 "<embedded>/clojure/test.clj"
	bool o27;
	clj_value t27 = clj_c_var_borrow(V[40], &o27);
	if (t27 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t28 = K[77];
	bool o29;
	clj_value t29 = clj_c_var_borrow(V[43], &o29);
	if (t29 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	goto L2;
	}
	clj_value t30 = K[78];
	clj_value t31 = l0;
	clj_value a10[1] = {t31};
	(void)a10;
	clj_value t32 = clj_c_invoke(t30, a10, 1);
	(void)t31;
	(void)t30;
	if (t32 == CLJ_THROWN) {
	if (o29) clj_release(t29);
	if (o27) clj_release(t27);
	goto L2;
	}
	clj_value a9[1] = {t32};
	(void)a9;
	clj_value t33 = clj_c_invoke(t29, a9, 1);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (t33 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	goto L2;
	}
	clj_value a8[2] = {t28, t33};
	(void)a8;
	clj_value t34 = clj_c_invoke(t27, a8, 2);
	clj_release(t33);
	(void)t28;
	if (o27) clj_release(t27);
	if (t34 == CLJ_THROWN) {
	goto L2;
	}
	clj_release(t34);
#line 76 "<embedded>/clojure/test.clj"
	bool o35;
	clj_value t35 = clj_c_var_borrow(V[40], &o35);
	if (t35 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t36 = K[79];
	bool o37;
	clj_value t37 = clj_c_var_borrow(V[43], &o37);
	if (t37 == CLJ_THROWN) {
	if (o35) clj_release(t35);
	goto L2;
	}
	clj_value t38 = K[80];
	clj_value t39 = l0;
	clj_value a13[1] = {t39};
	(void)a13;
	clj_value t40 = clj_c_invoke(t38, a13, 1);
	(void)t39;
	(void)t38;
	if (t40 == CLJ_THROWN) {
	if (o37) clj_release(t37);
	if (o35) clj_release(t35);
	goto L2;
	}
	clj_value a12[1] = {t40};
	(void)a12;
	clj_value t41 = clj_c_invoke(t37, a12, 1);
	clj_release(t40);
	if (o37) clj_release(t37);
	if (t41 == CLJ_THROWN) {
	if (o35) clj_release(t35);
	goto L2;
	}
	clj_value a11[2] = {t36, t41};
	(void)a11;
	clj_value t42 = clj_c_invoke(t35, a11, 2);
	clj_release(t41);
	(void)t36;
	if (o35) clj_release(t35);
	if (t42 == CLJ_THROWN) {
	goto L2;
	}
	clj_release(t42);
#line 77 "<embedded>/clojure/test.clj"
	bool o43;
	clj_value t43 = clj_c_var_borrow(V[37], &o43);
	if (t43 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t44 = K[80];
	clj_value t45 = l0;
	clj_value a15[1] = {t45};
	(void)a15;
	clj_value t46 = clj_c_invoke(t44, a15, 1);
	(void)t45;
	(void)t44;
	if (t46 == CLJ_THROWN) {
	if (o43) clj_release(t43);
	goto L2;
	}
	clj_value a14[1] = {t46};
	(void)a14;
	clj_value t47;
#ifdef CLJC_DIRECT_clojure_test_print_trace_a1
	if (!CLJC_FN_clojure_test_print_trace_a1) CLJC_FN_clojure_test_print_trace_a1 = clj_compiled_symbol("clojure_test_print_trace_a1");
	t47 = CLJC_FN_clojure_test_print_trace_a1 ? CLJC_FN_clojure_test_print_trace_a1(clj_var_root_relaxed(V[37]), NULL, a14, 1) : clj_c_invoke(t43, a14, 1);
#else
	t47 = clj_c_invoke(t43, a14, 1);
#endif
	clj_release(t46);
	if (o43) clj_release(t43);
	if (t47 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[9], &cc);
	clj_release(l1);
	clj_release(l2);
	return t47;
L2: ;
	clj_c_leave(&S[9], &cc);
L1: ;
	clj_release(l1);
	clj_release(l2);
	return CLJ_THROWN;
}

static clj_value clojure_test_form23__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_form23__0_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_23(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 70 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[32], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[74];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form23__0, NULL, 0, 0x2, 1, 1);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_24(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 70 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[32]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form25__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_ccall cc;
	if (!clj_c_enter(&S[10], &cc)) goto L1;
#line 79 "<embedded>/clojure/test.clj"
#line 80 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[40], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t1 = K[83];
	clj_value t2 = K[84];
	clj_value t3 = l0;
	clj_value a1[1] = {t3};
	(void)a1;
	clj_value t4 = clj_c_invoke(t2, a1, 1);
	(void)t3;
	(void)t2;
	if (t4 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t5 = K[85];
#line 81 "<embedded>/clojure/test.clj"
	bool o6;
	clj_value t6 = clj_c_var_borrow(V[44], &o6);
	if (t6 == CLJ_THROWN) {
	clj_release(t4);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t7 = K[86];
	clj_value t8 = l0;
	clj_value a3[1] = {t8};
	(void)a3;
	clj_value t9 = clj_c_invoke(t7, a3, 1);
	(void)t8;
	(void)t7;
	if (t9 == CLJ_THROWN) {
	if (o6) clj_release(t6);
	clj_release(t4);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t10 = K[87];
	clj_value t11 = l0;
	clj_value a4[1] = {t11};
	(void)a4;
	clj_value t12 = clj_c_invoke(t10, a4, 1);
	(void)t11;
	(void)t10;
	if (t12 == CLJ_THROWN) {
	clj_release(t9);
	if (o6) clj_release(t6);
	clj_release(t4);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t13 = K[88];
	clj_value t14 = l0;
	clj_value a5[1] = {t14};
	(void)a5;
	clj_value t15 = clj_c_invoke(t13, a5, 1);
	(void)t14;
	(void)t13;
	if (t15 == CLJ_THROWN) {
	clj_release(t12);
	clj_release(t9);
	if (o6) clj_release(t6);
	clj_release(t4);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a2[3] = {t9, t12, t15};
	(void)a2;
	clj_value t16 = clj_c_invoke(t6, a2, 3);
	clj_release(t15);
	clj_release(t12);
	clj_release(t9);
	if (o6) clj_release(t6);
	if (t16 == CLJ_THROWN) {
	clj_release(t4);
	if (o0) clj_release(t0);
	goto L2;
	}
#line 80 "<embedded>/clojure/test.clj"
	clj_value t17 = K[89];
	clj_value a0[5] = {t1, t4, t5, t16, t17};
	(void)a0;
	clj_value t18 = clj_c_invoke(t0, a0, 5);
	(void)t17;
	clj_release(t16);
	(void)t5;
	clj_release(t4);
	(void)t1;
	if (o0) clj_release(t0);
	if (t18 == CLJ_THROWN) {
	goto L2;
	}
	clj_release(t18);
#line 82 "<embedded>/clojure/test.clj"
	bool o19;
	clj_value t19 = clj_c_var_borrow(V[40], &o19);
	if (t19 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t20 = K[87];
	clj_value t21 = l0;
	clj_value a7[1] = {t21};
	(void)a7;
	clj_value t22 = clj_c_invoke(t20, a7, 1);
	(void)t21;
	(void)t20;
	if (t22 == CLJ_THROWN) {
	if (o19) clj_release(t19);
	goto L2;
	}
	clj_value t23 = K[90];
	clj_value t24 = K[88];
	clj_value t25 = l0;
	clj_value a8[1] = {t25};
	(void)a8;
	clj_value t26 = clj_c_invoke(t24, a8, 1);
	(void)t25;
	(void)t24;
	if (t26 == CLJ_THROWN) {
	clj_release(t22);
	if (o19) clj_release(t19);
	goto L2;
	}
	clj_value t27 = K[91];
	clj_value a6[4] = {t22, t23, t26, t27};
	(void)a6;
	clj_value t28 = clj_c_invoke(t19, a6, 4);
	(void)t27;
	clj_release(t26);
	(void)t23;
	clj_release(t22);
	if (o19) clj_release(t19);
	if (t28 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[10], &cc);
	return t28;
L2: ;
	clj_c_leave(&S[10], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_form25__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_form25__0_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_25(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 79 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[32], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[82];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form25__0, NULL, 0, 0x2, 1, 1);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_26(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 79 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[32]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form27__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_ccall cc;
	if (!clj_c_enter(&S[11], &cc)) goto L1;
#line 85 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[40], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t1 = K[94];
	bool o2;
	clj_value t2 = clj_c_var_borrow(V[45], &o2);
	if (t2 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t3 = K[95];
	clj_value t4 = l0;
	clj_value a2[1] = {t4};
	(void)a2;
	clj_value t5 = clj_c_invoke(t3, a2, 1);
	(void)t4;
	(void)t3;
	if (t5 == CLJ_THROWN) {
	if (o2) clj_release(t2);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a1[1] = {t5};
	(void)a1;
	clj_value t6 = clj_c_invoke(t2, a1, 1);
	clj_release(t5);
	if (o2) clj_release(t2);
	if (t6 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a0[2] = {t1, t6};
	(void)a0;
	clj_value t7 = clj_c_invoke(t0, a0, 2);
	clj_release(t6);
	(void)t1;
	if (o0) clj_release(t0);
	if (t7 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[11], &cc);
	return t7;
L2: ;
	clj_c_leave(&S[11], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_form27__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_form27__0_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_27(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 84 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[32], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[93];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form27__0, NULL, 0, 0x2, 1, 1);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_28(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 84 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[32]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form29__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_ccall cc;
	if (!clj_c_enter(&S[12], &cc)) goto L1;
#line 87 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_retain(K[3]);
	clj_c_leave(&S[12], &cc);
	return t0;
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_form29__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_form29__0_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_29(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 87 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[32], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[97];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form29__0, NULL, 0, 0x2, 1, 1);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_30(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 87 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[32]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form31__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_ccall cc;
	if (!clj_c_enter(&S[13], &cc)) goto L1;
#line 88 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_retain(K[3]);
	clj_c_leave(&S[13], &cc);
	return t0;
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_form31__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_form31__0_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_31(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 88 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[32], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[99];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form31__0, NULL, 0, 0x2, 1, 1);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_32(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 88 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[32]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form33__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_ccall cc;
	if (!clj_c_enter(&S[14], &cc)) goto L1;
#line 89 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_retain(K[3]);
	clj_c_leave(&S[14], &cc);
	return t0;
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_form33__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_form33__0_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_33(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 89 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[32], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[101];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form33__0, NULL, 0, 0x2, 1, 1);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_34(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 89 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[32]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_do_report_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = CLJ_NIL;
	clj_value l2 = CLJ_NIL;
	clj_value l3 = CLJ_NIL;
	clj_value l4 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[15], &cc)) goto L1;
#line 94 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[32], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
#line 95 "<embedded>/clojure/test.clj"
	clj_value t1 = K[104];
	clj_value t2 = K[105];
	clj_value t3 = l0;
	clj_value a2[1] = {t3};
	(void)a2;
	clj_value t4 = clj_c_invoke(t2, a2, 1);
	(void)t3;
	(void)t2;
	if (t4 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a1[2] = {t1, t4};
	(void)a1;
	clj_value t5;
	if (CLJC_GUARD(V[47], B[5])) {
	t5 = clj_eq(t1, t4);
	} else {
	t5 = clj_c_intrinsic_fallback(V[47], a1, 2);
	}
	clj_release(t4);
	(void)t1;
	if (t5 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_c_rebind(&l1, t5);
	clj_value t6 = l1;
	bool t7 = clj_truthy(t6);
	(void)t6;
	clj_value t8;
	if (t7) {
	clj_value t9 = l1;
	l1 = CLJ_NIL;
	t8 = t9;
	} else {
	clj_value t10 = K[106];
	clj_value t11 = K[105];
	clj_value t12 = l0;
	clj_value a4[1] = {t12};
	(void)a4;
	clj_value t13 = clj_c_invoke(t11, a4, 1);
	(void)t12;
	(void)t11;
	if (t13 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a3[2] = {t10, t13};
	(void)a3;
	clj_value t14;
	if (CLJC_GUARD(V[47], B[5])) {
	t14 = clj_eq(t10, t13);
	} else {
	t14 = clj_c_intrinsic_fallback(V[47], a3, 2);
	}
	clj_release(t13);
	(void)t10;
	if (t14 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	t8 = t14;
	}
	clj_c_rebind(&l2, t8);
	clj_value t15 = l2;
	bool t16 = clj_truthy(t15);
	(void)t15;
	clj_value t17;
	if (t16) {
	clj_value t18 = K[107];
	clj_value t19 = l0;
	clj_value a6[1] = {t19};
	(void)a6;
	clj_value t20 = clj_c_invoke(t18, a6, 1);
	(void)t19;
	(void)t18;
	if (t20 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a5[1] = {t20};
	(void)a5;
	clj_value t21;
	if (CLJC_GUARD(V[48], B[6])) {
	t21 = clj_nil_p(t20);
	} else {
	t21 = clj_c_intrinsic_fallback(V[48], a5, 1);
	}
	clj_release(t20);
	if (t21 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	t17 = t21;
	} else {
	clj_value t22 = clj_retain(l2);
	t17 = t22;
	}
	bool t23 = clj_truthy(t17);
	clj_release(t17);
	clj_value t24;
	if (t23) {
#line 96 "<embedded>/clojure/test.clj"
	bool o25;
	clj_value t25 = clj_c_var_borrow(V[7], &o25);
	if (t25 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a7[1] = {t25};
	(void)a7;
	clj_value t26;
	if (CLJC_GUARD(V[19], B[3])) {
	t26 = clj_first(t25);
	} else {
	t26 = clj_c_intrinsic_fallback(V[19], a7, 1);
	}
	if (o25) clj_release(t25);
	if (t26 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_c_rebind(&l3, t26);
#line 97 "<embedded>/clojure/test.clj"
	bool o27;
	clj_value t27 = clj_c_var_borrow(V[49], &o27);
	if (t27 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t28 = clj_var_deref(V[10]);
	if (t28 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_c_rebind(&l4, t28);
	clj_value t29 = l4;
	bool t30 = clj_truthy(t29);
	(void)t29;
	clj_value t31;
	if (t30) {
	clj_value t32 = clj_retain(l4);
	t31 = t32;
	} else {
	clj_value t33 = K[107];
	clj_value t34 = K[107];
	bool o35;
	clj_value t35 = clj_c_var_borrow(V[24], &o35);
	if (t35 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t36 = l3;
	clj_value a11[1] = {t36};
	(void)a11;
	clj_value t37 = clj_c_invoke(t35, a11, 1);
	(void)t36;
	if (o35) clj_release(t35);
	if (t37 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a10[1] = {t37};
	(void)a10;
	clj_value t38 = clj_c_invoke(t34, a10, 1);
	clj_release(t37);
	(void)t34;
	if (t38 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t39 = K[108];
	clj_value t40 = K[108];
	bool o41;
	clj_value t41 = clj_c_var_borrow(V[24], &o41);
	if (t41 == CLJ_THROWN) {
	clj_release(t38);
	if (o27) clj_release(t27);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t42 = l3;
	clj_value a13[1] = {t42};
	(void)a13;
	clj_value t43 = clj_c_invoke(t41, a13, 1);
	(void)t42;
	if (o41) clj_release(t41);
	if (t43 == CLJ_THROWN) {
	clj_release(t38);
	if (o27) clj_release(t27);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a12[1] = {t43};
	(void)a12;
	clj_value t44 = clj_c_invoke(t40, a12, 1);
	clj_release(t43);
	(void)t40;
	if (t44 == CLJ_THROWN) {
	clj_release(t38);
	if (o27) clj_release(t27);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a9[4] = {t33, t38, t39, t44};
	(void)a9;
	clj_value t45 = clj_c_map_literal(a9, 4);
	clj_release(t44);
	(void)t39;
	clj_release(t38);
	(void)t33;
	if (t45 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	if (o0) clj_release(t0);
	goto L2;
	}
	t31 = t45;
	}
	clj_value t46 = l0;
	clj_value a8[2] = {t31, t46};
	(void)a8;
	clj_value t47 = clj_c_invoke(t27, a8, 2);
	(void)t46;
	clj_release(t31);
	if (o27) clj_release(t27);
	if (t47 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	t24 = t47;
	} else {
#line 95 "<embedded>/clojure/test.clj"
	clj_value t48 = clj_retain(l0);
	t24 = t48;
	}
	clj_value a0[1] = {t24};
	(void)a0;
	clj_value t49 = clj_c_invoke(t0, a0, 1);
	clj_release(t24);
	if (o0) clj_release(t0);
	if (t49 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[15], &cc);
	clj_release(l1);
	clj_release(l2);
	clj_release(l3);
	clj_release(l4);
	return t49;
L2: ;
	clj_c_leave(&S[15], &cc);
L1: ;
	clj_release(l1);
	clj_release(l2);
	clj_release(l3);
	clj_release(l4);
	return CLJ_THROWN;
}

static clj_value clojure_test_do_report(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_do_report_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_35(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 91 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[103], clojure_test_do_report, NULL, 0, 0x2, 1, 1);
	clj_var_bind_root(V[46], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[109]);
	clj_value t2 = clj_c_def(V[46], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_function_QMARK__a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = CLJ_NIL;
	clj_value l2 = CLJ_NIL;
	clj_value l3 = CLJ_NIL;
	clj_value l4 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[16], &cc)) goto L1;
#line 105 "<embedded>/clojure/test.clj"
	clj_value t0 = l0;
	clj_value a0[1] = {t0};
	(void)a0;
	clj_value t1;
	if (CLJC_GUARD(V[51], B[7])) {
	t1 = clj_symbol_p(t0);
	} else {
	t1 = clj_c_intrinsic_fallback(V[51], a0, 1);
	}
	(void)t0;
	if (t1 == CLJ_THROWN) {
	goto L2;
	}
	bool t2 = clj_truthy(t1);
	clj_release(t1);
	clj_value t3;
	if (t2) {
#line 106 "<embedded>/clojure/test.clj"
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[52], &o4);
	if (t4 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t5 = l0;
	clj_value a1[1] = {t5};
	(void)a1;
	clj_value t6 = clj_c_invoke(t4, a1, 1);
	(void)t5;
	if (o4) clj_release(t4);
	if (t6 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l1, t6);
	clj_value t7 = l1;
	bool t8 = clj_truthy(t7);
	(void)t7;
	clj_value t9;
	if (t8) {
	clj_value t10 = l1;
	l1 = CLJ_NIL;
	clj_c_rebind(&l2, t10);
#line 107 "<embedded>/clojure/test.clj"
	bool o11;
	clj_value t11 = clj_c_var_borrow(V[33], &o11);
	if (t11 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t12 = l2;
	clj_value a2[1] = {t12};
	(void)a2;
	clj_value t13 = clj_c_invoke(t11, a2, 1);
	(void)t12;
	if (o11) clj_release(t11);
	if (t13 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l3, t13);
	clj_value t14 = l3;
	bool t15 = clj_truthy(t14);
	(void)t14;
	clj_value t16;
	if (t15) {
	clj_value t17 = K[112];
	bool o18;
	clj_value t18 = clj_c_var_borrow(V[24], &o18);
	if (t18 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t19 = l2;
	clj_value a5[1] = {t19};
	(void)a5;
	clj_value t20 = clj_c_invoke(t18, a5, 1);
	(void)t19;
	if (o18) clj_release(t18);
	if (t20 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a4[1] = {t20};
	(void)a4;
	clj_value t21 = clj_c_invoke(t17, a4, 1);
	clj_release(t20);
	(void)t17;
	if (t21 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a3[1] = {t21};
	(void)a3;
	clj_value t22;
	if (CLJC_GUARD(V[53], B[8])) {
	t22 = clj_not(t21);
	} else {
	t22 = clj_c_intrinsic_fallback(V[53], a3, 1);
	}
	clj_release(t21);
	if (t22 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l4, t22);
	clj_value t23 = l4;
	bool t24 = clj_truthy(t23);
	(void)t23;
	clj_value t25;
	if (t24) {
	bool o26;
	clj_value t26 = clj_c_var_borrow(V[55], &o26);
	if (t26 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t27 = l2;
	clj_value a7[1] = {t27};
	(void)a7;
	clj_value t28 = clj_c_invoke(t26, a7, 1);
	(void)t27;
	if (o26) clj_release(t26);
	if (t28 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a6[1] = {t28};
	(void)a6;
	clj_value t29;
	if (CLJC_GUARD(V[54], B[9])) {
	t29 = clj_fn_p(t28);
	} else {
	t29 = clj_c_intrinsic_fallback(V[54], a6, 1);
	}
	clj_release(t28);
	if (t29 == CLJ_THROWN) {
	goto L2;
	}
	t25 = t29;
	} else {
	clj_value t30 = l4;
	l4 = CLJ_NIL;
	t25 = t30;
	}
	t16 = t25;
	} else {
	clj_value t31 = l3;
	l3 = CLJ_NIL;
	t16 = t31;
	}
	t9 = t16;
	} else {
	t9 = CLJ_NIL;
	}
	t3 = t9;
	} else {
#line 108 "<embedded>/clojure/test.clj"
	clj_value t32 = l0;
	clj_value a8[1] = {t32};
	(void)a8;
	clj_value t33;
	if (CLJC_GUARD(V[54], B[9])) {
	t33 = clj_fn_p(t32);
	} else {
	t33 = clj_c_intrinsic_fallback(V[54], a8, 1);
	}
	(void)t32;
	if (t33 == CLJ_THROWN) {
	goto L2;
	}
	t3 = t33;
	}
	clj_c_leave(&S[16], &cc);
	clj_release(l1);
	clj_release(l2);
	clj_release(l3);
	clj_release(l4);
	return t3;
L2: ;
	clj_c_leave(&S[16], &cc);
L1: ;
	clj_release(l1);
	clj_release(l2);
	clj_release(l3);
	clj_release(l4);
	return CLJ_THROWN;
}

static clj_value clojure_test_function_QMARK_(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_function_QMARK__a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_36(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 102 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[111], clojure_test_function_QMARK_, NULL, 0, 0x2, 1, 1);
	clj_var_bind_root(V[50], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[113]);
	clj_value t2 = clj_c_def(V[50], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_assert_predicate_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_value l2 = CLJ_NIL;
	clj_value l3 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[17], &cc)) goto L1;
#line 114 "<embedded>/clojure/test.clj"
	clj_value t0 = l1;
	clj_value a0[1] = {t0};
	(void)a0;
	clj_value t1;
	if (CLJC_GUARD(V[57], B[10])) {
	t1 = clj_rest(t0);
	} else {
	t1 = clj_c_intrinsic_fallback(V[57], a0, 1);
	}
	(void)t0;
	if (t1 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l2, t1);
#line 115 "<embedded>/clojure/test.clj"
	clj_value t2 = l1;
	clj_value a1[1] = {t2};
	(void)a1;
	clj_value t3;
	if (CLJC_GUARD(V[19], B[3])) {
	t3 = clj_first(t2);
	} else {
	t3 = clj_c_intrinsic_fallback(V[19], a1, 1);
	}
	(void)t2;
	if (t3 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l3, t3);
#line 114 "<embedded>/clojure/test.clj"
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[13], &o4);
	if (t4 == CLJ_THROWN) {
	goto L2;
	}
	bool o5;
	clj_value t5 = clj_c_var_borrow(V[8], &o5);
	if (t5 == CLJ_THROWN) {
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t6 = K[116];
	clj_value a4[1] = {t6};
	(void)a4;
	clj_value t7 = clj_c_invoke(t5, a4, 1);
	(void)t6;
	if (o5) clj_release(t5);
	if (t7 == CLJ_THROWN) {
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[8], &o8);
	if (t8 == CLJ_THROWN) {
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o9;
	clj_value t9 = clj_c_var_borrow(V[17], &o9);
	if (t9 == CLJ_THROWN) {
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o10;
	clj_value t10 = clj_c_var_borrow(V[58], &o10);
	if (t10 == CLJ_THROWN) {
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o11;
	clj_value t11 = clj_c_var_borrow(V[13], &o11);
	if (t11 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o12;
	clj_value t12 = clj_c_var_borrow(V[8], &o12);
	if (t12 == CLJ_THROWN) {
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t13 = K[117];
	clj_value a9[1] = {t13};
	(void)a9;
	clj_value t14 = clj_c_invoke(t12, a9, 1);
	(void)t13;
	if (o12) clj_release(t12);
	if (t14 == CLJ_THROWN) {
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o15;
	clj_value t15 = clj_c_var_borrow(V[8], &o15);
	if (t15 == CLJ_THROWN) {
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o16;
	clj_value t16 = clj_c_var_borrow(V[13], &o16);
	if (t16 == CLJ_THROWN) {
	if (o15) clj_release(t15);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o17;
	clj_value t17 = clj_c_var_borrow(V[8], &o17);
	if (t17 == CLJ_THROWN) {
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t18 = K[118];
	clj_value a13[1] = {t18};
	(void)a13;
	clj_value t19 = clj_c_invoke(t17, a13, 1);
	(void)t18;
	if (o17) clj_release(t17);
	if (t19 == CLJ_THROWN) {
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t20 = l2;
	clj_value a12[2] = {t19, t20};
	(void)a12;
	clj_value t21;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t21 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a12, 2) : clj_c_invoke(t16, a12, 2);
#else
	t21 = clj_c_invoke(t16, a12, 2);
#endif
	(void)t20;
	clj_release(t19);
	if (o16) clj_release(t16);
	if (t21 == CLJ_THROWN) {
	if (o15) clj_release(t15);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a11[1] = {t21};
	(void)a11;
	clj_value t22;
	if (CLJC_GUARD(V[12], B[0])) {
	t22 = clj_seq(t21);
	} else {
	t22 = clj_c_intrinsic_fallback(V[12], a11, 1);
	}
	clj_release(t21);
	if (t22 == CLJ_THROWN) {
	if (o15) clj_release(t15);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a10[1] = {t22};
	(void)a10;
	clj_value t23 = clj_c_invoke(t15, a10, 1);
	clj_release(t22);
	if (o15) clj_release(t15);
	if (t23 == CLJ_THROWN) {
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o24;
	clj_value t24 = clj_c_var_borrow(V[8], &o24);
	if (t24 == CLJ_THROWN) {
	clj_release(t23);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t25 = K[119];
	clj_value a14[1] = {t25};
	(void)a14;
	clj_value t26 = clj_c_invoke(t24, a14, 1);
	(void)t25;
	if (o24) clj_release(t24);
	if (t26 == CLJ_THROWN) {
	clj_release(t23);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o27;
	clj_value t27 = clj_c_var_borrow(V[8], &o27);
	if (t27 == CLJ_THROWN) {
	clj_release(t26);
	clj_release(t23);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o28;
	clj_value t28 = clj_c_var_borrow(V[13], &o28);
	if (t28 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	clj_release(t26);
	clj_release(t23);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o29;
	clj_value t29 = clj_c_var_borrow(V[8], &o29);
	if (t29 == CLJ_THROWN) {
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	clj_release(t26);
	clj_release(t23);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t30 = K[120];
	clj_value a18[1] = {t30};
	(void)a18;
	clj_value t31 = clj_c_invoke(t29, a18, 1);
	(void)t30;
	if (o29) clj_release(t29);
	if (t31 == CLJ_THROWN) {
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	clj_release(t26);
	clj_release(t23);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o32;
	clj_value t32 = clj_c_var_borrow(V[8], &o32);
	if (t32 == CLJ_THROWN) {
	clj_release(t31);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	clj_release(t26);
	clj_release(t23);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t33 = l3;
	clj_value a19[1] = {t33};
	(void)a19;
	clj_value t34 = clj_c_invoke(t32, a19, 1);
	(void)t33;
	if (o32) clj_release(t32);
	if (t34 == CLJ_THROWN) {
	clj_release(t31);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	clj_release(t26);
	clj_release(t23);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o35;
	clj_value t35 = clj_c_var_borrow(V[8], &o35);
	if (t35 == CLJ_THROWN) {
	clj_release(t34);
	clj_release(t31);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	clj_release(t26);
	clj_release(t23);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t36 = K[117];
	clj_value a20[1] = {t36};
	(void)a20;
	clj_value t37 = clj_c_invoke(t35, a20, 1);
	(void)t36;
	if (o35) clj_release(t35);
	if (t37 == CLJ_THROWN) {
	clj_release(t34);
	clj_release(t31);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	clj_release(t26);
	clj_release(t23);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a17[3] = {t31, t34, t37};
	(void)a17;
	clj_value t38 = clj_c_invoke(t28, a17, 3);
	clj_release(t37);
	clj_release(t34);
	clj_release(t31);
	if (o28) clj_release(t28);
	if (t38 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	clj_release(t26);
	clj_release(t23);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a16[1] = {t38};
	(void)a16;
	clj_value t39;
	if (CLJC_GUARD(V[12], B[0])) {
	t39 = clj_seq(t38);
	} else {
	t39 = clj_c_intrinsic_fallback(V[12], a16, 1);
	}
	clj_release(t38);
	if (t39 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	clj_release(t26);
	clj_release(t23);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a15[1] = {t39};
	(void)a15;
	clj_value t40 = clj_c_invoke(t27, a15, 1);
	clj_release(t39);
	if (o27) clj_release(t27);
	if (t40 == CLJ_THROWN) {
	clj_release(t26);
	clj_release(t23);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a8[4] = {t14, t23, t26, t40};
	(void)a8;
	clj_value t41 = clj_c_invoke(t11, a8, 4);
	clj_release(t40);
	clj_release(t26);
	clj_release(t23);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (t41 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a7[1] = {t41};
	(void)a7;
	clj_value t42;
	if (CLJC_GUARD(V[12], B[0])) {
	t42 = clj_seq(t41);
	} else {
	t42 = clj_c_intrinsic_fallback(V[12], a7, 1);
	}
	clj_release(t41);
	if (t42 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a6[2] = {t10, t42};
	(void)a6;
	clj_value t43 = clj_c_invoke(t9, a6, 2);
	clj_release(t42);
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	if (t43 == CLJ_THROWN) {
	if (o8) clj_release(t8);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a5[1] = {t43};
	(void)a5;
	clj_value t44 = clj_c_invoke(t8, a5, 1);
	clj_release(t43);
	if (o8) clj_release(t8);
	if (t44 == CLJ_THROWN) {
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o45;
	clj_value t45 = clj_c_var_borrow(V[8], &o45);
	if (t45 == CLJ_THROWN) {
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o46;
	clj_value t46 = clj_c_var_borrow(V[13], &o46);
	if (t46 == CLJ_THROWN) {
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o47;
	clj_value t47 = clj_c_var_borrow(V[8], &o47);
	if (t47 == CLJ_THROWN) {
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t48 = K[121];
	clj_value a24[1] = {t48};
	(void)a24;
	clj_value t49 = clj_c_invoke(t47, a24, 1);
	(void)t48;
	if (o47) clj_release(t47);
	if (t49 == CLJ_THROWN) {
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o50;
	clj_value t50 = clj_c_var_borrow(V[8], &o50);
	if (t50 == CLJ_THROWN) {
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t51 = K[119];
	clj_value a25[1] = {t51};
	(void)a25;
	clj_value t52 = clj_c_invoke(t50, a25, 1);
	(void)t51;
	if (o50) clj_release(t50);
	if (t52 == CLJ_THROWN) {
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o53;
	clj_value t53 = clj_c_var_borrow(V[8], &o53);
	if (t53 == CLJ_THROWN) {
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o54;
	clj_value t54 = clj_c_var_borrow(V[13], &o54);
	if (t54 == CLJ_THROWN) {
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o55;
	clj_value t55 = clj_c_var_borrow(V[8], &o55);
	if (t55 == CLJ_THROWN) {
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t56 = K[122];
	clj_value a29[1] = {t56};
	(void)a29;
	clj_value t57 = clj_c_invoke(t55, a29, 1);
	(void)t56;
	if (o55) clj_release(t55);
	if (t57 == CLJ_THROWN) {
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o58;
	clj_value t58 = clj_c_var_borrow(V[8], &o58);
	if (t58 == CLJ_THROWN) {
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o59;
	clj_value t59 = clj_c_var_borrow(V[17], &o59);
	if (t59 == CLJ_THROWN) {
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o60;
	clj_value t60 = clj_c_var_borrow(V[18], &o60);
	if (t60 == CLJ_THROWN) {
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o61;
	clj_value t61 = clj_c_var_borrow(V[13], &o61);
	if (t61 == CLJ_THROWN) {
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o62;
	clj_value t62 = clj_c_var_borrow(V[8], &o62);
	if (t62 == CLJ_THROWN) {
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t63 = K[123];
	clj_value a34[1] = {t63};
	(void)a34;
	clj_value t64 = clj_c_invoke(t62, a34, 1);
	(void)t63;
	if (o62) clj_release(t62);
	if (t64 == CLJ_THROWN) {
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o65;
	clj_value t65 = clj_c_var_borrow(V[8], &o65);
	if (t65 == CLJ_THROWN) {
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o66;
	clj_value t66 = clj_c_var_borrow(V[13], &o66);
	if (t66 == CLJ_THROWN) {
	if (o65) clj_release(t65);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o67;
	clj_value t67 = clj_c_var_borrow(V[8], &o67);
	if (t67 == CLJ_THROWN) {
	if (o66) clj_release(t66);
	if (o65) clj_release(t65);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t68 = K[124];
	clj_value a38[1] = {t68};
	(void)a38;
	clj_value t69 = clj_c_invoke(t67, a38, 1);
	(void)t68;
	if (o67) clj_release(t67);
	if (t69 == CLJ_THROWN) {
	if (o66) clj_release(t66);
	if (o65) clj_release(t65);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o70;
	clj_value t70 = clj_c_var_borrow(V[8], &o70);
	if (t70 == CLJ_THROWN) {
	clj_release(t69);
	if (o66) clj_release(t66);
	if (o65) clj_release(t65);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t71 = l1;
	clj_value a39[1] = {t71};
	(void)a39;
	clj_value t72 = clj_c_invoke(t70, a39, 1);
	(void)t71;
	if (o70) clj_release(t70);
	if (t72 == CLJ_THROWN) {
	clj_release(t69);
	if (o66) clj_release(t66);
	if (o65) clj_release(t65);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a37[2] = {t69, t72};
	(void)a37;
	clj_value t73;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t73 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a37, 2) : clj_c_invoke(t66, a37, 2);
#else
	t73 = clj_c_invoke(t66, a37, 2);
#endif
	clj_release(t72);
	clj_release(t69);
	if (o66) clj_release(t66);
	if (t73 == CLJ_THROWN) {
	if (o65) clj_release(t65);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a36[1] = {t73};
	(void)a36;
	clj_value t74;
	if (CLJC_GUARD(V[12], B[0])) {
	t74 = clj_seq(t73);
	} else {
	t74 = clj_c_intrinsic_fallback(V[12], a36, 1);
	}
	clj_release(t73);
	if (t74 == CLJ_THROWN) {
	if (o65) clj_release(t65);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a35[1] = {t74};
	(void)a35;
	clj_value t75 = clj_c_invoke(t65, a35, 1);
	clj_release(t74);
	if (o65) clj_release(t65);
	if (t75 == CLJ_THROWN) {
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o76;
	clj_value t76 = clj_c_var_borrow(V[8], &o76);
	if (t76 == CLJ_THROWN) {
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t77 = K[125];
	clj_value a40[1] = {t77};
	(void)a40;
	clj_value t78 = clj_c_invoke(t76, a40, 1);
	(void)t77;
	if (o76) clj_release(t76);
	if (t78 == CLJ_THROWN) {
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o79;
	clj_value t79 = clj_c_var_borrow(V[8], &o79);
	if (t79 == CLJ_THROWN) {
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t80 = K[126];
	clj_value a41[1] = {t80};
	(void)a41;
	clj_value t81 = clj_c_invoke(t79, a41, 1);
	(void)t80;
	if (o79) clj_release(t79);
	if (t81 == CLJ_THROWN) {
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o82;
	clj_value t82 = clj_c_var_borrow(V[8], &o82);
	if (t82 == CLJ_THROWN) {
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t83 = K[127];
	clj_value a42[1] = {t83};
	(void)a42;
	clj_value t84 = clj_c_invoke(t82, a42, 1);
	(void)t83;
	if (o82) clj_release(t82);
	if (t84 == CLJ_THROWN) {
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o85;
	clj_value t85 = clj_c_var_borrow(V[8], &o85);
	if (t85 == CLJ_THROWN) {
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o86;
	clj_value t86 = clj_c_var_borrow(V[13], &o86);
	if (t86 == CLJ_THROWN) {
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o87;
	clj_value t87 = clj_c_var_borrow(V[8], &o87);
	if (t87 == CLJ_THROWN) {
	if (o86) clj_release(t86);
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t88 = K[128];
	clj_value a46[1] = {t88};
	(void)a46;
	clj_value t89 = clj_c_invoke(t87, a46, 1);
	(void)t88;
	if (o87) clj_release(t87);
	if (t89 == CLJ_THROWN) {
	if (o86) clj_release(t86);
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o90;
	clj_value t90 = clj_c_var_borrow(V[8], &o90);
	if (t90 == CLJ_THROWN) {
	clj_release(t89);
	if (o86) clj_release(t86);
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o91;
	clj_value t91 = clj_c_var_borrow(V[13], &o91);
	if (t91 == CLJ_THROWN) {
	if (o90) clj_release(t90);
	clj_release(t89);
	if (o86) clj_release(t86);
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o92;
	clj_value t92 = clj_c_var_borrow(V[8], &o92);
	if (t92 == CLJ_THROWN) {
	if (o91) clj_release(t91);
	if (o90) clj_release(t90);
	clj_release(t89);
	if (o86) clj_release(t86);
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t93 = K[129];
	clj_value a50[1] = {t93};
	(void)a50;
	clj_value t94 = clj_c_invoke(t92, a50, 1);
	(void)t93;
	if (o92) clj_release(t92);
	if (t94 == CLJ_THROWN) {
	if (o91) clj_release(t91);
	if (o90) clj_release(t90);
	clj_release(t89);
	if (o86) clj_release(t86);
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o95;
	clj_value t95 = clj_c_var_borrow(V[8], &o95);
	if (t95 == CLJ_THROWN) {
	clj_release(t94);
	if (o91) clj_release(t91);
	if (o90) clj_release(t90);
	clj_release(t89);
	if (o86) clj_release(t86);
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t96 = l3;
	clj_value a51[1] = {t96};
	(void)a51;
	clj_value t97 = clj_c_invoke(t95, a51, 1);
	(void)t96;
	if (o95) clj_release(t95);
	if (t97 == CLJ_THROWN) {
	clj_release(t94);
	if (o91) clj_release(t91);
	if (o90) clj_release(t90);
	clj_release(t89);
	if (o86) clj_release(t86);
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a49[2] = {t94, t97};
	(void)a49;
	clj_value t98;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t98 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a49, 2) : clj_c_invoke(t91, a49, 2);
#else
	t98 = clj_c_invoke(t91, a49, 2);
#endif
	clj_release(t97);
	clj_release(t94);
	if (o91) clj_release(t91);
	if (t98 == CLJ_THROWN) {
	if (o90) clj_release(t90);
	clj_release(t89);
	if (o86) clj_release(t86);
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a48[1] = {t98};
	(void)a48;
	clj_value t99;
	if (CLJC_GUARD(V[12], B[0])) {
	t99 = clj_seq(t98);
	} else {
	t99 = clj_c_intrinsic_fallback(V[12], a48, 1);
	}
	clj_release(t98);
	if (t99 == CLJ_THROWN) {
	if (o90) clj_release(t90);
	clj_release(t89);
	if (o86) clj_release(t86);
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a47[1] = {t99};
	(void)a47;
	clj_value t100 = clj_c_invoke(t90, a47, 1);
	clj_release(t99);
	if (o90) clj_release(t90);
	if (t100 == CLJ_THROWN) {
	clj_release(t89);
	if (o86) clj_release(t86);
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o101;
	clj_value t101 = clj_c_var_borrow(V[8], &o101);
	if (t101 == CLJ_THROWN) {
	clj_release(t100);
	clj_release(t89);
	if (o86) clj_release(t86);
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t102 = K[117];
	clj_value a52[1] = {t102};
	(void)a52;
	clj_value t103 = clj_c_invoke(t101, a52, 1);
	(void)t102;
	if (o101) clj_release(t101);
	if (t103 == CLJ_THROWN) {
	clj_release(t100);
	clj_release(t89);
	if (o86) clj_release(t86);
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a45[3] = {t89, t100, t103};
	(void)a45;
	clj_value t104 = clj_c_invoke(t86, a45, 3);
	clj_release(t103);
	clj_release(t100);
	clj_release(t89);
	if (o86) clj_release(t86);
	if (t104 == CLJ_THROWN) {
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a44[1] = {t104};
	(void)a44;
	clj_value t105;
	if (CLJC_GUARD(V[12], B[0])) {
	t105 = clj_seq(t104);
	} else {
	t105 = clj_c_intrinsic_fallback(V[12], a44, 1);
	}
	clj_release(t104);
	if (t105 == CLJ_THROWN) {
	if (o85) clj_release(t85);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a43[1] = {t105};
	(void)a43;
	clj_value t106 = clj_c_invoke(t85, a43, 1);
	clj_release(t105);
	if (o85) clj_release(t85);
	if (t106 == CLJ_THROWN) {
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o107;
	clj_value t107 = clj_c_var_borrow(V[8], &o107);
	if (t107 == CLJ_THROWN) {
	clj_release(t106);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t108 = K[130];
	clj_value a53[1] = {t108};
	(void)a53;
	clj_value t109 = clj_c_invoke(t107, a53, 1);
	(void)t108;
	if (o107) clj_release(t107);
	if (t109 == CLJ_THROWN) {
	clj_release(t106);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o110;
	clj_value t110 = clj_c_var_borrow(V[8], &o110);
	if (t110 == CLJ_THROWN) {
	clj_release(t109);
	clj_release(t106);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t111 = l0;
	clj_value a54[1] = {t111};
	(void)a54;
	clj_value t112 = clj_c_invoke(t110, a54, 1);
	(void)t111;
	if (o110) clj_release(t110);
	if (t112 == CLJ_THROWN) {
	clj_release(t109);
	clj_release(t106);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a33[8] = {t64, t75, t78, t81, t84, t106, t109, t112};
	(void)a33;
	clj_value t113 = clj_c_invoke(t61, a33, 8);
	clj_release(t112);
	clj_release(t109);
	clj_release(t106);
	clj_release(t84);
	clj_release(t81);
	clj_release(t78);
	clj_release(t75);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (t113 == CLJ_THROWN) {
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a32[1] = {t113};
	(void)a32;
	clj_value t114;
	if (CLJC_GUARD(V[12], B[0])) {
	t114 = clj_seq(t113);
	} else {
	t114 = clj_c_intrinsic_fallback(V[12], a32, 1);
	}
	clj_release(t113);
	if (t114 == CLJ_THROWN) {
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a31[2] = {t60, t114};
	(void)a31;
	clj_value t115 = clj_c_invoke(t59, a31, 2);
	clj_release(t114);
	if (o60) clj_release(t60);
	if (o59) clj_release(t59);
	if (t115 == CLJ_THROWN) {
	if (o58) clj_release(t58);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a30[1] = {t115};
	(void)a30;
	clj_value t116 = clj_c_invoke(t58, a30, 1);
	clj_release(t115);
	if (o58) clj_release(t58);
	if (t116 == CLJ_THROWN) {
	clj_release(t57);
	if (o54) clj_release(t54);
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a28[2] = {t57, t116};
	(void)a28;
	clj_value t117;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t117 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a28, 2) : clj_c_invoke(t54, a28, 2);
#else
	t117 = clj_c_invoke(t54, a28, 2);
#endif
	clj_release(t116);
	clj_release(t57);
	if (o54) clj_release(t54);
	if (t117 == CLJ_THROWN) {
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a27[1] = {t117};
	(void)a27;
	clj_value t118;
	if (CLJC_GUARD(V[12], B[0])) {
	t118 = clj_seq(t117);
	} else {
	t118 = clj_c_intrinsic_fallback(V[12], a27, 1);
	}
	clj_release(t117);
	if (t118 == CLJ_THROWN) {
	if (o53) clj_release(t53);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a26[1] = {t118};
	(void)a26;
	clj_value t119 = clj_c_invoke(t53, a26, 1);
	clj_release(t118);
	if (o53) clj_release(t53);
	if (t119 == CLJ_THROWN) {
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o120;
	clj_value t120 = clj_c_var_borrow(V[8], &o120);
	if (t120 == CLJ_THROWN) {
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o121;
	clj_value t121 = clj_c_var_borrow(V[13], &o121);
	if (t121 == CLJ_THROWN) {
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o122;
	clj_value t122 = clj_c_var_borrow(V[8], &o122);
	if (t122 == CLJ_THROWN) {
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t123 = K[131];
	clj_value a58[1] = {t123};
	(void)a58;
	clj_value t124 = clj_c_invoke(t122, a58, 1);
	(void)t123;
	if (o122) clj_release(t122);
	if (t124 == CLJ_THROWN) {
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o125;
	clj_value t125 = clj_c_var_borrow(V[8], &o125);
	if (t125 == CLJ_THROWN) {
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o126;
	clj_value t126 = clj_c_var_borrow(V[17], &o126);
	if (t126 == CLJ_THROWN) {
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o127;
	clj_value t127 = clj_c_var_borrow(V[18], &o127);
	if (t127 == CLJ_THROWN) {
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o128;
	clj_value t128 = clj_c_var_borrow(V[13], &o128);
	if (t128 == CLJ_THROWN) {
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o129;
	clj_value t129 = clj_c_var_borrow(V[8], &o129);
	if (t129 == CLJ_THROWN) {
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t130 = K[123];
	clj_value a63[1] = {t130};
	(void)a63;
	clj_value t131 = clj_c_invoke(t129, a63, 1);
	(void)t130;
	if (o129) clj_release(t129);
	if (t131 == CLJ_THROWN) {
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o132;
	clj_value t132 = clj_c_var_borrow(V[8], &o132);
	if (t132 == CLJ_THROWN) {
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o133;
	clj_value t133 = clj_c_var_borrow(V[13], &o133);
	if (t133 == CLJ_THROWN) {
	if (o132) clj_release(t132);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o134;
	clj_value t134 = clj_c_var_borrow(V[8], &o134);
	if (t134 == CLJ_THROWN) {
	if (o133) clj_release(t133);
	if (o132) clj_release(t132);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t135 = K[132];
	clj_value a67[1] = {t135};
	(void)a67;
	clj_value t136 = clj_c_invoke(t134, a67, 1);
	(void)t135;
	if (o134) clj_release(t134);
	if (t136 == CLJ_THROWN) {
	if (o133) clj_release(t133);
	if (o132) clj_release(t132);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o137;
	clj_value t137 = clj_c_var_borrow(V[8], &o137);
	if (t137 == CLJ_THROWN) {
	clj_release(t136);
	if (o133) clj_release(t133);
	if (o132) clj_release(t132);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t138 = l1;
	clj_value a68[1] = {t138};
	(void)a68;
	clj_value t139 = clj_c_invoke(t137, a68, 1);
	(void)t138;
	if (o137) clj_release(t137);
	if (t139 == CLJ_THROWN) {
	clj_release(t136);
	if (o133) clj_release(t133);
	if (o132) clj_release(t132);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a66[2] = {t136, t139};
	(void)a66;
	clj_value t140;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t140 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a66, 2) : clj_c_invoke(t133, a66, 2);
#else
	t140 = clj_c_invoke(t133, a66, 2);
#endif
	clj_release(t139);
	clj_release(t136);
	if (o133) clj_release(t133);
	if (t140 == CLJ_THROWN) {
	if (o132) clj_release(t132);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a65[1] = {t140};
	(void)a65;
	clj_value t141;
	if (CLJC_GUARD(V[12], B[0])) {
	t141 = clj_seq(t140);
	} else {
	t141 = clj_c_intrinsic_fallback(V[12], a65, 1);
	}
	clj_release(t140);
	if (t141 == CLJ_THROWN) {
	if (o132) clj_release(t132);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a64[1] = {t141};
	(void)a64;
	clj_value t142 = clj_c_invoke(t132, a64, 1);
	clj_release(t141);
	if (o132) clj_release(t132);
	if (t142 == CLJ_THROWN) {
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o143;
	clj_value t143 = clj_c_var_borrow(V[8], &o143);
	if (t143 == CLJ_THROWN) {
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t144 = K[125];
	clj_value a69[1] = {t144};
	(void)a69;
	clj_value t145 = clj_c_invoke(t143, a69, 1);
	(void)t144;
	if (o143) clj_release(t143);
	if (t145 == CLJ_THROWN) {
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o146;
	clj_value t146 = clj_c_var_borrow(V[8], &o146);
	if (t146 == CLJ_THROWN) {
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t147 = K[133];
	clj_value a70[1] = {t147};
	(void)a70;
	clj_value t148 = clj_c_invoke(t146, a70, 1);
	(void)t147;
	if (o146) clj_release(t146);
	if (t148 == CLJ_THROWN) {
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o149;
	clj_value t149 = clj_c_var_borrow(V[8], &o149);
	if (t149 == CLJ_THROWN) {
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t150 = K[127];
	clj_value a71[1] = {t150};
	(void)a71;
	clj_value t151 = clj_c_invoke(t149, a71, 1);
	(void)t150;
	if (o149) clj_release(t149);
	if (t151 == CLJ_THROWN) {
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o152;
	clj_value t152 = clj_c_var_borrow(V[8], &o152);
	if (t152 == CLJ_THROWN) {
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o153;
	clj_value t153 = clj_c_var_borrow(V[13], &o153);
	if (t153 == CLJ_THROWN) {
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o154;
	clj_value t154 = clj_c_var_borrow(V[8], &o154);
	if (t154 == CLJ_THROWN) {
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t155 = K[134];
	clj_value a75[1] = {t155};
	(void)a75;
	clj_value t156 = clj_c_invoke(t154, a75, 1);
	(void)t155;
	if (o154) clj_release(t154);
	if (t156 == CLJ_THROWN) {
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o157;
	clj_value t157 = clj_c_var_borrow(V[8], &o157);
	if (t157 == CLJ_THROWN) {
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o158;
	clj_value t158 = clj_c_var_borrow(V[13], &o158);
	if (t158 == CLJ_THROWN) {
	if (o157) clj_release(t157);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o159;
	clj_value t159 = clj_c_var_borrow(V[8], &o159);
	if (t159 == CLJ_THROWN) {
	if (o158) clj_release(t158);
	if (o157) clj_release(t157);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t160 = K[135];
	clj_value a79[1] = {t160};
	(void)a79;
	clj_value t161 = clj_c_invoke(t159, a79, 1);
	(void)t160;
	if (o159) clj_release(t159);
	if (t161 == CLJ_THROWN) {
	if (o158) clj_release(t158);
	if (o157) clj_release(t157);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o162;
	clj_value t162 = clj_c_var_borrow(V[8], &o162);
	if (t162 == CLJ_THROWN) {
	clj_release(t161);
	if (o158) clj_release(t158);
	if (o157) clj_release(t157);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t163 = K[136];
	clj_value a80[1] = {t163};
	(void)a80;
	clj_value t164 = clj_c_invoke(t162, a80, 1);
	(void)t163;
	if (o162) clj_release(t162);
	if (t164 == CLJ_THROWN) {
	clj_release(t161);
	if (o158) clj_release(t158);
	if (o157) clj_release(t157);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a78[2] = {t161, t164};
	(void)a78;
	clj_value t165;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t165 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a78, 2) : clj_c_invoke(t158, a78, 2);
#else
	t165 = clj_c_invoke(t158, a78, 2);
#endif
	clj_release(t164);
	clj_release(t161);
	if (o158) clj_release(t158);
	if (t165 == CLJ_THROWN) {
	if (o157) clj_release(t157);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a77[1] = {t165};
	(void)a77;
	clj_value t166;
	if (CLJC_GUARD(V[12], B[0])) {
	t166 = clj_seq(t165);
	} else {
	t166 = clj_c_intrinsic_fallback(V[12], a77, 1);
	}
	clj_release(t165);
	if (t166 == CLJ_THROWN) {
	if (o157) clj_release(t157);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a76[1] = {t166};
	(void)a76;
	clj_value t167 = clj_c_invoke(t157, a76, 1);
	clj_release(t166);
	if (o157) clj_release(t157);
	if (t167 == CLJ_THROWN) {
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o168;
	clj_value t168 = clj_c_var_borrow(V[8], &o168);
	if (t168 == CLJ_THROWN) {
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o169;
	clj_value t169 = clj_c_var_borrow(V[13], &o169);
	if (t169 == CLJ_THROWN) {
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o170;
	clj_value t170 = clj_c_var_borrow(V[8], &o170);
	if (t170 == CLJ_THROWN) {
	if (o169) clj_release(t169);
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t171 = K[137];
	clj_value a84[1] = {t171};
	(void)a84;
	clj_value t172 = clj_c_invoke(t170, a84, 1);
	(void)t171;
	if (o170) clj_release(t170);
	if (t172 == CLJ_THROWN) {
	if (o169) clj_release(t169);
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o173;
	clj_value t173 = clj_c_var_borrow(V[8], &o173);
	if (t173 == CLJ_THROWN) {
	clj_release(t172);
	if (o169) clj_release(t169);
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o174;
	clj_value t174 = clj_c_var_borrow(V[13], &o174);
	if (t174 == CLJ_THROWN) {
	if (o173) clj_release(t173);
	clj_release(t172);
	if (o169) clj_release(t169);
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o175;
	clj_value t175 = clj_c_var_borrow(V[8], &o175);
	if (t175 == CLJ_THROWN) {
	if (o174) clj_release(t174);
	if (o173) clj_release(t173);
	clj_release(t172);
	if (o169) clj_release(t169);
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t176 = K[138];
	clj_value a88[1] = {t176};
	(void)a88;
	clj_value t177 = clj_c_invoke(t175, a88, 1);
	(void)t176;
	if (o175) clj_release(t175);
	if (t177 == CLJ_THROWN) {
	if (o174) clj_release(t174);
	if (o173) clj_release(t173);
	clj_release(t172);
	if (o169) clj_release(t169);
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o178;
	clj_value t178 = clj_c_var_borrow(V[8], &o178);
	if (t178 == CLJ_THROWN) {
	clj_release(t177);
	if (o174) clj_release(t174);
	if (o173) clj_release(t173);
	clj_release(t172);
	if (o169) clj_release(t169);
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t179 = l3;
	clj_value a89[1] = {t179};
	(void)a89;
	clj_value t180 = clj_c_invoke(t178, a89, 1);
	(void)t179;
	if (o178) clj_release(t178);
	if (t180 == CLJ_THROWN) {
	clj_release(t177);
	if (o174) clj_release(t174);
	if (o173) clj_release(t173);
	clj_release(t172);
	if (o169) clj_release(t169);
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a87[2] = {t177, t180};
	(void)a87;
	clj_value t181;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t181 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a87, 2) : clj_c_invoke(t174, a87, 2);
#else
	t181 = clj_c_invoke(t174, a87, 2);
#endif
	clj_release(t180);
	clj_release(t177);
	if (o174) clj_release(t174);
	if (t181 == CLJ_THROWN) {
	if (o173) clj_release(t173);
	clj_release(t172);
	if (o169) clj_release(t169);
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a86[1] = {t181};
	(void)a86;
	clj_value t182;
	if (CLJC_GUARD(V[12], B[0])) {
	t182 = clj_seq(t181);
	} else {
	t182 = clj_c_intrinsic_fallback(V[12], a86, 1);
	}
	clj_release(t181);
	if (t182 == CLJ_THROWN) {
	if (o173) clj_release(t173);
	clj_release(t172);
	if (o169) clj_release(t169);
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a85[1] = {t182};
	(void)a85;
	clj_value t183 = clj_c_invoke(t173, a85, 1);
	clj_release(t182);
	if (o173) clj_release(t173);
	if (t183 == CLJ_THROWN) {
	clj_release(t172);
	if (o169) clj_release(t169);
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o184;
	clj_value t184 = clj_c_var_borrow(V[8], &o184);
	if (t184 == CLJ_THROWN) {
	clj_release(t183);
	clj_release(t172);
	if (o169) clj_release(t169);
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t185 = K[117];
	clj_value a90[1] = {t185};
	(void)a90;
	clj_value t186 = clj_c_invoke(t184, a90, 1);
	(void)t185;
	if (o184) clj_release(t184);
	if (t186 == CLJ_THROWN) {
	clj_release(t183);
	clj_release(t172);
	if (o169) clj_release(t169);
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a83[3] = {t172, t183, t186};
	(void)a83;
	clj_value t187 = clj_c_invoke(t169, a83, 3);
	clj_release(t186);
	clj_release(t183);
	clj_release(t172);
	if (o169) clj_release(t169);
	if (t187 == CLJ_THROWN) {
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a82[1] = {t187};
	(void)a82;
	clj_value t188;
	if (CLJC_GUARD(V[12], B[0])) {
	t188 = clj_seq(t187);
	} else {
	t188 = clj_c_intrinsic_fallback(V[12], a82, 1);
	}
	clj_release(t187);
	if (t188 == CLJ_THROWN) {
	if (o168) clj_release(t168);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a81[1] = {t188};
	(void)a81;
	clj_value t189 = clj_c_invoke(t168, a81, 1);
	clj_release(t188);
	if (o168) clj_release(t168);
	if (t189 == CLJ_THROWN) {
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a74[3] = {t156, t167, t189};
	(void)a74;
	clj_value t190 = clj_c_invoke(t153, a74, 3);
	clj_release(t189);
	clj_release(t167);
	clj_release(t156);
	if (o153) clj_release(t153);
	if (t190 == CLJ_THROWN) {
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a73[1] = {t190};
	(void)a73;
	clj_value t191;
	if (CLJC_GUARD(V[12], B[0])) {
	t191 = clj_seq(t190);
	} else {
	t191 = clj_c_intrinsic_fallback(V[12], a73, 1);
	}
	clj_release(t190);
	if (t191 == CLJ_THROWN) {
	if (o152) clj_release(t152);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a72[1] = {t191};
	(void)a72;
	clj_value t192 = clj_c_invoke(t152, a72, 1);
	clj_release(t191);
	if (o152) clj_release(t152);
	if (t192 == CLJ_THROWN) {
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o193;
	clj_value t193 = clj_c_var_borrow(V[8], &o193);
	if (t193 == CLJ_THROWN) {
	clj_release(t192);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t194 = K[130];
	clj_value a91[1] = {t194};
	(void)a91;
	clj_value t195 = clj_c_invoke(t193, a91, 1);
	(void)t194;
	if (o193) clj_release(t193);
	if (t195 == CLJ_THROWN) {
	clj_release(t192);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o196;
	clj_value t196 = clj_c_var_borrow(V[8], &o196);
	if (t196 == CLJ_THROWN) {
	clj_release(t195);
	clj_release(t192);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t197 = l0;
	clj_value a92[1] = {t197};
	(void)a92;
	clj_value t198 = clj_c_invoke(t196, a92, 1);
	(void)t197;
	if (o196) clj_release(t196);
	if (t198 == CLJ_THROWN) {
	clj_release(t195);
	clj_release(t192);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a62[8] = {t131, t142, t145, t148, t151, t192, t195, t198};
	(void)a62;
	clj_value t199 = clj_c_invoke(t128, a62, 8);
	clj_release(t198);
	clj_release(t195);
	clj_release(t192);
	clj_release(t151);
	clj_release(t148);
	clj_release(t145);
	clj_release(t142);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (t199 == CLJ_THROWN) {
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a61[1] = {t199};
	(void)a61;
	clj_value t200;
	if (CLJC_GUARD(V[12], B[0])) {
	t200 = clj_seq(t199);
	} else {
	t200 = clj_c_intrinsic_fallback(V[12], a61, 1);
	}
	clj_release(t199);
	if (t200 == CLJ_THROWN) {
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a60[2] = {t127, t200};
	(void)a60;
	clj_value t201 = clj_c_invoke(t126, a60, 2);
	clj_release(t200);
	if (o127) clj_release(t127);
	if (o126) clj_release(t126);
	if (t201 == CLJ_THROWN) {
	if (o125) clj_release(t125);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a59[1] = {t201};
	(void)a59;
	clj_value t202 = clj_c_invoke(t125, a59, 1);
	clj_release(t201);
	if (o125) clj_release(t125);
	if (t202 == CLJ_THROWN) {
	clj_release(t124);
	if (o121) clj_release(t121);
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a57[2] = {t124, t202};
	(void)a57;
	clj_value t203;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t203 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a57, 2) : clj_c_invoke(t121, a57, 2);
#else
	t203 = clj_c_invoke(t121, a57, 2);
#endif
	clj_release(t202);
	clj_release(t124);
	if (o121) clj_release(t121);
	if (t203 == CLJ_THROWN) {
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a56[1] = {t203};
	(void)a56;
	clj_value t204;
	if (CLJC_GUARD(V[12], B[0])) {
	t204 = clj_seq(t203);
	} else {
	t204 = clj_c_intrinsic_fallback(V[12], a56, 1);
	}
	clj_release(t203);
	if (t204 == CLJ_THROWN) {
	if (o120) clj_release(t120);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a55[1] = {t204};
	(void)a55;
	clj_value t205 = clj_c_invoke(t120, a55, 1);
	clj_release(t204);
	if (o120) clj_release(t120);
	if (t205 == CLJ_THROWN) {
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a23[4] = {t49, t52, t119, t205};
	(void)a23;
	clj_value t206 = clj_c_invoke(t46, a23, 4);
	clj_release(t205);
	clj_release(t119);
	clj_release(t52);
	clj_release(t49);
	if (o46) clj_release(t46);
	if (t206 == CLJ_THROWN) {
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a22[1] = {t206};
	(void)a22;
	clj_value t207;
	if (CLJC_GUARD(V[12], B[0])) {
	t207 = clj_seq(t206);
	} else {
	t207 = clj_c_intrinsic_fallback(V[12], a22, 1);
	}
	clj_release(t206);
	if (t207 == CLJ_THROWN) {
	if (o45) clj_release(t45);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a21[1] = {t207};
	(void)a21;
	clj_value t208 = clj_c_invoke(t45, a21, 1);
	clj_release(t207);
	if (o45) clj_release(t45);
	if (t208 == CLJ_THROWN) {
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	bool o209;
	clj_value t209 = clj_c_var_borrow(V[8], &o209);
	if (t209 == CLJ_THROWN) {
	clj_release(t208);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value t210 = K[119];
	clj_value a93[1] = {t210};
	(void)a93;
	clj_value t211 = clj_c_invoke(t209, a93, 1);
	(void)t210;
	if (o209) clj_release(t209);
	if (t211 == CLJ_THROWN) {
	clj_release(t208);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	goto L2;
	}
	clj_value a3[4] = {t7, t44, t208, t211};
	(void)a3;
	clj_value t212 = clj_c_invoke(t4, a3, 4);
	clj_release(t211);
	clj_release(t208);
	clj_release(t44);
	clj_release(t7);
	if (o4) clj_release(t4);
	if (t212 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a2[1] = {t212};
	(void)a2;
	clj_value t213;
	if (CLJC_GUARD(V[12], B[0])) {
	t213 = clj_seq(t212);
	} else {
	t213 = clj_c_intrinsic_fallback(V[12], a2, 1);
	}
	clj_release(t212);
	if (t213 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[17], &cc);
	clj_release(l2);
	clj_release(l3);
	return t213;
L2: ;
	clj_c_leave(&S[17], &cc);
L1: ;
	clj_release(l2);
	clj_release(l3);
	return CLJ_THROWN;
}

static clj_value clojure_test_assert_predicate(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 2: return clojure_test_assert_predicate_a2(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_37(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 110 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[115], clojure_test_assert_predicate, NULL, 0, 0x4, 2, 2);
	clj_var_bind_root(V[56], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[139]);
	clj_value t2 = clj_c_def(V[56], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_assert_any_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_ccall cc;
	if (!clj_c_enter(&S[18], &cc)) goto L1;
#line 125 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[13], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[8], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t2 = K[142];
	clj_value a2[1] = {t2};
	(void)a2;
	clj_value t3 = clj_c_invoke(t1, a2, 1);
	(void)t2;
	if (o1) clj_release(t1);
	if (t3 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[8], &o4);
	if (t4 == CLJ_THROWN) {
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o5;
	clj_value t5 = clj_c_var_borrow(V[17], &o5);
	if (t5 == CLJ_THROWN) {
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o6;
	clj_value t6 = clj_c_var_borrow(V[58], &o6);
	if (t6 == CLJ_THROWN) {
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o7;
	clj_value t7 = clj_c_var_borrow(V[13], &o7);
	if (t7 == CLJ_THROWN) {
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[8], &o8);
	if (t8 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t9 = K[143];
	clj_value a7[1] = {t9};
	(void)a7;
	clj_value t10 = clj_c_invoke(t8, a7, 1);
	(void)t9;
	if (o8) clj_release(t8);
	if (t10 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o11;
	clj_value t11 = clj_c_var_borrow(V[8], &o11);
	if (t11 == CLJ_THROWN) {
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t12 = l1;
	clj_value a8[1] = {t12};
	(void)a8;
	clj_value t13 = clj_c_invoke(t11, a8, 1);
	(void)t12;
	if (o11) clj_release(t11);
	if (t13 == CLJ_THROWN) {
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a6[2] = {t10, t13};
	(void)a6;
	clj_value t14;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t14 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a6, 2) : clj_c_invoke(t7, a6, 2);
#else
	t14 = clj_c_invoke(t7, a6, 2);
#endif
	clj_release(t13);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (t14 == CLJ_THROWN) {
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a5[1] = {t14};
	(void)a5;
	clj_value t15;
	if (CLJC_GUARD(V[12], B[0])) {
	t15 = clj_seq(t14);
	} else {
	t15 = clj_c_intrinsic_fallback(V[12], a5, 1);
	}
	clj_release(t14);
	if (t15 == CLJ_THROWN) {
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a4[2] = {t6, t15};
	(void)a4;
	clj_value t16 = clj_c_invoke(t5, a4, 2);
	clj_release(t15);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (t16 == CLJ_THROWN) {
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a3[1] = {t16};
	(void)a3;
	clj_value t17 = clj_c_invoke(t4, a3, 1);
	clj_release(t16);
	if (o4) clj_release(t4);
	if (t17 == CLJ_THROWN) {
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o18;
	clj_value t18 = clj_c_var_borrow(V[8], &o18);
	if (t18 == CLJ_THROWN) {
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o19;
	clj_value t19 = clj_c_var_borrow(V[13], &o19);
	if (t19 == CLJ_THROWN) {
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o20;
	clj_value t20 = clj_c_var_borrow(V[8], &o20);
	if (t20 == CLJ_THROWN) {
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t21 = K[144];
	clj_value a12[1] = {t21};
	(void)a12;
	clj_value t22 = clj_c_invoke(t20, a12, 1);
	(void)t21;
	if (o20) clj_release(t20);
	if (t22 == CLJ_THROWN) {
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o23;
	clj_value t23 = clj_c_var_borrow(V[8], &o23);
	if (t23 == CLJ_THROWN) {
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t24 = K[143];
	clj_value a13[1] = {t24};
	(void)a13;
	clj_value t25 = clj_c_invoke(t23, a13, 1);
	(void)t24;
	if (o23) clj_release(t23);
	if (t25 == CLJ_THROWN) {
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o26;
	clj_value t26 = clj_c_var_borrow(V[8], &o26);
	if (t26 == CLJ_THROWN) {
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o27;
	clj_value t27 = clj_c_var_borrow(V[13], &o27);
	if (t27 == CLJ_THROWN) {
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o28;
	clj_value t28 = clj_c_var_borrow(V[8], &o28);
	if (t28 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t29 = K[145];
	clj_value a17[1] = {t29};
	(void)a17;
	clj_value t30 = clj_c_invoke(t28, a17, 1);
	(void)t29;
	if (o28) clj_release(t28);
	if (t30 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o31;
	clj_value t31 = clj_c_var_borrow(V[8], &o31);
	if (t31 == CLJ_THROWN) {
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o32;
	clj_value t32 = clj_c_var_borrow(V[17], &o32);
	if (t32 == CLJ_THROWN) {
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o33;
	clj_value t33 = clj_c_var_borrow(V[18], &o33);
	if (t33 == CLJ_THROWN) {
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o34;
	clj_value t34 = clj_c_var_borrow(V[13], &o34);
	if (t34 == CLJ_THROWN) {
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o35;
	clj_value t35 = clj_c_var_borrow(V[8], &o35);
	if (t35 == CLJ_THROWN) {
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t36 = K[146];
	clj_value a22[1] = {t36};
	(void)a22;
	clj_value t37 = clj_c_invoke(t35, a22, 1);
	(void)t36;
	if (o35) clj_release(t35);
	if (t37 == CLJ_THROWN) {
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o38;
	clj_value t38 = clj_c_var_borrow(V[8], &o38);
	if (t38 == CLJ_THROWN) {
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o39;
	clj_value t39 = clj_c_var_borrow(V[13], &o39);
	if (t39 == CLJ_THROWN) {
	if (o38) clj_release(t38);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o40;
	clj_value t40 = clj_c_var_borrow(V[8], &o40);
	if (t40 == CLJ_THROWN) {
	if (o39) clj_release(t39);
	if (o38) clj_release(t38);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t41 = K[147];
	clj_value a26[1] = {t41};
	(void)a26;
	clj_value t42 = clj_c_invoke(t40, a26, 1);
	(void)t41;
	if (o40) clj_release(t40);
	if (t42 == CLJ_THROWN) {
	if (o39) clj_release(t39);
	if (o38) clj_release(t38);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o43;
	clj_value t43 = clj_c_var_borrow(V[8], &o43);
	if (t43 == CLJ_THROWN) {
	clj_release(t42);
	if (o39) clj_release(t39);
	if (o38) clj_release(t38);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t44 = l1;
	clj_value a27[1] = {t44};
	(void)a27;
	clj_value t45 = clj_c_invoke(t43, a27, 1);
	(void)t44;
	if (o43) clj_release(t43);
	if (t45 == CLJ_THROWN) {
	clj_release(t42);
	if (o39) clj_release(t39);
	if (o38) clj_release(t38);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a25[2] = {t42, t45};
	(void)a25;
	clj_value t46;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t46 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a25, 2) : clj_c_invoke(t39, a25, 2);
#else
	t46 = clj_c_invoke(t39, a25, 2);
#endif
	clj_release(t45);
	clj_release(t42);
	if (o39) clj_release(t39);
	if (t46 == CLJ_THROWN) {
	if (o38) clj_release(t38);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a24[1] = {t46};
	(void)a24;
	clj_value t47;
	if (CLJC_GUARD(V[12], B[0])) {
	t47 = clj_seq(t46);
	} else {
	t47 = clj_c_intrinsic_fallback(V[12], a24, 1);
	}
	clj_release(t46);
	if (t47 == CLJ_THROWN) {
	if (o38) clj_release(t38);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a23[1] = {t47};
	(void)a23;
	clj_value t48 = clj_c_invoke(t38, a23, 1);
	clj_release(t47);
	if (o38) clj_release(t38);
	if (t48 == CLJ_THROWN) {
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o49;
	clj_value t49 = clj_c_var_borrow(V[8], &o49);
	if (t49 == CLJ_THROWN) {
	clj_release(t48);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t50 = K[148];
	clj_value a28[1] = {t50};
	(void)a28;
	clj_value t51 = clj_c_invoke(t49, a28, 1);
	(void)t50;
	if (o49) clj_release(t49);
	if (t51 == CLJ_THROWN) {
	clj_release(t48);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o52;
	clj_value t52 = clj_c_var_borrow(V[8], &o52);
	if (t52 == CLJ_THROWN) {
	clj_release(t51);
	clj_release(t48);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t53 = K[149];
	clj_value a29[1] = {t53};
	(void)a29;
	clj_value t54 = clj_c_invoke(t52, a29, 1);
	(void)t53;
	if (o52) clj_release(t52);
	if (t54 == CLJ_THROWN) {
	clj_release(t51);
	clj_release(t48);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o55;
	clj_value t55 = clj_c_var_borrow(V[8], &o55);
	if (t55 == CLJ_THROWN) {
	clj_release(t54);
	clj_release(t51);
	clj_release(t48);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t56 = K[150];
	clj_value a30[1] = {t56};
	(void)a30;
	clj_value t57 = clj_c_invoke(t55, a30, 1);
	(void)t56;
	if (o55) clj_release(t55);
	if (t57 == CLJ_THROWN) {
	clj_release(t54);
	clj_release(t51);
	clj_release(t48);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o58;
	clj_value t58 = clj_c_var_borrow(V[8], &o58);
	if (t58 == CLJ_THROWN) {
	clj_release(t57);
	clj_release(t54);
	clj_release(t51);
	clj_release(t48);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t59 = K[143];
	clj_value a31[1] = {t59};
	(void)a31;
	clj_value t60 = clj_c_invoke(t58, a31, 1);
	(void)t59;
	if (o58) clj_release(t58);
	if (t60 == CLJ_THROWN) {
	clj_release(t57);
	clj_release(t54);
	clj_release(t51);
	clj_release(t48);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o61;
	clj_value t61 = clj_c_var_borrow(V[8], &o61);
	if (t61 == CLJ_THROWN) {
	clj_release(t60);
	clj_release(t57);
	clj_release(t54);
	clj_release(t51);
	clj_release(t48);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t62 = K[151];
	clj_value a32[1] = {t62};
	(void)a32;
	clj_value t63 = clj_c_invoke(t61, a32, 1);
	(void)t62;
	if (o61) clj_release(t61);
	if (t63 == CLJ_THROWN) {
	clj_release(t60);
	clj_release(t57);
	clj_release(t54);
	clj_release(t51);
	clj_release(t48);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o64;
	clj_value t64 = clj_c_var_borrow(V[8], &o64);
	if (t64 == CLJ_THROWN) {
	clj_release(t63);
	clj_release(t60);
	clj_release(t57);
	clj_release(t54);
	clj_release(t51);
	clj_release(t48);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t65 = l0;
	clj_value a33[1] = {t65};
	(void)a33;
	clj_value t66 = clj_c_invoke(t64, a33, 1);
	(void)t65;
	if (o64) clj_release(t64);
	if (t66 == CLJ_THROWN) {
	clj_release(t63);
	clj_release(t60);
	clj_release(t57);
	clj_release(t54);
	clj_release(t51);
	clj_release(t48);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a21[8] = {t37, t48, t51, t54, t57, t60, t63, t66};
	(void)a21;
	clj_value t67 = clj_c_invoke(t34, a21, 8);
	clj_release(t66);
	clj_release(t63);
	clj_release(t60);
	clj_release(t57);
	clj_release(t54);
	clj_release(t51);
	clj_release(t48);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (t67 == CLJ_THROWN) {
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a20[1] = {t67};
	(void)a20;
	clj_value t68;
	if (CLJC_GUARD(V[12], B[0])) {
	t68 = clj_seq(t67);
	} else {
	t68 = clj_c_intrinsic_fallback(V[12], a20, 1);
	}
	clj_release(t67);
	if (t68 == CLJ_THROWN) {
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a19[2] = {t33, t68};
	(void)a19;
	clj_value t69 = clj_c_invoke(t32, a19, 2);
	clj_release(t68);
	if (o33) clj_release(t33);
	if (o32) clj_release(t32);
	if (t69 == CLJ_THROWN) {
	if (o31) clj_release(t31);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a18[1] = {t69};
	(void)a18;
	clj_value t70 = clj_c_invoke(t31, a18, 1);
	clj_release(t69);
	if (o31) clj_release(t31);
	if (t70 == CLJ_THROWN) {
	clj_release(t30);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a16[2] = {t30, t70};
	(void)a16;
	clj_value t71;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t71 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a16, 2) : clj_c_invoke(t27, a16, 2);
#else
	t71 = clj_c_invoke(t27, a16, 2);
#endif
	clj_release(t70);
	clj_release(t30);
	if (o27) clj_release(t27);
	if (t71 == CLJ_THROWN) {
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a15[1] = {t71};
	(void)a15;
	clj_value t72;
	if (CLJC_GUARD(V[12], B[0])) {
	t72 = clj_seq(t71);
	} else {
	t72 = clj_c_intrinsic_fallback(V[12], a15, 1);
	}
	clj_release(t71);
	if (t72 == CLJ_THROWN) {
	if (o26) clj_release(t26);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a14[1] = {t72};
	(void)a14;
	clj_value t73 = clj_c_invoke(t26, a14, 1);
	clj_release(t72);
	if (o26) clj_release(t26);
	if (t73 == CLJ_THROWN) {
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o74;
	clj_value t74 = clj_c_var_borrow(V[8], &o74);
	if (t74 == CLJ_THROWN) {
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o75;
	clj_value t75 = clj_c_var_borrow(V[13], &o75);
	if (t75 == CLJ_THROWN) {
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o76;
	clj_value t76 = clj_c_var_borrow(V[8], &o76);
	if (t76 == CLJ_THROWN) {
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t77 = K[152];
	clj_value a37[1] = {t77};
	(void)a37;
	clj_value t78 = clj_c_invoke(t76, a37, 1);
	(void)t77;
	if (o76) clj_release(t76);
	if (t78 == CLJ_THROWN) {
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o79;
	clj_value t79 = clj_c_var_borrow(V[8], &o79);
	if (t79 == CLJ_THROWN) {
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o80;
	clj_value t80 = clj_c_var_borrow(V[17], &o80);
	if (t80 == CLJ_THROWN) {
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o81;
	clj_value t81 = clj_c_var_borrow(V[18], &o81);
	if (t81 == CLJ_THROWN) {
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o82;
	clj_value t82 = clj_c_var_borrow(V[13], &o82);
	if (t82 == CLJ_THROWN) {
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o83;
	clj_value t83 = clj_c_var_borrow(V[8], &o83);
	if (t83 == CLJ_THROWN) {
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t84 = K[146];
	clj_value a42[1] = {t84};
	(void)a42;
	clj_value t85 = clj_c_invoke(t83, a42, 1);
	(void)t84;
	if (o83) clj_release(t83);
	if (t85 == CLJ_THROWN) {
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o86;
	clj_value t86 = clj_c_var_borrow(V[8], &o86);
	if (t86 == CLJ_THROWN) {
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o87;
	clj_value t87 = clj_c_var_borrow(V[13], &o87);
	if (t87 == CLJ_THROWN) {
	if (o86) clj_release(t86);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o88;
	clj_value t88 = clj_c_var_borrow(V[8], &o88);
	if (t88 == CLJ_THROWN) {
	if (o87) clj_release(t87);
	if (o86) clj_release(t86);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t89 = K[153];
	clj_value a46[1] = {t89};
	(void)a46;
	clj_value t90 = clj_c_invoke(t88, a46, 1);
	(void)t89;
	if (o88) clj_release(t88);
	if (t90 == CLJ_THROWN) {
	if (o87) clj_release(t87);
	if (o86) clj_release(t86);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o91;
	clj_value t91 = clj_c_var_borrow(V[8], &o91);
	if (t91 == CLJ_THROWN) {
	clj_release(t90);
	if (o87) clj_release(t87);
	if (o86) clj_release(t86);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t92 = l1;
	clj_value a47[1] = {t92};
	(void)a47;
	clj_value t93 = clj_c_invoke(t91, a47, 1);
	(void)t92;
	if (o91) clj_release(t91);
	if (t93 == CLJ_THROWN) {
	clj_release(t90);
	if (o87) clj_release(t87);
	if (o86) clj_release(t86);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a45[2] = {t90, t93};
	(void)a45;
	clj_value t94;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t94 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a45, 2) : clj_c_invoke(t87, a45, 2);
#else
	t94 = clj_c_invoke(t87, a45, 2);
#endif
	clj_release(t93);
	clj_release(t90);
	if (o87) clj_release(t87);
	if (t94 == CLJ_THROWN) {
	if (o86) clj_release(t86);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a44[1] = {t94};
	(void)a44;
	clj_value t95;
	if (CLJC_GUARD(V[12], B[0])) {
	t95 = clj_seq(t94);
	} else {
	t95 = clj_c_intrinsic_fallback(V[12], a44, 1);
	}
	clj_release(t94);
	if (t95 == CLJ_THROWN) {
	if (o86) clj_release(t86);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a43[1] = {t95};
	(void)a43;
	clj_value t96 = clj_c_invoke(t86, a43, 1);
	clj_release(t95);
	if (o86) clj_release(t86);
	if (t96 == CLJ_THROWN) {
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o97;
	clj_value t97 = clj_c_var_borrow(V[8], &o97);
	if (t97 == CLJ_THROWN) {
	clj_release(t96);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t98 = K[148];
	clj_value a48[1] = {t98};
	(void)a48;
	clj_value t99 = clj_c_invoke(t97, a48, 1);
	(void)t98;
	if (o97) clj_release(t97);
	if (t99 == CLJ_THROWN) {
	clj_release(t96);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o100;
	clj_value t100 = clj_c_var_borrow(V[8], &o100);
	if (t100 == CLJ_THROWN) {
	clj_release(t99);
	clj_release(t96);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t101 = K[154];
	clj_value a49[1] = {t101};
	(void)a49;
	clj_value t102 = clj_c_invoke(t100, a49, 1);
	(void)t101;
	if (o100) clj_release(t100);
	if (t102 == CLJ_THROWN) {
	clj_release(t99);
	clj_release(t96);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o103;
	clj_value t103 = clj_c_var_borrow(V[8], &o103);
	if (t103 == CLJ_THROWN) {
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t104 = K[150];
	clj_value a50[1] = {t104};
	(void)a50;
	clj_value t105 = clj_c_invoke(t103, a50, 1);
	(void)t104;
	if (o103) clj_release(t103);
	if (t105 == CLJ_THROWN) {
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o106;
	clj_value t106 = clj_c_var_borrow(V[8], &o106);
	if (t106 == CLJ_THROWN) {
	clj_release(t105);
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t107 = K[143];
	clj_value a51[1] = {t107};
	(void)a51;
	clj_value t108 = clj_c_invoke(t106, a51, 1);
	(void)t107;
	if (o106) clj_release(t106);
	if (t108 == CLJ_THROWN) {
	clj_release(t105);
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o109;
	clj_value t109 = clj_c_var_borrow(V[8], &o109);
	if (t109 == CLJ_THROWN) {
	clj_release(t108);
	clj_release(t105);
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t110 = K[151];
	clj_value a52[1] = {t110};
	(void)a52;
	clj_value t111 = clj_c_invoke(t109, a52, 1);
	(void)t110;
	if (o109) clj_release(t109);
	if (t111 == CLJ_THROWN) {
	clj_release(t108);
	clj_release(t105);
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o112;
	clj_value t112 = clj_c_var_borrow(V[8], &o112);
	if (t112 == CLJ_THROWN) {
	clj_release(t111);
	clj_release(t108);
	clj_release(t105);
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t113 = l0;
	clj_value a53[1] = {t113};
	(void)a53;
	clj_value t114 = clj_c_invoke(t112, a53, 1);
	(void)t113;
	if (o112) clj_release(t112);
	if (t114 == CLJ_THROWN) {
	clj_release(t111);
	clj_release(t108);
	clj_release(t105);
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a41[8] = {t85, t96, t99, t102, t105, t108, t111, t114};
	(void)a41;
	clj_value t115 = clj_c_invoke(t82, a41, 8);
	clj_release(t114);
	clj_release(t111);
	clj_release(t108);
	clj_release(t105);
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (t115 == CLJ_THROWN) {
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a40[1] = {t115};
	(void)a40;
	clj_value t116;
	if (CLJC_GUARD(V[12], B[0])) {
	t116 = clj_seq(t115);
	} else {
	t116 = clj_c_intrinsic_fallback(V[12], a40, 1);
	}
	clj_release(t115);
	if (t116 == CLJ_THROWN) {
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a39[2] = {t81, t116};
	(void)a39;
	clj_value t117 = clj_c_invoke(t80, a39, 2);
	clj_release(t116);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (t117 == CLJ_THROWN) {
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a38[1] = {t117};
	(void)a38;
	clj_value t118 = clj_c_invoke(t79, a38, 1);
	clj_release(t117);
	if (o79) clj_release(t79);
	if (t118 == CLJ_THROWN) {
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a36[2] = {t78, t118};
	(void)a36;
	clj_value t119;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t119 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a36, 2) : clj_c_invoke(t75, a36, 2);
#else
	t119 = clj_c_invoke(t75, a36, 2);
#endif
	clj_release(t118);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (t119 == CLJ_THROWN) {
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a35[1] = {t119};
	(void)a35;
	clj_value t120;
	if (CLJC_GUARD(V[12], B[0])) {
	t120 = clj_seq(t119);
	} else {
	t120 = clj_c_intrinsic_fallback(V[12], a35, 1);
	}
	clj_release(t119);
	if (t120 == CLJ_THROWN) {
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a34[1] = {t120};
	(void)a34;
	clj_value t121 = clj_c_invoke(t74, a34, 1);
	clj_release(t120);
	if (o74) clj_release(t74);
	if (t121 == CLJ_THROWN) {
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a11[4] = {t22, t25, t73, t121};
	(void)a11;
	clj_value t122 = clj_c_invoke(t19, a11, 4);
	clj_release(t121);
	clj_release(t73);
	clj_release(t25);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (t122 == CLJ_THROWN) {
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a10[1] = {t122};
	(void)a10;
	clj_value t123;
	if (CLJC_GUARD(V[12], B[0])) {
	t123 = clj_seq(t122);
	} else {
	t123 = clj_c_intrinsic_fallback(V[12], a10, 1);
	}
	clj_release(t122);
	if (t123 == CLJ_THROWN) {
	if (o18) clj_release(t18);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a9[1] = {t123};
	(void)a9;
	clj_value t124 = clj_c_invoke(t18, a9, 1);
	clj_release(t123);
	if (o18) clj_release(t18);
	if (t124 == CLJ_THROWN) {
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o125;
	clj_value t125 = clj_c_var_borrow(V[8], &o125);
	if (t125 == CLJ_THROWN) {
	clj_release(t124);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t126 = K[143];
	clj_value a54[1] = {t126};
	(void)a54;
	clj_value t127 = clj_c_invoke(t125, a54, 1);
	(void)t126;
	if (o125) clj_release(t125);
	if (t127 == CLJ_THROWN) {
	clj_release(t124);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a1[4] = {t3, t17, t124, t127};
	(void)a1;
	clj_value t128 = clj_c_invoke(t0, a1, 4);
	clj_release(t127);
	clj_release(t124);
	clj_release(t17);
	clj_release(t3);
	if (o0) clj_release(t0);
	if (t128 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t128};
	(void)a0;
	clj_value t129;
	if (CLJC_GUARD(V[12], B[0])) {
	t129 = clj_seq(t128);
	} else {
	t129 = clj_c_intrinsic_fallback(V[12], a0, 1);
	}
	clj_release(t128);
	if (t129 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[18], &cc);
	return t129;
L2: ;
	clj_c_leave(&S[18], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_assert_any(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 2: return clojure_test_assert_any_a2(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_38(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 125 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[141], clojure_test_assert_any, NULL, 0, 0x4, 2, 2);
	clj_var_bind_root(V[59], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[155]);
	clj_value t2 = clj_c_def(V[59], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form39__0_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_ccall cc;
	if (!clj_c_enter(&S[19], &cc)) goto L1;
#line 139 "<embedded>/clojure/test.clj"
#line 140 "<embedded>/clojure/test.clj"
	clj_value t0 = l1;
	clj_value a0[1] = {t0};
	(void)a0;
	clj_value t1;
	if (CLJC_GUARD(V[48], B[6])) {
	t1 = clj_nil_p(t0);
	} else {
	t1 = clj_c_intrinsic_fallback(V[48], a0, 1);
	}
	(void)t0;
	if (t1 == CLJ_THROWN) {
	goto L2;
	}
	bool t2 = clj_truthy(t1);
	clj_release(t1);
	clj_value t3;
	if (t2) {
#line 139 "<embedded>/clojure/test.clj"
	clj_value t4 = clj_retain(K[158]);
	t3 = t4;
	} else {
#line 141 "<embedded>/clojure/test.clj"
	clj_value t5 = l1;
	clj_value a1[1] = {t5};
	(void)a1;
	clj_value t6;
	if (CLJC_GUARD(V[15], B[1])) {
	t6 = clj_seq_p(t5);
	} else {
	t6 = clj_c_intrinsic_fallback(V[15], a1, 1);
	}
	(void)t5;
	if (t6 == CLJ_THROWN) {
	goto L2;
	}
	bool t7 = clj_truthy(t6);
	clj_release(t6);
	clj_value t8;
	if (t7) {
	clj_value t9 = l1;
	clj_value a2[1] = {t9};
	(void)a2;
	clj_value t10;
	if (CLJC_GUARD(V[19], B[3])) {
	t10 = clj_first(t9);
	} else {
	t10 = clj_c_intrinsic_fallback(V[19], a2, 1);
	}
	(void)t9;
	if (t10 == CLJ_THROWN) {
	goto L2;
	}
	t8 = t10;
	} else {
#line 139 "<embedded>/clojure/test.clj"
	clj_value t11 = clj_retain(K[159]);
	t8 = t11;
	}
	t3 = t8;
	}
	clj_c_leave(&S[19], &cc);
	return t3;
L2: ;
	clj_c_leave(&S[19], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_form39__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 2: return clojure_test_form39__0_a2(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_39(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_value l0 = CLJ_NIL;
	clj_eval_top_enter();
#line 136 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_retain(K[157]);
	clj_value t1 = clj_c_def(V[60], t0, false, false);
	clj_release(t0);
	if (t1 == CLJ_THROWN) {
	goto L1;
	}
	clj_c_rebind(&l0, t1);
	bool o2;
	clj_value t2 = clj_c_var_borrow(V[33], &o2);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_value t3 = l0;
	clj_value a0[1] = {t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t2, a0, 1);
	(void)t3;
	if (o2) clj_release(t2);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	bool t5 = clj_truthy(t4);
	clj_release(t4);
	clj_value t6;
	if (t5) {
	clj_value t7 = clj_retain(K[3]);
	t6 = t7;
	} else {
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[34], &o8);
	if (t8 == CLJ_THROWN) {
	goto L1;
	}
	clj_value t9 = K[156];
#line 138 "<embedded>/clojure/test.clj"
	clj_value t10 = clj_c_closure(CLJ_NIL, clojure_test_form39__0, NULL, 0, 0x4, 2, 2);
#line 136 "<embedded>/clojure/test.clj"
	clj_value t11 = K[159];
	clj_value t12 = V[35];
	bool o13;
	clj_value t13 = clj_c_var_borrow(V[36], &o13);
	if (t13 == CLJ_THROWN) {
	clj_release(t10);
	if (o8) clj_release(t8);
	goto L1;
	}
	clj_value t14 = K[160];
	clj_value a2[1] = {t14};
	(void)a2;
	clj_value t15 = clj_c_invoke(t13, a2, 1);
	(void)t14;
	if (o13) clj_release(t13);
	if (t15 == CLJ_THROWN) {
	clj_release(t10);
	if (o8) clj_release(t8);
	goto L1;
	}
	bool o16;
	clj_value t16 = clj_c_var_borrow(V[36], &o16);
	if (t16 == CLJ_THROWN) {
	clj_release(t15);
	clj_release(t10);
	if (o8) clj_release(t8);
	goto L1;
	}
	clj_value t17 = K[160];
	clj_value a3[1] = {t17};
	(void)a3;
	clj_value t18 = clj_c_invoke(t16, a3, 1);
	(void)t17;
	if (o16) clj_release(t16);
	if (t18 == CLJ_THROWN) {
	clj_release(t15);
	clj_release(t10);
	if (o8) clj_release(t8);
	goto L1;
	}
	bool o19;
	clj_value t19 = clj_c_var_borrow(V[36], &o19);
	if (t19 == CLJ_THROWN) {
	clj_release(t18);
	clj_release(t15);
	clj_release(t10);
	if (o8) clj_release(t8);
	goto L1;
	}
	clj_value t20 = K[161];
	clj_value a4[1] = {t20};
	(void)a4;
	clj_value t21 = clj_c_invoke(t19, a4, 1);
	(void)t20;
	if (o19) clj_release(t19);
	if (t21 == CLJ_THROWN) {
	clj_release(t18);
	clj_release(t15);
	clj_release(t10);
	if (o8) clj_release(t8);
	goto L1;
	}
	clj_value a1[7] = {t9, t10, t11, t12, t15, t18, t21};
	(void)a1;
	clj_value t22;
#ifdef CLJC_DIRECT_clojure_core___GT_MultiFn_a7
	if (!CLJC_FN_clojure_core___GT_MultiFn_a7) CLJC_FN_clojure_core___GT_MultiFn_a7 = clj_compiled_symbol("clojure_core___GT_MultiFn_a7");
	t22 = CLJC_FN_clojure_core___GT_MultiFn_a7 ? CLJC_FN_clojure_core___GT_MultiFn_a7(clj_var_root_relaxed(V[34]), NULL, a1, 7) : clj_c_invoke(t8, a1, 7);
#else
	t22 = clj_c_invoke(t8, a1, 7);
#endif
	clj_release(t21);
	clj_release(t18);
	clj_release(t15);
	(void)t12;
	(void)t11;
	clj_release(t10);
	(void)t9;
	if (o8) clj_release(t8);
	if (t22 == CLJ_THROWN) {
	goto L1;
	}
	clj_var_bind_root(V[60], t22);
	clj_release(t22);
	clj_value t23 = clj_retain(K[162]);
	clj_value t24 = clj_c_def(V[60], t23, false, false);
	clj_release(t23);
	if (t24 == CLJ_THROWN) {
	goto L1;
	}
	t6 = t24;
	}
	clj_release(l0);
	clj_eval_top_leave();
	return t6;
L1: ;
	clj_release(l0);
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form40__0_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_ccall cc;
	if (!clj_c_enter(&S[20], &cc)) goto L1;
#line 144 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[13], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[8], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t2 = K[165];
	clj_value a2[1] = {t2};
	(void)a2;
	clj_value t3 = clj_c_invoke(t1, a2, 1);
	(void)t2;
	if (o1) clj_release(t1);
	if (t3 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[8], &o4);
	if (t4 == CLJ_THROWN) {
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o5;
	clj_value t5 = clj_c_var_borrow(V[17], &o5);
	if (t5 == CLJ_THROWN) {
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o6;
	clj_value t6 = clj_c_var_borrow(V[18], &o6);
	if (t6 == CLJ_THROWN) {
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o7;
	clj_value t7 = clj_c_var_borrow(V[13], &o7);
	if (t7 == CLJ_THROWN) {
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[8], &o8);
	if (t8 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t9 = K[166];
	clj_value a7[1] = {t9};
	(void)a7;
	clj_value t10 = clj_c_invoke(t8, a7, 1);
	(void)t9;
	if (o8) clj_release(t8);
	if (t10 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o11;
	clj_value t11 = clj_c_var_borrow(V[8], &o11);
	if (t11 == CLJ_THROWN) {
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t12 = K[167];
	clj_value a8[1] = {t12};
	(void)a8;
	clj_value t13 = clj_c_invoke(t11, a8, 1);
	(void)t12;
	if (o11) clj_release(t11);
	if (t13 == CLJ_THROWN) {
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o14;
	clj_value t14 = clj_c_var_borrow(V[8], &o14);
	if (t14 == CLJ_THROWN) {
	clj_release(t13);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t15 = K[168];
	clj_value a9[1] = {t15};
	(void)a9;
	clj_value t16 = clj_c_invoke(t14, a9, 1);
	(void)t15;
	if (o14) clj_release(t14);
	if (t16 == CLJ_THROWN) {
	clj_release(t13);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o17;
	clj_value t17 = clj_c_var_borrow(V[8], &o17);
	if (t17 == CLJ_THROWN) {
	clj_release(t16);
	clj_release(t13);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t18 = l0;
	clj_value a10[1] = {t18};
	(void)a10;
	clj_value t19 = clj_c_invoke(t17, a10, 1);
	(void)t18;
	if (o17) clj_release(t17);
	if (t19 == CLJ_THROWN) {
	clj_release(t16);
	clj_release(t13);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a6[4] = {t10, t13, t16, t19};
	(void)a6;
	clj_value t20 = clj_c_invoke(t7, a6, 4);
	clj_release(t19);
	clj_release(t16);
	clj_release(t13);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (t20 == CLJ_THROWN) {
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a5[1] = {t20};
	(void)a5;
	clj_value t21;
	if (CLJC_GUARD(V[12], B[0])) {
	t21 = clj_seq(t20);
	} else {
	t21 = clj_c_intrinsic_fallback(V[12], a5, 1);
	}
	clj_release(t20);
	if (t21 == CLJ_THROWN) {
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a4[2] = {t6, t21};
	(void)a4;
	clj_value t22 = clj_c_invoke(t5, a4, 2);
	clj_release(t21);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (t22 == CLJ_THROWN) {
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a3[1] = {t22};
	(void)a3;
	clj_value t23 = clj_c_invoke(t4, a3, 1);
	clj_release(t22);
	if (o4) clj_release(t4);
	if (t23 == CLJ_THROWN) {
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a1[2] = {t3, t23};
	(void)a1;
	clj_value t24;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t24 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a1, 2) : clj_c_invoke(t0, a1, 2);
#else
	t24 = clj_c_invoke(t0, a1, 2);
#endif
	clj_release(t23);
	clj_release(t3);
	if (o0) clj_release(t0);
	if (t24 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t24};
	(void)a0;
	clj_value t25;
	if (CLJC_GUARD(V[12], B[0])) {
	t25 = clj_seq(t24);
	} else {
	t25 = clj_c_intrinsic_fallback(V[12], a0, 1);
	}
	clj_release(t24);
	if (t25 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[20], &cc);
	return t25;
L2: ;
	clj_c_leave(&S[20], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_form40__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 2: return clojure_test_form40__0_a2(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_40(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 144 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[60], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[164];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form40__0, NULL, 0, 0x4, 2, 2);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_41(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 144 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[60]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form42__0_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_value l2 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[21], &cc)) goto L1;
#line 148 "<embedded>/clojure/test.clj"
	clj_value t0 = l1;
	clj_value a0[1] = {t0};
	(void)a0;
	clj_value t1;
	if (CLJC_GUARD(V[61], B[11])) {
	t1 = clj_sequential_p(t0);
	} else {
	t1 = clj_c_intrinsic_fallback(V[61], a0, 1);
	}
	(void)t0;
	if (t1 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l2, t1);
	clj_value t2 = l2;
	bool t3 = clj_truthy(t2);
	(void)t2;
	clj_value t4;
	if (t3) {
	bool o5;
	clj_value t5 = clj_c_var_borrow(V[50], &o5);
	if (t5 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t6 = l1;
	clj_value a2[1] = {t6};
	(void)a2;
	clj_value t7;
	if (CLJC_GUARD(V[19], B[3])) {
	t7 = clj_first(t6);
	} else {
	t7 = clj_c_intrinsic_fallback(V[19], a2, 1);
	}
	(void)t6;
	if (t7 == CLJ_THROWN) {
	if (o5) clj_release(t5);
	goto L2;
	}
	clj_value a1[1] = {t7};
	(void)a1;
	clj_value t8;
#ifdef CLJC_DIRECT_clojure_test_function_QMARK__a1
	if (!CLJC_FN_clojure_test_function_QMARK__a1) CLJC_FN_clojure_test_function_QMARK__a1 = clj_compiled_symbol("clojure_test_function_QMARK__a1");
	t8 = CLJC_FN_clojure_test_function_QMARK__a1 ? CLJC_FN_clojure_test_function_QMARK__a1(clj_var_root_relaxed(V[50]), NULL, a1, 1) : clj_c_invoke(t5, a1, 1);
#else
	t8 = clj_c_invoke(t5, a1, 1);
#endif
	clj_release(t7);
	if (o5) clj_release(t5);
	if (t8 == CLJ_THROWN) {
	goto L2;
	}
	t4 = t8;
	} else {
	clj_value t9 = clj_retain(l2);
	t4 = t9;
	}
	bool t10 = clj_truthy(t4);
	clj_release(t4);
	clj_value t11;
	if (t10) {
#line 149 "<embedded>/clojure/test.clj"
	bool o12;
	clj_value t12 = clj_c_var_borrow(V[56], &o12);
	if (t12 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t13 = l0;
	clj_value t14 = l1;
	clj_value a3[2] = {t13, t14};
	(void)a3;
	clj_value t15;
#ifdef CLJC_DIRECT_clojure_test_assert_predicate_a2
	if (!CLJC_FN_clojure_test_assert_predicate_a2) CLJC_FN_clojure_test_assert_predicate_a2 = clj_compiled_symbol("clojure_test_assert_predicate_a2");
	t15 = CLJC_FN_clojure_test_assert_predicate_a2 ? CLJC_FN_clojure_test_assert_predicate_a2(clj_var_root_relaxed(V[56]), NULL, a3, 2) : clj_c_invoke(t12, a3, 2);
#else
	t15 = clj_c_invoke(t12, a3, 2);
#endif
	(void)t14;
	(void)t13;
	if (o12) clj_release(t12);
	if (t15 == CLJ_THROWN) {
	goto L2;
	}
	t11 = t15;
	} else {
#line 150 "<embedded>/clojure/test.clj"
	bool o16;
	clj_value t16 = clj_c_var_borrow(V[59], &o16);
	if (t16 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t17 = l0;
	clj_value t18 = l1;
	clj_value a4[2] = {t17, t18};
	(void)a4;
	clj_value t19;
#ifdef CLJC_DIRECT_clojure_test_assert_any_a2
	if (!CLJC_FN_clojure_test_assert_any_a2) CLJC_FN_clojure_test_assert_any_a2 = clj_compiled_symbol("clojure_test_assert_any_a2");
	t19 = CLJC_FN_clojure_test_assert_any_a2 ? CLJC_FN_clojure_test_assert_any_a2(clj_var_root_relaxed(V[59]), NULL, a4, 2) : clj_c_invoke(t16, a4, 2);
#else
	t19 = clj_c_invoke(t16, a4, 2);
#endif
	(void)t18;
	(void)t17;
	if (o16) clj_release(t16);
	if (t19 == CLJ_THROWN) {
	goto L2;
	}
	t11 = t19;
	}
	clj_c_leave(&S[21], &cc);
	clj_release(l2);
	return t11;
L2: ;
	clj_c_leave(&S[21], &cc);
L1: ;
	clj_release(l2);
	return CLJ_THROWN;
}

static clj_value clojure_test_form42__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 2: return clojure_test_form42__0_a2(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_42(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 147 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[60], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[170];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form42__0, NULL, 0, 0x4, 2, 2);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_43(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 147 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[60]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form44__0_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_ccall cc;
	if (!clj_c_enter(&S[22], &cc)) goto L1;
#line 152 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[13], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[8], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t2 = K[173];
	clj_value a2[1] = {t2};
	(void)a2;
	clj_value t3 = clj_c_invoke(t1, a2, 1);
	(void)t2;
	if (o1) clj_release(t1);
	if (t3 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[8], &o4);
	if (t4 == CLJ_THROWN) {
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o5;
	clj_value t5 = clj_c_var_borrow(V[17], &o5);
	if (t5 == CLJ_THROWN) {
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o6;
	clj_value t6 = clj_c_var_borrow(V[58], &o6);
	if (t6 == CLJ_THROWN) {
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o7;
	clj_value t7 = clj_c_var_borrow(V[13], &o7);
	if (t7 == CLJ_THROWN) {
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[8], &o8);
	if (t8 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t9 = K[174];
	clj_value a7[1] = {t9};
	(void)a7;
	clj_value t10 = clj_c_invoke(t8, a7, 1);
	(void)t9;
	if (o8) clj_release(t8);
	if (t10 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o11;
	clj_value t11 = clj_c_var_borrow(V[8], &o11);
	if (t11 == CLJ_THROWN) {
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
#line 153 "<embedded>/clojure/test.clj"
	clj_value t12 = l1;
	clj_value t13 = K[175];
	clj_value a9[2] = {t12, t13};
	(void)a9;
	clj_value t14;
	if (CLJC_GUARD(V[62], B[12])) {
	t14 = clj_nth2(t12, t13);
	} else {
	t14 = clj_c_intrinsic_fallback(V[62], a9, 2);
	}
	(void)t13;
	(void)t12;
	if (t14 == CLJ_THROWN) {
	if (o11) clj_release(t11);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a8[1] = {t14};
	(void)a8;
	clj_value t15 = clj_c_invoke(t11, a8, 1);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (t15 == CLJ_THROWN) {
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
#line 152 "<embedded>/clojure/test.clj"
	bool o16;
	clj_value t16 = clj_c_var_borrow(V[8], &o16);
	if (t16 == CLJ_THROWN) {
	clj_release(t15);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t17 = K[176];
	clj_value a10[1] = {t17};
	(void)a10;
	clj_value t18 = clj_c_invoke(t16, a10, 1);
	(void)t17;
	if (o16) clj_release(t16);
	if (t18 == CLJ_THROWN) {
	clj_release(t15);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o19;
	clj_value t19 = clj_c_var_borrow(V[8], &o19);
	if (t19 == CLJ_THROWN) {
	clj_release(t18);
	clj_release(t15);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
#line 154 "<embedded>/clojure/test.clj"
	clj_value t20 = l1;
	clj_value t21 = K[177];
	clj_value a12[2] = {t20, t21};
	(void)a12;
	clj_value t22;
	if (CLJC_GUARD(V[62], B[12])) {
	t22 = clj_nth2(t20, t21);
	} else {
	t22 = clj_c_intrinsic_fallback(V[62], a12, 2);
	}
	(void)t21;
	(void)t20;
	if (t22 == CLJ_THROWN) {
	if (o19) clj_release(t19);
	clj_release(t18);
	clj_release(t15);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a11[1] = {t22};
	(void)a11;
	clj_value t23 = clj_c_invoke(t19, a11, 1);
	clj_release(t22);
	if (o19) clj_release(t19);
	if (t23 == CLJ_THROWN) {
	clj_release(t18);
	clj_release(t15);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a6[4] = {t10, t15, t18, t23};
	(void)a6;
	clj_value t24 = clj_c_invoke(t7, a6, 4);
	clj_release(t23);
	clj_release(t18);
	clj_release(t15);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (t24 == CLJ_THROWN) {
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a5[1] = {t24};
	(void)a5;
	clj_value t25;
	if (CLJC_GUARD(V[12], B[0])) {
	t25 = clj_seq(t24);
	} else {
	t25 = clj_c_intrinsic_fallback(V[12], a5, 1);
	}
	clj_release(t24);
	if (t25 == CLJ_THROWN) {
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a4[2] = {t6, t25};
	(void)a4;
	clj_value t26 = clj_c_invoke(t5, a4, 2);
	clj_release(t25);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (t26 == CLJ_THROWN) {
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a3[1] = {t26};
	(void)a3;
	clj_value t27 = clj_c_invoke(t4, a3, 1);
	clj_release(t26);
	if (o4) clj_release(t4);
	if (t27 == CLJ_THROWN) {
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
#line 152 "<embedded>/clojure/test.clj"
	bool o28;
	clj_value t28 = clj_c_var_borrow(V[8], &o28);
	if (t28 == CLJ_THROWN) {
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o29;
	clj_value t29 = clj_c_var_borrow(V[13], &o29);
	if (t29 == CLJ_THROWN) {
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o30;
	clj_value t30 = clj_c_var_borrow(V[8], &o30);
	if (t30 == CLJ_THROWN) {
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t31 = K[178];
	clj_value a16[1] = {t31};
	(void)a16;
	clj_value t32 = clj_c_invoke(t30, a16, 1);
	(void)t31;
	if (o30) clj_release(t30);
	if (t32 == CLJ_THROWN) {
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o33;
	clj_value t33 = clj_c_var_borrow(V[8], &o33);
	if (t33 == CLJ_THROWN) {
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o34;
	clj_value t34 = clj_c_var_borrow(V[17], &o34);
	if (t34 == CLJ_THROWN) {
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o35;
	clj_value t35 = clj_c_var_borrow(V[58], &o35);
	if (t35 == CLJ_THROWN) {
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o36;
	clj_value t36 = clj_c_var_borrow(V[13], &o36);
	if (t36 == CLJ_THROWN) {
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o37;
	clj_value t37 = clj_c_var_borrow(V[8], &o37);
	if (t37 == CLJ_THROWN) {
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t38 = K[179];
	clj_value a21[1] = {t38};
	(void)a21;
	clj_value t39 = clj_c_invoke(t37, a21, 1);
	(void)t38;
	if (o37) clj_release(t37);
	if (t39 == CLJ_THROWN) {
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o40;
	clj_value t40 = clj_c_var_borrow(V[8], &o40);
	if (t40 == CLJ_THROWN) {
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o41;
	clj_value t41 = clj_c_var_borrow(V[13], &o41);
	if (t41 == CLJ_THROWN) {
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o42;
	clj_value t42 = clj_c_var_borrow(V[8], &o42);
	if (t42 == CLJ_THROWN) {
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t43 = K[180];
	clj_value a25[1] = {t43};
	(void)a25;
	clj_value t44 = clj_c_invoke(t42, a25, 1);
	(void)t43;
	if (o42) clj_release(t42);
	if (t44 == CLJ_THROWN) {
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o45;
	clj_value t45 = clj_c_var_borrow(V[8], &o45);
	if (t45 == CLJ_THROWN) {
	clj_release(t44);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t46 = K[174];
	clj_value a26[1] = {t46};
	(void)a26;
	clj_value t47 = clj_c_invoke(t45, a26, 1);
	(void)t46;
	if (o45) clj_release(t45);
	if (t47 == CLJ_THROWN) {
	clj_release(t44);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o48;
	clj_value t48 = clj_c_var_borrow(V[8], &o48);
	if (t48 == CLJ_THROWN) {
	clj_release(t47);
	clj_release(t44);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t49 = K[176];
	clj_value a27[1] = {t49};
	(void)a27;
	clj_value t50 = clj_c_invoke(t48, a27, 1);
	(void)t49;
	if (o48) clj_release(t48);
	if (t50 == CLJ_THROWN) {
	clj_release(t47);
	clj_release(t44);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a24[3] = {t44, t47, t50};
	(void)a24;
	clj_value t51 = clj_c_invoke(t41, a24, 3);
	clj_release(t50);
	clj_release(t47);
	clj_release(t44);
	if (o41) clj_release(t41);
	if (t51 == CLJ_THROWN) {
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a23[1] = {t51};
	(void)a23;
	clj_value t52;
	if (CLJC_GUARD(V[12], B[0])) {
	t52 = clj_seq(t51);
	} else {
	t52 = clj_c_intrinsic_fallback(V[12], a23, 1);
	}
	clj_release(t51);
	if (t52 == CLJ_THROWN) {
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a22[1] = {t52};
	(void)a22;
	clj_value t53 = clj_c_invoke(t40, a22, 1);
	clj_release(t52);
	if (o40) clj_release(t40);
	if (t53 == CLJ_THROWN) {
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a20[2] = {t39, t53};
	(void)a20;
	clj_value t54;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t54 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a20, 2) : clj_c_invoke(t36, a20, 2);
#else
	t54 = clj_c_invoke(t36, a20, 2);
#endif
	clj_release(t53);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (t54 == CLJ_THROWN) {
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a19[1] = {t54};
	(void)a19;
	clj_value t55;
	if (CLJC_GUARD(V[12], B[0])) {
	t55 = clj_seq(t54);
	} else {
	t55 = clj_c_intrinsic_fallback(V[12], a19, 1);
	}
	clj_release(t54);
	if (t55 == CLJ_THROWN) {
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a18[2] = {t35, t55};
	(void)a18;
	clj_value t56 = clj_c_invoke(t34, a18, 2);
	clj_release(t55);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (t56 == CLJ_THROWN) {
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a17[1] = {t56};
	(void)a17;
	clj_value t57 = clj_c_invoke(t33, a17, 1);
	clj_release(t56);
	if (o33) clj_release(t33);
	if (t57 == CLJ_THROWN) {
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o58;
	clj_value t58 = clj_c_var_borrow(V[8], &o58);
	if (t58 == CLJ_THROWN) {
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o59;
	clj_value t59 = clj_c_var_borrow(V[13], &o59);
	if (t59 == CLJ_THROWN) {
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o60;
	clj_value t60 = clj_c_var_borrow(V[8], &o60);
	if (t60 == CLJ_THROWN) {
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t61 = K[181];
	clj_value a31[1] = {t61};
	(void)a31;
	clj_value t62 = clj_c_invoke(t60, a31, 1);
	(void)t61;
	if (o60) clj_release(t60);
	if (t62 == CLJ_THROWN) {
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o63;
	clj_value t63 = clj_c_var_borrow(V[8], &o63);
	if (t63 == CLJ_THROWN) {
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t64 = K[179];
	clj_value a32[1] = {t64};
	(void)a32;
	clj_value t65 = clj_c_invoke(t63, a32, 1);
	(void)t64;
	if (o63) clj_release(t63);
	if (t65 == CLJ_THROWN) {
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o66;
	clj_value t66 = clj_c_var_borrow(V[8], &o66);
	if (t66 == CLJ_THROWN) {
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o67;
	clj_value t67 = clj_c_var_borrow(V[13], &o67);
	if (t67 == CLJ_THROWN) {
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o68;
	clj_value t68 = clj_c_var_borrow(V[8], &o68);
	if (t68 == CLJ_THROWN) {
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t69 = K[182];
	clj_value a36[1] = {t69};
	(void)a36;
	clj_value t70 = clj_c_invoke(t68, a36, 1);
	(void)t69;
	if (o68) clj_release(t68);
	if (t70 == CLJ_THROWN) {
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o71;
	clj_value t71 = clj_c_var_borrow(V[8], &o71);
	if (t71 == CLJ_THROWN) {
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o72;
	clj_value t72 = clj_c_var_borrow(V[17], &o72);
	if (t72 == CLJ_THROWN) {
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o73;
	clj_value t73 = clj_c_var_borrow(V[18], &o73);
	if (t73 == CLJ_THROWN) {
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o74;
	clj_value t74 = clj_c_var_borrow(V[13], &o74);
	if (t74 == CLJ_THROWN) {
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o75;
	clj_value t75 = clj_c_var_borrow(V[8], &o75);
	if (t75 == CLJ_THROWN) {
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t76 = K[183];
	clj_value a41[1] = {t76};
	(void)a41;
	clj_value t77 = clj_c_invoke(t75, a41, 1);
	(void)t76;
	if (o75) clj_release(t75);
	if (t77 == CLJ_THROWN) {
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o78;
	clj_value t78 = clj_c_var_borrow(V[8], &o78);
	if (t78 == CLJ_THROWN) {
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o79;
	clj_value t79 = clj_c_var_borrow(V[13], &o79);
	if (t79 == CLJ_THROWN) {
	if (o78) clj_release(t78);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o80;
	clj_value t80 = clj_c_var_borrow(V[8], &o80);
	if (t80 == CLJ_THROWN) {
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t81 = K[184];
	clj_value a45[1] = {t81};
	(void)a45;
	clj_value t82 = clj_c_invoke(t80, a45, 1);
	(void)t81;
	if (o80) clj_release(t80);
	if (t82 == CLJ_THROWN) {
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o83;
	clj_value t83 = clj_c_var_borrow(V[8], &o83);
	if (t83 == CLJ_THROWN) {
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t84 = l1;
	clj_value a46[1] = {t84};
	(void)a46;
	clj_value t85 = clj_c_invoke(t83, a46, 1);
	(void)t84;
	if (o83) clj_release(t83);
	if (t85 == CLJ_THROWN) {
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a44[2] = {t82, t85};
	(void)a44;
	clj_value t86;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t86 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a44, 2) : clj_c_invoke(t79, a44, 2);
#else
	t86 = clj_c_invoke(t79, a44, 2);
#endif
	clj_release(t85);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (t86 == CLJ_THROWN) {
	if (o78) clj_release(t78);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a43[1] = {t86};
	(void)a43;
	clj_value t87;
	if (CLJC_GUARD(V[12], B[0])) {
	t87 = clj_seq(t86);
	} else {
	t87 = clj_c_intrinsic_fallback(V[12], a43, 1);
	}
	clj_release(t86);
	if (t87 == CLJ_THROWN) {
	if (o78) clj_release(t78);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a42[1] = {t87};
	(void)a42;
	clj_value t88 = clj_c_invoke(t78, a42, 1);
	clj_release(t87);
	if (o78) clj_release(t78);
	if (t88 == CLJ_THROWN) {
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o89;
	clj_value t89 = clj_c_var_borrow(V[8], &o89);
	if (t89 == CLJ_THROWN) {
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t90 = K[185];
	clj_value a47[1] = {t90};
	(void)a47;
	clj_value t91 = clj_c_invoke(t89, a47, 1);
	(void)t90;
	if (o89) clj_release(t89);
	if (t91 == CLJ_THROWN) {
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o92;
	clj_value t92 = clj_c_var_borrow(V[8], &o92);
	if (t92 == CLJ_THROWN) {
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t93 = K[186];
	clj_value a48[1] = {t93};
	(void)a48;
	clj_value t94 = clj_c_invoke(t92, a48, 1);
	(void)t93;
	if (o92) clj_release(t92);
	if (t94 == CLJ_THROWN) {
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o95;
	clj_value t95 = clj_c_var_borrow(V[8], &o95);
	if (t95 == CLJ_THROWN) {
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t96 = K[187];
	clj_value a49[1] = {t96};
	(void)a49;
	clj_value t97 = clj_c_invoke(t95, a49, 1);
	(void)t96;
	if (o95) clj_release(t95);
	if (t97 == CLJ_THROWN) {
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o98;
	clj_value t98 = clj_c_var_borrow(V[8], &o98);
	if (t98 == CLJ_THROWN) {
	clj_release(t97);
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o99;
	clj_value t99 = clj_c_var_borrow(V[13], &o99);
	if (t99 == CLJ_THROWN) {
	if (o98) clj_release(t98);
	clj_release(t97);
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o100;
	clj_value t100 = clj_c_var_borrow(V[8], &o100);
	if (t100 == CLJ_THROWN) {
	if (o99) clj_release(t99);
	if (o98) clj_release(t98);
	clj_release(t97);
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t101 = K[188];
	clj_value a53[1] = {t101};
	(void)a53;
	clj_value t102 = clj_c_invoke(t100, a53, 1);
	(void)t101;
	if (o100) clj_release(t100);
	if (t102 == CLJ_THROWN) {
	if (o99) clj_release(t99);
	if (o98) clj_release(t98);
	clj_release(t97);
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o103;
	clj_value t103 = clj_c_var_borrow(V[8], &o103);
	if (t103 == CLJ_THROWN) {
	clj_release(t102);
	if (o99) clj_release(t99);
	if (o98) clj_release(t98);
	clj_release(t97);
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t104 = K[176];
	clj_value a54[1] = {t104};
	(void)a54;
	clj_value t105 = clj_c_invoke(t103, a54, 1);
	(void)t104;
	if (o103) clj_release(t103);
	if (t105 == CLJ_THROWN) {
	clj_release(t102);
	if (o99) clj_release(t99);
	if (o98) clj_release(t98);
	clj_release(t97);
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a52[2] = {t102, t105};
	(void)a52;
	clj_value t106;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t106 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a52, 2) : clj_c_invoke(t99, a52, 2);
#else
	t106 = clj_c_invoke(t99, a52, 2);
#endif
	clj_release(t105);
	clj_release(t102);
	if (o99) clj_release(t99);
	if (t106 == CLJ_THROWN) {
	if (o98) clj_release(t98);
	clj_release(t97);
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a51[1] = {t106};
	(void)a51;
	clj_value t107;
	if (CLJC_GUARD(V[12], B[0])) {
	t107 = clj_seq(t106);
	} else {
	t107 = clj_c_intrinsic_fallback(V[12], a51, 1);
	}
	clj_release(t106);
	if (t107 == CLJ_THROWN) {
	if (o98) clj_release(t98);
	clj_release(t97);
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a50[1] = {t107};
	(void)a50;
	clj_value t108 = clj_c_invoke(t98, a50, 1);
	clj_release(t107);
	if (o98) clj_release(t98);
	if (t108 == CLJ_THROWN) {
	clj_release(t97);
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o109;
	clj_value t109 = clj_c_var_borrow(V[8], &o109);
	if (t109 == CLJ_THROWN) {
	clj_release(t108);
	clj_release(t97);
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t110 = K[189];
	clj_value a55[1] = {t110};
	(void)a55;
	clj_value t111 = clj_c_invoke(t109, a55, 1);
	(void)t110;
	if (o109) clj_release(t109);
	if (t111 == CLJ_THROWN) {
	clj_release(t108);
	clj_release(t97);
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o112;
	clj_value t112 = clj_c_var_borrow(V[8], &o112);
	if (t112 == CLJ_THROWN) {
	clj_release(t111);
	clj_release(t108);
	clj_release(t97);
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t113 = l0;
	clj_value a56[1] = {t113};
	(void)a56;
	clj_value t114 = clj_c_invoke(t112, a56, 1);
	(void)t113;
	if (o112) clj_release(t112);
	if (t114 == CLJ_THROWN) {
	clj_release(t111);
	clj_release(t108);
	clj_release(t97);
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a40[8] = {t77, t88, t91, t94, t97, t108, t111, t114};
	(void)a40;
	clj_value t115 = clj_c_invoke(t74, a40, 8);
	clj_release(t114);
	clj_release(t111);
	clj_release(t108);
	clj_release(t97);
	clj_release(t94);
	clj_release(t91);
	clj_release(t88);
	clj_release(t77);
	if (o74) clj_release(t74);
	if (t115 == CLJ_THROWN) {
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a39[1] = {t115};
	(void)a39;
	clj_value t116;
	if (CLJC_GUARD(V[12], B[0])) {
	t116 = clj_seq(t115);
	} else {
	t116 = clj_c_intrinsic_fallback(V[12], a39, 1);
	}
	clj_release(t115);
	if (t116 == CLJ_THROWN) {
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a38[2] = {t73, t116};
	(void)a38;
	clj_value t117 = clj_c_invoke(t72, a38, 2);
	clj_release(t116);
	if (o73) clj_release(t73);
	if (o72) clj_release(t72);
	if (t117 == CLJ_THROWN) {
	if (o71) clj_release(t71);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a37[1] = {t117};
	(void)a37;
	clj_value t118 = clj_c_invoke(t71, a37, 1);
	clj_release(t117);
	if (o71) clj_release(t71);
	if (t118 == CLJ_THROWN) {
	clj_release(t70);
	if (o67) clj_release(t67);
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a35[2] = {t70, t118};
	(void)a35;
	clj_value t119;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t119 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a35, 2) : clj_c_invoke(t67, a35, 2);
#else
	t119 = clj_c_invoke(t67, a35, 2);
#endif
	clj_release(t118);
	clj_release(t70);
	if (o67) clj_release(t67);
	if (t119 == CLJ_THROWN) {
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a34[1] = {t119};
	(void)a34;
	clj_value t120;
	if (CLJC_GUARD(V[12], B[0])) {
	t120 = clj_seq(t119);
	} else {
	t120 = clj_c_intrinsic_fallback(V[12], a34, 1);
	}
	clj_release(t119);
	if (t120 == CLJ_THROWN) {
	if (o66) clj_release(t66);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a33[1] = {t120};
	(void)a33;
	clj_value t121 = clj_c_invoke(t66, a33, 1);
	clj_release(t120);
	if (o66) clj_release(t66);
	if (t121 == CLJ_THROWN) {
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o122;
	clj_value t122 = clj_c_var_borrow(V[8], &o122);
	if (t122 == CLJ_THROWN) {
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o123;
	clj_value t123 = clj_c_var_borrow(V[13], &o123);
	if (t123 == CLJ_THROWN) {
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o124;
	clj_value t124 = clj_c_var_borrow(V[8], &o124);
	if (t124 == CLJ_THROWN) {
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t125 = K[190];
	clj_value a60[1] = {t125};
	(void)a60;
	clj_value t126 = clj_c_invoke(t124, a60, 1);
	(void)t125;
	if (o124) clj_release(t124);
	if (t126 == CLJ_THROWN) {
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o127;
	clj_value t127 = clj_c_var_borrow(V[8], &o127);
	if (t127 == CLJ_THROWN) {
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o128;
	clj_value t128 = clj_c_var_borrow(V[17], &o128);
	if (t128 == CLJ_THROWN) {
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o129;
	clj_value t129 = clj_c_var_borrow(V[18], &o129);
	if (t129 == CLJ_THROWN) {
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o130;
	clj_value t130 = clj_c_var_borrow(V[13], &o130);
	if (t130 == CLJ_THROWN) {
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o131;
	clj_value t131 = clj_c_var_borrow(V[8], &o131);
	if (t131 == CLJ_THROWN) {
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t132 = K[183];
	clj_value a65[1] = {t132};
	(void)a65;
	clj_value t133 = clj_c_invoke(t131, a65, 1);
	(void)t132;
	if (o131) clj_release(t131);
	if (t133 == CLJ_THROWN) {
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o134;
	clj_value t134 = clj_c_var_borrow(V[8], &o134);
	if (t134 == CLJ_THROWN) {
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o135;
	clj_value t135 = clj_c_var_borrow(V[13], &o135);
	if (t135 == CLJ_THROWN) {
	if (o134) clj_release(t134);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o136;
	clj_value t136 = clj_c_var_borrow(V[8], &o136);
	if (t136 == CLJ_THROWN) {
	if (o135) clj_release(t135);
	if (o134) clj_release(t134);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t137 = K[191];
	clj_value a69[1] = {t137};
	(void)a69;
	clj_value t138 = clj_c_invoke(t136, a69, 1);
	(void)t137;
	if (o136) clj_release(t136);
	if (t138 == CLJ_THROWN) {
	if (o135) clj_release(t135);
	if (o134) clj_release(t134);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o139;
	clj_value t139 = clj_c_var_borrow(V[8], &o139);
	if (t139 == CLJ_THROWN) {
	clj_release(t138);
	if (o135) clj_release(t135);
	if (o134) clj_release(t134);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t140 = l1;
	clj_value a70[1] = {t140};
	(void)a70;
	clj_value t141 = clj_c_invoke(t139, a70, 1);
	(void)t140;
	if (o139) clj_release(t139);
	if (t141 == CLJ_THROWN) {
	clj_release(t138);
	if (o135) clj_release(t135);
	if (o134) clj_release(t134);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a68[2] = {t138, t141};
	(void)a68;
	clj_value t142;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t142 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a68, 2) : clj_c_invoke(t135, a68, 2);
#else
	t142 = clj_c_invoke(t135, a68, 2);
#endif
	clj_release(t141);
	clj_release(t138);
	if (o135) clj_release(t135);
	if (t142 == CLJ_THROWN) {
	if (o134) clj_release(t134);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a67[1] = {t142};
	(void)a67;
	clj_value t143;
	if (CLJC_GUARD(V[12], B[0])) {
	t143 = clj_seq(t142);
	} else {
	t143 = clj_c_intrinsic_fallback(V[12], a67, 1);
	}
	clj_release(t142);
	if (t143 == CLJ_THROWN) {
	if (o134) clj_release(t134);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a66[1] = {t143};
	(void)a66;
	clj_value t144 = clj_c_invoke(t134, a66, 1);
	clj_release(t143);
	if (o134) clj_release(t134);
	if (t144 == CLJ_THROWN) {
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o145;
	clj_value t145 = clj_c_var_borrow(V[8], &o145);
	if (t145 == CLJ_THROWN) {
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t146 = K[185];
	clj_value a71[1] = {t146};
	(void)a71;
	clj_value t147 = clj_c_invoke(t145, a71, 1);
	(void)t146;
	if (o145) clj_release(t145);
	if (t147 == CLJ_THROWN) {
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o148;
	clj_value t148 = clj_c_var_borrow(V[8], &o148);
	if (t148 == CLJ_THROWN) {
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t149 = K[192];
	clj_value a72[1] = {t149};
	(void)a72;
	clj_value t150 = clj_c_invoke(t148, a72, 1);
	(void)t149;
	if (o148) clj_release(t148);
	if (t150 == CLJ_THROWN) {
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o151;
	clj_value t151 = clj_c_var_borrow(V[8], &o151);
	if (t151 == CLJ_THROWN) {
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t152 = K[187];
	clj_value a73[1] = {t152};
	(void)a73;
	clj_value t153 = clj_c_invoke(t151, a73, 1);
	(void)t152;
	if (o151) clj_release(t151);
	if (t153 == CLJ_THROWN) {
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o154;
	clj_value t154 = clj_c_var_borrow(V[8], &o154);
	if (t154 == CLJ_THROWN) {
	clj_release(t153);
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o155;
	clj_value t155 = clj_c_var_borrow(V[13], &o155);
	if (t155 == CLJ_THROWN) {
	if (o154) clj_release(t154);
	clj_release(t153);
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o156;
	clj_value t156 = clj_c_var_borrow(V[8], &o156);
	if (t156 == CLJ_THROWN) {
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	clj_release(t153);
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t157 = K[193];
	clj_value a77[1] = {t157};
	(void)a77;
	clj_value t158 = clj_c_invoke(t156, a77, 1);
	(void)t157;
	if (o156) clj_release(t156);
	if (t158 == CLJ_THROWN) {
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	clj_release(t153);
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o159;
	clj_value t159 = clj_c_var_borrow(V[8], &o159);
	if (t159 == CLJ_THROWN) {
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	clj_release(t153);
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t160 = K[176];
	clj_value a78[1] = {t160};
	(void)a78;
	clj_value t161 = clj_c_invoke(t159, a78, 1);
	(void)t160;
	if (o159) clj_release(t159);
	if (t161 == CLJ_THROWN) {
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	clj_release(t153);
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a76[2] = {t158, t161};
	(void)a76;
	clj_value t162;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t162 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a76, 2) : clj_c_invoke(t155, a76, 2);
#else
	t162 = clj_c_invoke(t155, a76, 2);
#endif
	clj_release(t161);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (t162 == CLJ_THROWN) {
	if (o154) clj_release(t154);
	clj_release(t153);
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a75[1] = {t162};
	(void)a75;
	clj_value t163;
	if (CLJC_GUARD(V[12], B[0])) {
	t163 = clj_seq(t162);
	} else {
	t163 = clj_c_intrinsic_fallback(V[12], a75, 1);
	}
	clj_release(t162);
	if (t163 == CLJ_THROWN) {
	if (o154) clj_release(t154);
	clj_release(t153);
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a74[1] = {t163};
	(void)a74;
	clj_value t164 = clj_c_invoke(t154, a74, 1);
	clj_release(t163);
	if (o154) clj_release(t154);
	if (t164 == CLJ_THROWN) {
	clj_release(t153);
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o165;
	clj_value t165 = clj_c_var_borrow(V[8], &o165);
	if (t165 == CLJ_THROWN) {
	clj_release(t164);
	clj_release(t153);
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t166 = K[189];
	clj_value a79[1] = {t166};
	(void)a79;
	clj_value t167 = clj_c_invoke(t165, a79, 1);
	(void)t166;
	if (o165) clj_release(t165);
	if (t167 == CLJ_THROWN) {
	clj_release(t164);
	clj_release(t153);
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o168;
	clj_value t168 = clj_c_var_borrow(V[8], &o168);
	if (t168 == CLJ_THROWN) {
	clj_release(t167);
	clj_release(t164);
	clj_release(t153);
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t169 = l0;
	clj_value a80[1] = {t169};
	(void)a80;
	clj_value t170 = clj_c_invoke(t168, a80, 1);
	(void)t169;
	if (o168) clj_release(t168);
	if (t170 == CLJ_THROWN) {
	clj_release(t167);
	clj_release(t164);
	clj_release(t153);
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a64[8] = {t133, t144, t147, t150, t153, t164, t167, t170};
	(void)a64;
	clj_value t171 = clj_c_invoke(t130, a64, 8);
	clj_release(t170);
	clj_release(t167);
	clj_release(t164);
	clj_release(t153);
	clj_release(t150);
	clj_release(t147);
	clj_release(t144);
	clj_release(t133);
	if (o130) clj_release(t130);
	if (t171 == CLJ_THROWN) {
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a63[1] = {t171};
	(void)a63;
	clj_value t172;
	if (CLJC_GUARD(V[12], B[0])) {
	t172 = clj_seq(t171);
	} else {
	t172 = clj_c_intrinsic_fallback(V[12], a63, 1);
	}
	clj_release(t171);
	if (t172 == CLJ_THROWN) {
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a62[2] = {t129, t172};
	(void)a62;
	clj_value t173 = clj_c_invoke(t128, a62, 2);
	clj_release(t172);
	if (o129) clj_release(t129);
	if (o128) clj_release(t128);
	if (t173 == CLJ_THROWN) {
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a61[1] = {t173};
	(void)a61;
	clj_value t174 = clj_c_invoke(t127, a61, 1);
	clj_release(t173);
	if (o127) clj_release(t127);
	if (t174 == CLJ_THROWN) {
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a59[2] = {t126, t174};
	(void)a59;
	clj_value t175;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t175 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a59, 2) : clj_c_invoke(t123, a59, 2);
#else
	t175 = clj_c_invoke(t123, a59, 2);
#endif
	clj_release(t174);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (t175 == CLJ_THROWN) {
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a58[1] = {t175};
	(void)a58;
	clj_value t176;
	if (CLJC_GUARD(V[12], B[0])) {
	t176 = clj_seq(t175);
	} else {
	t176 = clj_c_intrinsic_fallback(V[12], a58, 1);
	}
	clj_release(t175);
	if (t176 == CLJ_THROWN) {
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a57[1] = {t176};
	(void)a57;
	clj_value t177 = clj_c_invoke(t122, a57, 1);
	clj_release(t176);
	if (o122) clj_release(t122);
	if (t177 == CLJ_THROWN) {
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a30[4] = {t62, t65, t121, t177};
	(void)a30;
	clj_value t178 = clj_c_invoke(t59, a30, 4);
	clj_release(t177);
	clj_release(t121);
	clj_release(t65);
	clj_release(t62);
	if (o59) clj_release(t59);
	if (t178 == CLJ_THROWN) {
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a29[1] = {t178};
	(void)a29;
	clj_value t179;
	if (CLJC_GUARD(V[12], B[0])) {
	t179 = clj_seq(t178);
	} else {
	t179 = clj_c_intrinsic_fallback(V[12], a29, 1);
	}
	clj_release(t178);
	if (t179 == CLJ_THROWN) {
	if (o58) clj_release(t58);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a28[1] = {t179};
	(void)a28;
	clj_value t180 = clj_c_invoke(t58, a28, 1);
	clj_release(t179);
	if (o58) clj_release(t58);
	if (t180 == CLJ_THROWN) {
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o181;
	clj_value t181 = clj_c_var_borrow(V[8], &o181);
	if (t181 == CLJ_THROWN) {
	clj_release(t180);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t182 = K[179];
	clj_value a81[1] = {t182};
	(void)a81;
	clj_value t183 = clj_c_invoke(t181, a81, 1);
	(void)t182;
	if (o181) clj_release(t181);
	if (t183 == CLJ_THROWN) {
	clj_release(t180);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a15[4] = {t32, t57, t180, t183};
	(void)a15;
	clj_value t184 = clj_c_invoke(t29, a15, 4);
	clj_release(t183);
	clj_release(t180);
	clj_release(t57);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (t184 == CLJ_THROWN) {
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a14[1] = {t184};
	(void)a14;
	clj_value t185;
	if (CLJC_GUARD(V[12], B[0])) {
	t185 = clj_seq(t184);
	} else {
	t185 = clj_c_intrinsic_fallback(V[12], a14, 1);
	}
	clj_release(t184);
	if (t185 == CLJ_THROWN) {
	if (o28) clj_release(t28);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a13[1] = {t185};
	(void)a13;
	clj_value t186 = clj_c_invoke(t28, a13, 1);
	clj_release(t185);
	if (o28) clj_release(t28);
	if (t186 == CLJ_THROWN) {
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a1[3] = {t3, t27, t186};
	(void)a1;
	clj_value t187 = clj_c_invoke(t0, a1, 3);
	clj_release(t186);
	clj_release(t27);
	clj_release(t3);
	if (o0) clj_release(t0);
	if (t187 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t187};
	(void)a0;
	clj_value t188;
	if (CLJC_GUARD(V[12], B[0])) {
	t188 = clj_seq(t187);
	} else {
	t188 = clj_c_intrinsic_fallback(V[12], a0, 1);
	}
	clj_release(t187);
	if (t188 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[22], &cc);
	return t188;
L2: ;
	clj_c_leave(&S[22], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_form44__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 2: return clojure_test_form44__0_a2(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_44(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 152 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[60], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[172];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form44__0, NULL, 0, 0x4, 2, 2);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_45(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 152 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[60]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form46__0_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_value l2 = CLJ_NIL;
	clj_value l3 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[23], &cc)) goto L1;
#line 164 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[63], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t1 = l1;
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2 = clj_c_invoke(t0, a0, 1);
	(void)t1;
	if (o0) clj_release(t0);
	if (t2 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l2, t2);
#line 165 "<embedded>/clojure/test.clj"
	bool o3;
	clj_value t3 = clj_c_var_borrow(V[64], &o3);
	if (t3 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t4 = l1;
	clj_value t5 = K[177];
	clj_value a1[2] = {t4, t5};
	(void)a1;
	clj_value t6;
#ifdef CLJC_DIRECT_clojure_core_nthnext_a2
	if (!CLJC_FN_clojure_core_nthnext_a2) CLJC_FN_clojure_core_nthnext_a2 = clj_compiled_symbol("clojure_core_nthnext_a2");
	t6 = CLJC_FN_clojure_core_nthnext_a2 ? CLJC_FN_clojure_core_nthnext_a2(clj_var_root_relaxed(V[64]), NULL, a1, 2) : clj_c_invoke(t3, a1, 2);
#else
	t6 = clj_c_invoke(t3, a1, 2);
#endif
	(void)t5;
	(void)t4;
	if (o3) clj_release(t3);
	if (t6 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l3, t6);
#line 164 "<embedded>/clojure/test.clj"
	bool o7;
	clj_value t7 = clj_c_var_borrow(V[13], &o7);
	if (t7 == CLJ_THROWN) {
	goto L2;
	}
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[8], &o8);
	if (t8 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t9 = K[196];
	clj_value a4[1] = {t9};
	(void)a4;
	clj_value t10 = clj_c_invoke(t8, a4, 1);
	(void)t9;
	if (o8) clj_release(t8);
	if (t10 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t11 = l3;
	bool o12;
	clj_value t12 = clj_c_var_borrow(V[8], &o12);
	if (t12 == CLJ_THROWN) {
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o13;
	clj_value t13 = clj_c_var_borrow(V[13], &o13);
	if (t13 == CLJ_THROWN) {
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o14;
	clj_value t14 = clj_c_var_borrow(V[8], &o14);
	if (t14 == CLJ_THROWN) {
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t15 = K[197];
	clj_value a8[1] = {t15};
	(void)a8;
	clj_value t16 = clj_c_invoke(t14, a8, 1);
	(void)t15;
	if (o14) clj_release(t14);
	if (t16 == CLJ_THROWN) {
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o17;
	clj_value t17 = clj_c_var_borrow(V[8], &o17);
	if (t17 == CLJ_THROWN) {
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o18;
	clj_value t18 = clj_c_var_borrow(V[17], &o18);
	if (t18 == CLJ_THROWN) {
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o19;
	clj_value t19 = clj_c_var_borrow(V[18], &o19);
	if (t19 == CLJ_THROWN) {
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o20;
	clj_value t20 = clj_c_var_borrow(V[13], &o20);
	if (t20 == CLJ_THROWN) {
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o21;
	clj_value t21 = clj_c_var_borrow(V[8], &o21);
	if (t21 == CLJ_THROWN) {
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t22 = K[198];
	clj_value a13[1] = {t22};
	(void)a13;
	clj_value t23 = clj_c_invoke(t21, a13, 1);
	(void)t22;
	if (o21) clj_release(t21);
	if (t23 == CLJ_THROWN) {
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o24;
	clj_value t24 = clj_c_var_borrow(V[8], &o24);
	if (t24 == CLJ_THROWN) {
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o25;
	clj_value t25 = clj_c_var_borrow(V[13], &o25);
	if (t25 == CLJ_THROWN) {
	if (o24) clj_release(t24);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o26;
	clj_value t26 = clj_c_var_borrow(V[8], &o26);
	if (t26 == CLJ_THROWN) {
	if (o25) clj_release(t25);
	if (o24) clj_release(t24);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t27 = K[199];
	clj_value a17[1] = {t27};
	(void)a17;
	clj_value t28 = clj_c_invoke(t26, a17, 1);
	(void)t27;
	if (o26) clj_release(t26);
	if (t28 == CLJ_THROWN) {
	if (o25) clj_release(t25);
	if (o24) clj_release(t24);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o29;
	clj_value t29 = clj_c_var_borrow(V[8], &o29);
	if (t29 == CLJ_THROWN) {
	clj_release(t28);
	if (o25) clj_release(t25);
	if (o24) clj_release(t24);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t30 = l1;
	clj_value a18[1] = {t30};
	(void)a18;
	clj_value t31 = clj_c_invoke(t29, a18, 1);
	(void)t30;
	if (o29) clj_release(t29);
	if (t31 == CLJ_THROWN) {
	clj_release(t28);
	if (o25) clj_release(t25);
	if (o24) clj_release(t24);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a16[2] = {t28, t31};
	(void)a16;
	clj_value t32;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t32 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a16, 2) : clj_c_invoke(t25, a16, 2);
#else
	t32 = clj_c_invoke(t25, a16, 2);
#endif
	clj_release(t31);
	clj_release(t28);
	if (o25) clj_release(t25);
	if (t32 == CLJ_THROWN) {
	if (o24) clj_release(t24);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a15[1] = {t32};
	(void)a15;
	clj_value t33;
	if (CLJC_GUARD(V[12], B[0])) {
	t33 = clj_seq(t32);
	} else {
	t33 = clj_c_intrinsic_fallback(V[12], a15, 1);
	}
	clj_release(t32);
	if (t33 == CLJ_THROWN) {
	if (o24) clj_release(t24);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a14[1] = {t33};
	(void)a14;
	clj_value t34 = clj_c_invoke(t24, a14, 1);
	clj_release(t33);
	if (o24) clj_release(t24);
	if (t34 == CLJ_THROWN) {
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o35;
	clj_value t35 = clj_c_var_borrow(V[8], &o35);
	if (t35 == CLJ_THROWN) {
	clj_release(t34);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t36 = K[200];
	clj_value a19[1] = {t36};
	(void)a19;
	clj_value t37 = clj_c_invoke(t35, a19, 1);
	(void)t36;
	if (o35) clj_release(t35);
	if (t37 == CLJ_THROWN) {
	clj_release(t34);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o38;
	clj_value t38 = clj_c_var_borrow(V[8], &o38);
	if (t38 == CLJ_THROWN) {
	clj_release(t37);
	clj_release(t34);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t39 = K[201];
	clj_value a20[1] = {t39};
	(void)a20;
	clj_value t40 = clj_c_invoke(t38, a20, 1);
	(void)t39;
	if (o38) clj_release(t38);
	if (t40 == CLJ_THROWN) {
	clj_release(t37);
	clj_release(t34);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o41;
	clj_value t41 = clj_c_var_borrow(V[8], &o41);
	if (t41 == CLJ_THROWN) {
	clj_release(t40);
	clj_release(t37);
	clj_release(t34);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t42 = K[202];
	clj_value a21[1] = {t42};
	(void)a21;
	clj_value t43 = clj_c_invoke(t41, a21, 1);
	(void)t42;
	if (o41) clj_release(t41);
	if (t43 == CLJ_THROWN) {
	clj_release(t40);
	clj_release(t37);
	clj_release(t34);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o44;
	clj_value t44 = clj_c_var_borrow(V[8], &o44);
	if (t44 == CLJ_THROWN) {
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t34);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t45 = K[3];
	clj_value a22[1] = {t45};
	(void)a22;
	clj_value t46 = clj_c_invoke(t44, a22, 1);
	(void)t45;
	if (o44) clj_release(t44);
	if (t46 == CLJ_THROWN) {
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t34);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o47;
	clj_value t47 = clj_c_var_borrow(V[8], &o47);
	if (t47 == CLJ_THROWN) {
	clj_release(t46);
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t34);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t48 = K[203];
	clj_value a23[1] = {t48};
	(void)a23;
	clj_value t49 = clj_c_invoke(t47, a23, 1);
	(void)t48;
	if (o47) clj_release(t47);
	if (t49 == CLJ_THROWN) {
	clj_release(t46);
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t34);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o50;
	clj_value t50 = clj_c_var_borrow(V[8], &o50);
	if (t50 == CLJ_THROWN) {
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t34);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t51 = l0;
	clj_value a24[1] = {t51};
	(void)a24;
	clj_value t52 = clj_c_invoke(t50, a24, 1);
	(void)t51;
	if (o50) clj_release(t50);
	if (t52 == CLJ_THROWN) {
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t34);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a12[8] = {t23, t34, t37, t40, t43, t46, t49, t52};
	(void)a12;
	clj_value t53 = clj_c_invoke(t20, a12, 8);
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t34);
	clj_release(t23);
	if (o20) clj_release(t20);
	if (t53 == CLJ_THROWN) {
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a11[1] = {t53};
	(void)a11;
	clj_value t54;
	if (CLJC_GUARD(V[12], B[0])) {
	t54 = clj_seq(t53);
	} else {
	t54 = clj_c_intrinsic_fallback(V[12], a11, 1);
	}
	clj_release(t53);
	if (t54 == CLJ_THROWN) {
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a10[2] = {t19, t54};
	(void)a10;
	clj_value t55 = clj_c_invoke(t18, a10, 2);
	clj_release(t54);
	if (o19) clj_release(t19);
	if (o18) clj_release(t18);
	if (t55 == CLJ_THROWN) {
	if (o17) clj_release(t17);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a9[1] = {t55};
	(void)a9;
	clj_value t56 = clj_c_invoke(t17, a9, 1);
	clj_release(t55);
	if (o17) clj_release(t17);
	if (t56 == CLJ_THROWN) {
	clj_release(t16);
	if (o13) clj_release(t13);
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a7[2] = {t16, t56};
	(void)a7;
	clj_value t57;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t57 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a7, 2) : clj_c_invoke(t13, a7, 2);
#else
	t57 = clj_c_invoke(t13, a7, 2);
#endif
	clj_release(t56);
	clj_release(t16);
	if (o13) clj_release(t13);
	if (t57 == CLJ_THROWN) {
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a6[1] = {t57};
	(void)a6;
	clj_value t58;
	if (CLJC_GUARD(V[12], B[0])) {
	t58 = clj_seq(t57);
	} else {
	t58 = clj_c_intrinsic_fallback(V[12], a6, 1);
	}
	clj_release(t57);
	if (t58 == CLJ_THROWN) {
	if (o12) clj_release(t12);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a5[1] = {t58};
	(void)a5;
	clj_value t59 = clj_c_invoke(t12, a5, 1);
	clj_release(t58);
	if (o12) clj_release(t12);
	if (t59 == CLJ_THROWN) {
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o60;
	clj_value t60 = clj_c_var_borrow(V[8], &o60);
	if (t60 == CLJ_THROWN) {
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o61;
	clj_value t61 = clj_c_var_borrow(V[13], &o61);
	if (t61 == CLJ_THROWN) {
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o62;
	clj_value t62 = clj_c_var_borrow(V[8], &o62);
	if (t62 == CLJ_THROWN) {
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t63 = K[204];
	clj_value a28[1] = {t63};
	(void)a28;
	clj_value t64 = clj_c_invoke(t62, a28, 1);
	(void)t63;
	if (o62) clj_release(t62);
	if (t64 == CLJ_THROWN) {
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o65;
	clj_value t65 = clj_c_var_borrow(V[8], &o65);
	if (t65 == CLJ_THROWN) {
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t66 = l2;
	clj_value a29[1] = {t66};
	(void)a29;
	clj_value t67 = clj_c_invoke(t65, a29, 1);
	(void)t66;
	if (o65) clj_release(t65);
	if (t67 == CLJ_THROWN) {
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o68;
	clj_value t68 = clj_c_var_borrow(V[8], &o68);
	if (t68 == CLJ_THROWN) {
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t69 = K[205];
	clj_value a30[1] = {t69};
	(void)a30;
	clj_value t70 = clj_c_invoke(t68, a30, 1);
	(void)t69;
	if (o68) clj_release(t68);
	if (t70 == CLJ_THROWN) {
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o71;
	clj_value t71 = clj_c_var_borrow(V[8], &o71);
	if (t71 == CLJ_THROWN) {
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o72;
	clj_value t72 = clj_c_var_borrow(V[13], &o72);
	if (t72 == CLJ_THROWN) {
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o73;
	clj_value t73 = clj_c_var_borrow(V[8], &o73);
	if (t73 == CLJ_THROWN) {
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t74 = K[206];
	clj_value a34[1] = {t74};
	(void)a34;
	clj_value t75 = clj_c_invoke(t73, a34, 1);
	(void)t74;
	if (o73) clj_release(t73);
	if (t75 == CLJ_THROWN) {
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o76;
	clj_value t76 = clj_c_var_borrow(V[8], &o76);
	if (t76 == CLJ_THROWN) {
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o77;
	clj_value t77 = clj_c_var_borrow(V[17], &o77);
	if (t77 == CLJ_THROWN) {
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o78;
	clj_value t78 = clj_c_var_borrow(V[18], &o78);
	if (t78 == CLJ_THROWN) {
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o79;
	clj_value t79 = clj_c_var_borrow(V[13], &o79);
	if (t79 == CLJ_THROWN) {
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o80;
	clj_value t80 = clj_c_var_borrow(V[8], &o80);
	if (t80 == CLJ_THROWN) {
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t81 = K[198];
	clj_value a39[1] = {t81};
	(void)a39;
	clj_value t82 = clj_c_invoke(t80, a39, 1);
	(void)t81;
	if (o80) clj_release(t80);
	if (t82 == CLJ_THROWN) {
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o83;
	clj_value t83 = clj_c_var_borrow(V[8], &o83);
	if (t83 == CLJ_THROWN) {
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o84;
	clj_value t84 = clj_c_var_borrow(V[13], &o84);
	if (t84 == CLJ_THROWN) {
	if (o83) clj_release(t83);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o85;
	clj_value t85 = clj_c_var_borrow(V[8], &o85);
	if (t85 == CLJ_THROWN) {
	if (o84) clj_release(t84);
	if (o83) clj_release(t83);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t86 = K[207];
	clj_value a43[1] = {t86};
	(void)a43;
	clj_value t87 = clj_c_invoke(t85, a43, 1);
	(void)t86;
	if (o85) clj_release(t85);
	if (t87 == CLJ_THROWN) {
	if (o84) clj_release(t84);
	if (o83) clj_release(t83);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o88;
	clj_value t88 = clj_c_var_borrow(V[8], &o88);
	if (t88 == CLJ_THROWN) {
	clj_release(t87);
	if (o84) clj_release(t84);
	if (o83) clj_release(t83);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t89 = l1;
	clj_value a44[1] = {t89};
	(void)a44;
	clj_value t90 = clj_c_invoke(t88, a44, 1);
	(void)t89;
	if (o88) clj_release(t88);
	if (t90 == CLJ_THROWN) {
	clj_release(t87);
	if (o84) clj_release(t84);
	if (o83) clj_release(t83);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a42[2] = {t87, t90};
	(void)a42;
	clj_value t91;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t91 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a42, 2) : clj_c_invoke(t84, a42, 2);
#else
	t91 = clj_c_invoke(t84, a42, 2);
#endif
	clj_release(t90);
	clj_release(t87);
	if (o84) clj_release(t84);
	if (t91 == CLJ_THROWN) {
	if (o83) clj_release(t83);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a41[1] = {t91};
	(void)a41;
	clj_value t92;
	if (CLJC_GUARD(V[12], B[0])) {
	t92 = clj_seq(t91);
	} else {
	t92 = clj_c_intrinsic_fallback(V[12], a41, 1);
	}
	clj_release(t91);
	if (t92 == CLJ_THROWN) {
	if (o83) clj_release(t83);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a40[1] = {t92};
	(void)a40;
	clj_value t93 = clj_c_invoke(t83, a40, 1);
	clj_release(t92);
	if (o83) clj_release(t83);
	if (t93 == CLJ_THROWN) {
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o94;
	clj_value t94 = clj_c_var_borrow(V[8], &o94);
	if (t94 == CLJ_THROWN) {
	clj_release(t93);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t95 = K[200];
	clj_value a45[1] = {t95};
	(void)a45;
	clj_value t96 = clj_c_invoke(t94, a45, 1);
	(void)t95;
	if (o94) clj_release(t94);
	if (t96 == CLJ_THROWN) {
	clj_release(t93);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o97;
	clj_value t97 = clj_c_var_borrow(V[8], &o97);
	if (t97 == CLJ_THROWN) {
	clj_release(t96);
	clj_release(t93);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t98 = K[208];
	clj_value a46[1] = {t98};
	(void)a46;
	clj_value t99 = clj_c_invoke(t97, a46, 1);
	(void)t98;
	if (o97) clj_release(t97);
	if (t99 == CLJ_THROWN) {
	clj_release(t96);
	clj_release(t93);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o100;
	clj_value t100 = clj_c_var_borrow(V[8], &o100);
	if (t100 == CLJ_THROWN) {
	clj_release(t99);
	clj_release(t96);
	clj_release(t93);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t101 = K[202];
	clj_value a47[1] = {t101};
	(void)a47;
	clj_value t102 = clj_c_invoke(t100, a47, 1);
	(void)t101;
	if (o100) clj_release(t100);
	if (t102 == CLJ_THROWN) {
	clj_release(t99);
	clj_release(t96);
	clj_release(t93);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o103;
	clj_value t103 = clj_c_var_borrow(V[8], &o103);
	if (t103 == CLJ_THROWN) {
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t93);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t104 = K[205];
	clj_value a48[1] = {t104};
	(void)a48;
	clj_value t105 = clj_c_invoke(t103, a48, 1);
	(void)t104;
	if (o103) clj_release(t103);
	if (t105 == CLJ_THROWN) {
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t93);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o106;
	clj_value t106 = clj_c_var_borrow(V[8], &o106);
	if (t106 == CLJ_THROWN) {
	clj_release(t105);
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t93);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t107 = K[203];
	clj_value a49[1] = {t107};
	(void)a49;
	clj_value t108 = clj_c_invoke(t106, a49, 1);
	(void)t107;
	if (o106) clj_release(t106);
	if (t108 == CLJ_THROWN) {
	clj_release(t105);
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t93);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o109;
	clj_value t109 = clj_c_var_borrow(V[8], &o109);
	if (t109 == CLJ_THROWN) {
	clj_release(t108);
	clj_release(t105);
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t93);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t110 = l0;
	clj_value a50[1] = {t110};
	(void)a50;
	clj_value t111 = clj_c_invoke(t109, a50, 1);
	(void)t110;
	if (o109) clj_release(t109);
	if (t111 == CLJ_THROWN) {
	clj_release(t108);
	clj_release(t105);
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t93);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a38[8] = {t82, t93, t96, t99, t102, t105, t108, t111};
	(void)a38;
	clj_value t112 = clj_c_invoke(t79, a38, 8);
	clj_release(t111);
	clj_release(t108);
	clj_release(t105);
	clj_release(t102);
	clj_release(t99);
	clj_release(t96);
	clj_release(t93);
	clj_release(t82);
	if (o79) clj_release(t79);
	if (t112 == CLJ_THROWN) {
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a37[1] = {t112};
	(void)a37;
	clj_value t113;
	if (CLJC_GUARD(V[12], B[0])) {
	t113 = clj_seq(t112);
	} else {
	t113 = clj_c_intrinsic_fallback(V[12], a37, 1);
	}
	clj_release(t112);
	if (t113 == CLJ_THROWN) {
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a36[2] = {t78, t113};
	(void)a36;
	clj_value t114 = clj_c_invoke(t77, a36, 2);
	clj_release(t113);
	if (o78) clj_release(t78);
	if (o77) clj_release(t77);
	if (t114 == CLJ_THROWN) {
	if (o76) clj_release(t76);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a35[1] = {t114};
	(void)a35;
	clj_value t115 = clj_c_invoke(t76, a35, 1);
	clj_release(t114);
	if (o76) clj_release(t76);
	if (t115 == CLJ_THROWN) {
	clj_release(t75);
	if (o72) clj_release(t72);
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a33[2] = {t75, t115};
	(void)a33;
	clj_value t116;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t116 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a33, 2) : clj_c_invoke(t72, a33, 2);
#else
	t116 = clj_c_invoke(t72, a33, 2);
#endif
	clj_release(t115);
	clj_release(t75);
	if (o72) clj_release(t72);
	if (t116 == CLJ_THROWN) {
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a32[1] = {t116};
	(void)a32;
	clj_value t117;
	if (CLJC_GUARD(V[12], B[0])) {
	t117 = clj_seq(t116);
	} else {
	t117 = clj_c_intrinsic_fallback(V[12], a32, 1);
	}
	clj_release(t116);
	if (t117 == CLJ_THROWN) {
	if (o71) clj_release(t71);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a31[1] = {t117};
	(void)a31;
	clj_value t118 = clj_c_invoke(t71, a31, 1);
	clj_release(t117);
	if (o71) clj_release(t71);
	if (t118 == CLJ_THROWN) {
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o119;
	clj_value t119 = clj_c_var_borrow(V[8], &o119);
	if (t119 == CLJ_THROWN) {
	clj_release(t118);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t120 = K[205];
	clj_value a51[1] = {t120};
	(void)a51;
	clj_value t121 = clj_c_invoke(t119, a51, 1);
	(void)t120;
	if (o119) clj_release(t119);
	if (t121 == CLJ_THROWN) {
	clj_release(t118);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a27[5] = {t64, t67, t70, t118, t121};
	(void)a27;
	clj_value t122 = clj_c_invoke(t61, a27, 5);
	clj_release(t121);
	clj_release(t118);
	clj_release(t70);
	clj_release(t67);
	clj_release(t64);
	if (o61) clj_release(t61);
	if (t122 == CLJ_THROWN) {
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a26[1] = {t122};
	(void)a26;
	clj_value t123;
	if (CLJC_GUARD(V[12], B[0])) {
	t123 = clj_seq(t122);
	} else {
	t123 = clj_c_intrinsic_fallback(V[12], a26, 1);
	}
	clj_release(t122);
	if (t123 == CLJ_THROWN) {
	if (o60) clj_release(t60);
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a25[1] = {t123};
	(void)a25;
	clj_value t124 = clj_c_invoke(t60, a25, 1);
	clj_release(t123);
	if (o60) clj_release(t60);
	if (t124 == CLJ_THROWN) {
	clj_release(t59);
	clj_release(t10);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a3[4] = {t10, t11, t59, t124};
	(void)a3;
	clj_value t125 = clj_c_invoke(t7, a3, 4);
	clj_release(t124);
	clj_release(t59);
	(void)t11;
	clj_release(t10);
	if (o7) clj_release(t7);
	if (t125 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a2[1] = {t125};
	(void)a2;
	clj_value t126;
	if (CLJC_GUARD(V[12], B[0])) {
	t126 = clj_seq(t125);
	} else {
	t126 = clj_c_intrinsic_fallback(V[12], a2, 1);
	}
	clj_release(t125);
	if (t126 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[23], &cc);
	clj_release(l2);
	clj_release(l3);
	return t126;
L2: ;
	clj_c_leave(&S[23], &cc);
L1: ;
	clj_release(l2);
	clj_release(l3);
	return CLJ_THROWN;
}

static clj_value clojure_test_form46__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 2: return clojure_test_form46__0_a2(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_46(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 163 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[60], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[195];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form46__0, NULL, 0, 0x4, 2, 2);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_47(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 163 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[60]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form48__0_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_value l2 = CLJ_NIL;
	clj_value l3 = CLJ_NIL;
	clj_value l4 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[24], &cc)) goto L1;
#line 175 "<embedded>/clojure/test.clj"
	clj_value t0 = l1;
	clj_value t1 = K[175];
	clj_value a0[2] = {t0, t1};
	(void)a0;
	clj_value t2;
	if (CLJC_GUARD(V[62], B[12])) {
	t2 = clj_nth2(t0, t1);
	} else {
	t2 = clj_c_intrinsic_fallback(V[62], a0, 2);
	}
	(void)t1;
	(void)t0;
	if (t2 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l2, t2);
#line 176 "<embedded>/clojure/test.clj"
	clj_value t3 = l1;
	clj_value t4 = K[177];
	clj_value a1[2] = {t3, t4};
	(void)a1;
	clj_value t5;
	if (CLJC_GUARD(V[62], B[12])) {
	t5 = clj_nth2(t3, t4);
	} else {
	t5 = clj_c_intrinsic_fallback(V[62], a1, 2);
	}
	(void)t4;
	(void)t3;
	if (t5 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l3, t5);
#line 177 "<embedded>/clojure/test.clj"
	bool o6;
	clj_value t6 = clj_c_var_borrow(V[64], &o6);
	if (t6 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t7 = l1;
	clj_value t8 = K[211];
	clj_value a2[2] = {t7, t8};
	(void)a2;
	clj_value t9;
#ifdef CLJC_DIRECT_clojure_core_nthnext_a2
	if (!CLJC_FN_clojure_core_nthnext_a2) CLJC_FN_clojure_core_nthnext_a2 = clj_compiled_symbol("clojure_core_nthnext_a2");
	t9 = CLJC_FN_clojure_core_nthnext_a2 ? CLJC_FN_clojure_core_nthnext_a2(clj_var_root_relaxed(V[64]), NULL, a2, 2) : clj_c_invoke(t6, a2, 2);
#else
	t9 = clj_c_invoke(t6, a2, 2);
#endif
	(void)t8;
	(void)t7;
	if (o6) clj_release(t6);
	if (t9 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l4, t9);
#line 175 "<embedded>/clojure/test.clj"
	bool o10;
	clj_value t10 = clj_c_var_borrow(V[13], &o10);
	if (t10 == CLJ_THROWN) {
	goto L2;
	}
	bool o11;
	clj_value t11 = clj_c_var_borrow(V[8], &o11);
	if (t11 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t12 = K[212];
	clj_value a5[1] = {t12};
	(void)a5;
	clj_value t13 = clj_c_invoke(t11, a5, 1);
	(void)t12;
	if (o11) clj_release(t11);
	if (t13 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t14 = l4;
	bool o15;
	clj_value t15 = clj_c_var_borrow(V[8], &o15);
	if (t15 == CLJ_THROWN) {
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o16;
	clj_value t16 = clj_c_var_borrow(V[13], &o16);
	if (t16 == CLJ_THROWN) {
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o17;
	clj_value t17 = clj_c_var_borrow(V[8], &o17);
	if (t17 == CLJ_THROWN) {
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t18 = K[213];
	clj_value a9[1] = {t18};
	(void)a9;
	clj_value t19 = clj_c_invoke(t17, a9, 1);
	(void)t18;
	if (o17) clj_release(t17);
	if (t19 == CLJ_THROWN) {
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o20;
	clj_value t20 = clj_c_var_borrow(V[8], &o20);
	if (t20 == CLJ_THROWN) {
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o21;
	clj_value t21 = clj_c_var_borrow(V[17], &o21);
	if (t21 == CLJ_THROWN) {
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o22;
	clj_value t22 = clj_c_var_borrow(V[18], &o22);
	if (t22 == CLJ_THROWN) {
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o23;
	clj_value t23 = clj_c_var_borrow(V[13], &o23);
	if (t23 == CLJ_THROWN) {
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o24;
	clj_value t24 = clj_c_var_borrow(V[8], &o24);
	if (t24 == CLJ_THROWN) {
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t25 = K[214];
	clj_value a14[1] = {t25};
	(void)a14;
	clj_value t26 = clj_c_invoke(t24, a14, 1);
	(void)t25;
	if (o24) clj_release(t24);
	if (t26 == CLJ_THROWN) {
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o27;
	clj_value t27 = clj_c_var_borrow(V[8], &o27);
	if (t27 == CLJ_THROWN) {
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o28;
	clj_value t28 = clj_c_var_borrow(V[13], &o28);
	if (t28 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o29;
	clj_value t29 = clj_c_var_borrow(V[8], &o29);
	if (t29 == CLJ_THROWN) {
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t30 = K[215];
	clj_value a18[1] = {t30};
	(void)a18;
	clj_value t31 = clj_c_invoke(t29, a18, 1);
	(void)t30;
	if (o29) clj_release(t29);
	if (t31 == CLJ_THROWN) {
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o32;
	clj_value t32 = clj_c_var_borrow(V[8], &o32);
	if (t32 == CLJ_THROWN) {
	clj_release(t31);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t33 = l1;
	clj_value a19[1] = {t33};
	(void)a19;
	clj_value t34 = clj_c_invoke(t32, a19, 1);
	(void)t33;
	if (o32) clj_release(t32);
	if (t34 == CLJ_THROWN) {
	clj_release(t31);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a17[2] = {t31, t34};
	(void)a17;
	clj_value t35;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t35 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a17, 2) : clj_c_invoke(t28, a17, 2);
#else
	t35 = clj_c_invoke(t28, a17, 2);
#endif
	clj_release(t34);
	clj_release(t31);
	if (o28) clj_release(t28);
	if (t35 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a16[1] = {t35};
	(void)a16;
	clj_value t36;
	if (CLJC_GUARD(V[12], B[0])) {
	t36 = clj_seq(t35);
	} else {
	t36 = clj_c_intrinsic_fallback(V[12], a16, 1);
	}
	clj_release(t35);
	if (t36 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a15[1] = {t36};
	(void)a15;
	clj_value t37 = clj_c_invoke(t27, a15, 1);
	clj_release(t36);
	if (o27) clj_release(t27);
	if (t37 == CLJ_THROWN) {
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o38;
	clj_value t38 = clj_c_var_borrow(V[8], &o38);
	if (t38 == CLJ_THROWN) {
	clj_release(t37);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t39 = K[216];
	clj_value a20[1] = {t39};
	(void)a20;
	clj_value t40 = clj_c_invoke(t38, a20, 1);
	(void)t39;
	if (o38) clj_release(t38);
	if (t40 == CLJ_THROWN) {
	clj_release(t37);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o41;
	clj_value t41 = clj_c_var_borrow(V[8], &o41);
	if (t41 == CLJ_THROWN) {
	clj_release(t40);
	clj_release(t37);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t42 = K[217];
	clj_value a21[1] = {t42};
	(void)a21;
	clj_value t43 = clj_c_invoke(t41, a21, 1);
	(void)t42;
	if (o41) clj_release(t41);
	if (t43 == CLJ_THROWN) {
	clj_release(t40);
	clj_release(t37);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o44;
	clj_value t44 = clj_c_var_borrow(V[8], &o44);
	if (t44 == CLJ_THROWN) {
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t45 = K[218];
	clj_value a22[1] = {t45};
	(void)a22;
	clj_value t46 = clj_c_invoke(t44, a22, 1);
	(void)t45;
	if (o44) clj_release(t44);
	if (t46 == CLJ_THROWN) {
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o47;
	clj_value t47 = clj_c_var_borrow(V[8], &o47);
	if (t47 == CLJ_THROWN) {
	clj_release(t46);
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t48 = K[3];
	clj_value a23[1] = {t48};
	(void)a23;
	clj_value t49 = clj_c_invoke(t47, a23, 1);
	(void)t48;
	if (o47) clj_release(t47);
	if (t49 == CLJ_THROWN) {
	clj_release(t46);
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o50;
	clj_value t50 = clj_c_var_borrow(V[8], &o50);
	if (t50 == CLJ_THROWN) {
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t51 = K[219];
	clj_value a24[1] = {t51};
	(void)a24;
	clj_value t52 = clj_c_invoke(t50, a24, 1);
	(void)t51;
	if (o50) clj_release(t50);
	if (t52 == CLJ_THROWN) {
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o53;
	clj_value t53 = clj_c_var_borrow(V[8], &o53);
	if (t53 == CLJ_THROWN) {
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t54 = l0;
	clj_value a25[1] = {t54};
	(void)a25;
	clj_value t55 = clj_c_invoke(t53, a25, 1);
	(void)t54;
	if (o53) clj_release(t53);
	if (t55 == CLJ_THROWN) {
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a13[8] = {t26, t37, t40, t43, t46, t49, t52, t55};
	(void)a13;
	clj_value t56 = clj_c_invoke(t23, a13, 8);
	clj_release(t55);
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t40);
	clj_release(t37);
	clj_release(t26);
	if (o23) clj_release(t23);
	if (t56 == CLJ_THROWN) {
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a12[1] = {t56};
	(void)a12;
	clj_value t57;
	if (CLJC_GUARD(V[12], B[0])) {
	t57 = clj_seq(t56);
	} else {
	t57 = clj_c_intrinsic_fallback(V[12], a12, 1);
	}
	clj_release(t56);
	if (t57 == CLJ_THROWN) {
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a11[2] = {t22, t57};
	(void)a11;
	clj_value t58 = clj_c_invoke(t21, a11, 2);
	clj_release(t57);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (t58 == CLJ_THROWN) {
	if (o20) clj_release(t20);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a10[1] = {t58};
	(void)a10;
	clj_value t59 = clj_c_invoke(t20, a10, 1);
	clj_release(t58);
	if (o20) clj_release(t20);
	if (t59 == CLJ_THROWN) {
	clj_release(t19);
	if (o16) clj_release(t16);
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a8[2] = {t19, t59};
	(void)a8;
	clj_value t60;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t60 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a8, 2) : clj_c_invoke(t16, a8, 2);
#else
	t60 = clj_c_invoke(t16, a8, 2);
#endif
	clj_release(t59);
	clj_release(t19);
	if (o16) clj_release(t16);
	if (t60 == CLJ_THROWN) {
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a7[1] = {t60};
	(void)a7;
	clj_value t61;
	if (CLJC_GUARD(V[12], B[0])) {
	t61 = clj_seq(t60);
	} else {
	t61 = clj_c_intrinsic_fallback(V[12], a7, 1);
	}
	clj_release(t60);
	if (t61 == CLJ_THROWN) {
	if (o15) clj_release(t15);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a6[1] = {t61};
	(void)a6;
	clj_value t62 = clj_c_invoke(t15, a6, 1);
	clj_release(t61);
	if (o15) clj_release(t15);
	if (t62 == CLJ_THROWN) {
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o63;
	clj_value t63 = clj_c_var_borrow(V[8], &o63);
	if (t63 == CLJ_THROWN) {
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o64;
	clj_value t64 = clj_c_var_borrow(V[13], &o64);
	if (t64 == CLJ_THROWN) {
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o65;
	clj_value t65 = clj_c_var_borrow(V[8], &o65);
	if (t65 == CLJ_THROWN) {
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t66 = K[220];
	clj_value a29[1] = {t66};
	(void)a29;
	clj_value t67 = clj_c_invoke(t65, a29, 1);
	(void)t66;
	if (o65) clj_release(t65);
	if (t67 == CLJ_THROWN) {
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o68;
	clj_value t68 = clj_c_var_borrow(V[8], &o68);
	if (t68 == CLJ_THROWN) {
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t69 = l2;
	clj_value a30[1] = {t69};
	(void)a30;
	clj_value t70 = clj_c_invoke(t68, a30, 1);
	(void)t69;
	if (o68) clj_release(t68);
	if (t70 == CLJ_THROWN) {
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o71;
	clj_value t71 = clj_c_var_borrow(V[8], &o71);
	if (t71 == CLJ_THROWN) {
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t72 = K[221];
	clj_value a31[1] = {t72};
	(void)a31;
	clj_value t73 = clj_c_invoke(t71, a31, 1);
	(void)t72;
	if (o71) clj_release(t71);
	if (t73 == CLJ_THROWN) {
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o74;
	clj_value t74 = clj_c_var_borrow(V[8], &o74);
	if (t74 == CLJ_THROWN) {
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o75;
	clj_value t75 = clj_c_var_borrow(V[13], &o75);
	if (t75 == CLJ_THROWN) {
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o76;
	clj_value t76 = clj_c_var_borrow(V[8], &o76);
	if (t76 == CLJ_THROWN) {
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t77 = K[222];
	clj_value a35[1] = {t77};
	(void)a35;
	clj_value t78 = clj_c_invoke(t76, a35, 1);
	(void)t77;
	if (o76) clj_release(t76);
	if (t78 == CLJ_THROWN) {
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o79;
	clj_value t79 = clj_c_var_borrow(V[8], &o79);
	if (t79 == CLJ_THROWN) {
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o80;
	clj_value t80 = clj_c_var_borrow(V[17], &o80);
	if (t80 == CLJ_THROWN) {
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o81;
	clj_value t81 = clj_c_var_borrow(V[58], &o81);
	if (t81 == CLJ_THROWN) {
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o82;
	clj_value t82 = clj_c_var_borrow(V[13], &o82);
	if (t82 == CLJ_THROWN) {
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o83;
	clj_value t83 = clj_c_var_borrow(V[8], &o83);
	if (t83 == CLJ_THROWN) {
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t84 = K[223];
	clj_value a40[1] = {t84};
	(void)a40;
	clj_value t85 = clj_c_invoke(t83, a40, 1);
	(void)t84;
	if (o83) clj_release(t83);
	if (t85 == CLJ_THROWN) {
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o86;
	clj_value t86 = clj_c_var_borrow(V[8], &o86);
	if (t86 == CLJ_THROWN) {
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o87;
	clj_value t87 = clj_c_var_borrow(V[13], &o87);
	if (t87 == CLJ_THROWN) {
	if (o86) clj_release(t86);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o88;
	clj_value t88 = clj_c_var_borrow(V[8], &o88);
	if (t88 == CLJ_THROWN) {
	if (o87) clj_release(t87);
	if (o86) clj_release(t86);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t89 = K[224];
	clj_value a44[1] = {t89};
	(void)a44;
	clj_value t90 = clj_c_invoke(t88, a44, 1);
	(void)t89;
	if (o88) clj_release(t88);
	if (t90 == CLJ_THROWN) {
	if (o87) clj_release(t87);
	if (o86) clj_release(t86);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o91;
	clj_value t91 = clj_c_var_borrow(V[8], &o91);
	if (t91 == CLJ_THROWN) {
	clj_release(t90);
	if (o87) clj_release(t87);
	if (o86) clj_release(t86);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t92 = K[221];
	clj_value a45[1] = {t92};
	(void)a45;
	clj_value t93 = clj_c_invoke(t91, a45, 1);
	(void)t92;
	if (o91) clj_release(t91);
	if (t93 == CLJ_THROWN) {
	clj_release(t90);
	if (o87) clj_release(t87);
	if (o86) clj_release(t86);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a43[2] = {t90, t93};
	(void)a43;
	clj_value t94;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t94 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a43, 2) : clj_c_invoke(t87, a43, 2);
#else
	t94 = clj_c_invoke(t87, a43, 2);
#endif
	clj_release(t93);
	clj_release(t90);
	if (o87) clj_release(t87);
	if (t94 == CLJ_THROWN) {
	if (o86) clj_release(t86);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a42[1] = {t94};
	(void)a42;
	clj_value t95;
	if (CLJC_GUARD(V[12], B[0])) {
	t95 = clj_seq(t94);
	} else {
	t95 = clj_c_intrinsic_fallback(V[12], a42, 1);
	}
	clj_release(t94);
	if (t95 == CLJ_THROWN) {
	if (o86) clj_release(t86);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a41[1] = {t95};
	(void)a41;
	clj_value t96 = clj_c_invoke(t86, a41, 1);
	clj_release(t95);
	if (o86) clj_release(t86);
	if (t96 == CLJ_THROWN) {
	clj_release(t85);
	if (o82) clj_release(t82);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a39[2] = {t85, t96};
	(void)a39;
	clj_value t97;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t97 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a39, 2) : clj_c_invoke(t82, a39, 2);
#else
	t97 = clj_c_invoke(t82, a39, 2);
#endif
	clj_release(t96);
	clj_release(t85);
	if (o82) clj_release(t82);
	if (t97 == CLJ_THROWN) {
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a38[1] = {t97};
	(void)a38;
	clj_value t98;
	if (CLJC_GUARD(V[12], B[0])) {
	t98 = clj_seq(t97);
	} else {
	t98 = clj_c_intrinsic_fallback(V[12], a38, 1);
	}
	clj_release(t97);
	if (t98 == CLJ_THROWN) {
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a37[2] = {t81, t98};
	(void)a37;
	clj_value t99 = clj_c_invoke(t80, a37, 2);
	clj_release(t98);
	if (o81) clj_release(t81);
	if (o80) clj_release(t80);
	if (t99 == CLJ_THROWN) {
	if (o79) clj_release(t79);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a36[1] = {t99};
	(void)a36;
	clj_value t100 = clj_c_invoke(t79, a36, 1);
	clj_release(t99);
	if (o79) clj_release(t79);
	if (t100 == CLJ_THROWN) {
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o101;
	clj_value t101 = clj_c_var_borrow(V[8], &o101);
	if (t101 == CLJ_THROWN) {
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o102;
	clj_value t102 = clj_c_var_borrow(V[13], &o102);
	if (t102 == CLJ_THROWN) {
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o103;
	clj_value t103 = clj_c_var_borrow(V[8], &o103);
	if (t103 == CLJ_THROWN) {
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t104 = K[225];
	clj_value a49[1] = {t104};
	(void)a49;
	clj_value t105 = clj_c_invoke(t103, a49, 1);
	(void)t104;
	if (o103) clj_release(t103);
	if (t105 == CLJ_THROWN) {
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o106;
	clj_value t106 = clj_c_var_borrow(V[8], &o106);
	if (t106 == CLJ_THROWN) {
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o107;
	clj_value t107 = clj_c_var_borrow(V[13], &o107);
	if (t107 == CLJ_THROWN) {
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o108;
	clj_value t108 = clj_c_var_borrow(V[8], &o108);
	if (t108 == CLJ_THROWN) {
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t109 = K[226];
	clj_value a53[1] = {t109};
	(void)a53;
	clj_value t110 = clj_c_invoke(t108, a53, 1);
	(void)t109;
	if (o108) clj_release(t108);
	if (t110 == CLJ_THROWN) {
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o111;
	clj_value t111 = clj_c_var_borrow(V[8], &o111);
	if (t111 == CLJ_THROWN) {
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o112;
	clj_value t112 = clj_c_var_borrow(V[13], &o112);
	if (t112 == CLJ_THROWN) {
	if (o111) clj_release(t111);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o113;
	clj_value t113 = clj_c_var_borrow(V[8], &o113);
	if (t113 == CLJ_THROWN) {
	if (o112) clj_release(t112);
	if (o111) clj_release(t111);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t114 = K[227];
	clj_value a57[1] = {t114};
	(void)a57;
	clj_value t115 = clj_c_invoke(t113, a57, 1);
	(void)t114;
	if (o113) clj_release(t113);
	if (t115 == CLJ_THROWN) {
	if (o112) clj_release(t112);
	if (o111) clj_release(t111);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o116;
	clj_value t116 = clj_c_var_borrow(V[8], &o116);
	if (t116 == CLJ_THROWN) {
	clj_release(t115);
	if (o112) clj_release(t112);
	if (o111) clj_release(t111);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t117 = K[223];
	clj_value a58[1] = {t117};
	(void)a58;
	clj_value t118 = clj_c_invoke(t116, a58, 1);
	(void)t117;
	if (o116) clj_release(t116);
	if (t118 == CLJ_THROWN) {
	clj_release(t115);
	if (o112) clj_release(t112);
	if (o111) clj_release(t111);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a56[2] = {t115, t118};
	(void)a56;
	clj_value t119;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t119 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a56, 2) : clj_c_invoke(t112, a56, 2);
#else
	t119 = clj_c_invoke(t112, a56, 2);
#endif
	clj_release(t118);
	clj_release(t115);
	if (o112) clj_release(t112);
	if (t119 == CLJ_THROWN) {
	if (o111) clj_release(t111);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a55[1] = {t119};
	(void)a55;
	clj_value t120;
	if (CLJC_GUARD(V[12], B[0])) {
	t120 = clj_seq(t119);
	} else {
	t120 = clj_c_intrinsic_fallback(V[12], a55, 1);
	}
	clj_release(t119);
	if (t120 == CLJ_THROWN) {
	if (o111) clj_release(t111);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a54[1] = {t120};
	(void)a54;
	clj_value t121 = clj_c_invoke(t111, a54, 1);
	clj_release(t120);
	if (o111) clj_release(t111);
	if (t121 == CLJ_THROWN) {
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o122;
	clj_value t122 = clj_c_var_borrow(V[8], &o122);
	if (t122 == CLJ_THROWN) {
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o123;
	clj_value t123 = clj_c_var_borrow(V[13], &o123);
	if (t123 == CLJ_THROWN) {
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o124;
	clj_value t124 = clj_c_var_borrow(V[8], &o124);
	if (t124 == CLJ_THROWN) {
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t125 = K[228];
	clj_value a62[1] = {t125};
	(void)a62;
	clj_value t126 = clj_c_invoke(t124, a62, 1);
	(void)t125;
	if (o124) clj_release(t124);
	if (t126 == CLJ_THROWN) {
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o127;
	clj_value t127 = clj_c_var_borrow(V[8], &o127);
	if (t127 == CLJ_THROWN) {
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o128;
	clj_value t128 = clj_c_var_borrow(V[13], &o128);
	if (t128 == CLJ_THROWN) {
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o129;
	clj_value t129 = clj_c_var_borrow(V[8], &o129);
	if (t129 == CLJ_THROWN) {
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t130 = K[229];
	clj_value a66[1] = {t130};
	(void)a66;
	clj_value t131 = clj_c_invoke(t129, a66, 1);
	(void)t130;
	if (o129) clj_release(t129);
	if (t131 == CLJ_THROWN) {
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o132;
	clj_value t132 = clj_c_var_borrow(V[8], &o132);
	if (t132 == CLJ_THROWN) {
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t133 = l3;
	clj_value a67[1] = {t133};
	(void)a67;
	clj_value t134 = clj_c_invoke(t132, a67, 1);
	(void)t133;
	if (o132) clj_release(t132);
	if (t134 == CLJ_THROWN) {
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o135;
	clj_value t135 = clj_c_var_borrow(V[8], &o135);
	if (t135 == CLJ_THROWN) {
	clj_release(t134);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t136 = K[223];
	clj_value a68[1] = {t136};
	(void)a68;
	clj_value t137 = clj_c_invoke(t135, a68, 1);
	(void)t136;
	if (o135) clj_release(t135);
	if (t137 == CLJ_THROWN) {
	clj_release(t134);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a65[3] = {t131, t134, t137};
	(void)a65;
	clj_value t138 = clj_c_invoke(t128, a65, 3);
	clj_release(t137);
	clj_release(t134);
	clj_release(t131);
	if (o128) clj_release(t128);
	if (t138 == CLJ_THROWN) {
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a64[1] = {t138};
	(void)a64;
	clj_value t139;
	if (CLJC_GUARD(V[12], B[0])) {
	t139 = clj_seq(t138);
	} else {
	t139 = clj_c_intrinsic_fallback(V[12], a64, 1);
	}
	clj_release(t138);
	if (t139 == CLJ_THROWN) {
	if (o127) clj_release(t127);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a63[1] = {t139};
	(void)a63;
	clj_value t140 = clj_c_invoke(t127, a63, 1);
	clj_release(t139);
	if (o127) clj_release(t127);
	if (t140 == CLJ_THROWN) {
	clj_release(t126);
	if (o123) clj_release(t123);
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a61[2] = {t126, t140};
	(void)a61;
	clj_value t141;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t141 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a61, 2) : clj_c_invoke(t123, a61, 2);
#else
	t141 = clj_c_invoke(t123, a61, 2);
#endif
	clj_release(t140);
	clj_release(t126);
	if (o123) clj_release(t123);
	if (t141 == CLJ_THROWN) {
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a60[1] = {t141};
	(void)a60;
	clj_value t142;
	if (CLJC_GUARD(V[12], B[0])) {
	t142 = clj_seq(t141);
	} else {
	t142 = clj_c_intrinsic_fallback(V[12], a60, 1);
	}
	clj_release(t141);
	if (t142 == CLJ_THROWN) {
	if (o122) clj_release(t122);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a59[1] = {t142};
	(void)a59;
	clj_value t143 = clj_c_invoke(t122, a59, 1);
	clj_release(t142);
	if (o122) clj_release(t122);
	if (t143 == CLJ_THROWN) {
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a52[3] = {t110, t121, t143};
	(void)a52;
	clj_value t144 = clj_c_invoke(t107, a52, 3);
	clj_release(t143);
	clj_release(t121);
	clj_release(t110);
	if (o107) clj_release(t107);
	if (t144 == CLJ_THROWN) {
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a51[1] = {t144};
	(void)a51;
	clj_value t145;
	if (CLJC_GUARD(V[12], B[0])) {
	t145 = clj_seq(t144);
	} else {
	t145 = clj_c_intrinsic_fallback(V[12], a51, 1);
	}
	clj_release(t144);
	if (t145 == CLJ_THROWN) {
	if (o106) clj_release(t106);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a50[1] = {t145};
	(void)a50;
	clj_value t146 = clj_c_invoke(t106, a50, 1);
	clj_release(t145);
	if (o106) clj_release(t106);
	if (t146 == CLJ_THROWN) {
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o147;
	clj_value t147 = clj_c_var_borrow(V[8], &o147);
	if (t147 == CLJ_THROWN) {
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o148;
	clj_value t148 = clj_c_var_borrow(V[13], &o148);
	if (t148 == CLJ_THROWN) {
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o149;
	clj_value t149 = clj_c_var_borrow(V[8], &o149);
	if (t149 == CLJ_THROWN) {
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t150 = K[230];
	clj_value a72[1] = {t150};
	(void)a72;
	clj_value t151 = clj_c_invoke(t149, a72, 1);
	(void)t150;
	if (o149) clj_release(t149);
	if (t151 == CLJ_THROWN) {
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o152;
	clj_value t152 = clj_c_var_borrow(V[8], &o152);
	if (t152 == CLJ_THROWN) {
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o153;
	clj_value t153 = clj_c_var_borrow(V[17], &o153);
	if (t153 == CLJ_THROWN) {
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o154;
	clj_value t154 = clj_c_var_borrow(V[18], &o154);
	if (t154 == CLJ_THROWN) {
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o155;
	clj_value t155 = clj_c_var_borrow(V[13], &o155);
	if (t155 == CLJ_THROWN) {
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o156;
	clj_value t156 = clj_c_var_borrow(V[8], &o156);
	if (t156 == CLJ_THROWN) {
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t157 = K[214];
	clj_value a77[1] = {t157};
	(void)a77;
	clj_value t158 = clj_c_invoke(t156, a77, 1);
	(void)t157;
	if (o156) clj_release(t156);
	if (t158 == CLJ_THROWN) {
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o159;
	clj_value t159 = clj_c_var_borrow(V[8], &o159);
	if (t159 == CLJ_THROWN) {
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o160;
	clj_value t160 = clj_c_var_borrow(V[13], &o160);
	if (t160 == CLJ_THROWN) {
	if (o159) clj_release(t159);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o161;
	clj_value t161 = clj_c_var_borrow(V[8], &o161);
	if (t161 == CLJ_THROWN) {
	if (o160) clj_release(t160);
	if (o159) clj_release(t159);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t162 = K[231];
	clj_value a81[1] = {t162};
	(void)a81;
	clj_value t163 = clj_c_invoke(t161, a81, 1);
	(void)t162;
	if (o161) clj_release(t161);
	if (t163 == CLJ_THROWN) {
	if (o160) clj_release(t160);
	if (o159) clj_release(t159);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o164;
	clj_value t164 = clj_c_var_borrow(V[8], &o164);
	if (t164 == CLJ_THROWN) {
	clj_release(t163);
	if (o160) clj_release(t160);
	if (o159) clj_release(t159);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t165 = l1;
	clj_value a82[1] = {t165};
	(void)a82;
	clj_value t166 = clj_c_invoke(t164, a82, 1);
	(void)t165;
	if (o164) clj_release(t164);
	if (t166 == CLJ_THROWN) {
	clj_release(t163);
	if (o160) clj_release(t160);
	if (o159) clj_release(t159);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a80[2] = {t163, t166};
	(void)a80;
	clj_value t167;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t167 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a80, 2) : clj_c_invoke(t160, a80, 2);
#else
	t167 = clj_c_invoke(t160, a80, 2);
#endif
	clj_release(t166);
	clj_release(t163);
	if (o160) clj_release(t160);
	if (t167 == CLJ_THROWN) {
	if (o159) clj_release(t159);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a79[1] = {t167};
	(void)a79;
	clj_value t168;
	if (CLJC_GUARD(V[12], B[0])) {
	t168 = clj_seq(t167);
	} else {
	t168 = clj_c_intrinsic_fallback(V[12], a79, 1);
	}
	clj_release(t167);
	if (t168 == CLJ_THROWN) {
	if (o159) clj_release(t159);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a78[1] = {t168};
	(void)a78;
	clj_value t169 = clj_c_invoke(t159, a78, 1);
	clj_release(t168);
	if (o159) clj_release(t159);
	if (t169 == CLJ_THROWN) {
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o170;
	clj_value t170 = clj_c_var_borrow(V[8], &o170);
	if (t170 == CLJ_THROWN) {
	clj_release(t169);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t171 = K[216];
	clj_value a83[1] = {t171};
	(void)a83;
	clj_value t172 = clj_c_invoke(t170, a83, 1);
	(void)t171;
	if (o170) clj_release(t170);
	if (t172 == CLJ_THROWN) {
	clj_release(t169);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o173;
	clj_value t173 = clj_c_var_borrow(V[8], &o173);
	if (t173 == CLJ_THROWN) {
	clj_release(t172);
	clj_release(t169);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t174 = K[232];
	clj_value a84[1] = {t174};
	(void)a84;
	clj_value t175 = clj_c_invoke(t173, a84, 1);
	(void)t174;
	if (o173) clj_release(t173);
	if (t175 == CLJ_THROWN) {
	clj_release(t172);
	clj_release(t169);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o176;
	clj_value t176 = clj_c_var_borrow(V[8], &o176);
	if (t176 == CLJ_THROWN) {
	clj_release(t175);
	clj_release(t172);
	clj_release(t169);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t177 = K[218];
	clj_value a85[1] = {t177};
	(void)a85;
	clj_value t178 = clj_c_invoke(t176, a85, 1);
	(void)t177;
	if (o176) clj_release(t176);
	if (t178 == CLJ_THROWN) {
	clj_release(t175);
	clj_release(t172);
	clj_release(t169);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o179;
	clj_value t179 = clj_c_var_borrow(V[8], &o179);
	if (t179 == CLJ_THROWN) {
	clj_release(t178);
	clj_release(t175);
	clj_release(t172);
	clj_release(t169);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t180 = K[221];
	clj_value a86[1] = {t180};
	(void)a86;
	clj_value t181 = clj_c_invoke(t179, a86, 1);
	(void)t180;
	if (o179) clj_release(t179);
	if (t181 == CLJ_THROWN) {
	clj_release(t178);
	clj_release(t175);
	clj_release(t172);
	clj_release(t169);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o182;
	clj_value t182 = clj_c_var_borrow(V[8], &o182);
	if (t182 == CLJ_THROWN) {
	clj_release(t181);
	clj_release(t178);
	clj_release(t175);
	clj_release(t172);
	clj_release(t169);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t183 = K[219];
	clj_value a87[1] = {t183};
	(void)a87;
	clj_value t184 = clj_c_invoke(t182, a87, 1);
	(void)t183;
	if (o182) clj_release(t182);
	if (t184 == CLJ_THROWN) {
	clj_release(t181);
	clj_release(t178);
	clj_release(t175);
	clj_release(t172);
	clj_release(t169);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o185;
	clj_value t185 = clj_c_var_borrow(V[8], &o185);
	if (t185 == CLJ_THROWN) {
	clj_release(t184);
	clj_release(t181);
	clj_release(t178);
	clj_release(t175);
	clj_release(t172);
	clj_release(t169);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t186 = l0;
	clj_value a88[1] = {t186};
	(void)a88;
	clj_value t187 = clj_c_invoke(t185, a88, 1);
	(void)t186;
	if (o185) clj_release(t185);
	if (t187 == CLJ_THROWN) {
	clj_release(t184);
	clj_release(t181);
	clj_release(t178);
	clj_release(t175);
	clj_release(t172);
	clj_release(t169);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a76[8] = {t158, t169, t172, t175, t178, t181, t184, t187};
	(void)a76;
	clj_value t188 = clj_c_invoke(t155, a76, 8);
	clj_release(t187);
	clj_release(t184);
	clj_release(t181);
	clj_release(t178);
	clj_release(t175);
	clj_release(t172);
	clj_release(t169);
	clj_release(t158);
	if (o155) clj_release(t155);
	if (t188 == CLJ_THROWN) {
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a75[1] = {t188};
	(void)a75;
	clj_value t189;
	if (CLJC_GUARD(V[12], B[0])) {
	t189 = clj_seq(t188);
	} else {
	t189 = clj_c_intrinsic_fallback(V[12], a75, 1);
	}
	clj_release(t188);
	if (t189 == CLJ_THROWN) {
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a74[2] = {t154, t189};
	(void)a74;
	clj_value t190 = clj_c_invoke(t153, a74, 2);
	clj_release(t189);
	if (o154) clj_release(t154);
	if (o153) clj_release(t153);
	if (t190 == CLJ_THROWN) {
	if (o152) clj_release(t152);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a73[1] = {t190};
	(void)a73;
	clj_value t191 = clj_c_invoke(t152, a73, 1);
	clj_release(t190);
	if (o152) clj_release(t152);
	if (t191 == CLJ_THROWN) {
	clj_release(t151);
	if (o148) clj_release(t148);
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a71[2] = {t151, t191};
	(void)a71;
	clj_value t192;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t192 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a71, 2) : clj_c_invoke(t148, a71, 2);
#else
	t192 = clj_c_invoke(t148, a71, 2);
#endif
	clj_release(t191);
	clj_release(t151);
	if (o148) clj_release(t148);
	if (t192 == CLJ_THROWN) {
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a70[1] = {t192};
	(void)a70;
	clj_value t193;
	if (CLJC_GUARD(V[12], B[0])) {
	t193 = clj_seq(t192);
	} else {
	t193 = clj_c_intrinsic_fallback(V[12], a70, 1);
	}
	clj_release(t192);
	if (t193 == CLJ_THROWN) {
	if (o147) clj_release(t147);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a69[1] = {t193};
	(void)a69;
	clj_value t194 = clj_c_invoke(t147, a69, 1);
	clj_release(t193);
	if (o147) clj_release(t147);
	if (t194 == CLJ_THROWN) {
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o195;
	clj_value t195 = clj_c_var_borrow(V[8], &o195);
	if (t195 == CLJ_THROWN) {
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o196;
	clj_value t196 = clj_c_var_borrow(V[13], &o196);
	if (t196 == CLJ_THROWN) {
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o197;
	clj_value t197 = clj_c_var_borrow(V[8], &o197);
	if (t197 == CLJ_THROWN) {
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t198 = K[233];
	clj_value a92[1] = {t198};
	(void)a92;
	clj_value t199 = clj_c_invoke(t197, a92, 1);
	(void)t198;
	if (o197) clj_release(t197);
	if (t199 == CLJ_THROWN) {
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o200;
	clj_value t200 = clj_c_var_borrow(V[8], &o200);
	if (t200 == CLJ_THROWN) {
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o201;
	clj_value t201 = clj_c_var_borrow(V[17], &o201);
	if (t201 == CLJ_THROWN) {
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o202;
	clj_value t202 = clj_c_var_borrow(V[18], &o202);
	if (t202 == CLJ_THROWN) {
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o203;
	clj_value t203 = clj_c_var_borrow(V[13], &o203);
	if (t203 == CLJ_THROWN) {
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o204;
	clj_value t204 = clj_c_var_borrow(V[8], &o204);
	if (t204 == CLJ_THROWN) {
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t205 = K[214];
	clj_value a97[1] = {t205};
	(void)a97;
	clj_value t206 = clj_c_invoke(t204, a97, 1);
	(void)t205;
	if (o204) clj_release(t204);
	if (t206 == CLJ_THROWN) {
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o207;
	clj_value t207 = clj_c_var_borrow(V[8], &o207);
	if (t207 == CLJ_THROWN) {
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o208;
	clj_value t208 = clj_c_var_borrow(V[13], &o208);
	if (t208 == CLJ_THROWN) {
	if (o207) clj_release(t207);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o209;
	clj_value t209 = clj_c_var_borrow(V[8], &o209);
	if (t209 == CLJ_THROWN) {
	if (o208) clj_release(t208);
	if (o207) clj_release(t207);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t210 = K[234];
	clj_value a101[1] = {t210};
	(void)a101;
	clj_value t211 = clj_c_invoke(t209, a101, 1);
	(void)t210;
	if (o209) clj_release(t209);
	if (t211 == CLJ_THROWN) {
	if (o208) clj_release(t208);
	if (o207) clj_release(t207);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o212;
	clj_value t212 = clj_c_var_borrow(V[8], &o212);
	if (t212 == CLJ_THROWN) {
	clj_release(t211);
	if (o208) clj_release(t208);
	if (o207) clj_release(t207);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t213 = l1;
	clj_value a102[1] = {t213};
	(void)a102;
	clj_value t214 = clj_c_invoke(t212, a102, 1);
	(void)t213;
	if (o212) clj_release(t212);
	if (t214 == CLJ_THROWN) {
	clj_release(t211);
	if (o208) clj_release(t208);
	if (o207) clj_release(t207);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a100[2] = {t211, t214};
	(void)a100;
	clj_value t215;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t215 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a100, 2) : clj_c_invoke(t208, a100, 2);
#else
	t215 = clj_c_invoke(t208, a100, 2);
#endif
	clj_release(t214);
	clj_release(t211);
	if (o208) clj_release(t208);
	if (t215 == CLJ_THROWN) {
	if (o207) clj_release(t207);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a99[1] = {t215};
	(void)a99;
	clj_value t216;
	if (CLJC_GUARD(V[12], B[0])) {
	t216 = clj_seq(t215);
	} else {
	t216 = clj_c_intrinsic_fallback(V[12], a99, 1);
	}
	clj_release(t215);
	if (t216 == CLJ_THROWN) {
	if (o207) clj_release(t207);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a98[1] = {t216};
	(void)a98;
	clj_value t217 = clj_c_invoke(t207, a98, 1);
	clj_release(t216);
	if (o207) clj_release(t207);
	if (t217 == CLJ_THROWN) {
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o218;
	clj_value t218 = clj_c_var_borrow(V[8], &o218);
	if (t218 == CLJ_THROWN) {
	clj_release(t217);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t219 = K[216];
	clj_value a103[1] = {t219};
	(void)a103;
	clj_value t220 = clj_c_invoke(t218, a103, 1);
	(void)t219;
	if (o218) clj_release(t218);
	if (t220 == CLJ_THROWN) {
	clj_release(t217);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o221;
	clj_value t221 = clj_c_var_borrow(V[8], &o221);
	if (t221 == CLJ_THROWN) {
	clj_release(t220);
	clj_release(t217);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t222 = K[217];
	clj_value a104[1] = {t222};
	(void)a104;
	clj_value t223 = clj_c_invoke(t221, a104, 1);
	(void)t222;
	if (o221) clj_release(t221);
	if (t223 == CLJ_THROWN) {
	clj_release(t220);
	clj_release(t217);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o224;
	clj_value t224 = clj_c_var_borrow(V[8], &o224);
	if (t224 == CLJ_THROWN) {
	clj_release(t223);
	clj_release(t220);
	clj_release(t217);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t225 = K[218];
	clj_value a105[1] = {t225};
	(void)a105;
	clj_value t226 = clj_c_invoke(t224, a105, 1);
	(void)t225;
	if (o224) clj_release(t224);
	if (t226 == CLJ_THROWN) {
	clj_release(t223);
	clj_release(t220);
	clj_release(t217);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o227;
	clj_value t227 = clj_c_var_borrow(V[8], &o227);
	if (t227 == CLJ_THROWN) {
	clj_release(t226);
	clj_release(t223);
	clj_release(t220);
	clj_release(t217);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t228 = K[221];
	clj_value a106[1] = {t228};
	(void)a106;
	clj_value t229 = clj_c_invoke(t227, a106, 1);
	(void)t228;
	if (o227) clj_release(t227);
	if (t229 == CLJ_THROWN) {
	clj_release(t226);
	clj_release(t223);
	clj_release(t220);
	clj_release(t217);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o230;
	clj_value t230 = clj_c_var_borrow(V[8], &o230);
	if (t230 == CLJ_THROWN) {
	clj_release(t229);
	clj_release(t226);
	clj_release(t223);
	clj_release(t220);
	clj_release(t217);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t231 = K[219];
	clj_value a107[1] = {t231};
	(void)a107;
	clj_value t232 = clj_c_invoke(t230, a107, 1);
	(void)t231;
	if (o230) clj_release(t230);
	if (t232 == CLJ_THROWN) {
	clj_release(t229);
	clj_release(t226);
	clj_release(t223);
	clj_release(t220);
	clj_release(t217);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o233;
	clj_value t233 = clj_c_var_borrow(V[8], &o233);
	if (t233 == CLJ_THROWN) {
	clj_release(t232);
	clj_release(t229);
	clj_release(t226);
	clj_release(t223);
	clj_release(t220);
	clj_release(t217);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t234 = l0;
	clj_value a108[1] = {t234};
	(void)a108;
	clj_value t235 = clj_c_invoke(t233, a108, 1);
	(void)t234;
	if (o233) clj_release(t233);
	if (t235 == CLJ_THROWN) {
	clj_release(t232);
	clj_release(t229);
	clj_release(t226);
	clj_release(t223);
	clj_release(t220);
	clj_release(t217);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a96[8] = {t206, t217, t220, t223, t226, t229, t232, t235};
	(void)a96;
	clj_value t236 = clj_c_invoke(t203, a96, 8);
	clj_release(t235);
	clj_release(t232);
	clj_release(t229);
	clj_release(t226);
	clj_release(t223);
	clj_release(t220);
	clj_release(t217);
	clj_release(t206);
	if (o203) clj_release(t203);
	if (t236 == CLJ_THROWN) {
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a95[1] = {t236};
	(void)a95;
	clj_value t237;
	if (CLJC_GUARD(V[12], B[0])) {
	t237 = clj_seq(t236);
	} else {
	t237 = clj_c_intrinsic_fallback(V[12], a95, 1);
	}
	clj_release(t236);
	if (t237 == CLJ_THROWN) {
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a94[2] = {t202, t237};
	(void)a94;
	clj_value t238 = clj_c_invoke(t201, a94, 2);
	clj_release(t237);
	if (o202) clj_release(t202);
	if (o201) clj_release(t201);
	if (t238 == CLJ_THROWN) {
	if (o200) clj_release(t200);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a93[1] = {t238};
	(void)a93;
	clj_value t239 = clj_c_invoke(t200, a93, 1);
	clj_release(t238);
	if (o200) clj_release(t200);
	if (t239 == CLJ_THROWN) {
	clj_release(t199);
	if (o196) clj_release(t196);
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a91[2] = {t199, t239};
	(void)a91;
	clj_value t240;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t240 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a91, 2) : clj_c_invoke(t196, a91, 2);
#else
	t240 = clj_c_invoke(t196, a91, 2);
#endif
	clj_release(t239);
	clj_release(t199);
	if (o196) clj_release(t196);
	if (t240 == CLJ_THROWN) {
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a90[1] = {t240};
	(void)a90;
	clj_value t241;
	if (CLJC_GUARD(V[12], B[0])) {
	t241 = clj_seq(t240);
	} else {
	t241 = clj_c_intrinsic_fallback(V[12], a90, 1);
	}
	clj_release(t240);
	if (t241 == CLJ_THROWN) {
	if (o195) clj_release(t195);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a89[1] = {t241};
	(void)a89;
	clj_value t242 = clj_c_invoke(t195, a89, 1);
	clj_release(t241);
	if (o195) clj_release(t195);
	if (t242 == CLJ_THROWN) {
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a48[4] = {t105, t146, t194, t242};
	(void)a48;
	clj_value t243 = clj_c_invoke(t102, a48, 4);
	clj_release(t242);
	clj_release(t194);
	clj_release(t146);
	clj_release(t105);
	if (o102) clj_release(t102);
	if (t243 == CLJ_THROWN) {
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a47[1] = {t243};
	(void)a47;
	clj_value t244;
	if (CLJC_GUARD(V[12], B[0])) {
	t244 = clj_seq(t243);
	} else {
	t244 = clj_c_intrinsic_fallback(V[12], a47, 1);
	}
	clj_release(t243);
	if (t244 == CLJ_THROWN) {
	if (o101) clj_release(t101);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a46[1] = {t244};
	(void)a46;
	clj_value t245 = clj_c_invoke(t101, a46, 1);
	clj_release(t244);
	if (o101) clj_release(t101);
	if (t245 == CLJ_THROWN) {
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a34[3] = {t78, t100, t245};
	(void)a34;
	clj_value t246 = clj_c_invoke(t75, a34, 3);
	clj_release(t245);
	clj_release(t100);
	clj_release(t78);
	if (o75) clj_release(t75);
	if (t246 == CLJ_THROWN) {
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a33[1] = {t246};
	(void)a33;
	clj_value t247;
	if (CLJC_GUARD(V[12], B[0])) {
	t247 = clj_seq(t246);
	} else {
	t247 = clj_c_intrinsic_fallback(V[12], a33, 1);
	}
	clj_release(t246);
	if (t247 == CLJ_THROWN) {
	if (o74) clj_release(t74);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a32[1] = {t247};
	(void)a32;
	clj_value t248 = clj_c_invoke(t74, a32, 1);
	clj_release(t247);
	if (o74) clj_release(t74);
	if (t248 == CLJ_THROWN) {
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	bool o249;
	clj_value t249 = clj_c_var_borrow(V[8], &o249);
	if (t249 == CLJ_THROWN) {
	clj_release(t248);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value t250 = K[221];
	clj_value a109[1] = {t250};
	(void)a109;
	clj_value t251 = clj_c_invoke(t249, a109, 1);
	(void)t250;
	if (o249) clj_release(t249);
	if (t251 == CLJ_THROWN) {
	clj_release(t248);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a28[5] = {t67, t70, t73, t248, t251};
	(void)a28;
	clj_value t252 = clj_c_invoke(t64, a28, 5);
	clj_release(t251);
	clj_release(t248);
	clj_release(t73);
	clj_release(t70);
	clj_release(t67);
	if (o64) clj_release(t64);
	if (t252 == CLJ_THROWN) {
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a27[1] = {t252};
	(void)a27;
	clj_value t253;
	if (CLJC_GUARD(V[12], B[0])) {
	t253 = clj_seq(t252);
	} else {
	t253 = clj_c_intrinsic_fallback(V[12], a27, 1);
	}
	clj_release(t252);
	if (t253 == CLJ_THROWN) {
	if (o63) clj_release(t63);
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a26[1] = {t253};
	(void)a26;
	clj_value t254 = clj_c_invoke(t63, a26, 1);
	clj_release(t253);
	if (o63) clj_release(t63);
	if (t254 == CLJ_THROWN) {
	clj_release(t62);
	clj_release(t13);
	if (o10) clj_release(t10);
	goto L2;
	}
	clj_value a4[4] = {t13, t14, t62, t254};
	(void)a4;
	clj_value t255 = clj_c_invoke(t10, a4, 4);
	clj_release(t254);
	clj_release(t62);
	(void)t14;
	clj_release(t13);
	if (o10) clj_release(t10);
	if (t255 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a3[1] = {t255};
	(void)a3;
	clj_value t256;
	if (CLJC_GUARD(V[12], B[0])) {
	t256 = clj_seq(t255);
	} else {
	t256 = clj_c_intrinsic_fallback(V[12], a3, 1);
	}
	clj_release(t255);
	if (t256 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[24], &cc);
	clj_release(l2);
	clj_release(l3);
	clj_release(l4);
	return t256;
L2: ;
	clj_c_leave(&S[24], &cc);
L1: ;
	clj_release(l2);
	clj_release(l3);
	clj_release(l4);
	return CLJ_THROWN;
}

static clj_value clojure_test_form48__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 2: return clojure_test_form48__0_a2(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_48(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 174 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[60], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[210];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form48__0, NULL, 0, 0x4, 2, 2);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_49(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 174 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[60]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_try_expr_a4(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_value l2 = args[2];
	(void)l2;
	clj_value l3 = args[3];
	(void)l3;
	clj_ccall cc;
	if (!clj_c_enter(&S[25], &cc)) goto L1;
#line 189 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[13], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[8], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t2 = K[237];
	clj_value a2[1] = {t2};
	(void)a2;
	clj_value t3 = clj_c_invoke(t1, a2, 1);
	(void)t2;
	if (o1) clj_release(t1);
	if (t3 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[8], &o4);
	if (t4 == CLJ_THROWN) {
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
#line 192 "<embedded>/clojure/test.clj"
	bool o5;
	clj_value t5 = clj_c_var_borrow(V[60], &o5);
	if (t5 == CLJ_THROWN) {
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t6 = l2;
	clj_value t7 = l3;
	clj_value a4[2] = {t6, t7};
	(void)a4;
	clj_value t8 = clj_c_invoke(t5, a4, 2);
	(void)t7;
	(void)t6;
	if (o5) clj_release(t5);
	if (t8 == CLJ_THROWN) {
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a3[1] = {t8};
	(void)a3;
	clj_value t9 = clj_c_invoke(t4, a3, 1);
	clj_release(t8);
	if (o4) clj_release(t4);
	if (t9 == CLJ_THROWN) {
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
#line 189 "<embedded>/clojure/test.clj"
	bool o10;
	clj_value t10 = clj_c_var_borrow(V[8], &o10);
	if (t10 == CLJ_THROWN) {
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o11;
	clj_value t11 = clj_c_var_borrow(V[13], &o11);
	if (t11 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o12;
	clj_value t12 = clj_c_var_borrow(V[8], &o12);
	if (t12 == CLJ_THROWN) {
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t13 = K[238];
	clj_value a8[1] = {t13};
	(void)a8;
	clj_value t14 = clj_c_invoke(t12, a8, 1);
	(void)t13;
	if (o12) clj_release(t12);
	if (t14 == CLJ_THROWN) {
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o15;
	clj_value t15 = clj_c_var_borrow(V[8], &o15);
	if (t15 == CLJ_THROWN) {
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t16 = K[239];
	clj_value a9[1] = {t16};
	(void)a9;
	clj_value t17 = clj_c_invoke(t15, a9, 1);
	(void)t16;
	if (o15) clj_release(t15);
	if (t17 == CLJ_THROWN) {
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o18;
	clj_value t18 = clj_c_var_borrow(V[8], &o18);
	if (t18 == CLJ_THROWN) {
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t19 = K[240];
	clj_value a10[1] = {t19};
	(void)a10;
	clj_value t20 = clj_c_invoke(t18, a10, 1);
	(void)t19;
	if (o18) clj_release(t18);
	if (t20 == CLJ_THROWN) {
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o21;
	clj_value t21 = clj_c_var_borrow(V[8], &o21);
	if (t21 == CLJ_THROWN) {
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o22;
	clj_value t22 = clj_c_var_borrow(V[13], &o22);
	if (t22 == CLJ_THROWN) {
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o23;
	clj_value t23 = clj_c_var_borrow(V[8], &o23);
	if (t23 == CLJ_THROWN) {
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t24 = K[241];
	clj_value a14[1] = {t24};
	(void)a14;
	clj_value t25 = clj_c_invoke(t23, a14, 1);
	(void)t24;
	if (o23) clj_release(t23);
	if (t25 == CLJ_THROWN) {
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o26;
	clj_value t26 = clj_c_var_borrow(V[8], &o26);
	if (t26 == CLJ_THROWN) {
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o27;
	clj_value t27 = clj_c_var_borrow(V[17], &o27);
	if (t27 == CLJ_THROWN) {
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o28;
	clj_value t28 = clj_c_var_borrow(V[18], &o28);
	if (t28 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o29;
	clj_value t29 = clj_c_var_borrow(V[13], &o29);
	if (t29 == CLJ_THROWN) {
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o30;
	clj_value t30 = clj_c_var_borrow(V[8], &o30);
	if (t30 == CLJ_THROWN) {
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t31 = K[242];
	clj_value a19[1] = {t31};
	(void)a19;
	clj_value t32 = clj_c_invoke(t30, a19, 1);
	(void)t31;
	if (o30) clj_release(t30);
	if (t32 == CLJ_THROWN) {
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o33;
	clj_value t33 = clj_c_var_borrow(V[8], &o33);
	if (t33 == CLJ_THROWN) {
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o34;
	clj_value t34 = clj_c_var_borrow(V[13], &o34);
	if (t34 == CLJ_THROWN) {
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o35;
	clj_value t35 = clj_c_var_borrow(V[8], &o35);
	if (t35 == CLJ_THROWN) {
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t36 = K[243];
	clj_value a23[1] = {t36};
	(void)a23;
	clj_value t37 = clj_c_invoke(t35, a23, 1);
	(void)t36;
	if (o35) clj_release(t35);
	if (t37 == CLJ_THROWN) {
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o38;
	clj_value t38 = clj_c_var_borrow(V[8], &o38);
	if (t38 == CLJ_THROWN) {
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t39 = l3;
	clj_value a24[1] = {t39};
	(void)a24;
	clj_value t40 = clj_c_invoke(t38, a24, 1);
	(void)t39;
	if (o38) clj_release(t38);
	if (t40 == CLJ_THROWN) {
	clj_release(t37);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a22[2] = {t37, t40};
	(void)a22;
	clj_value t41;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t41 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a22, 2) : clj_c_invoke(t34, a22, 2);
#else
	t41 = clj_c_invoke(t34, a22, 2);
#endif
	clj_release(t40);
	clj_release(t37);
	if (o34) clj_release(t34);
	if (t41 == CLJ_THROWN) {
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a21[1] = {t41};
	(void)a21;
	clj_value t42;
	if (CLJC_GUARD(V[12], B[0])) {
	t42 = clj_seq(t41);
	} else {
	t42 = clj_c_intrinsic_fallback(V[12], a21, 1);
	}
	clj_release(t41);
	if (t42 == CLJ_THROWN) {
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a20[1] = {t42};
	(void)a20;
	clj_value t43 = clj_c_invoke(t33, a20, 1);
	clj_release(t42);
	if (o33) clj_release(t33);
	if (t43 == CLJ_THROWN) {
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o44;
	clj_value t44 = clj_c_var_borrow(V[8], &o44);
	if (t44 == CLJ_THROWN) {
	clj_release(t43);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t45 = K[244];
	clj_value a25[1] = {t45};
	(void)a25;
	clj_value t46 = clj_c_invoke(t44, a25, 1);
	(void)t45;
	if (o44) clj_release(t44);
	if (t46 == CLJ_THROWN) {
	clj_release(t43);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o47;
	clj_value t47 = clj_c_var_borrow(V[8], &o47);
	if (t47 == CLJ_THROWN) {
	clj_release(t46);
	clj_release(t43);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t48 = K[245];
	clj_value a26[1] = {t48};
	(void)a26;
	clj_value t49 = clj_c_invoke(t47, a26, 1);
	(void)t48;
	if (o47) clj_release(t47);
	if (t49 == CLJ_THROWN) {
	clj_release(t46);
	clj_release(t43);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o50;
	clj_value t50 = clj_c_var_borrow(V[8], &o50);
	if (t50 == CLJ_THROWN) {
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t51 = K[246];
	clj_value a27[1] = {t51};
	(void)a27;
	clj_value t52 = clj_c_invoke(t50, a27, 1);
	(void)t51;
	if (o50) clj_release(t50);
	if (t52 == CLJ_THROWN) {
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o53;
	clj_value t53 = clj_c_var_borrow(V[8], &o53);
	if (t53 == CLJ_THROWN) {
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t54 = K[240];
	clj_value a28[1] = {t54};
	(void)a28;
	clj_value t55 = clj_c_invoke(t53, a28, 1);
	(void)t54;
	if (o53) clj_release(t53);
	if (t55 == CLJ_THROWN) {
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o56;
	clj_value t56 = clj_c_var_borrow(V[8], &o56);
	if (t56 == CLJ_THROWN) {
	clj_release(t55);
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t57 = K[247];
	clj_value a29[1] = {t57};
	(void)a29;
	clj_value t58 = clj_c_invoke(t56, a29, 1);
	(void)t57;
	if (o56) clj_release(t56);
	if (t58 == CLJ_THROWN) {
	clj_release(t55);
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o59;
	clj_value t59 = clj_c_var_borrow(V[8], &o59);
	if (t59 == CLJ_THROWN) {
	clj_release(t58);
	clj_release(t55);
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t60 = l2;
	clj_value a30[1] = {t60};
	(void)a30;
	clj_value t61 = clj_c_invoke(t59, a30, 1);
	(void)t60;
	if (o59) clj_release(t59);
	if (t61 == CLJ_THROWN) {
	clj_release(t58);
	clj_release(t55);
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a18[8] = {t32, t43, t46, t49, t52, t55, t58, t61};
	(void)a18;
	clj_value t62 = clj_c_invoke(t29, a18, 8);
	clj_release(t61);
	clj_release(t58);
	clj_release(t55);
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	clj_release(t43);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (t62 == CLJ_THROWN) {
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a17[1] = {t62};
	(void)a17;
	clj_value t63;
	if (CLJC_GUARD(V[12], B[0])) {
	t63 = clj_seq(t62);
	} else {
	t63 = clj_c_intrinsic_fallback(V[12], a17, 1);
	}
	clj_release(t62);
	if (t63 == CLJ_THROWN) {
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a16[2] = {t28, t63};
	(void)a16;
	clj_value t64 = clj_c_invoke(t27, a16, 2);
	clj_release(t63);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (t64 == CLJ_THROWN) {
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a15[1] = {t64};
	(void)a15;
	clj_value t65 = clj_c_invoke(t26, a15, 1);
	clj_release(t64);
	if (o26) clj_release(t26);
	if (t65 == CLJ_THROWN) {
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a13[2] = {t25, t65};
	(void)a13;
	clj_value t66;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t66 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a13, 2) : clj_c_invoke(t22, a13, 2);
#else
	t66 = clj_c_invoke(t22, a13, 2);
#endif
	clj_release(t65);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (t66 == CLJ_THROWN) {
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a12[1] = {t66};
	(void)a12;
	clj_value t67;
	if (CLJC_GUARD(V[12], B[0])) {
	t67 = clj_seq(t66);
	} else {
	t67 = clj_c_intrinsic_fallback(V[12], a12, 1);
	}
	clj_release(t66);
	if (t67 == CLJ_THROWN) {
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a11[1] = {t67};
	(void)a11;
	clj_value t68 = clj_c_invoke(t21, a11, 1);
	clj_release(t67);
	if (o21) clj_release(t21);
	if (t68 == CLJ_THROWN) {
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a7[4] = {t14, t17, t20, t68};
	(void)a7;
	clj_value t69 = clj_c_invoke(t11, a7, 4);
	clj_release(t68);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (t69 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a6[1] = {t69};
	(void)a6;
	clj_value t70;
	if (CLJC_GUARD(V[12], B[0])) {
	t70 = clj_seq(t69);
	} else {
	t70 = clj_c_intrinsic_fallback(V[12], a6, 1);
	}
	clj_release(t69);
	if (t70 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a5[1] = {t70};
	(void)a5;
	clj_value t71 = clj_c_invoke(t10, a5, 1);
	clj_release(t70);
	if (o10) clj_release(t10);
	if (t71 == CLJ_THROWN) {
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a1[3] = {t3, t9, t71};
	(void)a1;
	clj_value t72 = clj_c_invoke(t0, a1, 3);
	clj_release(t71);
	clj_release(t9);
	clj_release(t3);
	if (o0) clj_release(t0);
	if (t72 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t72};
	(void)a0;
	clj_value t73;
	if (CLJC_GUARD(V[12], B[0])) {
	t73 = clj_seq(t72);
	} else {
	t73 = clj_c_intrinsic_fallback(V[12], a0, 1);
	}
	clj_release(t72);
	if (t73 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[25], &cc);
	return t73;
L2: ;
	clj_c_leave(&S[25], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_try_expr(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 4: return clojure_test_try_expr_a4(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_50(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 189 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[236], clojure_test_try_expr, NULL, 0, 0x10, 4, 4);
	clj_var_bind_root(V[65], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[248]);
	clj_value t2 = clj_c_def(V[65], t1, true, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_is_a3(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_value l2 = args[2];
	(void)l2;
	clj_ccall cc;
	if (!clj_c_enter(&S[26], &cc)) goto L1;
#line 200 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[13], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[8], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t2 = K[251];
	clj_value a3[1] = {t2};
	(void)a3;
	clj_value t3 = clj_c_invoke(t1, a3, 1);
	(void)t2;
	if (o1) clj_release(t1);
	if (t3 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[8], &o4);
	if (t4 == CLJ_THROWN) {
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t5 = l2;
	clj_value a4[1] = {t5};
	(void)a4;
	clj_value t6 = clj_c_invoke(t4, a4, 1);
	(void)t5;
	if (o4) clj_release(t4);
	if (t6 == CLJ_THROWN) {
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o7;
	clj_value t7 = clj_c_var_borrow(V[8], &o7);
	if (t7 == CLJ_THROWN) {
	clj_release(t6);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t8 = K[3];
	clj_value a5[1] = {t8};
	(void)a5;
	clj_value t9 = clj_c_invoke(t7, a5, 1);
	(void)t8;
	if (o7) clj_release(t7);
	if (t9 == CLJ_THROWN) {
	clj_release(t6);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a2[3] = {t3, t6, t9};
	(void)a2;
	clj_value t10 = clj_c_invoke(t0, a2, 3);
	clj_release(t9);
	clj_release(t6);
	clj_release(t3);
	if (o0) clj_release(t0);
	if (t10 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a1[1] = {t10};
	(void)a1;
	clj_value t11;
	if (CLJC_GUARD(V[12], B[0])) {
	t11 = clj_seq(t10);
	} else {
	t11 = clj_c_intrinsic_fallback(V[12], a1, 1);
	}
	clj_release(t10);
	if (t11 == CLJ_THROWN) {
	goto L2;
	}
	bool o12;
	clj_value t12 = clj_c_var_borrow(V[24], &o12);
	if (t12 == CLJ_THROWN) {
	clj_release(t11);
	goto L2;
	}
	clj_value t13 = l0;
	clj_value a6[1] = {t13};
	(void)a6;
	clj_value t14 = clj_c_invoke(t12, a6, 1);
	(void)t13;
	if (o12) clj_release(t12);
	if (t14 == CLJ_THROWN) {
	clj_release(t11);
	goto L2;
	}
	clj_value a0[2] = {t11, t14};
	(void)a0;
	clj_value t15;
	if (CLJC_GUARD(V[67], B[13])) {
	t15 = clj_with_meta(t11, t14);
	} else {
	t15 = clj_c_intrinsic_fallback(V[67], a0, 2);
	clj_release(t11);
	}
	clj_release(t14);
	if (t15 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[26], &cc);
	return t15;
L2: ;
	clj_c_leave(&S[26], &cc);
L1: ;
	return CLJ_THROWN;
}

clj_value clojure_test_is_a4(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_value l2 = args[2];
	(void)l2;
	clj_value l3 = args[3];
	(void)l3;
	clj_value l4 = CLJ_NIL;
	clj_value l5 = CLJ_NIL;
	clj_value l6 = CLJ_NIL;
	clj_value l7 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[26], &cc)) goto L1;
#line 202 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[24], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t1 = l0;
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2 = clj_c_invoke(t0, a0, 1);
	(void)t1;
	if (o0) clj_release(t0);
	if (t2 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l4, t2);
	clj_value t3 = l4;
	clj_value a1[1] = {t3};
	(void)a1;
	clj_value t4;
	if (CLJC_GUARD(V[15], B[1])) {
	t4 = clj_seq_p(t3);
	} else {
	t4 = clj_c_intrinsic_fallback(V[15], a1, 1);
	}
	(void)t3;
	if (t4 == CLJ_THROWN) {
	goto L2;
	}
	bool t5 = clj_truthy(t4);
	clj_release(t4);
	clj_value t6;
	if (t5) {
	clj_value t7 = l4;
	clj_value a2[1] = {t7};
	(void)a2;
	clj_value t8;
	if (CLJC_GUARD(V[16], B[2])) {
	t8 = clj_next(t7);
	} else {
	t8 = clj_c_intrinsic_fallback(V[16], a2, 1);
	}
	(void)t7;
	if (t8 == CLJ_THROWN) {
	goto L2;
	}
	bool t9 = clj_truthy(t8);
	clj_release(t8);
	clj_value t10;
	if (t9) {
	bool o11;
	clj_value t11 = clj_c_var_borrow(V[17], &o11);
	if (t11 == CLJ_THROWN) {
	goto L2;
	}
	bool o12;
	clj_value t12 = clj_c_var_borrow(V[18], &o12);
	if (t12 == CLJ_THROWN) {
	if (o11) clj_release(t11);
	goto L2;
	}
	clj_value t13 = l4;
	clj_value a3[2] = {t12, t13};
	(void)a3;
	clj_value t14 = clj_c_invoke(t11, a3, 2);
	(void)t13;
	if (o12) clj_release(t12);
	if (o11) clj_release(t11);
	if (t14 == CLJ_THROWN) {
	goto L2;
	}
	t10 = t14;
	} else {
	clj_value t15 = l4;
	clj_value a4[1] = {t15};
	(void)a4;
	clj_value t16;
	if (CLJC_GUARD(V[12], B[0])) {
	t16 = clj_seq(t15);
	} else {
	t16 = clj_c_intrinsic_fallback(V[12], a4, 1);
	}
	(void)t15;
	if (t16 == CLJ_THROWN) {
	goto L2;
	}
	bool t17 = clj_truthy(t16);
	clj_release(t16);
	clj_value t18;
	if (t17) {
	clj_value t19 = l4;
	clj_value a5[1] = {t19};
	(void)a5;
	clj_value t20;
	if (CLJC_GUARD(V[19], B[3])) {
	t20 = clj_first(t19);
	} else {
	t20 = clj_c_intrinsic_fallback(V[19], a5, 1);
	}
	(void)t19;
	if (t20 == CLJ_THROWN) {
	goto L2;
	}
	t18 = t20;
	} else {
	clj_value t21 = clj_retain(K[252]);
	t18 = t21;
	}
	t10 = t18;
	}
	t6 = t10;
	} else {
	clj_value t22 = l4;
	l4 = CLJ_NIL;
	t6 = t22;
	}
	clj_c_rebind(&l5, t6);
	clj_value t23 = l5;
	clj_value t24 = K[253];
	clj_value a6[2] = {t23, t24};
	(void)a6;
	clj_value t25;
	if (CLJC_GUARD(V[20], B[4])) {
	t25 = clj_get2(t23, t24);
	} else {
	t25 = clj_c_intrinsic_fallback(V[20], a6, 2);
	}
	(void)t24;
	(void)t23;
	if (t25 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l6, t25);
	clj_value t26 = l5;
	clj_value t27 = K[254];
	clj_value a7[2] = {t26, t27};
	(void)a7;
	clj_value t28;
	if (CLJC_GUARD(V[20], B[4])) {
	t28 = clj_get2(t26, t27);
	} else {
	t28 = clj_c_intrinsic_fallback(V[20], a7, 2);
	}
	(void)t27;
	(void)t26;
	if (t28 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l7, t28);
	bool o29;
	clj_value t29 = clj_c_var_borrow(V[13], &o29);
	if (t29 == CLJ_THROWN) {
	goto L2;
	}
	bool o30;
	clj_value t30 = clj_c_var_borrow(V[8], &o30);
	if (t30 == CLJ_THROWN) {
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value t31 = K[255];
	clj_value a10[1] = {t31};
	(void)a10;
	clj_value t32 = clj_c_invoke(t30, a10, 1);
	(void)t31;
	if (o30) clj_release(t30);
	if (t32 == CLJ_THROWN) {
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o33;
	clj_value t33 = clj_c_var_borrow(V[8], &o33);
	if (t33 == CLJ_THROWN) {
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o34;
	clj_value t34 = clj_c_var_borrow(V[17], &o34);
	if (t34 == CLJ_THROWN) {
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o35;
	clj_value t35 = clj_c_var_borrow(V[58], &o35);
	if (t35 == CLJ_THROWN) {
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o36;
	clj_value t36 = clj_c_var_borrow(V[13], &o36);
	if (t36 == CLJ_THROWN) {
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o37;
	clj_value t37 = clj_c_var_borrow(V[8], &o37);
	if (t37 == CLJ_THROWN) {
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value t38 = K[256];
	clj_value a15[1] = {t38};
	(void)a15;
	clj_value t39 = clj_c_invoke(t37, a15, 1);
	(void)t38;
	if (o37) clj_release(t37);
	if (t39 == CLJ_THROWN) {
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o40;
	clj_value t40 = clj_c_var_borrow(V[8], &o40);
	if (t40 == CLJ_THROWN) {
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o41;
	clj_value t41 = clj_c_var_borrow(V[17], &o41);
	if (t41 == CLJ_THROWN) {
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o42;
	clj_value t42 = clj_c_var_borrow(V[18], &o42);
	if (t42 == CLJ_THROWN) {
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o43;
	clj_value t43 = clj_c_var_borrow(V[13], &o43);
	if (t43 == CLJ_THROWN) {
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o44;
	clj_value t44 = clj_c_var_borrow(V[8], &o44);
	if (t44 == CLJ_THROWN) {
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value t45 = K[257];
	clj_value a20[1] = {t45};
	(void)a20;
	clj_value t46 = clj_c_invoke(t44, a20, 1);
	(void)t45;
	if (o44) clj_release(t44);
	if (t46 == CLJ_THROWN) {
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o47;
	clj_value t47 = clj_c_var_borrow(V[8], &o47);
	if (t47 == CLJ_THROWN) {
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o48;
	clj_value t48 = clj_c_var_borrow(V[68], &o48);
	if (t48 == CLJ_THROWN) {
	if (o47) clj_release(t47);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value a21[1] = {t48};
	(void)a21;
	clj_value t49 = clj_c_invoke(t47, a21, 1);
	if (o48) clj_release(t48);
	if (o47) clj_release(t47);
	if (t49 == CLJ_THROWN) {
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o50;
	clj_value t50 = clj_c_var_borrow(V[8], &o50);
	if (t50 == CLJ_THROWN) {
	clj_release(t49);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value t51 = K[253];
	clj_value a22[1] = {t51};
	(void)a22;
	clj_value t52 = clj_c_invoke(t50, a22, 1);
	(void)t51;
	if (o50) clj_release(t50);
	if (t52 == CLJ_THROWN) {
	clj_release(t49);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o53;
	clj_value t53 = clj_c_var_borrow(V[8], &o53);
	if (t53 == CLJ_THROWN) {
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value t54 = l6;
	clj_value a23[1] = {t54};
	(void)a23;
	clj_value t55 = clj_c_invoke(t53, a23, 1);
	(void)t54;
	if (o53) clj_release(t53);
	if (t55 == CLJ_THROWN) {
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o56;
	clj_value t56 = clj_c_var_borrow(V[8], &o56);
	if (t56 == CLJ_THROWN) {
	clj_release(t55);
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value t57 = K[254];
	clj_value a24[1] = {t57};
	(void)a24;
	clj_value t58 = clj_c_invoke(t56, a24, 1);
	(void)t57;
	if (o56) clj_release(t56);
	if (t58 == CLJ_THROWN) {
	clj_release(t55);
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o59;
	clj_value t59 = clj_c_var_borrow(V[8], &o59);
	if (t59 == CLJ_THROWN) {
	clj_release(t58);
	clj_release(t55);
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value t60 = l7;
	clj_value a25[1] = {t60};
	(void)a25;
	clj_value t61 = clj_c_invoke(t59, a25, 1);
	(void)t60;
	if (o59) clj_release(t59);
	if (t61 == CLJ_THROWN) {
	clj_release(t58);
	clj_release(t55);
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value a19[6] = {t46, t49, t52, t55, t58, t61};
	(void)a19;
	clj_value t62 = clj_c_invoke(t43, a19, 6);
	clj_release(t61);
	clj_release(t58);
	clj_release(t55);
	clj_release(t52);
	clj_release(t49);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (t62 == CLJ_THROWN) {
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value a18[1] = {t62};
	(void)a18;
	clj_value t63;
	if (CLJC_GUARD(V[12], B[0])) {
	t63 = clj_seq(t62);
	} else {
	t63 = clj_c_intrinsic_fallback(V[12], a18, 1);
	}
	clj_release(t62);
	if (t63 == CLJ_THROWN) {
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value a17[2] = {t42, t63};
	(void)a17;
	clj_value t64 = clj_c_invoke(t41, a17, 2);
	clj_release(t63);
	if (o42) clj_release(t42);
	if (o41) clj_release(t41);
	if (t64 == CLJ_THROWN) {
	if (o40) clj_release(t40);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value a16[1] = {t64};
	(void)a16;
	clj_value t65 = clj_c_invoke(t40, a16, 1);
	clj_release(t64);
	if (o40) clj_release(t40);
	if (t65 == CLJ_THROWN) {
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value a14[2] = {t39, t65};
	(void)a14;
	clj_value t66;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t66 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a14, 2) : clj_c_invoke(t36, a14, 2);
#else
	t66 = clj_c_invoke(t36, a14, 2);
#endif
	clj_release(t65);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (t66 == CLJ_THROWN) {
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value a13[1] = {t66};
	(void)a13;
	clj_value t67;
	if (CLJC_GUARD(V[12], B[0])) {
	t67 = clj_seq(t66);
	} else {
	t67 = clj_c_intrinsic_fallback(V[12], a13, 1);
	}
	clj_release(t66);
	if (t67 == CLJ_THROWN) {
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value a12[2] = {t35, t67};
	(void)a12;
	clj_value t68 = clj_c_invoke(t34, a12, 2);
	clj_release(t67);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	if (t68 == CLJ_THROWN) {
	if (o33) clj_release(t33);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value a11[1] = {t68};
	(void)a11;
	clj_value t69 = clj_c_invoke(t33, a11, 1);
	clj_release(t68);
	if (o33) clj_release(t33);
	if (t69 == CLJ_THROWN) {
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o70;
	clj_value t70 = clj_c_var_borrow(V[8], &o70);
	if (t70 == CLJ_THROWN) {
	clj_release(t69);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o71;
	clj_value t71 = clj_c_var_borrow(V[13], &o71);
	if (t71 == CLJ_THROWN) {
	if (o70) clj_release(t70);
	clj_release(t69);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o72;
	clj_value t72 = clj_c_var_borrow(V[8], &o72);
	if (t72 == CLJ_THROWN) {
	if (o71) clj_release(t71);
	if (o70) clj_release(t70);
	clj_release(t69);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value t73 = K[258];
	clj_value a29[1] = {t73};
	(void)a29;
	clj_value t74 = clj_c_invoke(t72, a29, 1);
	(void)t73;
	if (o72) clj_release(t72);
	if (t74 == CLJ_THROWN) {
	if (o71) clj_release(t71);
	if (o70) clj_release(t70);
	clj_release(t69);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o75;
	clj_value t75 = clj_c_var_borrow(V[8], &o75);
	if (t75 == CLJ_THROWN) {
	clj_release(t74);
	if (o71) clj_release(t71);
	if (o70) clj_release(t70);
	clj_release(t69);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value t76 = l3;
	clj_value a30[1] = {t76};
	(void)a30;
	clj_value t77 = clj_c_invoke(t75, a30, 1);
	(void)t76;
	if (o75) clj_release(t75);
	if (t77 == CLJ_THROWN) {
	clj_release(t74);
	if (o71) clj_release(t71);
	if (o70) clj_release(t70);
	clj_release(t69);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	bool o78;
	clj_value t78 = clj_c_var_borrow(V[8], &o78);
	if (t78 == CLJ_THROWN) {
	clj_release(t77);
	clj_release(t74);
	if (o71) clj_release(t71);
	if (o70) clj_release(t70);
	clj_release(t69);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value t79 = l2;
	clj_value a31[1] = {t79};
	(void)a31;
	clj_value t80 = clj_c_invoke(t78, a31, 1);
	(void)t79;
	if (o78) clj_release(t78);
	if (t80 == CLJ_THROWN) {
	clj_release(t77);
	clj_release(t74);
	if (o71) clj_release(t71);
	if (o70) clj_release(t70);
	clj_release(t69);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value a28[3] = {t74, t77, t80};
	(void)a28;
	clj_value t81 = clj_c_invoke(t71, a28, 3);
	clj_release(t80);
	clj_release(t77);
	clj_release(t74);
	if (o71) clj_release(t71);
	if (t81 == CLJ_THROWN) {
	if (o70) clj_release(t70);
	clj_release(t69);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value a27[1] = {t81};
	(void)a27;
	clj_value t82;
	if (CLJC_GUARD(V[12], B[0])) {
	t82 = clj_seq(t81);
	} else {
	t82 = clj_c_intrinsic_fallback(V[12], a27, 1);
	}
	clj_release(t81);
	if (t82 == CLJ_THROWN) {
	if (o70) clj_release(t70);
	clj_release(t69);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value a26[1] = {t82};
	(void)a26;
	clj_value t83 = clj_c_invoke(t70, a26, 1);
	clj_release(t82);
	if (o70) clj_release(t70);
	if (t83 == CLJ_THROWN) {
	clj_release(t69);
	clj_release(t32);
	if (o29) clj_release(t29);
	goto L2;
	}
	clj_value a9[3] = {t32, t69, t83};
	(void)a9;
	clj_value t84 = clj_c_invoke(t29, a9, 3);
	clj_release(t83);
	clj_release(t69);
	clj_release(t32);
	if (o29) clj_release(t29);
	if (t84 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a8[1] = {t84};
	(void)a8;
	clj_value t85;
	if (CLJC_GUARD(V[12], B[0])) {
	t85 = clj_seq(t84);
	} else {
	t85 = clj_c_intrinsic_fallback(V[12], a8, 1);
	}
	clj_release(t84);
	if (t85 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[26], &cc);
	clj_release(l4);
	clj_release(l5);
	clj_release(l6);
	clj_release(l7);
	return t85;
L2: ;
	clj_c_leave(&S[26], &cc);
L1: ;
	clj_release(l4);
	clj_release(l5);
	clj_release(l6);
	clj_release(l7);
	return CLJ_THROWN;
}

static clj_value clojure_test_is(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 3: return clojure_test_is_a3(clj_from_ptr((void *)f), f->env, args, n);
	case 4: return clojure_test_is_a4(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_51(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 197 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[250], clojure_test_is, NULL, 0, 0x18, 3, 4);
	clj_var_bind_root(V[66], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[259]);
	clj_value t2 = clj_c_def(V[66], t1, true, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_are_v4(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_value l2 = args[2];
	(void)l2;
	clj_value l3 = args[3];
	(void)l3;
	clj_value l4 = CLJ_NIL;
	clj_value l5 = CLJ_NIL;
	clj_value l6 = CLJ_NIL;
	clj_value l7 = CLJ_NIL;
	clj_value l8 = CLJ_NIL;
	l4 = nargs > 4 ? clj_list_from_array(args + 4, nargs - 4) : CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[27], &cc)) goto L1;
#line 209 "<embedded>/clojure/test.clj"
#line 210 "<embedded>/clojure/test.clj"
	clj_value t0 = l2;
	clj_value a0[1] = {t0};
	(void)a0;
	clj_value t1;
	if (CLJC_GUARD(V[70], B[14])) {
	t1 = clj_empty_p(t0);
	} else {
	t1 = clj_c_intrinsic_fallback(V[70], a0, 1);
	}
	(void)t0;
	if (t1 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l5, t1);
	clj_value t2 = l5;
	bool t3 = clj_truthy(t2);
	(void)t2;
	clj_value t4;
	if (t3) {
	clj_value t5 = l4;
	clj_value a1[1] = {t5};
	(void)a1;
	clj_value t6;
	if (CLJC_GUARD(V[70], B[14])) {
	t6 = clj_empty_p(t5);
	} else {
	t6 = clj_c_intrinsic_fallback(V[70], a1, 1);
	}
	(void)t5;
	if (t6 == CLJ_THROWN) {
	goto L2;
	}
	t4 = t6;
	} else {
	clj_value t7 = l5;
	l5 = CLJ_NIL;
	t4 = t7;
	}
	clj_c_rebind(&l6, t4);
#line 209 "<embedded>/clojure/test.clj"
	clj_value t8 = l6;
	bool t9 = clj_truthy(t8);
	(void)t8;
	clj_value t10;
	if (t9) {
	clj_value t11 = clj_retain(l6);
	t10 = t11;
	} else {
#line 211 "<embedded>/clojure/test.clj"
	clj_value t12 = l2;
	clj_value a3[1] = {t12};
	(void)a3;
	clj_value t13;
	if (CLJC_GUARD(V[72], B[16])) {
	t13 = clj_count(t12);
	} else {
	t13 = clj_c_intrinsic_fallback(V[72], a3, 1);
	}
	(void)t12;
	if (t13 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t14;
	clj_value a2[1];
	if (CLJC_GUARD(V[71], B[15])) {
	if (1 && clj_is_fixnum(t13)) {
	int64_t i15 = clj_fixnum_val(t13);
	t14 = clj_bool(i15 > INT64_C(0));
	} else {
	a2[0] = t13;
	t14 = clj_pos_p(a2[0]);
	}
	} else {
	a2[0] = t13;
	t14 = clj_c_intrinsic_fallback(V[71], a2, 1);
	}
	clj_release(t13);
	if (t14 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l7, t14);
#line 209 "<embedded>/clojure/test.clj"
	clj_value t16 = l7;
	bool t17 = clj_truthy(t16);
	(void)t16;
	clj_value t18;
	if (t17) {
#line 212 "<embedded>/clojure/test.clj"
	clj_value t19 = l4;
	clj_value a5[1] = {t19};
	(void)a5;
	clj_value t20;
	if (CLJC_GUARD(V[72], B[16])) {
	t20 = clj_count(t19);
	} else {
	t20 = clj_c_intrinsic_fallback(V[72], a5, 1);
	}
	(void)t19;
	if (t20 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t21;
	clj_value a4[1];
	if (CLJC_GUARD(V[71], B[15])) {
	if (1 && clj_is_fixnum(t20)) {
	int64_t i22 = clj_fixnum_val(t20);
	t21 = clj_bool(i22 > INT64_C(0));
	} else {
	a4[0] = t20;
	t21 = clj_pos_p(a4[0]);
	}
	} else {
	a4[0] = t20;
	t21 = clj_c_intrinsic_fallback(V[71], a4, 1);
	}
	clj_release(t20);
	if (t21 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l8, t21);
#line 209 "<embedded>/clojure/test.clj"
	clj_value t23 = l8;
	bool t24 = clj_truthy(t23);
	(void)t23;
	clj_value t25;
	if (t24) {
#line 213 "<embedded>/clojure/test.clj"
	bool o26;
	clj_value t26 = clj_c_var_borrow(V[74], &o26);
	if (t26 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t27 = l4;
	clj_value a8[1] = {t27};
	(void)a8;
	clj_value t28;
	if (CLJC_GUARD(V[72], B[16])) {
	t28 = clj_count(t27);
	} else {
	t28 = clj_c_intrinsic_fallback(V[72], a8, 1);
	}
	(void)t27;
	if (t28 == CLJ_THROWN) {
	if (o26) clj_release(t26);
	goto L2;
	}
	clj_value t29 = l2;
	clj_value a9[1] = {t29};
	(void)a9;
	clj_value t30;
	if (CLJC_GUARD(V[72], B[16])) {
	t30 = clj_count(t29);
	} else {
	t30 = clj_c_intrinsic_fallback(V[72], a9, 1);
	}
	(void)t29;
	if (t30 == CLJ_THROWN) {
	clj_release(t28);
	if (o26) clj_release(t26);
	goto L2;
	}
	clj_value a7[2] = {t28, t30};
	(void)a7;
	clj_value t31;
#ifdef CLJC_DIRECT_clojure_core_mod_a2
	if (!CLJC_FN_clojure_core_mod_a2) CLJC_FN_clojure_core_mod_a2 = clj_compiled_symbol("clojure_core_mod_a2");
	t31 = CLJC_FN_clojure_core_mod_a2 ? CLJC_FN_clojure_core_mod_a2(clj_var_root_relaxed(V[74]), NULL, a7, 2) : clj_c_invoke(t26, a7, 2);
#else
	t31 = clj_c_invoke(t26, a7, 2);
#endif
	clj_release(t30);
	clj_release(t28);
	if (o26) clj_release(t26);
	if (t31 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a6[1] = {t31};
	(void)a6;
	clj_value t32;
	if (CLJC_GUARD(V[73], B[17])) {
	t32 = clj_zero_p(t31);
	} else {
	t32 = clj_c_intrinsic_fallback(V[73], a6, 1);
	}
	clj_release(t31);
	if (t32 == CLJ_THROWN) {
	goto L2;
	}
	t25 = t32;
	} else {
#line 209 "<embedded>/clojure/test.clj"
	clj_value t33 = clj_retain(l8);
	t25 = t33;
	}
	t18 = t25;
	} else {
	clj_value t34 = clj_retain(l7);
	t18 = t34;
	}
	t10 = t18;
	}
	bool t35 = clj_truthy(t10);
	clj_release(t10);
	clj_value t36;
	if (t35) {
	bool o37;
	clj_value t37 = clj_c_var_borrow(V[13], &o37);
	if (t37 == CLJ_THROWN) {
	goto L2;
	}
	bool o38;
	clj_value t38 = clj_c_var_borrow(V[8], &o38);
	if (t38 == CLJ_THROWN) {
	if (o37) clj_release(t37);
	goto L2;
	}
	clj_value t39 = K[262];
	clj_value a12[1] = {t39};
	(void)a12;
	clj_value t40 = clj_c_invoke(t38, a12, 1);
	(void)t39;
	if (o38) clj_release(t38);
	if (t40 == CLJ_THROWN) {
	if (o37) clj_release(t37);
	goto L2;
	}
	bool o41;
	clj_value t41 = clj_c_var_borrow(V[8], &o41);
	if (t41 == CLJ_THROWN) {
	clj_release(t40);
	if (o37) clj_release(t37);
	goto L2;
	}
	clj_value t42 = l2;
	clj_value a13[1] = {t42};
	(void)a13;
	clj_value t43 = clj_c_invoke(t41, a13, 1);
	(void)t42;
	if (o41) clj_release(t41);
	if (t43 == CLJ_THROWN) {
	clj_release(t40);
	if (o37) clj_release(t37);
	goto L2;
	}
	bool o44;
	clj_value t44 = clj_c_var_borrow(V[8], &o44);
	if (t44 == CLJ_THROWN) {
	clj_release(t43);
	clj_release(t40);
	if (o37) clj_release(t37);
	goto L2;
	}
	bool o45;
	clj_value t45 = clj_c_var_borrow(V[13], &o45);
	if (t45 == CLJ_THROWN) {
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t40);
	if (o37) clj_release(t37);
	goto L2;
	}
	bool o46;
	clj_value t46 = clj_c_var_borrow(V[8], &o46);
	if (t46 == CLJ_THROWN) {
	if (o45) clj_release(t45);
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t40);
	if (o37) clj_release(t37);
	goto L2;
	}
	clj_value t47 = K[263];
	clj_value a17[1] = {t47};
	(void)a17;
	clj_value t48 = clj_c_invoke(t46, a17, 1);
	(void)t47;
	if (o46) clj_release(t46);
	if (t48 == CLJ_THROWN) {
	if (o45) clj_release(t45);
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t40);
	if (o37) clj_release(t37);
	goto L2;
	}
	bool o49;
	clj_value t49 = clj_c_var_borrow(V[8], &o49);
	if (t49 == CLJ_THROWN) {
	clj_release(t48);
	if (o45) clj_release(t45);
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t40);
	if (o37) clj_release(t37);
	goto L2;
	}
	clj_value t50 = l3;
	clj_value a18[1] = {t50};
	(void)a18;
	clj_value t51 = clj_c_invoke(t49, a18, 1);
	(void)t50;
	if (o49) clj_release(t49);
	if (t51 == CLJ_THROWN) {
	clj_release(t48);
	if (o45) clj_release(t45);
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t40);
	if (o37) clj_release(t37);
	goto L2;
	}
	clj_value a16[2] = {t48, t51};
	(void)a16;
	clj_value t52;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t52 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a16, 2) : clj_c_invoke(t45, a16, 2);
#else
	t52 = clj_c_invoke(t45, a16, 2);
#endif
	clj_release(t51);
	clj_release(t48);
	if (o45) clj_release(t45);
	if (t52 == CLJ_THROWN) {
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t40);
	if (o37) clj_release(t37);
	goto L2;
	}
	clj_value a15[1] = {t52};
	(void)a15;
	clj_value t53;
	if (CLJC_GUARD(V[12], B[0])) {
	t53 = clj_seq(t52);
	} else {
	t53 = clj_c_intrinsic_fallback(V[12], a15, 1);
	}
	clj_release(t52);
	if (t53 == CLJ_THROWN) {
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t40);
	if (o37) clj_release(t37);
	goto L2;
	}
	clj_value a14[1] = {t53};
	(void)a14;
	clj_value t54 = clj_c_invoke(t44, a14, 1);
	clj_release(t53);
	if (o44) clj_release(t44);
	if (t54 == CLJ_THROWN) {
	clj_release(t43);
	clj_release(t40);
	if (o37) clj_release(t37);
	goto L2;
	}
	clj_value t55 = l4;
	clj_value a11[4] = {t40, t43, t54, t55};
	(void)a11;
	clj_value t56 = clj_c_invoke(t37, a11, 4);
	(void)t55;
	clj_release(t54);
	clj_release(t43);
	clj_release(t40);
	if (o37) clj_release(t37);
	if (t56 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a10[1] = {t56};
	(void)a10;
	clj_value t57;
	if (CLJC_GUARD(V[12], B[0])) {
	t57 = clj_seq(t56);
	} else {
	t57 = clj_c_intrinsic_fallback(V[12], a10, 1);
	}
	clj_release(t56);
	if (t57 == CLJ_THROWN) {
	goto L2;
	}
	t36 = t57;
	} else {
#line 215 "<embedded>/clojure/test.clj"
	bool o58;
	clj_value t58 = clj_c_var_borrow(V[75], &o58);
	if (t58 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t59 = K[264];
	clj_value t60 = K[265];
	clj_value a19[2] = {t59, t60};
	(void)a19;
	clj_value t61 = clj_c_invoke(t58, a19, 2);
	(void)t60;
	(void)t59;
	if (o58) clj_release(t58);
	if (t61 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t62 = clj_throw(t61);
	if (t62 == CLJ_THROWN) {
	goto L2;
	}
	t36 = t62;
	}
	clj_c_leave(&S[27], &cc);
	clj_release(l4);
	clj_release(l5);
	clj_release(l6);
	clj_release(l7);
	clj_release(l8);
	return t36;
L2: ;
	clj_c_leave(&S[27], &cc);
L1: ;
	clj_release(l4);
	clj_release(l5);
	clj_release(l6);
	clj_release(l7);
	clj_release(l8);
	return CLJ_THROWN;
}

static clj_value clojure_test_are(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	default: break;
	}
	if (n >= 4) return clojure_test_are_v4(clj_from_ptr((void *)f), f->env, args, n);
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_52(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 206 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[261], clojure_test_are, NULL, 0, 0x0, 4, CLJ_ARITY_ANY);
	clj_var_bind_root(V[69], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[266]);
	clj_value t2 = clj_c_def(V[69], t1, true, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_testing_v3(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_value l2 = args[2];
	(void)l2;
	clj_value l3 = CLJ_NIL;
	l3 = nargs > 3 ? clj_list_from_array(args + 3, nargs - 3) : CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[28], &cc)) goto L1;
#line 217 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[13], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[8], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t2 = K[269];
	clj_value a2[1] = {t2};
	(void)a2;
	clj_value t3 = clj_c_invoke(t1, a2, 1);
	(void)t2;
	if (o1) clj_release(t1);
	if (t3 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[8], &o4);
	if (t4 == CLJ_THROWN) {
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o5;
	clj_value t5 = clj_c_var_borrow(V[17], &o5);
	if (t5 == CLJ_THROWN) {
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o6;
	clj_value t6 = clj_c_var_borrow(V[58], &o6);
	if (t6 == CLJ_THROWN) {
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o7;
	clj_value t7 = clj_c_var_borrow(V[13], &o7);
	if (t7 == CLJ_THROWN) {
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[8], &o8);
	if (t8 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t9 = K[270];
	clj_value a7[1] = {t9};
	(void)a7;
	clj_value t10 = clj_c_invoke(t8, a7, 1);
	(void)t9;
	if (o8) clj_release(t8);
	if (t10 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o11;
	clj_value t11 = clj_c_var_borrow(V[8], &o11);
	if (t11 == CLJ_THROWN) {
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o12;
	clj_value t12 = clj_c_var_borrow(V[13], &o12);
	if (t12 == CLJ_THROWN) {
	if (o11) clj_release(t11);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o13;
	clj_value t13 = clj_c_var_borrow(V[8], &o13);
	if (t13 == CLJ_THROWN) {
	if (o12) clj_release(t12);
	if (o11) clj_release(t11);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t14 = K[271];
	clj_value a11[1] = {t14};
	(void)a11;
	clj_value t15 = clj_c_invoke(t13, a11, 1);
	(void)t14;
	if (o13) clj_release(t13);
	if (t15 == CLJ_THROWN) {
	if (o12) clj_release(t12);
	if (o11) clj_release(t11);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o16;
	clj_value t16 = clj_c_var_borrow(V[8], &o16);
	if (t16 == CLJ_THROWN) {
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o11) clj_release(t11);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t17 = K[272];
	clj_value a12[1] = {t17};
	(void)a12;
	clj_value t18 = clj_c_invoke(t16, a12, 1);
	(void)t17;
	if (o16) clj_release(t16);
	if (t18 == CLJ_THROWN) {
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o11) clj_release(t11);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o19;
	clj_value t19 = clj_c_var_borrow(V[8], &o19);
	if (t19 == CLJ_THROWN) {
	clj_release(t18);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o11) clj_release(t11);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t20 = l2;
	clj_value a13[1] = {t20};
	(void)a13;
	clj_value t21 = clj_c_invoke(t19, a13, 1);
	(void)t20;
	if (o19) clj_release(t19);
	if (t21 == CLJ_THROWN) {
	clj_release(t18);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o11) clj_release(t11);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a10[3] = {t15, t18, t21};
	(void)a10;
	clj_value t22 = clj_c_invoke(t12, a10, 3);
	clj_release(t21);
	clj_release(t18);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (t22 == CLJ_THROWN) {
	if (o11) clj_release(t11);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a9[1] = {t22};
	(void)a9;
	clj_value t23;
	if (CLJC_GUARD(V[12], B[0])) {
	t23 = clj_seq(t22);
	} else {
	t23 = clj_c_intrinsic_fallback(V[12], a9, 1);
	}
	clj_release(t22);
	if (t23 == CLJ_THROWN) {
	if (o11) clj_release(t11);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a8[1] = {t23};
	(void)a8;
	clj_value t24 = clj_c_invoke(t11, a8, 1);
	clj_release(t23);
	if (o11) clj_release(t11);
	if (t24 == CLJ_THROWN) {
	clj_release(t10);
	if (o7) clj_release(t7);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a6[2] = {t10, t24};
	(void)a6;
	clj_value t25;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t25 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a6, 2) : clj_c_invoke(t7, a6, 2);
#else
	t25 = clj_c_invoke(t7, a6, 2);
#endif
	clj_release(t24);
	clj_release(t10);
	if (o7) clj_release(t7);
	if (t25 == CLJ_THROWN) {
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a5[1] = {t25};
	(void)a5;
	clj_value t26;
	if (CLJC_GUARD(V[12], B[0])) {
	t26 = clj_seq(t25);
	} else {
	t26 = clj_c_intrinsic_fallback(V[12], a5, 1);
	}
	clj_release(t25);
	if (t26 == CLJ_THROWN) {
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a4[2] = {t6, t26};
	(void)a4;
	clj_value t27 = clj_c_invoke(t5, a4, 2);
	clj_release(t26);
	if (o6) clj_release(t6);
	if (o5) clj_release(t5);
	if (t27 == CLJ_THROWN) {
	if (o4) clj_release(t4);
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a3[1] = {t27};
	(void)a3;
	clj_value t28 = clj_c_invoke(t4, a3, 1);
	clj_release(t27);
	if (o4) clj_release(t4);
	if (t28 == CLJ_THROWN) {
	clj_release(t3);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t29 = l3;
	clj_value a1[3] = {t3, t28, t29};
	(void)a1;
	clj_value t30 = clj_c_invoke(t0, a1, 3);
	(void)t29;
	clj_release(t28);
	clj_release(t3);
	if (o0) clj_release(t0);
	if (t30 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t30};
	(void)a0;
	clj_value t31;
	if (CLJC_GUARD(V[12], B[0])) {
	t31 = clj_seq(t30);
	} else {
	t31 = clj_c_intrinsic_fallback(V[12], a0, 1);
	}
	clj_release(t30);
	if (t31 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[28], &cc);
	clj_release(l3);
	return t31;
L2: ;
	clj_c_leave(&S[28], &cc);
L1: ;
	clj_release(l3);
	return CLJ_THROWN;
}

static clj_value clojure_test_testing(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	default: break;
	}
	if (n >= 3) return clojure_test_testing_v3(clj_from_ptr((void *)f), f->env, args, n);
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_53(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 217 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[268], clojure_test_testing, NULL, 0, 0x0, 3, CLJ_ARITY_ANY);
	clj_var_bind_root(V[76], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[273]);
	clj_value t2 = clj_c_def(V[76], t1, true, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_with_test_v3(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_value l2 = args[2];
	(void)l2;
	clj_value l3 = CLJ_NIL;
	l3 = nargs > 3 ? clj_list_from_array(args + 3, nargs - 3) : CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[29], &cc)) goto L1;
#line 228 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[3], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool t1 = clj_truthy(t0);
	if (o0) clj_release(t0);
	clj_value t2;
	if (t1) {
	bool o3;
	clj_value t3 = clj_c_var_borrow(V[13], &o3);
	if (t3 == CLJ_THROWN) {
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[8], &o4);
	if (t4 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t5 = K[276];
	clj_value a2[1] = {t5};
	(void)a2;
	clj_value t6 = clj_c_invoke(t4, a2, 1);
	(void)t5;
	if (o4) clj_release(t4);
	if (t6 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o7;
	clj_value t7 = clj_c_var_borrow(V[8], &o7);
	if (t7 == CLJ_THROWN) {
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t8 = l2;
	clj_value a3[1] = {t8};
	(void)a3;
	clj_value t9 = clj_c_invoke(t7, a3, 1);
	(void)t8;
	if (o7) clj_release(t7);
	if (t9 == CLJ_THROWN) {
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o10;
	clj_value t10 = clj_c_var_borrow(V[8], &o10);
	if (t10 == CLJ_THROWN) {
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o11;
	clj_value t11 = clj_c_var_borrow(V[13], &o11);
	if (t11 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o12;
	clj_value t12 = clj_c_var_borrow(V[8], &o12);
	if (t12 == CLJ_THROWN) {
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t13 = K[277];
	clj_value a7[1] = {t13};
	(void)a7;
	clj_value t14 = clj_c_invoke(t12, a7, 1);
	(void)t13;
	if (o12) clj_release(t12);
	if (t14 == CLJ_THROWN) {
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o15;
	clj_value t15 = clj_c_var_borrow(V[8], &o15);
	if (t15 == CLJ_THROWN) {
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t16 = K[278];
	clj_value a8[1] = {t16};
	(void)a8;
	clj_value t17 = clj_c_invoke(t15, a8, 1);
	(void)t16;
	if (o15) clj_release(t15);
	if (t17 == CLJ_THROWN) {
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o18;
	clj_value t18 = clj_c_var_borrow(V[8], &o18);
	if (t18 == CLJ_THROWN) {
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t19 = K[279];
	clj_value a9[1] = {t19};
	(void)a9;
	clj_value t20 = clj_c_invoke(t18, a9, 1);
	(void)t19;
	if (o18) clj_release(t18);
	if (t20 == CLJ_THROWN) {
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o21;
	clj_value t21 = clj_c_var_borrow(V[8], &o21);
	if (t21 == CLJ_THROWN) {
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o22;
	clj_value t22 = clj_c_var_borrow(V[13], &o22);
	if (t22 == CLJ_THROWN) {
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o23;
	clj_value t23 = clj_c_var_borrow(V[8], &o23);
	if (t23 == CLJ_THROWN) {
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t24 = K[280];
	clj_value a13[1] = {t24};
	(void)a13;
	clj_value t25 = clj_c_invoke(t23, a13, 1);
	(void)t24;
	if (o23) clj_release(t23);
	if (t25 == CLJ_THROWN) {
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o26;
	clj_value t26 = clj_c_var_borrow(V[8], &o26);
	if (t26 == CLJ_THROWN) {
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o27;
	clj_value t27 = clj_c_var_borrow(V[17], &o27);
	if (t27 == CLJ_THROWN) {
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o28;
	clj_value t28 = clj_c_var_borrow(V[58], &o28);
	if (t28 == CLJ_THROWN) {
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o29;
	clj_value t29 = clj_c_var_borrow(V[13], &o29);
	if (t29 == CLJ_THROWN) {
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	const clj_value *a17 = NULL;
	clj_value t30;
#ifdef CLJC_DIRECT_clojure_core_concat_a0
	if (!CLJC_FN_clojure_core_concat_a0) CLJC_FN_clojure_core_concat_a0 = clj_compiled_symbol("clojure_core_concat_a0");
	t30 = CLJC_FN_clojure_core_concat_a0 ? CLJC_FN_clojure_core_concat_a0(clj_var_root_relaxed(V[13]), NULL, a17, 0) : clj_c_invoke(t29, a17, 0);
#else
	t30 = clj_c_invoke(t29, a17, 0);
#endif
	if (o29) clj_release(t29);
	if (t30 == CLJ_THROWN) {
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a16[1] = {t30};
	(void)a16;
	clj_value t31;
	if (CLJC_GUARD(V[12], B[0])) {
	t31 = clj_seq(t30);
	} else {
	t31 = clj_c_intrinsic_fallback(V[12], a16, 1);
	}
	clj_release(t30);
	if (t31 == CLJ_THROWN) {
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a15[2] = {t28, t31};
	(void)a15;
	clj_value t32 = clj_c_invoke(t27, a15, 2);
	clj_release(t31);
	if (o28) clj_release(t28);
	if (o27) clj_release(t27);
	if (t32 == CLJ_THROWN) {
	if (o26) clj_release(t26);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a14[1] = {t32};
	(void)a14;
	clj_value t33 = clj_c_invoke(t26, a14, 1);
	clj_release(t32);
	if (o26) clj_release(t26);
	if (t33 == CLJ_THROWN) {
	clj_release(t25);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t34 = l3;
	clj_value a12[3] = {t25, t33, t34};
	(void)a12;
	clj_value t35 = clj_c_invoke(t22, a12, 3);
	(void)t34;
	clj_release(t33);
	clj_release(t25);
	if (o22) clj_release(t22);
	if (t35 == CLJ_THROWN) {
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a11[1] = {t35};
	(void)a11;
	clj_value t36;
	if (CLJC_GUARD(V[12], B[0])) {
	t36 = clj_seq(t35);
	} else {
	t36 = clj_c_intrinsic_fallback(V[12], a11, 1);
	}
	clj_release(t35);
	if (t36 == CLJ_THROWN) {
	if (o21) clj_release(t21);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a10[1] = {t36};
	(void)a10;
	clj_value t37 = clj_c_invoke(t21, a10, 1);
	clj_release(t36);
	if (o21) clj_release(t21);
	if (t37 == CLJ_THROWN) {
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a6[4] = {t14, t17, t20, t37};
	(void)a6;
	clj_value t38 = clj_c_invoke(t11, a6, 4);
	clj_release(t37);
	clj_release(t20);
	clj_release(t17);
	clj_release(t14);
	if (o11) clj_release(t11);
	if (t38 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a5[1] = {t38};
	(void)a5;
	clj_value t39;
	if (CLJC_GUARD(V[12], B[0])) {
	t39 = clj_seq(t38);
	} else {
	t39 = clj_c_intrinsic_fallback(V[12], a5, 1);
	}
	clj_release(t38);
	if (t39 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a4[1] = {t39};
	(void)a4;
	clj_value t40 = clj_c_invoke(t10, a4, 1);
	clj_release(t39);
	if (o10) clj_release(t10);
	if (t40 == CLJ_THROWN) {
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a1[3] = {t6, t9, t40};
	(void)a1;
	clj_value t41 = clj_c_invoke(t3, a1, 3);
	clj_release(t40);
	clj_release(t9);
	clj_release(t6);
	if (o3) clj_release(t3);
	if (t41 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t41};
	(void)a0;
	clj_value t42;
	if (CLJC_GUARD(V[12], B[0])) {
	t42 = clj_seq(t41);
	} else {
	t42 = clj_c_intrinsic_fallback(V[12], a0, 1);
	}
	clj_release(t41);
	if (t42 == CLJ_THROWN) {
	goto L2;
	}
	t2 = t42;
	} else {
	clj_value t43 = clj_retain(l2);
	t2 = t43;
	}
	clj_c_leave(&S[29], &cc);
	clj_release(l3);
	return t2;
L2: ;
	clj_c_leave(&S[29], &cc);
L1: ;
	clj_release(l3);
	return CLJ_THROWN;
}

static clj_value clojure_test_with_test(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	default: break;
	}
	if (n >= 3) return clojure_test_with_test_v3(clj_from_ptr((void *)f), f->env, args, n);
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_54(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 225 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[275], clojure_test_with_test, NULL, 0, 0x0, 3, CLJ_ARITY_ANY);
	clj_var_bind_root(V[77], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[281]);
	clj_value t2 = clj_c_def(V[77], t1, true, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_deftest_v3(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_value l2 = args[2];
	(void)l2;
	clj_value l3 = CLJ_NIL;
	l3 = nargs > 3 ? clj_list_from_array(args + 3, nargs - 3) : CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[30], &cc)) goto L1;
#line 235 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[3], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool t1 = clj_truthy(t0);
	if (o0) clj_release(t0);
	clj_value t2;
	if (t1) {
	bool o3;
	clj_value t3 = clj_c_var_borrow(V[13], &o3);
	if (t3 == CLJ_THROWN) {
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[8], &o4);
	if (t4 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t5 = K[284];
	clj_value a2[1] = {t5};
	(void)a2;
	clj_value t6 = clj_c_invoke(t4, a2, 1);
	(void)t5;
	if (o4) clj_release(t4);
	if (t6 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o7;
	clj_value t7 = clj_c_var_borrow(V[8], &o7);
	if (t7 == CLJ_THROWN) {
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
#line 236 "<embedded>/clojure/test.clj"
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[79], &o8);
	if (t8 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t9 = l2;
	bool o10;
	clj_value t10 = clj_c_var_borrow(V[80], &o10);
	if (t10 == CLJ_THROWN) {
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t11 = K[285];
	bool o12;
	clj_value t12 = clj_c_var_borrow(V[13], &o12);
	if (t12 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o13;
	clj_value t13 = clj_c_var_borrow(V[8], &o13);
	if (t13 == CLJ_THROWN) {
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t14 = K[286];
	clj_value a7[1] = {t14};
	(void)a7;
	clj_value t15 = clj_c_invoke(t13, a7, 1);
	(void)t14;
	if (o13) clj_release(t13);
	if (t15 == CLJ_THROWN) {
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o16;
	clj_value t16 = clj_c_var_borrow(V[8], &o16);
	if (t16 == CLJ_THROWN) {
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o17;
	clj_value t17 = clj_c_var_borrow(V[17], &o17);
	if (t17 == CLJ_THROWN) {
	if (o16) clj_release(t16);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o18;
	clj_value t18 = clj_c_var_borrow(V[58], &o18);
	if (t18 == CLJ_THROWN) {
	if (o17) clj_release(t17);
	if (o16) clj_release(t16);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o19;
	clj_value t19 = clj_c_var_borrow(V[13], &o19);
	if (t19 == CLJ_THROWN) {
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	if (o16) clj_release(t16);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	const clj_value *a11 = NULL;
	clj_value t20;
#ifdef CLJC_DIRECT_clojure_core_concat_a0
	if (!CLJC_FN_clojure_core_concat_a0) CLJC_FN_clojure_core_concat_a0 = clj_compiled_symbol("clojure_core_concat_a0");
	t20 = CLJC_FN_clojure_core_concat_a0 ? CLJC_FN_clojure_core_concat_a0(clj_var_root_relaxed(V[13]), NULL, a11, 0) : clj_c_invoke(t19, a11, 0);
#else
	t20 = clj_c_invoke(t19, a11, 0);
#endif
	if (o19) clj_release(t19);
	if (t20 == CLJ_THROWN) {
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	if (o16) clj_release(t16);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a10[1] = {t20};
	(void)a10;
	clj_value t21;
	if (CLJC_GUARD(V[12], B[0])) {
	t21 = clj_seq(t20);
	} else {
	t21 = clj_c_intrinsic_fallback(V[12], a10, 1);
	}
	clj_release(t20);
	if (t21 == CLJ_THROWN) {
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	if (o16) clj_release(t16);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a9[2] = {t18, t21};
	(void)a9;
	clj_value t22 = clj_c_invoke(t17, a9, 2);
	clj_release(t21);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	if (t22 == CLJ_THROWN) {
	if (o16) clj_release(t16);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a8[1] = {t22};
	(void)a8;
	clj_value t23 = clj_c_invoke(t16, a8, 1);
	clj_release(t22);
	if (o16) clj_release(t16);
	if (t23 == CLJ_THROWN) {
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t24 = l3;
	clj_value a6[3] = {t15, t23, t24};
	(void)a6;
	clj_value t25 = clj_c_invoke(t12, a6, 3);
	(void)t24;
	clj_release(t23);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (t25 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a5[1] = {t25};
	(void)a5;
	clj_value t26;
	if (CLJC_GUARD(V[12], B[0])) {
	t26 = clj_seq(t25);
	} else {
	t26 = clj_c_intrinsic_fallback(V[12], a5, 1);
	}
	clj_release(t25);
	if (t26 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a4[4] = {t9, t10, t11, t26};
	(void)a4;
	clj_value t27 = clj_c_invoke(t8, a4, 4);
	clj_release(t26);
	(void)t11;
	if (o10) clj_release(t10);
	(void)t9;
	if (o8) clj_release(t8);
	if (t27 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a3[1] = {t27};
	(void)a3;
	clj_value t28 = clj_c_invoke(t7, a3, 1);
	clj_release(t27);
	if (o7) clj_release(t7);
	if (t28 == CLJ_THROWN) {
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
#line 235 "<embedded>/clojure/test.clj"
	bool o29;
	clj_value t29 = clj_c_var_borrow(V[8], &o29);
	if (t29 == CLJ_THROWN) {
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o30;
	clj_value t30 = clj_c_var_borrow(V[13], &o30);
	if (t30 == CLJ_THROWN) {
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o31;
	clj_value t31 = clj_c_var_borrow(V[8], &o31);
	if (t31 == CLJ_THROWN) {
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t32 = K[287];
	clj_value a15[1] = {t32};
	(void)a15;
	clj_value t33 = clj_c_invoke(t31, a15, 1);
	(void)t32;
	if (o31) clj_release(t31);
	if (t33 == CLJ_THROWN) {
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o34;
	clj_value t34 = clj_c_var_borrow(V[8], &o34);
	if (t34 == CLJ_THROWN) {
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o35;
	clj_value t35 = clj_c_var_borrow(V[17], &o35);
	if (t35 == CLJ_THROWN) {
	if (o34) clj_release(t34);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o36;
	clj_value t36 = clj_c_var_borrow(V[58], &o36);
	if (t36 == CLJ_THROWN) {
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o37;
	clj_value t37 = clj_c_var_borrow(V[13], &o37);
	if (t37 == CLJ_THROWN) {
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	const clj_value *a19 = NULL;
	clj_value t38;
#ifdef CLJC_DIRECT_clojure_core_concat_a0
	if (!CLJC_FN_clojure_core_concat_a0) CLJC_FN_clojure_core_concat_a0 = clj_compiled_symbol("clojure_core_concat_a0");
	t38 = CLJC_FN_clojure_core_concat_a0 ? CLJC_FN_clojure_core_concat_a0(clj_var_root_relaxed(V[13]), NULL, a19, 0) : clj_c_invoke(t37, a19, 0);
#else
	t38 = clj_c_invoke(t37, a19, 0);
#endif
	if (o37) clj_release(t37);
	if (t38 == CLJ_THROWN) {
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a18[1] = {t38};
	(void)a18;
	clj_value t39;
	if (CLJC_GUARD(V[12], B[0])) {
	t39 = clj_seq(t38);
	} else {
	t39 = clj_c_intrinsic_fallback(V[12], a18, 1);
	}
	clj_release(t38);
	if (t39 == CLJ_THROWN) {
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (o34) clj_release(t34);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a17[2] = {t36, t39};
	(void)a17;
	clj_value t40 = clj_c_invoke(t35, a17, 2);
	clj_release(t39);
	if (o36) clj_release(t36);
	if (o35) clj_release(t35);
	if (t40 == CLJ_THROWN) {
	if (o34) clj_release(t34);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a16[1] = {t40};
	(void)a16;
	clj_value t41 = clj_c_invoke(t34, a16, 1);
	clj_release(t40);
	if (o34) clj_release(t34);
	if (t41 == CLJ_THROWN) {
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o42;
	clj_value t42 = clj_c_var_borrow(V[8], &o42);
	if (t42 == CLJ_THROWN) {
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o43;
	clj_value t43 = clj_c_var_borrow(V[13], &o43);
	if (t43 == CLJ_THROWN) {
	if (o42) clj_release(t42);
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o44;
	clj_value t44 = clj_c_var_borrow(V[8], &o44);
	if (t44 == CLJ_THROWN) {
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t45 = K[288];
	clj_value a23[1] = {t45};
	(void)a23;
	clj_value t46 = clj_c_invoke(t44, a23, 1);
	(void)t45;
	if (o44) clj_release(t44);
	if (t46 == CLJ_THROWN) {
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o47;
	clj_value t47 = clj_c_var_borrow(V[8], &o47);
	if (t47 == CLJ_THROWN) {
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o48;
	clj_value t48 = clj_c_var_borrow(V[13], &o48);
	if (t48 == CLJ_THROWN) {
	if (o47) clj_release(t47);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o49;
	clj_value t49 = clj_c_var_borrow(V[8], &o49);
	if (t49 == CLJ_THROWN) {
	if (o48) clj_release(t48);
	if (o47) clj_release(t47);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t50 = K[289];
	clj_value a27[1] = {t50};
	(void)a27;
	clj_value t51 = clj_c_invoke(t49, a27, 1);
	(void)t50;
	if (o49) clj_release(t49);
	if (t51 == CLJ_THROWN) {
	if (o48) clj_release(t48);
	if (o47) clj_release(t47);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o52;
	clj_value t52 = clj_c_var_borrow(V[8], &o52);
	if (t52 == CLJ_THROWN) {
	clj_release(t51);
	if (o48) clj_release(t48);
	if (o47) clj_release(t47);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t53 = l2;
	clj_value a28[1] = {t53};
	(void)a28;
	clj_value t54 = clj_c_invoke(t52, a28, 1);
	(void)t53;
	if (o52) clj_release(t52);
	if (t54 == CLJ_THROWN) {
	clj_release(t51);
	if (o48) clj_release(t48);
	if (o47) clj_release(t47);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a26[2] = {t51, t54};
	(void)a26;
	clj_value t55;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t55 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a26, 2) : clj_c_invoke(t48, a26, 2);
#else
	t55 = clj_c_invoke(t48, a26, 2);
#endif
	clj_release(t54);
	clj_release(t51);
	if (o48) clj_release(t48);
	if (t55 == CLJ_THROWN) {
	if (o47) clj_release(t47);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a25[1] = {t55};
	(void)a25;
	clj_value t56;
	if (CLJC_GUARD(V[12], B[0])) {
	t56 = clj_seq(t55);
	} else {
	t56 = clj_c_intrinsic_fallback(V[12], a25, 1);
	}
	clj_release(t55);
	if (t56 == CLJ_THROWN) {
	if (o47) clj_release(t47);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a24[1] = {t56};
	(void)a24;
	clj_value t57 = clj_c_invoke(t47, a24, 1);
	clj_release(t56);
	if (o47) clj_release(t47);
	if (t57 == CLJ_THROWN) {
	clj_release(t46);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a22[2] = {t46, t57};
	(void)a22;
	clj_value t58;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t58 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a22, 2) : clj_c_invoke(t43, a22, 2);
#else
	t58 = clj_c_invoke(t43, a22, 2);
#endif
	clj_release(t57);
	clj_release(t46);
	if (o43) clj_release(t43);
	if (t58 == CLJ_THROWN) {
	if (o42) clj_release(t42);
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a21[1] = {t58};
	(void)a21;
	clj_value t59;
	if (CLJC_GUARD(V[12], B[0])) {
	t59 = clj_seq(t58);
	} else {
	t59 = clj_c_intrinsic_fallback(V[12], a21, 1);
	}
	clj_release(t58);
	if (t59 == CLJ_THROWN) {
	if (o42) clj_release(t42);
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a20[1] = {t59};
	(void)a20;
	clj_value t60 = clj_c_invoke(t42, a20, 1);
	clj_release(t59);
	if (o42) clj_release(t42);
	if (t60 == CLJ_THROWN) {
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a14[3] = {t33, t41, t60};
	(void)a14;
	clj_value t61 = clj_c_invoke(t30, a14, 3);
	clj_release(t60);
	clj_release(t41);
	clj_release(t33);
	if (o30) clj_release(t30);
	if (t61 == CLJ_THROWN) {
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a13[1] = {t61};
	(void)a13;
	clj_value t62;
	if (CLJC_GUARD(V[12], B[0])) {
	t62 = clj_seq(t61);
	} else {
	t62 = clj_c_intrinsic_fallback(V[12], a13, 1);
	}
	clj_release(t61);
	if (t62 == CLJ_THROWN) {
	if (o29) clj_release(t29);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a12[1] = {t62};
	(void)a12;
	clj_value t63 = clj_c_invoke(t29, a12, 1);
	clj_release(t62);
	if (o29) clj_release(t29);
	if (t63 == CLJ_THROWN) {
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a1[3] = {t6, t28, t63};
	(void)a1;
	clj_value t64 = clj_c_invoke(t3, a1, 3);
	clj_release(t63);
	clj_release(t28);
	clj_release(t6);
	if (o3) clj_release(t3);
	if (t64 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t64};
	(void)a0;
	clj_value t65;
	if (CLJC_GUARD(V[12], B[0])) {
	t65 = clj_seq(t64);
	} else {
	t65 = clj_c_intrinsic_fallback(V[12], a0, 1);
	}
	clj_release(t64);
	if (t65 == CLJ_THROWN) {
	goto L2;
	}
	t2 = t65;
	} else {
	t2 = CLJ_NIL;
	}
	clj_c_leave(&S[30], &cc);
	clj_release(l3);
	return t2;
L2: ;
	clj_c_leave(&S[30], &cc);
L1: ;
	clj_release(l3);
	return CLJ_THROWN;
}

static clj_value clojure_test_deftest(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	default: break;
	}
	if (n >= 3) return clojure_test_deftest_v3(clj_from_ptr((void *)f), f->env, args, n);
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_55(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 232 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[283], clojure_test_deftest, NULL, 0, 0x0, 3, CLJ_ARITY_ANY);
	clj_var_bind_root(V[78], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[290]);
	clj_value t2 = clj_c_def(V[78], t1, true, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_deftest__v3(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_value l2 = args[2];
	(void)l2;
	clj_value l3 = CLJ_NIL;
	l3 = nargs > 3 ? clj_list_from_array(args + 3, nargs - 3) : CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[31], &cc)) goto L1;
#line 242 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[3], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool t1 = clj_truthy(t0);
	if (o0) clj_release(t0);
	clj_value t2;
	if (t1) {
	bool o3;
	clj_value t3 = clj_c_var_borrow(V[13], &o3);
	if (t3 == CLJ_THROWN) {
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[8], &o4);
	if (t4 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t5 = K[293];
	clj_value a2[1] = {t5};
	(void)a2;
	clj_value t6 = clj_c_invoke(t4, a2, 1);
	(void)t5;
	if (o4) clj_release(t4);
	if (t6 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o7;
	clj_value t7 = clj_c_var_borrow(V[8], &o7);
	if (t7 == CLJ_THROWN) {
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
#line 243 "<embedded>/clojure/test.clj"
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[79], &o8);
	if (t8 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t9 = l2;
	bool o10;
	clj_value t10 = clj_c_var_borrow(V[80], &o10);
	if (t10 == CLJ_THROWN) {
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t11 = K[294];
	bool o12;
	clj_value t12 = clj_c_var_borrow(V[13], &o12);
	if (t12 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o13;
	clj_value t13 = clj_c_var_borrow(V[8], &o13);
	if (t13 == CLJ_THROWN) {
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t14 = K[295];
	clj_value a7[1] = {t14};
	(void)a7;
	clj_value t15 = clj_c_invoke(t13, a7, 1);
	(void)t14;
	if (o13) clj_release(t13);
	if (t15 == CLJ_THROWN) {
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o16;
	clj_value t16 = clj_c_var_borrow(V[8], &o16);
	if (t16 == CLJ_THROWN) {
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o17;
	clj_value t17 = clj_c_var_borrow(V[17], &o17);
	if (t17 == CLJ_THROWN) {
	if (o16) clj_release(t16);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o18;
	clj_value t18 = clj_c_var_borrow(V[58], &o18);
	if (t18 == CLJ_THROWN) {
	if (o17) clj_release(t17);
	if (o16) clj_release(t16);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o19;
	clj_value t19 = clj_c_var_borrow(V[13], &o19);
	if (t19 == CLJ_THROWN) {
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	if (o16) clj_release(t16);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	const clj_value *a11 = NULL;
	clj_value t20;
#ifdef CLJC_DIRECT_clojure_core_concat_a0
	if (!CLJC_FN_clojure_core_concat_a0) CLJC_FN_clojure_core_concat_a0 = clj_compiled_symbol("clojure_core_concat_a0");
	t20 = CLJC_FN_clojure_core_concat_a0 ? CLJC_FN_clojure_core_concat_a0(clj_var_root_relaxed(V[13]), NULL, a11, 0) : clj_c_invoke(t19, a11, 0);
#else
	t20 = clj_c_invoke(t19, a11, 0);
#endif
	if (o19) clj_release(t19);
	if (t20 == CLJ_THROWN) {
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	if (o16) clj_release(t16);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a10[1] = {t20};
	(void)a10;
	clj_value t21;
	if (CLJC_GUARD(V[12], B[0])) {
	t21 = clj_seq(t20);
	} else {
	t21 = clj_c_intrinsic_fallback(V[12], a10, 1);
	}
	clj_release(t20);
	if (t21 == CLJ_THROWN) {
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	if (o16) clj_release(t16);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a9[2] = {t18, t21};
	(void)a9;
	clj_value t22 = clj_c_invoke(t17, a9, 2);
	clj_release(t21);
	if (o18) clj_release(t18);
	if (o17) clj_release(t17);
	if (t22 == CLJ_THROWN) {
	if (o16) clj_release(t16);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a8[1] = {t22};
	(void)a8;
	clj_value t23 = clj_c_invoke(t16, a8, 1);
	clj_release(t22);
	if (o16) clj_release(t16);
	if (t23 == CLJ_THROWN) {
	clj_release(t15);
	if (o12) clj_release(t12);
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t24 = l3;
	clj_value a6[3] = {t15, t23, t24};
	(void)a6;
	clj_value t25 = clj_c_invoke(t12, a6, 3);
	(void)t24;
	clj_release(t23);
	clj_release(t15);
	if (o12) clj_release(t12);
	if (t25 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a5[1] = {t25};
	(void)a5;
	clj_value t26;
	if (CLJC_GUARD(V[12], B[0])) {
	t26 = clj_seq(t25);
	} else {
	t26 = clj_c_intrinsic_fallback(V[12], a5, 1);
	}
	clj_release(t25);
	if (t26 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t27 = K[296];
	clj_value t28 = K[5];
	clj_value a4[6] = {t9, t10, t11, t26, t27, t28};
	(void)a4;
	clj_value t29 = clj_c_invoke(t8, a4, 6);
	(void)t28;
	(void)t27;
	clj_release(t26);
	(void)t11;
	if (o10) clj_release(t10);
	(void)t9;
	if (o8) clj_release(t8);
	if (t29 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a3[1] = {t29};
	(void)a3;
	clj_value t30 = clj_c_invoke(t7, a3, 1);
	clj_release(t29);
	if (o7) clj_release(t7);
	if (t30 == CLJ_THROWN) {
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
#line 242 "<embedded>/clojure/test.clj"
	bool o31;
	clj_value t31 = clj_c_var_borrow(V[8], &o31);
	if (t31 == CLJ_THROWN) {
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o32;
	clj_value t32 = clj_c_var_borrow(V[13], &o32);
	if (t32 == CLJ_THROWN) {
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o33;
	clj_value t33 = clj_c_var_borrow(V[8], &o33);
	if (t33 == CLJ_THROWN) {
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t34 = K[297];
	clj_value a15[1] = {t34};
	(void)a15;
	clj_value t35 = clj_c_invoke(t33, a15, 1);
	(void)t34;
	if (o33) clj_release(t33);
	if (t35 == CLJ_THROWN) {
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o36;
	clj_value t36 = clj_c_var_borrow(V[8], &o36);
	if (t36 == CLJ_THROWN) {
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o37;
	clj_value t37 = clj_c_var_borrow(V[17], &o37);
	if (t37 == CLJ_THROWN) {
	if (o36) clj_release(t36);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o38;
	clj_value t38 = clj_c_var_borrow(V[58], &o38);
	if (t38 == CLJ_THROWN) {
	if (o37) clj_release(t37);
	if (o36) clj_release(t36);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o39;
	clj_value t39 = clj_c_var_borrow(V[13], &o39);
	if (t39 == CLJ_THROWN) {
	if (o38) clj_release(t38);
	if (o37) clj_release(t37);
	if (o36) clj_release(t36);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	const clj_value *a19 = NULL;
	clj_value t40;
#ifdef CLJC_DIRECT_clojure_core_concat_a0
	if (!CLJC_FN_clojure_core_concat_a0) CLJC_FN_clojure_core_concat_a0 = clj_compiled_symbol("clojure_core_concat_a0");
	t40 = CLJC_FN_clojure_core_concat_a0 ? CLJC_FN_clojure_core_concat_a0(clj_var_root_relaxed(V[13]), NULL, a19, 0) : clj_c_invoke(t39, a19, 0);
#else
	t40 = clj_c_invoke(t39, a19, 0);
#endif
	if (o39) clj_release(t39);
	if (t40 == CLJ_THROWN) {
	if (o38) clj_release(t38);
	if (o37) clj_release(t37);
	if (o36) clj_release(t36);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a18[1] = {t40};
	(void)a18;
	clj_value t41;
	if (CLJC_GUARD(V[12], B[0])) {
	t41 = clj_seq(t40);
	} else {
	t41 = clj_c_intrinsic_fallback(V[12], a18, 1);
	}
	clj_release(t40);
	if (t41 == CLJ_THROWN) {
	if (o38) clj_release(t38);
	if (o37) clj_release(t37);
	if (o36) clj_release(t36);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a17[2] = {t38, t41};
	(void)a17;
	clj_value t42 = clj_c_invoke(t37, a17, 2);
	clj_release(t41);
	if (o38) clj_release(t38);
	if (o37) clj_release(t37);
	if (t42 == CLJ_THROWN) {
	if (o36) clj_release(t36);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a16[1] = {t42};
	(void)a16;
	clj_value t43 = clj_c_invoke(t36, a16, 1);
	clj_release(t42);
	if (o36) clj_release(t36);
	if (t43 == CLJ_THROWN) {
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o44;
	clj_value t44 = clj_c_var_borrow(V[8], &o44);
	if (t44 == CLJ_THROWN) {
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o45;
	clj_value t45 = clj_c_var_borrow(V[13], &o45);
	if (t45 == CLJ_THROWN) {
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o46;
	clj_value t46 = clj_c_var_borrow(V[8], &o46);
	if (t46 == CLJ_THROWN) {
	if (o45) clj_release(t45);
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t47 = K[298];
	clj_value a23[1] = {t47};
	(void)a23;
	clj_value t48 = clj_c_invoke(t46, a23, 1);
	(void)t47;
	if (o46) clj_release(t46);
	if (t48 == CLJ_THROWN) {
	if (o45) clj_release(t45);
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o49;
	clj_value t49 = clj_c_var_borrow(V[8], &o49);
	if (t49 == CLJ_THROWN) {
	clj_release(t48);
	if (o45) clj_release(t45);
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o50;
	clj_value t50 = clj_c_var_borrow(V[13], &o50);
	if (t50 == CLJ_THROWN) {
	if (o49) clj_release(t49);
	clj_release(t48);
	if (o45) clj_release(t45);
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o51;
	clj_value t51 = clj_c_var_borrow(V[8], &o51);
	if (t51 == CLJ_THROWN) {
	if (o50) clj_release(t50);
	if (o49) clj_release(t49);
	clj_release(t48);
	if (o45) clj_release(t45);
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t52 = K[299];
	clj_value a27[1] = {t52};
	(void)a27;
	clj_value t53 = clj_c_invoke(t51, a27, 1);
	(void)t52;
	if (o51) clj_release(t51);
	if (t53 == CLJ_THROWN) {
	if (o50) clj_release(t50);
	if (o49) clj_release(t49);
	clj_release(t48);
	if (o45) clj_release(t45);
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o54;
	clj_value t54 = clj_c_var_borrow(V[8], &o54);
	if (t54 == CLJ_THROWN) {
	clj_release(t53);
	if (o50) clj_release(t50);
	if (o49) clj_release(t49);
	clj_release(t48);
	if (o45) clj_release(t45);
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t55 = l2;
	clj_value a28[1] = {t55};
	(void)a28;
	clj_value t56 = clj_c_invoke(t54, a28, 1);
	(void)t55;
	if (o54) clj_release(t54);
	if (t56 == CLJ_THROWN) {
	clj_release(t53);
	if (o50) clj_release(t50);
	if (o49) clj_release(t49);
	clj_release(t48);
	if (o45) clj_release(t45);
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a26[2] = {t53, t56};
	(void)a26;
	clj_value t57;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t57 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a26, 2) : clj_c_invoke(t50, a26, 2);
#else
	t57 = clj_c_invoke(t50, a26, 2);
#endif
	clj_release(t56);
	clj_release(t53);
	if (o50) clj_release(t50);
	if (t57 == CLJ_THROWN) {
	if (o49) clj_release(t49);
	clj_release(t48);
	if (o45) clj_release(t45);
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a25[1] = {t57};
	(void)a25;
	clj_value t58;
	if (CLJC_GUARD(V[12], B[0])) {
	t58 = clj_seq(t57);
	} else {
	t58 = clj_c_intrinsic_fallback(V[12], a25, 1);
	}
	clj_release(t57);
	if (t58 == CLJ_THROWN) {
	if (o49) clj_release(t49);
	clj_release(t48);
	if (o45) clj_release(t45);
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a24[1] = {t58};
	(void)a24;
	clj_value t59 = clj_c_invoke(t49, a24, 1);
	clj_release(t58);
	if (o49) clj_release(t49);
	if (t59 == CLJ_THROWN) {
	clj_release(t48);
	if (o45) clj_release(t45);
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a22[2] = {t48, t59};
	(void)a22;
	clj_value t60;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t60 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a22, 2) : clj_c_invoke(t45, a22, 2);
#else
	t60 = clj_c_invoke(t45, a22, 2);
#endif
	clj_release(t59);
	clj_release(t48);
	if (o45) clj_release(t45);
	if (t60 == CLJ_THROWN) {
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a21[1] = {t60};
	(void)a21;
	clj_value t61;
	if (CLJC_GUARD(V[12], B[0])) {
	t61 = clj_seq(t60);
	} else {
	t61 = clj_c_intrinsic_fallback(V[12], a21, 1);
	}
	clj_release(t60);
	if (t61 == CLJ_THROWN) {
	if (o44) clj_release(t44);
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a20[1] = {t61};
	(void)a20;
	clj_value t62 = clj_c_invoke(t44, a20, 1);
	clj_release(t61);
	if (o44) clj_release(t44);
	if (t62 == CLJ_THROWN) {
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a14[3] = {t35, t43, t62};
	(void)a14;
	clj_value t63 = clj_c_invoke(t32, a14, 3);
	clj_release(t62);
	clj_release(t43);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (t63 == CLJ_THROWN) {
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a13[1] = {t63};
	(void)a13;
	clj_value t64;
	if (CLJC_GUARD(V[12], B[0])) {
	t64 = clj_seq(t63);
	} else {
	t64 = clj_c_intrinsic_fallback(V[12], a13, 1);
	}
	clj_release(t63);
	if (t64 == CLJ_THROWN) {
	if (o31) clj_release(t31);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a12[1] = {t64};
	(void)a12;
	clj_value t65 = clj_c_invoke(t31, a12, 1);
	clj_release(t64);
	if (o31) clj_release(t31);
	if (t65 == CLJ_THROWN) {
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a1[3] = {t6, t30, t65};
	(void)a1;
	clj_value t66 = clj_c_invoke(t3, a1, 3);
	clj_release(t65);
	clj_release(t30);
	clj_release(t6);
	if (o3) clj_release(t3);
	if (t66 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t66};
	(void)a0;
	clj_value t67;
	if (CLJC_GUARD(V[12], B[0])) {
	t67 = clj_seq(t66);
	} else {
	t67 = clj_c_intrinsic_fallback(V[12], a0, 1);
	}
	clj_release(t66);
	if (t67 == CLJ_THROWN) {
	goto L2;
	}
	t2 = t67;
	} else {
	t2 = CLJ_NIL;
	}
	clj_c_leave(&S[31], &cc);
	clj_release(l3);
	return t2;
L2: ;
	clj_c_leave(&S[31], &cc);
L1: ;
	clj_release(l3);
	return CLJ_THROWN;
}

static clj_value clojure_test_deftest_(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	default: break;
	}
	if (n >= 3) return clojure_test_deftest__v3(clj_from_ptr((void *)f), f->env, args, n);
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_56(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 239 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[292], clojure_test_deftest_, NULL, 0, 0x0, 3, CLJ_ARITY_ANY);
	clj_var_bind_root(V[81], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[300]);
	clj_value t2 = clj_c_def(V[81], t1, true, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_set_test_v3(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_value l2 = args[2];
	(void)l2;
	clj_value l3 = CLJ_NIL;
	l3 = nargs > 3 ? clj_list_from_array(args + 3, nargs - 3) : CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[32], &cc)) goto L1;
#line 249 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[3], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool t1 = clj_truthy(t0);
	if (o0) clj_release(t0);
	clj_value t2;
	if (t1) {
	bool o3;
	clj_value t3 = clj_c_var_borrow(V[13], &o3);
	if (t3 == CLJ_THROWN) {
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[8], &o4);
	if (t4 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t5 = K[303];
	clj_value a2[1] = {t5};
	(void)a2;
	clj_value t6 = clj_c_invoke(t4, a2, 1);
	(void)t5;
	if (o4) clj_release(t4);
	if (t6 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o7;
	clj_value t7 = clj_c_var_borrow(V[8], &o7);
	if (t7 == CLJ_THROWN) {
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[13], &o8);
	if (t8 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o9;
	clj_value t9 = clj_c_var_borrow(V[8], &o9);
	if (t9 == CLJ_THROWN) {
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t10 = K[304];
	clj_value a6[1] = {t10};
	(void)a6;
	clj_value t11 = clj_c_invoke(t9, a6, 1);
	(void)t10;
	if (o9) clj_release(t9);
	if (t11 == CLJ_THROWN) {
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o12;
	clj_value t12 = clj_c_var_borrow(V[8], &o12);
	if (t12 == CLJ_THROWN) {
	clj_release(t11);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t13 = l2;
	clj_value a7[1] = {t13};
	(void)a7;
	clj_value t14 = clj_c_invoke(t12, a7, 1);
	(void)t13;
	if (o12) clj_release(t12);
	if (t14 == CLJ_THROWN) {
	clj_release(t11);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a5[2] = {t11, t14};
	(void)a5;
	clj_value t15;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t15 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a5, 2) : clj_c_invoke(t8, a5, 2);
#else
	t15 = clj_c_invoke(t8, a5, 2);
#endif
	clj_release(t14);
	clj_release(t11);
	if (o8) clj_release(t8);
	if (t15 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a4[1] = {t15};
	(void)a4;
	clj_value t16;
	if (CLJC_GUARD(V[12], B[0])) {
	t16 = clj_seq(t15);
	} else {
	t16 = clj_c_intrinsic_fallback(V[12], a4, 1);
	}
	clj_release(t15);
	if (t16 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a3[1] = {t16};
	(void)a3;
	clj_value t17 = clj_c_invoke(t7, a3, 1);
	clj_release(t16);
	if (o7) clj_release(t7);
	if (t17 == CLJ_THROWN) {
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o18;
	clj_value t18 = clj_c_var_borrow(V[8], &o18);
	if (t18 == CLJ_THROWN) {
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t19 = K[305];
	clj_value a8[1] = {t19};
	(void)a8;
	clj_value t20 = clj_c_invoke(t18, a8, 1);
	(void)t19;
	if (o18) clj_release(t18);
	if (t20 == CLJ_THROWN) {
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o21;
	clj_value t21 = clj_c_var_borrow(V[8], &o21);
	if (t21 == CLJ_THROWN) {
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t22 = K[306];
	clj_value a9[1] = {t22};
	(void)a9;
	clj_value t23 = clj_c_invoke(t21, a9, 1);
	(void)t22;
	if (o21) clj_release(t21);
	if (t23 == CLJ_THROWN) {
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o24;
	clj_value t24 = clj_c_var_borrow(V[8], &o24);
	if (t24 == CLJ_THROWN) {
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o25;
	clj_value t25 = clj_c_var_borrow(V[13], &o25);
	if (t25 == CLJ_THROWN) {
	if (o24) clj_release(t24);
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o26;
	clj_value t26 = clj_c_var_borrow(V[8], &o26);
	if (t26 == CLJ_THROWN) {
	if (o25) clj_release(t25);
	if (o24) clj_release(t24);
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t27 = K[307];
	clj_value a13[1] = {t27};
	(void)a13;
	clj_value t28 = clj_c_invoke(t26, a13, 1);
	(void)t27;
	if (o26) clj_release(t26);
	if (t28 == CLJ_THROWN) {
	if (o25) clj_release(t25);
	if (o24) clj_release(t24);
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o29;
	clj_value t29 = clj_c_var_borrow(V[8], &o29);
	if (t29 == CLJ_THROWN) {
	clj_release(t28);
	if (o25) clj_release(t25);
	if (o24) clj_release(t24);
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o30;
	clj_value t30 = clj_c_var_borrow(V[17], &o30);
	if (t30 == CLJ_THROWN) {
	if (o29) clj_release(t29);
	clj_release(t28);
	if (o25) clj_release(t25);
	if (o24) clj_release(t24);
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o31;
	clj_value t31 = clj_c_var_borrow(V[58], &o31);
	if (t31 == CLJ_THROWN) {
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	if (o25) clj_release(t25);
	if (o24) clj_release(t24);
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	bool o32;
	clj_value t32 = clj_c_var_borrow(V[13], &o32);
	if (t32 == CLJ_THROWN) {
	if (o31) clj_release(t31);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	if (o25) clj_release(t25);
	if (o24) clj_release(t24);
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	const clj_value *a17 = NULL;
	clj_value t33;
#ifdef CLJC_DIRECT_clojure_core_concat_a0
	if (!CLJC_FN_clojure_core_concat_a0) CLJC_FN_clojure_core_concat_a0 = clj_compiled_symbol("clojure_core_concat_a0");
	t33 = CLJC_FN_clojure_core_concat_a0 ? CLJC_FN_clojure_core_concat_a0(clj_var_root_relaxed(V[13]), NULL, a17, 0) : clj_c_invoke(t32, a17, 0);
#else
	t33 = clj_c_invoke(t32, a17, 0);
#endif
	if (o32) clj_release(t32);
	if (t33 == CLJ_THROWN) {
	if (o31) clj_release(t31);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	if (o25) clj_release(t25);
	if (o24) clj_release(t24);
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a16[1] = {t33};
	(void)a16;
	clj_value t34;
	if (CLJC_GUARD(V[12], B[0])) {
	t34 = clj_seq(t33);
	} else {
	t34 = clj_c_intrinsic_fallback(V[12], a16, 1);
	}
	clj_release(t33);
	if (t34 == CLJ_THROWN) {
	if (o31) clj_release(t31);
	if (o30) clj_release(t30);
	if (o29) clj_release(t29);
	clj_release(t28);
	if (o25) clj_release(t25);
	if (o24) clj_release(t24);
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a15[2] = {t31, t34};
	(void)a15;
	clj_value t35 = clj_c_invoke(t30, a15, 2);
	clj_release(t34);
	if (o31) clj_release(t31);
	if (o30) clj_release(t30);
	if (t35 == CLJ_THROWN) {
	if (o29) clj_release(t29);
	clj_release(t28);
	if (o25) clj_release(t25);
	if (o24) clj_release(t24);
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a14[1] = {t35};
	(void)a14;
	clj_value t36 = clj_c_invoke(t29, a14, 1);
	clj_release(t35);
	if (o29) clj_release(t29);
	if (t36 == CLJ_THROWN) {
	clj_release(t28);
	if (o25) clj_release(t25);
	if (o24) clj_release(t24);
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value t37 = l3;
	clj_value a12[3] = {t28, t36, t37};
	(void)a12;
	clj_value t38 = clj_c_invoke(t25, a12, 3);
	(void)t37;
	clj_release(t36);
	clj_release(t28);
	if (o25) clj_release(t25);
	if (t38 == CLJ_THROWN) {
	if (o24) clj_release(t24);
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a11[1] = {t38};
	(void)a11;
	clj_value t39;
	if (CLJC_GUARD(V[12], B[0])) {
	t39 = clj_seq(t38);
	} else {
	t39 = clj_c_intrinsic_fallback(V[12], a11, 1);
	}
	clj_release(t38);
	if (t39 == CLJ_THROWN) {
	if (o24) clj_release(t24);
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a10[1] = {t39};
	(void)a10;
	clj_value t40 = clj_c_invoke(t24, a10, 1);
	clj_release(t39);
	if (o24) clj_release(t24);
	if (t40 == CLJ_THROWN) {
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	goto L2;
	}
	clj_value a1[5] = {t6, t17, t20, t23, t40};
	(void)a1;
	clj_value t41 = clj_c_invoke(t3, a1, 5);
	clj_release(t40);
	clj_release(t23);
	clj_release(t20);
	clj_release(t17);
	clj_release(t6);
	if (o3) clj_release(t3);
	if (t41 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t41};
	(void)a0;
	clj_value t42;
	if (CLJC_GUARD(V[12], B[0])) {
	t42 = clj_seq(t41);
	} else {
	t42 = clj_c_intrinsic_fallback(V[12], a0, 1);
	}
	clj_release(t41);
	if (t42 == CLJ_THROWN) {
	goto L2;
	}
	t2 = t42;
	} else {
	t2 = CLJ_NIL;
	}
	clj_c_leave(&S[32], &cc);
	clj_release(l3);
	return t2;
L2: ;
	clj_c_leave(&S[32], &cc);
L1: ;
	clj_release(l3);
	return CLJ_THROWN;
}

static clj_value clojure_test_set_test(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	default: break;
	}
	if (n >= 3) return clojure_test_set_test_v3(clj_from_ptr((void *)f), f->env, args, n);
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_57(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 246 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[302], clojure_test_set_test, NULL, 0, 0x0, 3, CLJ_ARITY_ANY);
	clj_var_bind_root(V[82], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[308]);
	clj_value t2 = clj_c_def(V[82], t1, true, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_58(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 254 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[36], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_value t1 = K[310];
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2 = clj_c_invoke(t0, a0, 1);
	(void)t1;
	if (o0) clj_release(t0);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_var_bind_root(V[83], t2);
	clj_release(t2);
	clj_value t3 = clj_retain(K[311]);
	clj_value t4 = clj_c_def(V[83], t3, false, false);
	clj_release(t3);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form59__0_v1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = CLJ_NIL;
	l1 = nargs > 1 ? clj_list_from_array(args + 1, nargs - 1) : CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[33], &cc)) goto L1;
#line 259 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_retain(l0);
	clj_c_leave(&S[33], &cc);
	clj_release(l1);
	return t0;
L1: ;
	clj_release(l1);
	return CLJ_THROWN;
}

static clj_value clojure_test_form59__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	default: break;
	}
	if (n >= 1) return clojure_test_form59__0_v1(clj_from_ptr((void *)f), f->env, args, n);
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_59(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_value l0 = CLJ_NIL;
	clj_eval_top_enter();
#line 256 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_retain(K[313]);
	clj_value t1 = clj_c_def(V[84], t0, false, false);
	clj_release(t0);
	if (t1 == CLJ_THROWN) {
	goto L1;
	}
	clj_c_rebind(&l0, t1);
	bool o2;
	clj_value t2 = clj_c_var_borrow(V[33], &o2);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_value t3 = l0;
	clj_value a0[1] = {t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t2, a0, 1);
	(void)t3;
	if (o2) clj_release(t2);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	bool t5 = clj_truthy(t4);
	clj_release(t4);
	clj_value t6;
	if (t5) {
	clj_value t7 = clj_retain(K[3]);
	t6 = t7;
	} else {
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[34], &o8);
	if (t8 == CLJ_THROWN) {
	goto L1;
	}
	clj_value t9 = K[312];
#line 259 "<embedded>/clojure/test.clj"
	clj_value t10 = clj_c_closure(CLJ_NIL, clojure_test_form59__0, NULL, 0, 0x0, 1, CLJ_ARITY_ANY);
#line 256 "<embedded>/clojure/test.clj"
	clj_value t11 = K[314];
	clj_value t12 = V[35];
	bool o13;
	clj_value t13 = clj_c_var_borrow(V[36], &o13);
	if (t13 == CLJ_THROWN) {
	clj_release(t10);
	if (o8) clj_release(t8);
	goto L1;
	}
	clj_value t14 = K[315];
	clj_value a2[1] = {t14};
	(void)a2;
	clj_value t15 = clj_c_invoke(t13, a2, 1);
	(void)t14;
	if (o13) clj_release(t13);
	if (t15 == CLJ_THROWN) {
	clj_release(t10);
	if (o8) clj_release(t8);
	goto L1;
	}
	bool o16;
	clj_value t16 = clj_c_var_borrow(V[36], &o16);
	if (t16 == CLJ_THROWN) {
	clj_release(t15);
	clj_release(t10);
	if (o8) clj_release(t8);
	goto L1;
	}
	clj_value t17 = K[315];
	clj_value a3[1] = {t17};
	(void)a3;
	clj_value t18 = clj_c_invoke(t16, a3, 1);
	(void)t17;
	if (o16) clj_release(t16);
	if (t18 == CLJ_THROWN) {
	clj_release(t15);
	clj_release(t10);
	if (o8) clj_release(t8);
	goto L1;
	}
	bool o19;
	clj_value t19 = clj_c_var_borrow(V[36], &o19);
	if (t19 == CLJ_THROWN) {
	clj_release(t18);
	clj_release(t15);
	clj_release(t10);
	if (o8) clj_release(t8);
	goto L1;
	}
	clj_value t20 = K[316];
	clj_value a4[1] = {t20};
	(void)a4;
	clj_value t21 = clj_c_invoke(t19, a4, 1);
	(void)t20;
	if (o19) clj_release(t19);
	if (t21 == CLJ_THROWN) {
	clj_release(t18);
	clj_release(t15);
	clj_release(t10);
	if (o8) clj_release(t8);
	goto L1;
	}
	clj_value a1[7] = {t9, t10, t11, t12, t15, t18, t21};
	(void)a1;
	clj_value t22;
#ifdef CLJC_DIRECT_clojure_core___GT_MultiFn_a7
	if (!CLJC_FN_clojure_core___GT_MultiFn_a7) CLJC_FN_clojure_core___GT_MultiFn_a7 = clj_compiled_symbol("clojure_core___GT_MultiFn_a7");
	t22 = CLJC_FN_clojure_core___GT_MultiFn_a7 ? CLJC_FN_clojure_core___GT_MultiFn_a7(clj_var_root_relaxed(V[34]), NULL, a1, 7) : clj_c_invoke(t8, a1, 7);
#else
	t22 = clj_c_invoke(t8, a1, 7);
#endif
	clj_release(t21);
	clj_release(t18);
	clj_release(t15);
	(void)t12;
	(void)t11;
	clj_release(t10);
	(void)t9;
	if (o8) clj_release(t8);
	if (t22 == CLJ_THROWN) {
	goto L1;
	}
	clj_var_bind_root(V[84], t22);
	clj_release(t22);
	clj_value t23 = clj_retain(K[317]);
	clj_value t24 = clj_c_def(V[84], t23, false, false);
	clj_release(t23);
	if (t24 == CLJ_THROWN) {
	goto L1;
	}
	t6 = t24;
	}
	clj_release(l0);
	clj_eval_top_leave();
	return t6;
L1: ;
	clj_release(l0);
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form60__0_v1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = CLJ_NIL;
	l1 = nargs > 1 ? clj_list_from_array(args + 1, nargs - 1) : CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[34], &cc)) goto L1;
#line 262 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[28], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[83], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o2;
	clj_value t2 = clj_c_var_borrow(V[85], &o2);
	if (t2 == CLJ_THROWN) {
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o3;
	clj_value t3 = clj_c_var_borrow(V[45], &o3);
	if (t3 == CLJ_THROWN) {
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[86], &o4);
	if (t4 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a2[1] = {t4};
	(void)a2;
	clj_value t5 = clj_c_invoke(t3, a2, 1);
	if (o4) clj_release(t4);
	if (o3) clj_release(t3);
	if (t5 == CLJ_THROWN) {
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t6 = K[319];
	clj_value a1[2] = {t5, t6};
	(void)a1;
	clj_value t7 = clj_vector_from_array(a1, 2);
	(void)t6;
	clj_release(t5);
	if (t7 == CLJ_THROWN) {
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t8 = l1;
	clj_value a0[4] = {t1, t2, t7, t8};
	(void)a0;
	clj_value t9 = clj_c_invoke(t0, a0, 4);
	(void)t8;
	clj_release(t7);
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t9 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[34], &cc);
	clj_release(l1);
	return t9;
L2: ;
	clj_c_leave(&S[34], &cc);
L1: ;
	clj_release(l1);
	return CLJ_THROWN;
}

static clj_value clojure_test_form60__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	default: break;
	}
	if (n >= 1) return clojure_test_form60__0_v1(clj_from_ptr((void *)f), f->env, args, n);
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_60(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 261 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[84], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[319];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form60__0, NULL, 0, 0x0, 1, CLJ_ARITY_ANY);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_61(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 261 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[84]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_form62__0_v1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = CLJ_NIL;
	l1 = nargs > 1 ? clj_list_from_array(args + 1, nargs - 1) : CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[35], &cc)) goto L1;
#line 265 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[28], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[83], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o2;
	clj_value t2 = clj_c_var_borrow(V[85], &o2);
	if (t2 == CLJ_THROWN) {
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o3;
	clj_value t3 = clj_c_var_borrow(V[45], &o3);
	if (t3 == CLJ_THROWN) {
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[86], &o4);
	if (t4 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a2[1] = {t4};
	(void)a2;
	clj_value t5 = clj_c_invoke(t3, a2, 1);
	if (o4) clj_release(t4);
	if (o3) clj_release(t3);
	if (t5 == CLJ_THROWN) {
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t6 = K[321];
	clj_value a1[2] = {t5, t6};
	(void)a1;
	clj_value t7 = clj_vector_from_array(a1, 2);
	(void)t6;
	clj_release(t5);
	if (t7 == CLJ_THROWN) {
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t8 = l1;
	clj_value a0[4] = {t1, t2, t7, t8};
	(void)a0;
	clj_value t9 = clj_c_invoke(t0, a0, 4);
	(void)t8;
	clj_release(t7);
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t9 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[35], &cc);
	clj_release(l1);
	return t9;
L2: ;
	clj_c_leave(&S[35], &cc);
L1: ;
	clj_release(l1);
	return CLJ_THROWN;
}

static clj_value clojure_test_form62__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	default: break;
	}
	if (n >= 1) return clojure_test_form62__0_v1(clj_from_ptr((void *)f), f->env, args, n);
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_62(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 264 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[41], &o0);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[84], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L1;
	}
	clj_value t2 = K[321];
	clj_value t3 = clj_c_closure(CLJ_NIL, clojure_test_form62__0, NULL, 0, 0x0, 1, CLJ_ARITY_ANY);
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t4;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value top_63(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 264 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_var_deref(V[84]);
	if (t0 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t0;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_default_fixture_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_ccall cc;
	if (!clj_c_enter(&S[36], &cc)) goto L1;
#line 270 "<embedded>/clojure/test.clj"
	clj_value t0 = l0;
	const clj_value *a0 = NULL;
	clj_value t1 = clj_c_invoke(t0, a0, 0);
	(void)t0;
	if (t1 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[36], &cc);
	return t1;
L2: ;
	clj_c_leave(&S[36], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_default_fixture(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_default_fixture_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_64(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 267 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[323], clojure_test_default_fixture, NULL, 0, 0x2, 1, 1);
	clj_var_bind_root(V[87], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[324]);
	clj_value t2 = clj_c_def(V[87], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_compose_fixtures__0__1_a0(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_ccall cc;
	if (!clj_c_enter(&S[39], &cc)) goto L1;
#line 275 "<embedded>/clojure/test.clj"
	clj_value t0 = fr.captured[0];
	clj_value t1 = fr.captured[1];
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2 = clj_c_invoke(t0, a0, 1);
	(void)t1;
	(void)t0;
	if (t2 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[39], &cc);
	return t2;
L2: ;
	clj_c_leave(&S[39], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_compose_fixtures__0__1(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 0: return clojure_test_compose_fixtures__0__1_a0(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value clojure_test_compose_fixtures__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_value s[1];
	clj_cframe fr = {s, captured, 0, NULL};
	(void)fr;
	s[0] = args[0];
	clj_ccall cc;
	if (!clj_c_enter(&S[38], &cc)) goto L1;
#line 275 "<embedded>/clojure/test.clj"
	clj_value t0 = fr.captured[0];
	clj_value c1[2] = {fr.captured[1], fr.slots[0]};
	clj_value t1 = clj_c_closure(CLJ_NIL, clojure_test_compose_fixtures__0__1, c1, 2, 0x1, 0, 0);
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2 = clj_c_invoke(t0, a0, 1);
	clj_release(t1);
	(void)t0;
	if (t2 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[38], &cc);
	clj_c_release_slots(&fr, 1);
	return t2;
L2: ;
	clj_c_leave(&S[38], &cc);
L1: ;
	clj_c_release_slots(&fr, 1);
	return CLJ_THROWN;
}

static clj_value clojure_test_compose_fixtures__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_compose_fixtures__0_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

clj_value clojure_test_compose_fixtures_a2(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_value s[2];
	clj_cframe fr = {s, captured, 0, NULL};
	(void)fr;
	s[0] = args[0];
	s[1] = args[1];
	clj_ccall cc;
	if (!clj_c_enter(&S[37], &cc)) goto L1;
#line 275 "<embedded>/clojure/test.clj"
	clj_value c0[2] = {fr.slots[0], fr.slots[1]};
	clj_value t0 = clj_c_closure(CLJ_NIL, clojure_test_compose_fixtures__0, c0, 2, 0x2, 1, 1);
	clj_c_leave(&S[37], &cc);
	clj_c_release_slots(&fr, 2);
	return t0;
L1: ;
	clj_c_release_slots(&fr, 2);
	return CLJ_THROWN;
}

static clj_value clojure_test_compose_fixtures(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 2: return clojure_test_compose_fixtures_a2(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_65(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 272 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[326], clojure_test_compose_fixtures, NULL, 0, 0x4, 2, 2);
	clj_var_bind_root(V[88], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[327]);
	clj_value t2 = clj_c_def(V[88], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_join_fixtures_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_ccall cc;
	if (!clj_c_enter(&S[40], &cc)) goto L1;
#line 280 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[90], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[88], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o2;
	clj_value t2 = clj_c_var_borrow(V[87], &o2);
	if (t2 == CLJ_THROWN) {
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t3 = l0;
	clj_value a0[3] = {t1, t2, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 3);
	(void)t3;
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[40], &cc);
	return t4;
L2: ;
	clj_c_leave(&S[40], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_join_fixtures(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_join_fixtures_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_66(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 277 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[329], clojure_test_join_fixtures, NULL, 0, 0x2, 1, 1);
	clj_var_bind_root(V[89], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[330]);
	clj_value t2 = clj_c_def(V[89], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_ns_key_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_ccall cc;
	if (!clj_c_enter(&S[41], &cc)) goto L1;
#line 282 "<embedded>/clojure/test.clj"
	clj_value t0 = l0;
	clj_value a0[1] = {t0};
	(void)a0;
	clj_value t1;
	if (CLJC_GUARD(V[51], B[7])) {
	t1 = clj_symbol_p(t0);
	} else {
	t1 = clj_c_intrinsic_fallback(V[51], a0, 1);
	}
	(void)t0;
	if (t1 == CLJ_THROWN) {
	goto L2;
	}
	bool t2 = clj_truthy(t1);
	clj_release(t1);
	clj_value t3;
	if (t2) {
	clj_value t4 = clj_retain(l0);
	t3 = t4;
	} else {
	bool o5;
	clj_value t5 = clj_c_var_borrow(V[45], &o5);
	if (t5 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t6 = l0;
	clj_value a1[1] = {t6};
	(void)a1;
	clj_value t7 = clj_c_invoke(t5, a1, 1);
	(void)t6;
	if (o5) clj_release(t5);
	if (t7 == CLJ_THROWN) {
	goto L2;
	}
	t3 = t7;
	}
	clj_c_leave(&S[41], &cc);
	return t3;
L2: ;
	clj_c_leave(&S[41], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_ns_key(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_ns_key_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_67(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 282 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[332], clojure_test_ns_key, NULL, 0, 0x2, 1, 1);
	clj_var_bind_root(V[91], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[333]);
	clj_value t2 = clj_c_def(V[91], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_test_var_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = CLJ_NIL;
	clj_value l2 = CLJ_NIL;
	clj_value l3 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[42], &cc)) goto L1;
#line 289 "<embedded>/clojure/test.clj"
	clj_value t0 = K[336];
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[24], &o1);
	if (t1 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t2 = l0;
	clj_value a1[1] = {t2};
	(void)a1;
	clj_value t3 = clj_c_invoke(t1, a1, 1);
	(void)t2;
	if (o1) clj_release(t1);
	if (t3 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 1);
	clj_release(t3);
	(void)t0;
	if (t4 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l1, t4);
	clj_value t5 = l1;
	bool t6 = clj_truthy(t5);
	(void)t5;
	clj_value t7;
	if (t6) {
	clj_value t8 = l1;
	l1 = CLJ_NIL;
	clj_c_rebind(&l2, t8);
#line 290 "<embedded>/clojure/test.clj"
	bool o9;
	clj_value t9 = clj_c_var_borrow(V[93], &o9);
	if (t9 == CLJ_THROWN) {
	goto L2;
	}
	bool o10;
	clj_value t10 = clj_c_var_borrow(V[18], &o10);
	if (t10 == CLJ_THROWN) {
	if (o9) clj_release(t9);
	goto L2;
	}
	clj_value t11 = V[7];
	bool o12;
	clj_value t12 = clj_c_var_borrow(V[7], &o12);
	if (t12 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	goto L2;
	}
	clj_value t13 = l0;
	clj_value a4[2] = {t12, t13};
	(void)a4;
	clj_value t14;
	if (CLJC_GUARD(V[94], B[18])) {
	if (o12) {
	t14 = clj_conj(t12, t13);
	o12 = false;
	} else {
	t14 = clj_conj2(t12, t13);
	}
	} else {
	t14 = clj_c_intrinsic_fallback(V[94], a4, 2);
	}
	(void)t13;
	if (o12) clj_release(t12);
	if (t14 == CLJ_THROWN) {
	if (o10) clj_release(t10);
	if (o9) clj_release(t9);
	goto L2;
	}
	clj_value a3[2] = {t11, t14};
	(void)a3;
	clj_value t15 = clj_c_invoke(t10, a3, 2);
	clj_release(t14);
	(void)t11;
	if (o10) clj_release(t10);
	if (t15 == CLJ_THROWN) {
	if (o9) clj_release(t9);
	goto L2;
	}
	clj_value a2[1] = {t15};
	(void)a2;
	clj_value t16 = clj_c_invoke(t9, a2, 1);
	clj_release(t15);
	if (o9) clj_release(t9);
	if (t16 == CLJ_THROWN) {
	goto L2;
	}
	clj_release(t16);
	clj_value t17;
#line 291 "<embedded>/clojure/test.clj"
	bool o18;
	clj_value t18 = clj_c_var_borrow(V[46], &o18);
	if (t18 == CLJ_THROWN) {
	goto L4;
	}
	clj_value t19 = K[337];
	clj_value t20 = l0;
	clj_value t21 = K[338];
	clj_value t22 = K[339];
	clj_value a7[4] = {t19, t20, t21, t22};
	(void)a7;
	clj_value t23 = clj_c_map_literal(a7, 4);
	(void)t22;
	(void)t21;
	(void)t20;
	(void)t19;
	if (t23 == CLJ_THROWN) {
	if (o18) clj_release(t18);
	goto L4;
	}
	clj_value a6[1] = {t23};
	(void)a6;
	clj_value t24;
#ifdef CLJC_DIRECT_clojure_test_do_report_a1
	if (!CLJC_FN_clojure_test_do_report_a1) CLJC_FN_clojure_test_do_report_a1 = clj_compiled_symbol("clojure_test_do_report_a1");
	t24 = CLJC_FN_clojure_test_do_report_a1 ? CLJC_FN_clojure_test_do_report_a1(clj_var_root_relaxed(V[46]), NULL, a6, 1) : clj_c_invoke(t18, a6, 1);
#else
	t24 = clj_c_invoke(t18, a6, 1);
#endif
	clj_release(t23);
	if (o18) clj_release(t18);
	if (t24 == CLJ_THROWN) {
	goto L4;
	}
	clj_release(t24);
#line 292 "<embedded>/clojure/test.clj"
	bool o25;
	clj_value t25 = clj_c_var_borrow(V[27], &o25);
	if (t25 == CLJ_THROWN) {
	goto L4;
	}
	clj_value t26 = K[336];
	clj_value a8[1] = {t26};
	(void)a8;
	clj_value t27;
#ifdef CLJC_DIRECT_clojure_test_inc_report_counter_a1
	if (!CLJC_FN_clojure_test_inc_report_counter_a1) CLJC_FN_clojure_test_inc_report_counter_a1 = clj_compiled_symbol("clojure_test_inc_report_counter_a1");
	t27 = CLJC_FN_clojure_test_inc_report_counter_a1 ? CLJC_FN_clojure_test_inc_report_counter_a1(clj_var_root_relaxed(V[27]), NULL, a8, 1) : clj_c_invoke(t25, a8, 1);
#else
	t27 = clj_c_invoke(t25, a8, 1);
#endif
	(void)t26;
	if (o25) clj_release(t25);
	if (t27 == CLJ_THROWN) {
	goto L4;
	}
	clj_release(t27);
#line 293 "<embedded>/clojure/test.clj"
	clj_value t28;
	clj_value t29 = l2;
	const clj_value *a10 = NULL;
	clj_value t30 = clj_c_invoke(t29, a10, 0);
	(void)t29;
	if (t30 == CLJ_THROWN) {
	goto L6;
	}
	t28 = t30;
	goto L7;
L6: ;
	t28 = CLJ_THROWN;
L7: ;
	if (t28 == CLJ_THROWN) {
	clj_value tr9 = clj_take_pending_trace();
	clj_value ex9 = clj_take_pending();
	bool h9 = false;
	if (true) {
	clj_c_rebind(&l3, ex9);
	h9 = true;
#line 295 "<embedded>/clojure/test.clj"
	bool o31;
	clj_value t31 = clj_c_var_borrow(V[46], &o31);
	if (t31 == CLJ_THROWN) {
	goto L8;
	}
	clj_value t32 = K[340];
	clj_value t33 = K[3];
	clj_value t34 = K[338];
	clj_value t35 = K[341];
	clj_value t36 = K[342];
	clj_value t37 = l3;
	clj_value t38 = K[343];
	clj_value t39 = K[344];
	clj_value a12[8] = {t32, t33, t34, t35, t36, t37, t38, t39};
	(void)a12;
	clj_value t40 = clj_c_map_literal(a12, 8);
	(void)t39;
	(void)t38;
	(void)t37;
	(void)t36;
	(void)t35;
	(void)t34;
	(void)t33;
	(void)t32;
	if (t40 == CLJ_THROWN) {
	if (o31) clj_release(t31);
	goto L8;
	}
	clj_value a11[1] = {t40};
	(void)a11;
	clj_value t41;
#ifdef CLJC_DIRECT_clojure_test_do_report_a1
	if (!CLJC_FN_clojure_test_do_report_a1) CLJC_FN_clojure_test_do_report_a1 = clj_compiled_symbol("clojure_test_do_report_a1");
	t41 = CLJC_FN_clojure_test_do_report_a1 ? CLJC_FN_clojure_test_do_report_a1(clj_var_root_relaxed(V[46]), NULL, a11, 1) : clj_c_invoke(t31, a11, 1);
#else
	t41 = clj_c_invoke(t31, a11, 1);
#endif
	clj_release(t40);
	if (o31) clj_release(t31);
	if (t41 == CLJ_THROWN) {
	goto L8;
	}
	t28 = t41;
	}
	goto L9;
L8: ;
	t28 = CLJ_THROWN;
L9: ;
	if (h9) clj_release(tr9); else clj_throw_traced(ex9, tr9);
	}
	clj_value t42 = t28;
	if (t42 == CLJ_THROWN) {
	goto L4;
	}
	clj_release(t42);
#line 297 "<embedded>/clojure/test.clj"
	bool o43;
	clj_value t43 = clj_c_var_borrow(V[46], &o43);
	if (t43 == CLJ_THROWN) {
	goto L4;
	}
	clj_value t44 = K[337];
	clj_value t45 = l0;
	clj_value t46 = K[338];
	clj_value t47 = K[345];
	clj_value a14[4] = {t44, t45, t46, t47};
	(void)a14;
	clj_value t48 = clj_c_map_literal(a14, 4);
	(void)t47;
	(void)t46;
	(void)t45;
	(void)t44;
	if (t48 == CLJ_THROWN) {
	if (o43) clj_release(t43);
	goto L4;
	}
	clj_value a13[1] = {t48};
	(void)a13;
	clj_value t49;
#ifdef CLJC_DIRECT_clojure_test_do_report_a1
	if (!CLJC_FN_clojure_test_do_report_a1) CLJC_FN_clojure_test_do_report_a1 = clj_compiled_symbol("clojure_test_do_report_a1");
	t49 = CLJC_FN_clojure_test_do_report_a1 ? CLJC_FN_clojure_test_do_report_a1(clj_var_root_relaxed(V[46]), NULL, a13, 1) : clj_c_invoke(t43, a13, 1);
#else
	t49 = clj_c_invoke(t43, a13, 1);
#endif
	clj_release(t48);
	if (o43) clj_release(t43);
	if (t49 == CLJ_THROWN) {
	goto L4;
	}
	t17 = t49;
	goto L5;
L4: ;
	t17 = CLJ_THROWN;
L5: ;
	clj_value pt5 = t17 == CLJ_THROWN ? clj_take_pending_trace() : CLJ_NIL;
	clj_value p5 = t17 == CLJ_THROWN ? clj_take_pending() : CLJ_NIL;
#line 290 "<embedded>/clojure/test.clj"
	bool o50;
	clj_value t50 = clj_c_var_borrow(V[95], &o50);
	if (t50 == CLJ_THROWN) {
	goto L10;
	}
	const clj_value *a15 = NULL;
	clj_value t51 = clj_c_invoke(t50, a15, 0);
	if (o50) clj_release(t50);
	if (t51 == CLJ_THROWN) {
	goto L10;
	}
	clj_release(t51);
	if (t17 == CLJ_THROWN) clj_throw_traced(p5, pt5);
	goto L11;
L10: ;
	clj_release(t17 == CLJ_THROWN ? p5 : t17);
	clj_release(pt5);
	t17 = CLJ_THROWN;
L11: ;
	clj_value t52 = t17;
	if (t52 == CLJ_THROWN) {
	goto L2;
	}
	t7 = t52;
	} else {
	t7 = CLJ_NIL;
	}
	clj_c_leave(&S[42], &cc);
	clj_release(l1);
	clj_release(l2);
	clj_release(l3);
	return t7;
L2: ;
	clj_c_leave(&S[42], &cc);
L1: ;
	clj_release(l1);
	clj_release(l2);
	clj_release(l3);
	return CLJ_THROWN;
}

static clj_value clojure_test_test_var(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_test_var_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_68(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 286 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[335], clojure_test_test_var, NULL, 0, 0x2, 1, 1);
	clj_var_bind_root(V[92], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[346]);
	clj_value t2 = clj_c_def(V[92], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_test_vars__0__1_a0(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_ccall cc;
	if (!clj_c_enter(&S[45], &cc)) goto L1;
#line 311 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[92], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t1 = fr.captured[0];
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2;
#ifdef CLJC_DIRECT_clojure_test_test_var_a1
	if (!CLJC_FN_clojure_test_test_var_a1) CLJC_FN_clojure_test_test_var_a1 = clj_compiled_symbol("clojure_test_test_var_a1");
	t2 = CLJC_FN_clojure_test_test_var_a1 ? CLJC_FN_clojure_test_test_var_a1(clj_var_root_relaxed(V[92]), NULL, a0, 1) : clj_c_invoke(t0, a0, 1);
#else
	t2 = clj_c_invoke(t0, a0, 1);
#endif
	(void)t1;
	if (o0) clj_release(t0);
	if (t2 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[45], &cc);
	return t2;
L2: ;
	clj_c_leave(&S[45], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_test_vars__0__1(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 0: return clojure_test_test_vars__0__1_a0(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value clojure_test_test_vars__0_a0(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_value s[2];
	clj_cframe fr = {s, captured, 0, NULL};
	(void)fr;
	for (uint32_t i = 0; i < 2; i++) s[i] = CLJ_NIL;
	clj_value l0 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[44], &cc)) goto L1;
#line 309 "<embedded>/clojure/test.clj"
	clj_value t0 = fr.captured[0];
	clj_value a0[1] = {t0};
	(void)a0;
	clj_value t1;
	if (CLJC_GUARD(V[12], B[0])) {
	t1 = clj_seq(t0);
	} else {
	t1 = clj_c_intrinsic_fallback(V[12], a0, 1);
	}
	(void)t0;
	if (t1 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l0, t1);
L4: ;
	clj_value t2 = l0;
	bool t3 = clj_truthy(t2);
	(void)t2;
	clj_value t4;
	if (t3) {
	clj_value t5 = l0;
	clj_value a1[1] = {t5};
	(void)a1;
	clj_value t6;
	if (CLJC_GUARD(V[19], B[3])) {
	t6 = clj_first(t5);
	} else {
	t6 = clj_c_intrinsic_fallback(V[19], a1, 1);
	}
	(void)t5;
	if (t6 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_set(&fr, 1, t6);
#line 310 "<embedded>/clojure/test.clj"
	clj_value t7 = K[352];
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[24], &o8);
	if (t8 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t9 = fr.slots[1];
	clj_value a3[1] = {t9};
	(void)a3;
	clj_value t10 = clj_c_invoke(t8, a3, 1);
	(void)t9;
	if (o8) clj_release(t8);
	if (t10 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a2[1] = {t10};
	(void)a2;
	clj_value t11 = clj_c_invoke(t7, a2, 1);
	clj_release(t10);
	(void)t7;
	if (t11 == CLJ_THROWN) {
	goto L2;
	}
	bool t12 = clj_truthy(t11);
	clj_release(t11);
	clj_value t13;
	if (t12) {
#line 311 "<embedded>/clojure/test.clj"
	clj_value t14 = fr.captured[1];
	clj_value c5[1] = {fr.slots[1]};
	clj_value t15 = clj_c_closure(CLJ_NIL, clojure_test_test_vars__0__1, c5, 1, 0x1, 0, 0);
	clj_value a4[1] = {t15};
	(void)a4;
	clj_value t16 = clj_c_invoke(t14, a4, 1);
	clj_release(t15);
	(void)t14;
	if (t16 == CLJ_THROWN) {
	goto L2;
	}
	t13 = t16;
	} else {
	t13 = CLJ_NIL;
	}
	clj_release(t13);
#line 309 "<embedded>/clojure/test.clj"
	clj_value t17 = l0;
	clj_value a6[1] = {t17};
	(void)a6;
	clj_value t18;
	if (CLJC_GUARD(V[16], B[2])) {
	t18 = clj_next(t17);
	} else {
	t18 = clj_c_intrinsic_fallback(V[16], a6, 1);
	}
	(void)t17;
	if (t18 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l0, t18);
	if (clj_c_loop_tick()) {
	goto L2;
	}
	goto L4;
	clj_value t19 = CLJ_NIL;
	t4 = t19;
	} else {
	t4 = CLJ_NIL;
	}
	clj_c_leave(&S[44], &cc);
	clj_release(l0);
	clj_c_release_slots(&fr, 2);
	return t4;
L2: ;
	clj_c_leave(&S[44], &cc);
L1: ;
	clj_release(l0);
	clj_c_release_slots(&fr, 2);
	return CLJ_THROWN;
}

static clj_value clojure_test_test_vars__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 0: return clojure_test_test_vars__0_a0(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

clj_value clojure_test_test_vars_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_value s[8];
	clj_cframe fr = {s, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	for (uint32_t i = 1; i < 8; i++) s[i] = CLJ_NIL;
	clj_value l1 = CLJ_NIL;
	clj_value l2 = CLJ_NIL;
	clj_value l3 = CLJ_NIL;
	clj_value l5 = CLJ_NIL;
	clj_value l6 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[43], &cc)) goto L1;
#line 303 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[97], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[98], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t2 = K[349];
	bool o3;
	clj_value t3 = clj_c_var_borrow(V[24], &o3);
	if (t3 == CLJ_THROWN) {
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a2[2] = {t2, t3};
	(void)a2;
	clj_value t4;
#ifdef CLJC_DIRECT_clojure_core_comp_a2
	if (!CLJC_FN_clojure_core_comp_a2) CLJC_FN_clojure_core_comp_a2 = clj_compiled_symbol("clojure_core_comp_a2");
	t4 = CLJC_FN_clojure_core_comp_a2 ? CLJC_FN_clojure_core_comp_a2(clj_var_root_relaxed(V[98]), NULL, a2, 2) : clj_c_invoke(t1, a2, 2);
#else
	t4 = clj_c_invoke(t1, a2, 2);
#endif
	if (o3) clj_release(t3);
	(void)t2;
	if (o1) clj_release(t1);
	if (t4 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t5 = l0;
	clj_value a1[2] = {t4, t5};
	(void)a1;
	clj_value t6;
#ifdef CLJC_DIRECT_clojure_core_group_by_a2
	if (!CLJC_FN_clojure_core_group_by_a2) CLJC_FN_clojure_core_group_by_a2 = clj_compiled_symbol("clojure_core_group_by_a2");
	t6 = CLJC_FN_clojure_core_group_by_a2 ? CLJC_FN_clojure_core_group_by_a2(clj_var_root_relaxed(V[97]), NULL, a1, 2) : clj_c_invoke(t0, a1, 2);
#else
	t6 = clj_c_invoke(t0, a1, 2);
#endif
	(void)t5;
	clj_release(t4);
	if (o0) clj_release(t0);
	if (t6 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t6};
	(void)a0;
	clj_value t7;
	if (CLJC_GUARD(V[12], B[0])) {
	t7 = clj_seq(t6);
	} else {
	t7 = clj_c_intrinsic_fallback(V[12], a0, 1);
	}
	clj_release(t6);
	if (t7 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l1, t7);
L4: ;
	clj_value t8 = l1;
	bool t9 = clj_truthy(t8);
	(void)t8;
	clj_value t10;
	if (t9) {
	clj_value t11 = l1;
	clj_value a3[1] = {t11};
	(void)a3;
	clj_value t12;
	if (CLJC_GUARD(V[19], B[3])) {
	t12 = clj_first(t11);
	} else {
	t12 = clj_c_intrinsic_fallback(V[19], a3, 1);
	}
	(void)t11;
	if (t12 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l2, t12);
	clj_value t13 = l2;
	clj_value t14 = K[41];
	clj_value t15 = K[3];
	clj_value a4[3] = {t13, t14, t15};
	(void)a4;
	clj_value t16;
	if (CLJC_GUARD(V[62], B[19])) {
	t16 = clj_nth3(t13, t14, t15);
	} else {
	t16 = clj_c_intrinsic_fallback(V[62], a4, 3);
	}
	(void)t15;
	(void)t14;
	(void)t13;
	if (t16 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l3, t16);
	clj_value t17 = l2;
	clj_value t18 = K[175];
	clj_value t19 = K[3];
	clj_value a5[3] = {t17, t18, t19};
	(void)a5;
	clj_value t20;
	if (CLJC_GUARD(V[62], B[19])) {
	t20 = clj_nth3(t17, t18, t19);
	} else {
	t20 = clj_c_intrinsic_fallback(V[62], a5, 3);
	}
	(void)t19;
	(void)t18;
	(void)t17;
	if (t20 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_set(&fr, 4, t20);
#line 304 "<embedded>/clojure/test.clj"
	bool o21;
	clj_value t21 = clj_c_var_borrow(V[55], &o21);
	if (t21 == CLJ_THROWN) {
	goto L2;
	}
	bool o22;
	clj_value t22 = clj_c_var_borrow(V[83], &o22);
	if (t22 == CLJ_THROWN) {
	if (o21) clj_release(t21);
	goto L2;
	}
	clj_value a7[1] = {t22};
	(void)a7;
	clj_value t23 = clj_c_invoke(t21, a7, 1);
	if (o22) clj_release(t22);
	if (o21) clj_release(t21);
	if (t23 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t24 = l3;
	clj_value a6[2] = {t23, t24};
	(void)a6;
	clj_value t25;
	if (CLJC_GUARD(V[20], B[4])) {
	t25 = clj_get2(t23, t24);
	} else {
	t25 = clj_c_intrinsic_fallback(V[20], a6, 2);
	}
	(void)t24;
	clj_release(t23);
	if (t25 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l5, t25);
#line 305 "<embedded>/clojure/test.clj"
	bool o26;
	clj_value t26 = clj_c_var_borrow(V[89], &o26);
	if (t26 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t27 = K[350];
	clj_value t28 = l5;
	clj_value a9[1] = {t28};
	(void)a9;
	clj_value t29 = clj_c_invoke(t27, a9, 1);
	(void)t28;
	(void)t27;
	if (t29 == CLJ_THROWN) {
	if (o26) clj_release(t26);
	goto L2;
	}
	clj_value a8[1] = {t29};
	(void)a8;
	clj_value t30;
#ifdef CLJC_DIRECT_clojure_test_join_fixtures_a1
	if (!CLJC_FN_clojure_test_join_fixtures_a1) CLJC_FN_clojure_test_join_fixtures_a1 = clj_compiled_symbol("clojure_test_join_fixtures_a1");
	t30 = CLJC_FN_clojure_test_join_fixtures_a1 ? CLJC_FN_clojure_test_join_fixtures_a1(clj_var_root_relaxed(V[89]), NULL, a8, 1) : clj_c_invoke(t26, a8, 1);
#else
	t30 = clj_c_invoke(t26, a8, 1);
#endif
	clj_release(t29);
	if (o26) clj_release(t26);
	if (t30 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l6, t30);
#line 306 "<embedded>/clojure/test.clj"
	bool o31;
	clj_value t31 = clj_c_var_borrow(V[89], &o31);
	if (t31 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t32 = K[351];
	clj_value t33 = l5;
	clj_value a11[1] = {t33};
	(void)a11;
	clj_value t34 = clj_c_invoke(t32, a11, 1);
	(void)t33;
	(void)t32;
	if (t34 == CLJ_THROWN) {
	if (o31) clj_release(t31);
	goto L2;
	}
	clj_value a10[1] = {t34};
	(void)a10;
	clj_value t35;
#ifdef CLJC_DIRECT_clojure_test_join_fixtures_a1
	if (!CLJC_FN_clojure_test_join_fixtures_a1) CLJC_FN_clojure_test_join_fixtures_a1 = clj_compiled_symbol("clojure_test_join_fixtures_a1");
	t35 = CLJC_FN_clojure_test_join_fixtures_a1 ? CLJC_FN_clojure_test_join_fixtures_a1(clj_var_root_relaxed(V[89]), NULL, a10, 1) : clj_c_invoke(t31, a10, 1);
#else
	t35 = clj_c_invoke(t31, a10, 1);
#endif
	clj_release(t34);
	if (o31) clj_release(t31);
	if (t35 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_set(&fr, 7, t35);
#line 307 "<embedded>/clojure/test.clj"
	clj_value t36 = l6;
#line 308 "<embedded>/clojure/test.clj"
	clj_value c13[2] = {fr.slots[4], fr.slots[7]};
	clj_value t37 = clj_c_closure(CLJ_NIL, clojure_test_test_vars__0, c13, 2, 0x1, 0, 0);
	clj_value a12[1] = {t37};
	(void)a12;
	clj_value t38 = clj_c_invoke(t36, a12, 1);
	clj_release(t37);
	(void)t36;
	if (t38 == CLJ_THROWN) {
	goto L2;
	}
	clj_release(t38);
#line 303 "<embedded>/clojure/test.clj"
	clj_value t39 = l1;
	clj_value a14[1] = {t39};
	(void)a14;
	clj_value t40;
	if (CLJC_GUARD(V[16], B[2])) {
	t40 = clj_next(t39);
	} else {
	t40 = clj_c_intrinsic_fallback(V[16], a14, 1);
	}
	(void)t39;
	if (t40 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l1, t40);
	if (clj_c_loop_tick()) {
	goto L2;
	}
	goto L4;
	clj_value t41 = CLJ_NIL;
	t10 = t41;
	} else {
	t10 = CLJ_NIL;
	}
	clj_c_leave(&S[43], &cc);
	clj_release(l1);
	clj_release(l2);
	clj_release(l3);
	clj_release(l5);
	clj_release(l6);
	clj_c_release_slots(&fr, 8);
	return t10;
L2: ;
	clj_c_leave(&S[43], &cc);
L1: ;
	clj_release(l1);
	clj_release(l2);
	clj_release(l3);
	clj_release(l5);
	clj_release(l6);
	clj_c_release_slots(&fr, 8);
	return CLJ_THROWN;
}

static clj_value clojure_test_test_vars(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_test_vars_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_69(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 299 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[348], clojure_test_test_vars, NULL, 0, 0x2, 1, 1);
	clj_var_bind_root(V[96], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[353]);
	clj_value t2 = clj_c_def(V[96], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static clj_value clojure_test_test_all_vars__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[47], &cc)) goto L1;
#line 316 "<embedded>/clojure/test.clj"
	clj_value t0 = K[356];
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[24], &o1);
	if (t1 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t2 = l0;
	clj_value a1[1] = {t2};
	(void)a1;
	clj_value t3 = clj_c_invoke(t1, a1, 1);
	(void)t2;
	if (o1) clj_release(t1);
	if (t3 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 1);
	clj_release(t3);
	(void)t0;
	if (t4 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l1, t4);
	clj_value t5 = l1;
	bool t6 = clj_truthy(t5);
	(void)t5;
	clj_value t7;
	if (t6) {
	clj_value t8 = l1;
	l1 = CLJ_NIL;
	t7 = t8;
	} else {
	clj_value t9 = clj_retain(K[41]);
	t7 = t9;
	}
	clj_c_leave(&S[47], &cc);
	clj_release(l1);
	return t7;
L2: ;
	clj_c_leave(&S[47], &cc);
L1: ;
	clj_release(l1);
	return CLJ_THROWN;
}

static clj_value clojure_test_test_all_vars__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_test_all_vars__0_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

clj_value clojure_test_test_all_vars_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_ccall cc;
	if (!clj_c_enter(&S[46], &cc)) goto L1;
#line 316 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[96], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[100], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t2 = clj_c_closure(CLJ_NIL, clojure_test_test_all_vars__0, NULL, 0, 0x2, 1, 1);
	bool o3;
	clj_value t3 = clj_c_var_borrow(V[101], &o3);
	if (t3 == CLJ_THROWN) {
	clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[102], &o4);
	if (t4 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t5 = l0;
	clj_value a3[1] = {t5};
	(void)a3;
	clj_value t6 = clj_c_invoke(t4, a3, 1);
	(void)t5;
	if (o4) clj_release(t4);
	if (t6 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a2[1] = {t6};
	(void)a2;
	clj_value t7;
#ifdef CLJC_DIRECT_clojure_core_vals_a1
	if (!CLJC_FN_clojure_core_vals_a1) CLJC_FN_clojure_core_vals_a1 = clj_compiled_symbol("clojure_core_vals_a1");
	t7 = CLJC_FN_clojure_core_vals_a1 ? CLJC_FN_clojure_core_vals_a1(clj_var_root_relaxed(V[101]), NULL, a2, 1) : clj_c_invoke(t3, a2, 1);
#else
	t7 = clj_c_invoke(t3, a2, 1);
#endif
	clj_release(t6);
	if (o3) clj_release(t3);
	if (t7 == CLJ_THROWN) {
	clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a1[2] = {t2, t7};
	(void)a1;
	clj_value t8;
#ifdef CLJC_DIRECT_clojure_core_sort_by_a2
	if (!CLJC_FN_clojure_core_sort_by_a2) CLJC_FN_clojure_core_sort_by_a2 = clj_compiled_symbol("clojure_core_sort_by_a2");
	t8 = CLJC_FN_clojure_core_sort_by_a2 ? CLJC_FN_clojure_core_sort_by_a2(clj_var_root_relaxed(V[100]), NULL, a1, 2) : clj_c_invoke(t1, a1, 2);
#else
	t8 = clj_c_invoke(t1, a1, 2);
#endif
	clj_release(t7);
	clj_release(t2);
	if (o1) clj_release(t1);
	if (t8 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a0[1] = {t8};
	(void)a0;
	clj_value t9;
#ifdef CLJC_DIRECT_clojure_test_test_vars_a1
	if (!CLJC_FN_clojure_test_test_vars_a1) CLJC_FN_clojure_test_test_vars_a1 = clj_compiled_symbol("clojure_test_test_vars_a1");
	t9 = CLJC_FN_clojure_test_test_vars_a1 ? CLJC_FN_clojure_test_test_vars_a1(clj_var_root_relaxed(V[96]), NULL, a0, 1) : clj_c_invoke(t0, a0, 1);
#else
	t9 = clj_c_invoke(t0, a0, 1);
#endif
	clj_release(t8);
	if (o0) clj_release(t0);
	if (t9 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[46], &cc);
	return t9;
L2: ;
	clj_c_leave(&S[46], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_test_all_vars(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_test_all_vars_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_70(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 313 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[355], clojure_test_test_all_vars, NULL, 0, 0x2, 1, 1);
	clj_var_bind_root(V[99], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[357]);
	clj_value t2 = clj_c_def(V[99], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_test_ns_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = CLJ_NIL;
	clj_value l2 = CLJ_NIL;
	clj_value l3 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[48], &cc)) goto L1;
#line 322 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[93], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[18], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t2 = V[5];
	bool o3;
	clj_value t3 = clj_c_var_borrow(V[36], &o3);
	if (t3 == CLJ_THROWN) {
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[6], &o4);
	if (t4 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a2[1] = {t4};
	(void)a2;
	clj_value t5 = clj_c_invoke(t3, a2, 1);
	if (o4) clj_release(t4);
	if (o3) clj_release(t3);
	if (t5 == CLJ_THROWN) {
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a1[2] = {t2, t5};
	(void)a1;
	clj_value t6 = clj_c_invoke(t1, a1, 2);
	clj_release(t5);
	(void)t2;
	if (o1) clj_release(t1);
	if (t6 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a0[1] = {t6};
	(void)a0;
	clj_value t7 = clj_c_invoke(t0, a0, 1);
	clj_release(t6);
	if (o0) clj_release(t0);
	if (t7 == CLJ_THROWN) {
	goto L2;
	}
	clj_release(t7);
	clj_value t8;
#line 323 "<embedded>/clojure/test.clj"
	bool o9;
	clj_value t9 = clj_c_var_borrow(V[104], &o9);
	if (t9 == CLJ_THROWN) {
	goto L4;
	}
	clj_value t10 = l0;
	clj_value a4[1] = {t10};
	(void)a4;
	clj_value t11 = clj_c_invoke(t9, a4, 1);
	(void)t10;
	if (o9) clj_release(t9);
	if (t11 == CLJ_THROWN) {
	goto L4;
	}
	clj_c_rebind(&l1, t11);
#line 324 "<embedded>/clojure/test.clj"
	bool o12;
	clj_value t12 = clj_c_var_borrow(V[46], &o12);
	if (t12 == CLJ_THROWN) {
	goto L4;
	}
	clj_value t13 = K[360];
	clj_value t14 = l1;
	clj_value t15 = K[361];
	clj_value t16 = K[362];
	clj_value a6[4] = {t13, t14, t15, t16};
	(void)a6;
	clj_value t17 = clj_c_map_literal(a6, 4);
	(void)t16;
	(void)t15;
	(void)t14;
	(void)t13;
	if (t17 == CLJ_THROWN) {
	if (o12) clj_release(t12);
	goto L4;
	}
	clj_value a5[1] = {t17};
	(void)a5;
	clj_value t18;
#ifdef CLJC_DIRECT_clojure_test_do_report_a1
	if (!CLJC_FN_clojure_test_do_report_a1) CLJC_FN_clojure_test_do_report_a1 = clj_compiled_symbol("clojure_test_do_report_a1");
	t18 = CLJC_FN_clojure_test_do_report_a1 ? CLJC_FN_clojure_test_do_report_a1(clj_var_root_relaxed(V[46]), NULL, a5, 1) : clj_c_invoke(t12, a5, 1);
#else
	t18 = clj_c_invoke(t12, a5, 1);
#endif
	clj_release(t17);
	if (o12) clj_release(t12);
	if (t18 == CLJ_THROWN) {
	goto L4;
	}
	clj_release(t18);
#line 325 "<embedded>/clojure/test.clj"
	bool o19;
	clj_value t19 = clj_c_var_borrow(V[102], &o19);
	if (t19 == CLJ_THROWN) {
	goto L4;
	}
	clj_value t20 = l1;
	clj_value a8[1] = {t20};
	(void)a8;
	clj_value t21 = clj_c_invoke(t19, a8, 1);
	(void)t20;
	if (o19) clj_release(t19);
	if (t21 == CLJ_THROWN) {
	goto L4;
	}
	clj_value t22 = K[363];
	clj_value a7[2] = {t21, t22};
	(void)a7;
	clj_value t23;
	if (CLJC_GUARD(V[20], B[4])) {
	t23 = clj_get2(t21, t22);
	} else {
	t23 = clj_c_intrinsic_fallback(V[20], a7, 2);
	}
	(void)t22;
	clj_release(t21);
	if (t23 == CLJ_THROWN) {
	goto L4;
	}
	clj_c_rebind(&l2, t23);
	clj_value t24 = l2;
	bool t25 = clj_truthy(t24);
	(void)t24;
	clj_value t26;
	if (t25) {
	clj_value t27 = l2;
	l2 = CLJ_NIL;
	clj_c_rebind(&l3, t27);
#line 326 "<embedded>/clojure/test.clj"
	bool o28;
	clj_value t28 = clj_c_var_borrow(V[105], &o28);
	if (t28 == CLJ_THROWN) {
	goto L4;
	}
	clj_value t29 = l3;
	clj_value a9[1] = {t29};
	(void)a9;
	clj_value t30 = clj_c_invoke(t28, a9, 1);
	(void)t29;
	if (o28) clj_release(t28);
	if (t30 == CLJ_THROWN) {
	goto L4;
	}
	const clj_value *a10 = NULL;
	clj_value t31 = clj_c_invoke(t30, a10, 0);
	clj_release(t30);
	if (t31 == CLJ_THROWN) {
	goto L4;
	}
	t26 = t31;
	} else {
#line 327 "<embedded>/clojure/test.clj"
	bool o32;
	clj_value t32 = clj_c_var_borrow(V[99], &o32);
	if (t32 == CLJ_THROWN) {
	goto L4;
	}
	clj_value t33 = l1;
	clj_value a11[1] = {t33};
	(void)a11;
	clj_value t34;
#ifdef CLJC_DIRECT_clojure_test_test_all_vars_a1
	if (!CLJC_FN_clojure_test_test_all_vars_a1) CLJC_FN_clojure_test_test_all_vars_a1 = clj_compiled_symbol("clojure_test_test_all_vars_a1");
	t34 = CLJC_FN_clojure_test_test_all_vars_a1 ? CLJC_FN_clojure_test_test_all_vars_a1(clj_var_root_relaxed(V[99]), NULL, a11, 1) : clj_c_invoke(t32, a11, 1);
#else
	t34 = clj_c_invoke(t32, a11, 1);
#endif
	(void)t33;
	if (o32) clj_release(t32);
	if (t34 == CLJ_THROWN) {
	goto L4;
	}
	t26 = t34;
	}
	clj_release(t26);
#line 328 "<embedded>/clojure/test.clj"
	bool o35;
	clj_value t35 = clj_c_var_borrow(V[46], &o35);
	if (t35 == CLJ_THROWN) {
	goto L4;
	}
	clj_value t36 = K[360];
	clj_value t37 = l1;
	clj_value t38 = K[361];
	clj_value t39 = K[364];
	clj_value a13[4] = {t36, t37, t38, t39};
	(void)a13;
	clj_value t40 = clj_c_map_literal(a13, 4);
	(void)t39;
	(void)t38;
	(void)t37;
	(void)t36;
	if (t40 == CLJ_THROWN) {
	if (o35) clj_release(t35);
	goto L4;
	}
	clj_value a12[1] = {t40};
	(void)a12;
	clj_value t41;
#ifdef CLJC_DIRECT_clojure_test_do_report_a1
	if (!CLJC_FN_clojure_test_do_report_a1) CLJC_FN_clojure_test_do_report_a1 = clj_compiled_symbol("clojure_test_do_report_a1");
	t41 = CLJC_FN_clojure_test_do_report_a1 ? CLJC_FN_clojure_test_do_report_a1(clj_var_root_relaxed(V[46]), NULL, a12, 1) : clj_c_invoke(t35, a12, 1);
#else
	t41 = clj_c_invoke(t35, a12, 1);
#endif
	clj_release(t40);
	if (o35) clj_release(t35);
	if (t41 == CLJ_THROWN) {
	goto L4;
	}
	clj_release(t41);
#line 322 "<embedded>/clojure/test.clj"
	bool o42;
	clj_value t42 = clj_c_var_borrow(V[55], &o42);
	if (t42 == CLJ_THROWN) {
	goto L4;
	}
	bool o43;
	clj_value t43 = clj_c_var_borrow(V[5], &o43);
	if (t43 == CLJ_THROWN) {
	if (o42) clj_release(t42);
	goto L4;
	}
	clj_value a14[1] = {t43};
	(void)a14;
	clj_value t44 = clj_c_invoke(t42, a14, 1);
	if (o43) clj_release(t43);
	if (o42) clj_release(t42);
	if (t44 == CLJ_THROWN) {
	goto L4;
	}
	t8 = t44;
	goto L5;
L4: ;
	t8 = CLJ_THROWN;
L5: ;
	clj_value pt3 = t8 == CLJ_THROWN ? clj_take_pending_trace() : CLJ_NIL;
	clj_value p3 = t8 == CLJ_THROWN ? clj_take_pending() : CLJ_NIL;
	bool o45;
	clj_value t45 = clj_c_var_borrow(V[95], &o45);
	if (t45 == CLJ_THROWN) {
	goto L6;
	}
	const clj_value *a15 = NULL;
	clj_value t46 = clj_c_invoke(t45, a15, 0);
	if (o45) clj_release(t45);
	if (t46 == CLJ_THROWN) {
	goto L6;
	}
	clj_release(t46);
	if (t8 == CLJ_THROWN) clj_throw_traced(p3, pt3);
	goto L7;
L6: ;
	clj_release(t8 == CLJ_THROWN ? p3 : t8);
	clj_release(pt3);
	t8 = CLJ_THROWN;
L7: ;
	clj_value t47 = t8;
	if (t47 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[48], &cc);
	clj_release(l1);
	clj_release(l2);
	clj_release(l3);
	return t47;
L2: ;
	clj_c_leave(&S[48], &cc);
L1: ;
	clj_release(l1);
	clj_release(l2);
	clj_release(l3);
	return CLJ_THROWN;
}

static clj_value clojure_test_test_ns(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_test_ns_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_71(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 318 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[359], clojure_test_test_ns, NULL, 0, 0x2, 1, 1);
	clj_var_bind_root(V[103], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[365]);
	clj_value t2 = clj_c_def(V[103], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_run_tests_a0(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_ccall cc;
	if (!clj_c_enter(&S[49], &cc)) goto L1;
#line 334 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[106], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[86], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2 = clj_c_invoke(t0, a0, 1);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t2 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[49], &cc);
	return t2;
L2: ;
	clj_c_leave(&S[49], &cc);
L1: ;
	return CLJ_THROWN;
}

clj_value clojure_test_run_tests_v0(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = CLJ_NIL;
	clj_value l1 = CLJ_NIL;
	l0 = nargs > 0 ? clj_list_from_array(args + 0, nargs - 0) : CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[49], &cc)) goto L1;
#line 336 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[17], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[107], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o2;
	clj_value t2 = clj_c_var_borrow(V[44], &o2);
	if (t2 == CLJ_THROWN) {
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o3;
	clj_value t3 = clj_c_var_borrow(V[23], &o3);
	if (t3 == CLJ_THROWN) {
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[103], &o4);
	if (t4 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t5 = l0;
	clj_value a2[2] = {t4, t5};
	(void)a2;
	clj_value t6;
#ifdef CLJC_DIRECT_clojure_core_map_a2
	if (!CLJC_FN_clojure_core_map_a2) CLJC_FN_clojure_core_map_a2 = clj_compiled_symbol("clojure_core_map_a2");
	t6 = CLJC_FN_clojure_core_map_a2 ? CLJC_FN_clojure_core_map_a2(clj_var_root_relaxed(V[23]), NULL, a2, 2) : clj_c_invoke(t3, a2, 2);
#else
	t6 = clj_c_invoke(t3, a2, 2);
#endif
	(void)t5;
	if (o4) clj_release(t4);
	if (o3) clj_release(t3);
	if (t6 == CLJ_THROWN) {
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a1[3] = {t1, t2, t6};
	(void)a1;
	clj_value t7 = clj_c_invoke(t0, a1, 3);
	clj_release(t6);
	if (o2) clj_release(t2);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t7 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t8 = K[368];
	clj_value t9 = K[369];
	clj_value a0[3] = {t7, t8, t9};
	(void)a0;
	clj_value t10;
	if (CLJC_GUARD(V[80], B[20])) {
	t10 = clj_assoc_owned(t7, t8, t9);
	} else {
	t10 = clj_c_intrinsic_fallback(V[80], a0, 3);
	clj_release(t7);
	}
	(void)t9;
	(void)t8;
	if (t10 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l1, t10);
#line 338 "<embedded>/clojure/test.clj"
	bool o11;
	clj_value t11 = clj_c_var_borrow(V[46], &o11);
	if (t11 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t12 = l1;
	clj_value a3[1] = {t12};
	(void)a3;
	clj_value t13;
#ifdef CLJC_DIRECT_clojure_test_do_report_a1
	if (!CLJC_FN_clojure_test_do_report_a1) CLJC_FN_clojure_test_do_report_a1 = clj_compiled_symbol("clojure_test_do_report_a1");
	t13 = CLJC_FN_clojure_test_do_report_a1 ? CLJC_FN_clojure_test_do_report_a1(clj_var_root_relaxed(V[46]), NULL, a3, 1) : clj_c_invoke(t11, a3, 1);
#else
	t13 = clj_c_invoke(t11, a3, 1);
#endif
	(void)t12;
	if (o11) clj_release(t11);
	if (t13 == CLJ_THROWN) {
	goto L2;
	}
	clj_release(t13);
#line 336 "<embedded>/clojure/test.clj"
	clj_value t14 = l1;
	l1 = CLJ_NIL;
	clj_c_leave(&S[49], &cc);
	clj_release(l0);
	clj_release(l1);
	return t14;
L2: ;
	clj_c_leave(&S[49], &cc);
L1: ;
	clj_release(l0);
	clj_release(l1);
	return CLJ_THROWN;
}

static clj_value clojure_test_run_tests(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 0: return clojure_test_run_tests_a0(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	if (n >= 0) return clojure_test_run_tests_v0(clj_from_ptr((void *)f), f->env, args, n);
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_72(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 331 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[367], clojure_test_run_tests, NULL, 0, 0x1, 0, CLJ_ARITY_ANY);
	clj_var_bind_root(V[106], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[370]);
	clj_value t2 = clj_c_def(V[106], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_run_all_tests_a0(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_ccall cc;
	if (!clj_c_enter(&S[50], &cc)) goto L1;
#line 344 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[17], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[106], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o2;
	clj_value t2 = clj_c_var_borrow(V[109], &o2);
	if (t2 == CLJ_THROWN) {
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	const clj_value *a1 = NULL;
	clj_value t3 = clj_c_invoke(t2, a1, 0);
	if (o2) clj_release(t2);
	if (t3 == CLJ_THROWN) {
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a0[2] = {t1, t3};
	(void)a0;
	clj_value t4 = clj_c_invoke(t0, a0, 2);
	clj_release(t3);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	if (t4 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[50], &cc);
	return t4;
L2: ;
	clj_c_leave(&S[50], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_run_all_tests__0_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_ccall cc;
	if (!clj_c_enter(&S[51], &cc)) goto L1;
#line 346 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[111], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t1 = fr.captured[0];
	clj_value t2 = l0;
	clj_value a0[2] = {t1, t2};
	(void)a0;
	clj_value t3 = clj_c_invoke(t0, a0, 2);
	(void)t2;
	(void)t1;
	if (o0) clj_release(t0);
	if (t3 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[51], &cc);
	return t3;
L2: ;
	clj_c_leave(&S[51], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_run_all_tests__0(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_run_all_tests__0_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value clojure_test_run_all_tests__1_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_ccall cc;
	if (!clj_c_enter(&S[52], &cc)) goto L1;
#line 347 "<embedded>/clojure/test.clj"
	clj_value t0 = fr.captured[0];
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[21], &o1);
	if (t1 == CLJ_THROWN) {
	goto L2;
	}
	bool o2;
	clj_value t2 = clj_c_var_borrow(V[45], &o2);
	if (t2 == CLJ_THROWN) {
	if (o1) clj_release(t1);
	goto L2;
	}
	clj_value t3 = l0;
	clj_value a2[1] = {t3};
	(void)a2;
	clj_value t4 = clj_c_invoke(t2, a2, 1);
	(void)t3;
	if (o2) clj_release(t2);
	if (t4 == CLJ_THROWN) {
	if (o1) clj_release(t1);
	goto L2;
	}
	clj_value a1[1] = {t4};
	(void)a1;
	clj_value t5 = clj_c_invoke(t1, a1, 1);
	clj_release(t4);
	if (o1) clj_release(t1);
	if (t5 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t5};
	(void)a0;
	clj_value t6 = clj_c_invoke(t0, a0, 1);
	clj_release(t5);
	(void)t0;
	if (t6 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[52], &cc);
	return t6;
L2: ;
	clj_c_leave(&S[52], &cc);
L1: ;
	return CLJ_THROWN;
}

static clj_value clojure_test_run_all_tests__1(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_run_all_tests__1_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

clj_value clojure_test_run_all_tests_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_value s[2];
	clj_cframe fr = {s, captured, 0, NULL};
	(void)fr;
	s[0] = args[0];
	for (uint32_t i = 1; i < 2; i++) s[i] = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[50], &cc)) goto L1;
#line 346 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[110], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t1 = fr.slots[0];
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2 = clj_c_invoke(t0, a0, 1);
	(void)t1;
	if (o0) clj_release(t0);
	if (t2 == CLJ_THROWN) {
	goto L2;
	}
	bool t3 = clj_truthy(t2);
	clj_release(t2);
	clj_value t4;
	if (t3) {
	clj_value c1[1] = {fr.slots[0]};
	clj_value t5 = clj_c_closure(CLJ_NIL, clojure_test_run_all_tests__0, c1, 1, 0x2, 1, 1);
	t4 = t5;
	} else {
	clj_value t6 = clj_c_local(&fr, 0, true);
	t4 = t6;
	}
	clj_c_set(&fr, 1, t4);
#line 347 "<embedded>/clojure/test.clj"
	bool o7;
	clj_value t7 = clj_c_var_borrow(V[17], &o7);
	if (t7 == CLJ_THROWN) {
	goto L2;
	}
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[106], &o8);
	if (t8 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	goto L2;
	}
	bool o9;
	clj_value t9 = clj_c_var_borrow(V[112], &o9);
	if (t9 == CLJ_THROWN) {
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value c4[1] = {fr.slots[1]};
	clj_value t10 = clj_c_closure(CLJ_NIL, clojure_test_run_all_tests__1, c4, 1, 0x2, 1, 1);
	bool o11;
	clj_value t11 = clj_c_var_borrow(V[109], &o11);
	if (t11 == CLJ_THROWN) {
	clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	goto L2;
	}
	const clj_value *a5 = NULL;
	clj_value t12 = clj_c_invoke(t11, a5, 0);
	if (o11) clj_release(t11);
	if (t12 == CLJ_THROWN) {
	clj_release(t10);
	if (o9) clj_release(t9);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a3[2] = {t10, t12};
	(void)a3;
	clj_value t13;
#ifdef CLJC_DIRECT_clojure_core_filter_a2
	if (!CLJC_FN_clojure_core_filter_a2) CLJC_FN_clojure_core_filter_a2 = clj_compiled_symbol("clojure_core_filter_a2");
	t13 = CLJC_FN_clojure_core_filter_a2 ? CLJC_FN_clojure_core_filter_a2(clj_var_root_relaxed(V[112]), NULL, a3, 2) : clj_c_invoke(t9, a3, 2);
#else
	t13 = clj_c_invoke(t9, a3, 2);
#endif
	clj_release(t12);
	clj_release(t10);
	if (o9) clj_release(t9);
	if (t13 == CLJ_THROWN) {
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value a2[2] = {t8, t13};
	(void)a2;
	clj_value t14 = clj_c_invoke(t7, a2, 2);
	clj_release(t13);
	if (o8) clj_release(t8);
	if (o7) clj_release(t7);
	if (t14 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[50], &cc);
	clj_c_release_slots(&fr, 2);
	return t14;
L2: ;
	clj_c_leave(&S[50], &cc);
L1: ;
	clj_c_release_slots(&fr, 2);
	return CLJ_THROWN;
}

static clj_value clojure_test_run_all_tests(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 0: return clojure_test_run_all_tests_a0(clj_from_ptr((void *)f), f->env, args, n);
	case 1: return clojure_test_run_all_tests_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_73(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 341 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[372], clojure_test_run_all_tests, NULL, 0, 0x3, 0, 1);
	clj_var_bind_root(V[108], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[373]);
	clj_value t2 = clj_c_def(V[108], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_run_test_var_a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[53], &cc)) goto L1;
#line 352 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[93], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	bool o1;
	clj_value t1 = clj_c_var_borrow(V[18], &o1);
	if (t1 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value t2 = V[5];
	bool o3;
	clj_value t3 = clj_c_var_borrow(V[36], &o3);
	if (t3 == CLJ_THROWN) {
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	bool o4;
	clj_value t4 = clj_c_var_borrow(V[6], &o4);
	if (t4 == CLJ_THROWN) {
	if (o3) clj_release(t3);
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a2[1] = {t4};
	(void)a2;
	clj_value t5 = clj_c_invoke(t3, a2, 1);
	if (o4) clj_release(t4);
	if (o3) clj_release(t3);
	if (t5 == CLJ_THROWN) {
	if (o1) clj_release(t1);
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a1[2] = {t2, t5};
	(void)a1;
	clj_value t6 = clj_c_invoke(t1, a1, 2);
	clj_release(t5);
	(void)t2;
	if (o1) clj_release(t1);
	if (t6 == CLJ_THROWN) {
	if (o0) clj_release(t0);
	goto L2;
	}
	clj_value a0[1] = {t6};
	(void)a0;
	clj_value t7 = clj_c_invoke(t0, a0, 1);
	clj_release(t6);
	if (o0) clj_release(t0);
	if (t7 == CLJ_THROWN) {
	goto L2;
	}
	clj_release(t7);
	clj_value t8;
#line 353 "<embedded>/clojure/test.clj"
	bool o9;
	clj_value t9 = clj_c_var_borrow(V[96], &o9);
	if (t9 == CLJ_THROWN) {
	goto L4;
	}
	clj_value t10 = l0;
	clj_value a5[1] = {t10};
	(void)a5;
	clj_value t11 = clj_vector_from_array(a5, 1);
	(void)t10;
	if (t11 == CLJ_THROWN) {
	if (o9) clj_release(t9);
	goto L4;
	}
	clj_value a4[1] = {t11};
	(void)a4;
	clj_value t12;
#ifdef CLJC_DIRECT_clojure_test_test_vars_a1
	if (!CLJC_FN_clojure_test_test_vars_a1) CLJC_FN_clojure_test_test_vars_a1 = clj_compiled_symbol("clojure_test_test_vars_a1");
	t12 = CLJC_FN_clojure_test_test_vars_a1 ? CLJC_FN_clojure_test_test_vars_a1(clj_var_root_relaxed(V[96]), NULL, a4, 1) : clj_c_invoke(t9, a4, 1);
#else
	t12 = clj_c_invoke(t9, a4, 1);
#endif
	clj_release(t11);
	if (o9) clj_release(t9);
	if (t12 == CLJ_THROWN) {
	goto L4;
	}
	clj_release(t12);
#line 354 "<embedded>/clojure/test.clj"
	bool o13;
	clj_value t13 = clj_c_var_borrow(V[55], &o13);
	if (t13 == CLJ_THROWN) {
	goto L4;
	}
	bool o14;
	clj_value t14 = clj_c_var_borrow(V[5], &o14);
	if (t14 == CLJ_THROWN) {
	if (o13) clj_release(t13);
	goto L4;
	}
	clj_value a7[1] = {t14};
	(void)a7;
	clj_value t15 = clj_c_invoke(t13, a7, 1);
	if (o14) clj_release(t14);
	if (o13) clj_release(t13);
	if (t15 == CLJ_THROWN) {
	goto L4;
	}
	clj_value t16 = K[376];
	clj_value t17 = K[377];
	clj_value a6[3] = {t15, t16, t17};
	(void)a6;
	clj_value t18;
	if (CLJC_GUARD(V[80], B[20])) {
	t18 = clj_assoc_owned(t15, t16, t17);
	} else {
	t18 = clj_c_intrinsic_fallback(V[80], a6, 3);
	clj_release(t15);
	}
	(void)t17;
	(void)t16;
	if (t18 == CLJ_THROWN) {
	goto L4;
	}
	clj_c_rebind(&l1, t18);
#line 355 "<embedded>/clojure/test.clj"
	bool o19;
	clj_value t19 = clj_c_var_borrow(V[46], &o19);
	if (t19 == CLJ_THROWN) {
	goto L4;
	}
	clj_value t20 = l1;
	clj_value a8[1] = {t20};
	(void)a8;
	clj_value t21;
#ifdef CLJC_DIRECT_clojure_test_do_report_a1
	if (!CLJC_FN_clojure_test_do_report_a1) CLJC_FN_clojure_test_do_report_a1 = clj_compiled_symbol("clojure_test_do_report_a1");
	t21 = CLJC_FN_clojure_test_do_report_a1 ? CLJC_FN_clojure_test_do_report_a1(clj_var_root_relaxed(V[46]), NULL, a8, 1) : clj_c_invoke(t19, a8, 1);
#else
	t21 = clj_c_invoke(t19, a8, 1);
#endif
	(void)t20;
	if (o19) clj_release(t19);
	if (t21 == CLJ_THROWN) {
	goto L4;
	}
	clj_release(t21);
#line 354 "<embedded>/clojure/test.clj"
	clj_value t22 = l1;
	l1 = CLJ_NIL;
	t8 = t22;
	goto L5;
L4: ;
	t8 = CLJ_THROWN;
L5: ;
	clj_value pt3 = t8 == CLJ_THROWN ? clj_take_pending_trace() : CLJ_NIL;
	clj_value p3 = t8 == CLJ_THROWN ? clj_take_pending() : CLJ_NIL;
#line 352 "<embedded>/clojure/test.clj"
	bool o23;
	clj_value t23 = clj_c_var_borrow(V[95], &o23);
	if (t23 == CLJ_THROWN) {
	goto L6;
	}
	const clj_value *a9 = NULL;
	clj_value t24 = clj_c_invoke(t23, a9, 0);
	if (o23) clj_release(t23);
	if (t24 == CLJ_THROWN) {
	goto L6;
	}
	clj_release(t24);
	if (t8 == CLJ_THROWN) clj_throw_traced(p3, pt3);
	goto L7;
L6: ;
	clj_release(t8 == CLJ_THROWN ? p3 : t8);
	clj_release(pt3);
	t8 = CLJ_THROWN;
L7: ;
	clj_value t25 = t8;
	if (t25 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_leave(&S[53], &cc);
	clj_release(l1);
	return t25;
L2: ;
	clj_c_leave(&S[53], &cc);
L1: ;
	clj_release(l1);
	return CLJ_THROWN;
}

static clj_value clojure_test_run_test_var(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_run_test_var_a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_74(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 349 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[375], clojure_test_run_test_var, NULL, 0, 0x2, 1, 1);
	clj_var_bind_root(V[113], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[378]);
	clj_value t2 = clj_c_def(V[113], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_run_test_a3(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = args[1];
	(void)l1;
	clj_value l2 = args[2];
	(void)l2;
	clj_value l3 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[54], &cc)) goto L1;
#line 361 "<embedded>/clojure/test.clj"
	bool o0;
	clj_value t0 = clj_c_var_borrow(V[52], &o0);
	if (t0 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t1 = l2;
	clj_value a0[1] = {t1};
	(void)a0;
	clj_value t2 = clj_c_invoke(t0, a0, 1);
	(void)t1;
	if (o0) clj_release(t0);
	if (t2 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l3, t2);
#line 362 "<embedded>/clojure/test.clj"
#line 363 "<embedded>/clojure/test.clj"
	clj_value t3 = l3;
	clj_value a1[1] = {t3};
	(void)a1;
	clj_value t4;
	if (CLJC_GUARD(V[48], B[6])) {
	t4 = clj_nil_p(t3);
	} else {
	t4 = clj_c_intrinsic_fallback(V[48], a1, 1);
	}
	(void)t3;
	if (t4 == CLJ_THROWN) {
	goto L2;
	}
	bool t5 = clj_truthy(t4);
	clj_release(t4);
	clj_value t6;
	if (t5) {
	bool o7;
	clj_value t7 = clj_c_var_borrow(V[75], &o7);
	if (t7 == CLJ_THROWN) {
	goto L2;
	}
	bool o8;
	clj_value t8 = clj_c_var_borrow(V[21], &o8);
	if (t8 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t9 = K[381];
	clj_value t10 = l2;
	clj_value t11 = K[382];
	clj_value a3[3] = {t9, t10, t11};
	(void)a3;
	clj_value t12 = clj_c_invoke(t8, a3, 3);
	(void)t11;
	(void)t10;
	(void)t9;
	if (o8) clj_release(t8);
	if (t12 == CLJ_THROWN) {
	if (o7) clj_release(t7);
	goto L2;
	}
	clj_value t13 = K[383];
	clj_value a2[2] = {t12, t13};
	(void)a2;
	clj_value t14 = clj_c_invoke(t7, a2, 2);
	(void)t13;
	clj_release(t12);
	if (o7) clj_release(t7);
	if (t14 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t15 = clj_throw(t14);
	if (t15 == CLJ_THROWN) {
	goto L2;
	}
	t6 = t15;
	} else {
#line 362 "<embedded>/clojure/test.clj"
#line 364 "<embedded>/clojure/test.clj"
	clj_value t16 = K[384];
	bool o17;
	clj_value t17 = clj_c_var_borrow(V[24], &o17);
	if (t17 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t18 = l3;
	clj_value a6[1] = {t18};
	(void)a6;
	clj_value t19 = clj_c_invoke(t17, a6, 1);
	(void)t18;
	if (o17) clj_release(t17);
	if (t19 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a5[1] = {t19};
	(void)a5;
	clj_value t20 = clj_c_invoke(t16, a5, 1);
	clj_release(t19);
	(void)t16;
	if (t20 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a4[1] = {t20};
	(void)a4;
	clj_value t21;
	if (CLJC_GUARD(V[53], B[8])) {
	t21 = clj_not(t20);
	} else {
	t21 = clj_c_intrinsic_fallback(V[53], a4, 1);
	}
	clj_release(t20);
	if (t21 == CLJ_THROWN) {
	goto L2;
	}
	bool t22 = clj_truthy(t21);
	clj_release(t21);
	clj_value t23;
	if (t22) {
	bool o24;
	clj_value t24 = clj_c_var_borrow(V[75], &o24);
	if (t24 == CLJ_THROWN) {
	goto L2;
	}
	bool o25;
	clj_value t25 = clj_c_var_borrow(V[21], &o25);
	if (t25 == CLJ_THROWN) {
	if (o24) clj_release(t24);
	goto L2;
	}
	clj_value t26 = l2;
	clj_value t27 = K[385];
	clj_value a8[2] = {t26, t27};
	(void)a8;
	clj_value t28 = clj_c_invoke(t25, a8, 2);
	(void)t27;
	(void)t26;
	if (o25) clj_release(t25);
	if (t28 == CLJ_THROWN) {
	if (o24) clj_release(t24);
	goto L2;
	}
	clj_value t29 = K[383];
	clj_value a7[2] = {t28, t29};
	(void)a7;
	clj_value t30 = clj_c_invoke(t24, a7, 2);
	(void)t29;
	clj_release(t28);
	if (o24) clj_release(t24);
	if (t30 == CLJ_THROWN) {
	goto L2;
	}
	clj_value t31 = clj_throw(t30);
	if (t31 == CLJ_THROWN) {
	goto L2;
	}
	t23 = t31;
	} else {
#line 362 "<embedded>/clojure/test.clj"
	bool o32;
	clj_value t32 = clj_c_var_borrow(V[13], &o32);
	if (t32 == CLJ_THROWN) {
	goto L2;
	}
	bool o33;
	clj_value t33 = clj_c_var_borrow(V[8], &o33);
	if (t33 == CLJ_THROWN) {
	if (o32) clj_release(t32);
	goto L2;
	}
	clj_value t34 = K[386];
	clj_value a11[1] = {t34};
	(void)a11;
	clj_value t35 = clj_c_invoke(t33, a11, 1);
	(void)t34;
	if (o33) clj_release(t33);
	if (t35 == CLJ_THROWN) {
	if (o32) clj_release(t32);
	goto L2;
	}
	bool o36;
	clj_value t36 = clj_c_var_borrow(V[8], &o36);
	if (t36 == CLJ_THROWN) {
	clj_release(t35);
	if (o32) clj_release(t32);
	goto L2;
	}
	clj_value t37 = l3;
	clj_value a12[1] = {t37};
	(void)a12;
	clj_value t38 = clj_c_invoke(t36, a12, 1);
	(void)t37;
	if (o36) clj_release(t36);
	if (t38 == CLJ_THROWN) {
	clj_release(t35);
	if (o32) clj_release(t32);
	goto L2;
	}
	clj_value a10[2] = {t35, t38};
	(void)a10;
	clj_value t39;
#ifdef CLJC_DIRECT_clojure_core_concat_a2
	if (!CLJC_FN_clojure_core_concat_a2) CLJC_FN_clojure_core_concat_a2 = clj_compiled_symbol("clojure_core_concat_a2");
	t39 = CLJC_FN_clojure_core_concat_a2 ? CLJC_FN_clojure_core_concat_a2(clj_var_root_relaxed(V[13]), NULL, a10, 2) : clj_c_invoke(t32, a10, 2);
#else
	t39 = clj_c_invoke(t32, a10, 2);
#endif
	clj_release(t38);
	clj_release(t35);
	if (o32) clj_release(t32);
	if (t39 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a9[1] = {t39};
	(void)a9;
	clj_value t40;
	if (CLJC_GUARD(V[12], B[0])) {
	t40 = clj_seq(t39);
	} else {
	t40 = clj_c_intrinsic_fallback(V[12], a9, 1);
	}
	clj_release(t39);
	if (t40 == CLJ_THROWN) {
	goto L2;
	}
	t23 = t40;
	}
	t6 = t23;
	}
	clj_c_leave(&S[54], &cc);
	clj_release(l3);
	return t6;
L2: ;
	clj_c_leave(&S[54], &cc);
L1: ;
	clj_release(l3);
	return CLJ_THROWN;
}

static clj_value clojure_test_run_test(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 3: return clojure_test_run_test_a3(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_75(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 358 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[380], clojure_test_run_test, NULL, 0, 0x8, 3, 3);
	clj_var_bind_root(V[114], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[387]);
	clj_value t2 = clj_c_def(V[114], t1, true, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

clj_value clojure_test_successful_QMARK__a1(clj_value self, const clj_value *captured, const clj_value *args, size_t nargs) {
	(void)self; (void)captured; (void)args; (void)nargs;
	clj_cframe fr = {NULL, captured, 0, NULL};
	(void)fr;
	clj_value l0 = args[0];
	(void)l0;
	clj_value l1 = CLJ_NIL;
	clj_ccall cc;
	if (!clj_c_enter(&S[55], &cc)) goto L1;
#line 370 "<embedded>/clojure/test.clj"
	clj_value t0 = K[390];
	clj_value t1 = l0;
	clj_value t2 = K[41];
	clj_value a1[2] = {t1, t2};
	(void)a1;
	clj_value t3 = clj_c_invoke(t0, a1, 2);
	(void)t2;
	(void)t1;
	(void)t0;
	if (t3 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a0[1] = {t3};
	(void)a0;
	clj_value t4;
	if (CLJC_GUARD(V[73], B[17])) {
	t4 = clj_zero_p(t3);
	} else {
	t4 = clj_c_intrinsic_fallback(V[73], a0, 1);
	}
	clj_release(t3);
	if (t4 == CLJ_THROWN) {
	goto L2;
	}
	clj_c_rebind(&l1, t4);
	clj_value t5 = l1;
	bool t6 = clj_truthy(t5);
	(void)t5;
	clj_value t7;
	if (t6) {
#line 371 "<embedded>/clojure/test.clj"
	clj_value t8 = K[391];
	clj_value t9 = l0;
	clj_value t10 = K[41];
	clj_value a3[2] = {t9, t10};
	(void)a3;
	clj_value t11 = clj_c_invoke(t8, a3, 2);
	(void)t10;
	(void)t9;
	(void)t8;
	if (t11 == CLJ_THROWN) {
	goto L2;
	}
	clj_value a2[1] = {t11};
	(void)a2;
	clj_value t12;
	if (CLJC_GUARD(V[73], B[17])) {
	t12 = clj_zero_p(t11);
	} else {
	t12 = clj_c_intrinsic_fallback(V[73], a2, 1);
	}
	clj_release(t11);
	if (t12 == CLJ_THROWN) {
	goto L2;
	}
	t7 = t12;
	} else {
#line 370 "<embedded>/clojure/test.clj"
	clj_value t13 = l1;
	l1 = CLJ_NIL;
	t7 = t13;
	}
	clj_c_leave(&S[55], &cc);
	clj_release(l1);
	return t7;
L2: ;
	clj_c_leave(&S[55], &cc);
L1: ;
	clj_release(l1);
	return CLJ_THROWN;
}

static clj_value clojure_test_successful_QMARK_(void *ctx, const clj_value *args, size_t n) {
	const clj_fn *f = ctx;
	switch (n) {
	case 1: return clojure_test_successful_QMARK__a1(clj_from_ptr((void *)f), f->env, args, n);
	default: break;
	}
	return clj_c_arity_error(clj_from_ptr((void *)f), n);
}

static clj_value top_76(void) {
	clj_cframe fr = {NULL, NULL, 0, NULL};
	(void)fr;
	clj_eval_top_enter();
#line 367 "<embedded>/clojure/test.clj"
	clj_value t0 = clj_c_closure(K[389], clojure_test_successful_QMARK_, NULL, 0, 0x2, 1, 1);
	clj_var_bind_root(V[115], t0);
	clj_release(t0);
	clj_value t1 = clj_retain(K[392]);
	clj_value t2 = clj_c_def(V[115], t1, false, false);
	clj_release(t1);
	if (t2 == CLJ_THROWN) {
	goto L1;
	}
	clj_eval_top_leave();
	return t2;
L1: ;
	clj_eval_top_leave();
	return CLJ_THROWN;
}

static bool pools_filled;

static void unit_pools(void) {
	(void)K; (void)V; (void)B; (void)OP; (void)F; (void)S;
	pools_filled = true;
	V[0] = clj_c_var("clojure.core", "in-ns");
	V[1] = clj_c_var("clojure.core", "refer-clojure");
	V[2] = clj_c_var("clojure.core", "require");
	V[3] = clj_c_var("clojure.test", "*load-tests*");
	V[4] = clj_c_var("clojure.test", "*stack-trace-depth*");
	V[5] = clj_c_var("clojure.test", "*report-counters*");
	V[6] = clj_c_var("clojure.test", "*initial-report-counters*");
	V[7] = clj_c_var("clojure.test", "*testing-vars*");
	V[8] = clj_c_var("clojure.core", "list");
	V[9] = clj_c_var("clojure.test", "*testing-contexts*");
	V[10] = clj_c_var("clojure.test", "*assertion-pos*");
	V[11] = clj_c_var("clojure.test", "with-test-out");
	V[12] = clj_c_var("clojure.core", "seq");
	V[13] = clj_c_var("clojure.core", "concat");
	V[14] = clj_c_var("clojure.test", "testing-vars-str");
	V[15] = clj_c_var("clojure.core", "seq?");
	V[16] = clj_c_var("clojure.core", "next");
	V[17] = clj_c_var("clojure.core", "apply");
	V[18] = clj_c_var("clojure.core", "hash-map");
	V[19] = clj_c_var("clojure.core", "first");
	V[20] = clj_c_var("clojure.core", "get");
	V[21] = clj_c_var("clojure.core", "str");
	V[22] = clj_c_var("clojure.core", "reverse");
	V[23] = clj_c_var("clojure.core", "map");
	V[24] = clj_c_var("clojure.core", "meta");
	V[25] = clj_c_var("clojure.test", "testing-contexts-str");
	V[26] = clj_c_var("clojure.core", "interpose");
	V[27] = clj_c_var("clojure.test", "inc-report-counter");
	V[28] = clj_c_var("clojure.core", "swap!");
	V[29] = clj_c_var("clojure.core", "update");
	V[30] = clj_c_var("clojure.core", "fnil");
	V[31] = clj_c_var("clojure.core", "inc");
	V[32] = clj_c_var("clojure.test", "report");
	V[33] = clj_c_var("clojure.core", "bound?");
	V[34] = clj_c_var("clojure.core", "->MultiFn");
	V[35] = clj_c_var("clojure.core", "global-hierarchy");
	V[36] = clj_c_var("clojure.core", "atom");
	V[37] = clj_c_var("clojure.test", "print-trace");
	V[38] = clj_c_var("clojure.core", "ex-trace");
	V[39] = clj_c_var("clojure.core", "take");
	V[40] = clj_c_var("clojure.core", "println");
	V[41] = clj_c_var("clojure.core", "-add-method");
	V[42] = clj_c_var("clojure.core", "prn");
	V[43] = clj_c_var("clojure.core", "pr-str");
	V[44] = clj_c_var("clojure.core", "+");
	V[45] = clj_c_var("clojure.core", "ns-name");
	V[46] = clj_c_var("clojure.test", "do-report");
	V[47] = clj_c_var("clojure.core", "=");
	V[48] = clj_c_var("clojure.core", "nil?");
	V[49] = clj_c_var("clojure.core", "merge");
	V[50] = clj_c_var("clojure.test", "function?");
	V[51] = clj_c_var("clojure.core", "symbol?");
	V[52] = clj_c_var("clojure.core", "resolve");
	V[53] = clj_c_var("clojure.core", "not");
	V[54] = clj_c_var("clojure.core", "fn?");
	V[55] = clj_c_var("clojure.core", "deref");
	V[56] = clj_c_var("clojure.test", "assert-predicate");
	V[57] = clj_c_var("clojure.core", "rest");
	V[58] = clj_c_var("clojure.core", "vector");
	V[59] = clj_c_var("clojure.test", "assert-any");
	V[60] = clj_c_var("clojure.test", "assert-expr");
	V[61] = clj_c_var("clojure.core", "sequential?");
	V[62] = clj_c_var("clojure.core", "nth");
	V[63] = clj_c_var("clojure.core", "second");
	V[64] = clj_c_var("clojure.core", "nthnext");
	V[65] = clj_c_var("clojure.test", "try-expr");
	V[66] = clj_c_var("clojure.test", "is");
	V[67] = clj_c_var("clojure.core", "with-meta");
	V[68] = clj_c_var("clojure.core", "*file*");
	V[69] = clj_c_var("clojure.test", "are");
	V[70] = clj_c_var("clojure.core", "empty?");
	V[71] = clj_c_var("clojure.core", "pos?");
	V[72] = clj_c_var("clojure.core", "count");
	V[73] = clj_c_var("clojure.core", "zero?");
	V[74] = clj_c_var("clojure.core", "mod");
	V[75] = clj_c_var("clojure.core", "ex-info");
	V[76] = clj_c_var("clojure.test", "testing");
	V[77] = clj_c_var("clojure.test", "with-test");
	V[78] = clj_c_var("clojure.test", "deftest");
	V[79] = clj_c_var("clojure.core", "vary-meta");
	V[80] = clj_c_var("clojure.core", "assoc");
	V[81] = clj_c_var("clojure.test", "deftest-");
	V[82] = clj_c_var("clojure.test", "set-test");
	V[83] = clj_c_var("clojure.test", "fixtures");
	V[84] = clj_c_var("clojure.test", "use-fixtures");
	V[85] = clj_c_var("clojure.core", "assoc-in");
	V[86] = clj_c_var("clojure.core", "*ns*");
	V[87] = clj_c_var("clojure.test", "default-fixture");
	V[88] = clj_c_var("clojure.test", "compose-fixtures");
	V[89] = clj_c_var("clojure.test", "join-fixtures");
	V[90] = clj_c_var("clojure.core", "reduce");
	V[91] = clj_c_var("clojure.test", "ns-key");
	V[92] = clj_c_var("clojure.test", "test-var");
	V[93] = clj_c_var("clojure.core", "push-thread-bindings");
	V[94] = clj_c_var("clojure.core", "conj");
	V[95] = clj_c_var("clojure.core", "pop-thread-bindings");
	V[96] = clj_c_var("clojure.test", "test-vars");
	V[97] = clj_c_var("clojure.core", "group-by");
	V[98] = clj_c_var("clojure.core", "comp");
	V[99] = clj_c_var("clojure.test", "test-all-vars");
	V[100] = clj_c_var("clojure.core", "sort-by");
	V[101] = clj_c_var("clojure.core", "vals");
	V[102] = clj_c_var("clojure.core", "ns-interns");
	V[103] = clj_c_var("clojure.test", "test-ns");
	V[104] = clj_c_var("clojure.core", "the-ns");
	V[105] = clj_c_var("clojure.core", "var-get");
	V[106] = clj_c_var("clojure.test", "run-tests");
	V[107] = clj_c_var("clojure.core", "merge-with");
	V[108] = clj_c_var("clojure.test", "run-all-tests");
	V[109] = clj_c_var("clojure.core", "all-ns");
	V[110] = clj_c_var("clojure.core", "regex?");
	V[111] = clj_c_var("clojure.core", "re-find");
	V[112] = clj_c_var("clojure.core", "filter");
	V[113] = clj_c_var("clojure.test", "run-test-var");
	V[114] = clj_c_var("clojure.test", "run-test");
	V[115] = clj_c_var("clojure.test", "successful?");
	K[0] = clj_c_const("\"<embedded>/clojure/test.clj\"", 29);
	K[1] = clj_c_const("clojure.test", 12);
	K[2] = clj_c_const("[clojure.template :as temp]", 27);
	K[3] = clj_c_const("nil", 3);
	K[4] = clj_c_const("*load-tests*", 12);
	K[5] = clj_c_const("true", 4);
	K[6] = clj_c_const("{:ns clojure.test, :dynamic true, :file \"<embedded>/clojure/test.clj\", :doc \"True by default. If set to false, no test functions will be created by deftest.\", :name *load-tests*, :line 6, :column 1}", 198);
	K[7] = clj_c_const("*stack-trace-depth*", 19);
	K[8] = clj_c_const("{:ns clojure.test, :dynamic true, :file \"<embedded>/clojure/test.clj\", :name *stack-trace-depth*, :line 10, :column 1}", 118);
	K[9] = clj_c_const("*report-counters*", 17);
	K[10] = clj_c_const("{:ns clojure.test, :dynamic true, :file \"<embedded>/clojure/test.clj\", :name *report-counters*, :line 12, :column 1}", 116);
	K[11] = clj_c_const("*initial-report-counters*", 25);
	K[12] = clj_c_const("{:pass 0, :test 0, :error 0, :fail 0}", 37);
	K[13] = clj_c_const("{:ns clojure.test, :dynamic true, :file \"<embedded>/clojure/test.clj\", :name *initial-report-counters*, :line 14, :column 1}", 124);
	K[14] = clj_c_const("*testing-vars*", 14);
	K[15] = clj_c_const("{:ns clojure.test, :dynamic true, :file \"<embedded>/clojure/test.clj\", :name *testing-vars*, :line 16, :column 1}", 113);
	K[16] = clj_c_const("*testing-contexts*", 18);
	K[17] = clj_c_const("{:ns clojure.test, :dynamic true, :file \"<embedded>/clojure/test.clj\", :name *testing-contexts*, :line 18, :column 1}", 117);
	K[18] = clj_c_const("*assertion-pos*", 15);
	K[19] = clj_c_const("{:ns clojure.test, :dynamic true, :file \"<embedded>/clojure/test.clj\", :doc \"{:file :line :column} of the `is` form being evaluated; nil outside one.\", :name *assertion-pos*, :line 20, :column 1}", 195);
	K[20] = clj_c_const("with-test-out", 13);
	K[21] = clj_c_const("clojure.test/with-test-out", 26);
	K[22] = clj_c_const("do", 2);
	K[23] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([& body]), :macro true, :doc \"Runs body; output goes wherever println goes here.\", :name with-test-out, :line 24, :column 1}", 191);
	K[24] = clj_c_const("testing-vars-str", 16);
	K[25] = clj_c_const("clojure.test/testing-vars-str", 29);
	K[26] = clj_c_const("{}", 2);
	K[27] = clj_c_const(":file", 5);
	K[28] = clj_c_const(":line", 5);
	K[29] = clj_c_const(":name", 5);
	K[30] = clj_c_const("\" (\"", 4);
	K[31] = clj_c_const("\"NO_SOURCE_PATH\"", 16);
	K[32] = clj_c_const("\":\"", 3);
	K[33] = clj_c_const("\")\"", 3);
	K[34] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([m]), :doc \"Returns a string representation of the current test, with the file and line of the assertion.\", :name testing-vars-str, :line 29, :column 1}", 219);
	K[35] = clj_c_const("testing-contexts-str", 20);
	K[36] = clj_c_const("clojure.test/testing-contexts-str", 33);
	K[37] = clj_c_const("\" \"", 3);
	K[38] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([]), :doc \"Returns a string representation of the current test context.\", :name testing-contexts-str, :line 36, :column 1}", 189);
	K[39] = clj_c_const("inc-report-counter", 18);
	K[40] = clj_c_const("clojure.test/inc-report-counter", 31);
	K[41] = clj_c_const("0", 1);
	K[42] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([name]), :doc \"Increments the named counter in *report-counters*, when bound.\", :name inc-report-counter, :line 41, :column 1}", 193);
	K[43] = clj_c_const("report", 6);
	K[44] = clj_c_const("{:ns clojure.test, :dynamic true, :file \"<embedded>/clojure/test.clj\", :doc \"Generic reporting function; rebind to plug in a custom reporter. Dispatches on :type.\", :name report, :line 47, :column 1}", 199);
	K[45] = clj_c_const(":type", 5);
	K[46] = clj_c_const(":default", 8);
	K[47] = clj_c_const("{}", 2);
	K[48] = clj_c_const("[:clojure.core/none {}]", 23);
	K[49] = clj_c_const("{:ns clojure.test, :dynamic true, :file \"<embedded>/clojure/test.clj\", :doc \"Generic reporting function; rebind to plug in a custom reporter. Dispatches on :type.\", :name report, :line 47, :column 1}", 199);
	K[50] = clj_c_const("print-trace", 11);
	K[51] = clj_c_const("clojure.test/print-trace", 24);
	K[52] = clj_c_const("\"    at\"", 8);
	K[53] = clj_c_const(":fn", 3);
	K[54] = clj_c_const("\"fn\"", 4);
	K[55] = clj_c_const("\"(\"", 3);
	K[56] = clj_c_const(":line", 5);
	K[57] = clj_c_const("\":\"", 3);
	K[58] = clj_c_const(":column", 7);
	K[59] = clj_c_const("\")\"", 3);
	K[60] = clj_c_const("{:ns clojure.test, :private true, :file \"<embedded>/clojure/test.clj\", :arglists ([e]), :name print-trace, :line 51, :column 1}", 127);
	K[61] = clj_c_const("report", 6);
	K[62] = clj_c_const(":default", 8);
	K[63] = clj_c_const("report", 6);
	K[64] = clj_c_const(":pass", 5);
	K[65] = clj_c_const("report", 6);
	K[66] = clj_c_const(":fail", 5);
	K[67] = clj_c_const("\"\\nFAIL in\"", 11);
	K[68] = clj_c_const(":message", 8);
	K[69] = clj_c_const("\"expected:\"", 11);
	K[70] = clj_c_const(":expected", 9);
	K[71] = clj_c_const("\"  actual:\"", 11);
	K[72] = clj_c_const(":actual", 7);
	K[73] = clj_c_const("report", 6);
	K[74] = clj_c_const(":error", 6);
	K[75] = clj_c_const("\"\\nERROR in\"", 12);
	K[76] = clj_c_const(":message", 8);
	K[77] = clj_c_const("\"expected:\"", 11);
	K[78] = clj_c_const(":expected", 9);
	K[79] = clj_c_const("\"  actual:\"", 11);
	K[80] = clj_c_const(":actual", 7);
	K[81] = clj_c_const("report", 6);
	K[82] = clj_c_const(":summary", 8);
	K[83] = clj_c_const("\"\\nRan\"", 7);
	K[84] = clj_c_const(":test", 5);
	K[85] = clj_c_const("\"tests containing\"", 18);
	K[86] = clj_c_const(":pass", 5);
	K[87] = clj_c_const(":fail", 5);
	K[88] = clj_c_const(":error", 6);
	K[89] = clj_c_const("\"assertions.\"", 13);
	K[90] = clj_c_const("\"failures,\"", 11);
	K[91] = clj_c_const("\"errors.\"", 9);
	K[92] = clj_c_const("report", 6);
	K[93] = clj_c_const(":begin-test-ns", 14);
	K[94] = clj_c_const("\"\\nTesting\"", 11);
	K[95] = clj_c_const(":ns", 3);
	K[96] = clj_c_const("report", 6);
	K[97] = clj_c_const(":end-test-ns", 12);
	K[98] = clj_c_const("report", 6);
	K[99] = clj_c_const(":begin-test-var", 15);
	K[100] = clj_c_const("report", 6);
	K[101] = clj_c_const(":end-test-var", 13);
	K[102] = clj_c_const("do-report", 9);
	K[103] = clj_c_const("clojure.test/do-report", 22);
	K[104] = clj_c_const(":fail", 5);
	K[105] = clj_c_const(":type", 5);
	K[106] = clj_c_const(":error", 6);
	K[107] = clj_c_const(":line", 5);
	K[108] = clj_c_const(":file", 5);
	K[109] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([m]), :doc \"Adds the position of the `is` form, or of the current test var, to a :fail or :error that carries none.\", :name do-report, :line 91, :column 1}", 222);
	K[110] = clj_c_const("function?", 9);
	K[111] = clj_c_const("clojure.test/function?", 22);
	K[112] = clj_c_const(":macro", 6);
	K[113] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([x]), :doc \"Returns true if argument is a function or a symbol that resolves to a function (not a macro).\", :name function?, :line 102, :column 1}", 213);
	K[114] = clj_c_const("assert-predicate", 16);
	K[115] = clj_c_const("clojure.test/assert-predicate", 29);
	K[116] = clj_c_const("clojure.core/let", 16);
	K[117] = clj_c_const("values__145__auto__", 19);
	K[118] = clj_c_const("clojure.core/list", 17);
	K[119] = clj_c_const("result__146__auto__", 19);
	K[120] = clj_c_const("clojure.core/apply", 18);
	K[121] = clj_c_const("if", 2);
	K[122] = clj_c_const("clojure.test/do-report", 22);
	K[123] = clj_c_const(":expected", 9);
	K[124] = clj_c_const("quote", 5);
	K[125] = clj_c_const(":type", 5);
	K[126] = clj_c_const(":pass", 5);
	K[127] = clj_c_const(":actual", 7);
	K[128] = clj_c_const("clojure.core/cons", 17);
	K[129] = clj_c_const("quote", 5);
	K[130] = clj_c_const(":message", 8);
	K[131] = clj_c_const("clojure.test/do-report", 22);
	K[132] = clj_c_const("quote", 5);
	K[133] = clj_c_const(":fail", 5);
	K[134] = clj_c_const("clojure.core/list", 17);
	K[135] = clj_c_const("quote", 5);
	K[136] = clj_c_const("not", 3);
	K[137] = clj_c_const("clojure.core/cons", 17);
	K[138] = clj_c_const("quote", 5);
	K[139] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([msg form]), :doc \"Returns generic assertion code for any functional predicate: the expected and actual arguments are\\n  evaluated once and reported with the predicate.\", :name assert-predicate, :line 110, :column 1}", 283);
	K[140] = clj_c_const("assert-any", 10);
	K[141] = clj_c_const("clojure.test/assert-any", 23);
	K[142] = clj_c_const("clojure.core/let", 16);
	K[143] = clj_c_const("value__147__auto__", 18);
	K[144] = clj_c_const("if", 2);
	K[145] = clj_c_const("clojure.test/do-report", 22);
	K[146] = clj_c_const(":expected", 9);
	K[147] = clj_c_const("quote", 5);
	K[148] = clj_c_const(":type", 5);
	K[149] = clj_c_const(":pass", 5);
	K[150] = clj_c_const(":actual", 7);
	K[151] = clj_c_const(":message", 8);
	K[152] = clj_c_const("clojure.test/do-report", 22);
	K[153] = clj_c_const("quote", 5);
	K[154] = clj_c_const(":fail", 5);
	K[155] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([msg form]), :doc \"Returns generic assertion code for any test, including macros, Java method calls, or isolated symbols.\", :name assert-any, :line 125, :column 1}", 230);
	K[156] = clj_c_const("assert-expr", 11);
	K[157] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :doc \"Extension point for `is`: dispatches on the head of the asserted form.\", :name assert-expr, :line 136, :column 1}", 175);
	K[158] = clj_c_const(":always-fail", 12);
	K[159] = clj_c_const(":default", 8);
	K[160] = clj_c_const("{}", 2);
	K[161] = clj_c_const("[:clojure.core/none {}]", 23);
	K[162] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :doc \"Extension point for `is`: dispatches on the head of the asserted form.\", :name assert-expr, :line 136, :column 1}", 175);
	K[163] = clj_c_const("assert-expr", 11);
	K[164] = clj_c_const(":always-fail", 12);
	K[165] = clj_c_const("clojure.test/do-report", 22);
	K[166] = clj_c_const(":type", 5);
	K[167] = clj_c_const(":fail", 5);
	K[168] = clj_c_const(":message", 8);
	K[169] = clj_c_const("assert-expr", 11);
	K[170] = clj_c_const(":default", 8);
	K[171] = clj_c_const("assert-expr", 11);
	K[172] = clj_c_const("instance?", 9);
	K[173] = clj_c_const("clojure.core/let", 16);
	K[174] = clj_c_const("klass__148__auto__", 18);
	K[175] = clj_c_const("1", 1);
	K[176] = clj_c_const("object__149__auto__", 19);
	K[177] = clj_c_const("2", 1);
	K[178] = clj_c_const("clojure.core/let", 16);
	K[179] = clj_c_const("result__150__auto__", 19);
	K[180] = clj_c_const("clojure.core/instance?", 22);
	K[181] = clj_c_const("if", 2);
	K[182] = clj_c_const("clojure.test/do-report", 22);
	K[183] = clj_c_const(":expected", 9);
	K[184] = clj_c_const("quote", 5);
	K[185] = clj_c_const(":type", 5);
	K[186] = clj_c_const(":pass", 5);
	K[187] = clj_c_const(":actual", 7);
	K[188] = clj_c_const("clojure.core/type", 17);
	K[189] = clj_c_const(":message", 8);
	K[190] = clj_c_const("clojure.test/do-report", 22);
	K[191] = clj_c_const("quote", 5);
	K[192] = clj_c_const(":fail", 5);
	K[193] = clj_c_const("clojure.core/type", 17);
	K[194] = clj_c_const("assert-expr", 11);
	K[195] = clj_c_const("thrown?", 7);
	K[196] = clj_c_const("try", 3);
	K[197] = clj_c_const("clojure.test/do-report", 22);
	K[198] = clj_c_const(":expected", 9);
	K[199] = clj_c_const("quote", 5);
	K[200] = clj_c_const(":type", 5);
	K[201] = clj_c_const(":fail", 5);
	K[202] = clj_c_const(":actual", 7);
	K[203] = clj_c_const(":message", 8);
	K[204] = clj_c_const("catch", 5);
	K[205] = clj_c_const("e__151__auto__", 14);
	K[206] = clj_c_const("clojure.test/do-report", 22);
	K[207] = clj_c_const("quote", 5);
	K[208] = clj_c_const(":pass", 5);
	K[209] = clj_c_const("assert-expr", 11);
	K[210] = clj_c_const("thrown-with-msg?", 16);
	K[211] = clj_c_const("3", 1);
	K[212] = clj_c_const("try", 3);
	K[213] = clj_c_const("clojure.test/do-report", 22);
	K[214] = clj_c_const(":expected", 9);
	K[215] = clj_c_const("quote", 5);
	K[216] = clj_c_const(":type", 5);
	K[217] = clj_c_const(":fail", 5);
	K[218] = clj_c_const(":actual", 7);
	K[219] = clj_c_const(":message", 8);
	K[220] = clj_c_const("catch", 5);
	K[221] = clj_c_const("e__152__auto__", 14);
	K[222] = clj_c_const("clojure.core/let", 16);
	K[223] = clj_c_const("m__153__auto__", 14);
	K[224] = clj_c_const("clojure.core/ex-message", 23);
	K[225] = clj_c_const("if", 2);
	K[226] = clj_c_const("clojure.core/and", 16);
	K[227] = clj_c_const("clojure.core/string?", 20);
	K[228] = clj_c_const("clojure.core/some?", 18);
	K[229] = clj_c_const("clojure.core/re-find", 20);
	K[230] = clj_c_const("clojure.test/do-report", 22);
	K[231] = clj_c_const("quote", 5);
	K[232] = clj_c_const(":pass", 5);
	K[233] = clj_c_const("clojure.test/do-report", 22);
	K[234] = clj_c_const("quote", 5);
	K[235] = clj_c_const("try-expr", 8);
	K[236] = clj_c_const("clojure.test/try-expr", 21);
	K[237] = clj_c_const("try", 3);
	K[238] = clj_c_const("catch", 5);
	K[239] = clj_c_const(":default", 8);
	K[240] = clj_c_const("t__154__auto__", 14);
	K[241] = clj_c_const("clojure.test/do-report", 22);
	K[242] = clj_c_const(":expected", 9);
	K[243] = clj_c_const("quote", 5);
	K[244] = clj_c_const(":type", 5);
	K[245] = clj_c_const(":error", 6);
	K[246] = clj_c_const(":actual", 7);
	K[247] = clj_c_const(":message", 8);
	K[248] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([msg form]), :macro true, :doc \"Used by the `is` macro to catch unexpected exceptions.\", :name try-expr, :line 189, :column 1}", 193);
	K[249] = clj_c_const("is", 2);
	K[250] = clj_c_const("clojure.test/is", 15);
	K[251] = clj_c_const("clojure.test/is", 15);
	K[252] = clj_c_const("{}", 2);
	K[253] = clj_c_const(":line", 5);
	K[254] = clj_c_const(":column", 7);
	K[255] = clj_c_const("clojure.core/binding", 20);
	K[256] = clj_c_const("clojure.test/*assertion-pos*", 28);
	K[257] = clj_c_const(":file", 5);
	K[258] = clj_c_const("clojure.test/try-expr", 21);
	K[259] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([form] [form msg]), :macro true, :doc \"Generic assertion macro. 'form' is any predicate test. 'msg' is an optional message to attach.\\n  Special forms: (is (thrown? c body)), (is (thrown-with-msg? c re body)).\", :name is, :line 197, :column 1}", 310);
	K[260] = clj_c_const("are", 3);
	K[261] = clj_c_const("clojure.test/are", 16);
	K[262] = clj_c_const("clojure.template/do-template", 28);
	K[263] = clj_c_const("clojure.test/is", 15);
	K[264] = clj_c_const("\"The number of args doesn't match are's argv or testing doesn't have any args\"", 78);
	K[265] = clj_c_const("{}", 2);
	K[266] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([argv expr & args]), :macro true, :doc \"Checks multiple assertions with a template expression: (are [x y] (= x y) 2 (+ 1 1) 4 (* 2 2)).\", :name are, :line 206, :column 1}", 237);
	K[267] = clj_c_const("testing", 7);
	K[268] = clj_c_const("clojure.test/testing", 20);
	K[269] = clj_c_const("clojure.core/binding", 20);
	K[270] = clj_c_const("clojure.test/*testing-contexts*", 31);
	K[271] = clj_c_const("clojure.core/conj", 17);
	K[272] = clj_c_const("clojure.test/*testing-contexts*", 31);
	K[273] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([string & body]), :macro true, :doc \"Adds a new string to the list of testing contexts for the extent of body.\", :name testing, :line 217, :column 1}", 216);
	K[274] = clj_c_const("with-test", 9);
	K[275] = clj_c_const("clojure.test/with-test", 22);
	K[276] = clj_c_const("clojure.core/doto", 17);
	K[277] = clj_c_const("clojure.core/alter-meta!", 24);
	K[278] = clj_c_const("clojure.core/assoc", 18);
	K[279] = clj_c_const(":test", 5);
	K[280] = clj_c_const("clojure.core/fn", 15);
	K[281] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([definition & body]), :macro true, :doc \"Adds a test fn to the var defined by definition.\", :name with-test, :line 225, :column 1}", 197);
	K[282] = clj_c_const("deftest", 7);
	K[283] = clj_c_const("clojure.test/deftest", 20);
	K[284] = clj_c_const("def", 3);
	K[285] = clj_c_const(":test", 5);
	K[286] = clj_c_const("clojure.core/fn", 15);
	K[287] = clj_c_const("clojure.core/fn", 15);
	K[288] = clj_c_const("clojure.test/test-var", 21);
	K[289] = clj_c_const("var", 3);
	K[290] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([name & body]), :macro true, :doc \"Defines a test function with no arguments; the body becomes the var's :test.\", :name deftest, :line 232, :column 1}", 217);
	K[291] = clj_c_const("deftest-", 8);
	K[292] = clj_c_const("clojure.test/deftest-", 21);
	K[293] = clj_c_const("def", 3);
	K[294] = clj_c_const(":test", 5);
	K[295] = clj_c_const("clojure.core/fn", 15);
	K[296] = clj_c_const(":private", 8);
	K[297] = clj_c_const("clojure.core/fn", 15);
	K[298] = clj_c_const("clojure.test/test-var", 21);
	K[299] = clj_c_const("var", 3);
	K[300] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([name & body]), :macro true, :doc \"Like deftest but creates a private var.\", :name deftest-, :line 239, :column 1}", 181);
	K[301] = clj_c_const("set-test", 8);
	K[302] = clj_c_const("clojure.test/set-test", 21);
	K[303] = clj_c_const("clojure.core/alter-meta!", 24);
	K[304] = clj_c_const("var", 3);
	K[305] = clj_c_const("clojure.core/assoc", 18);
	K[306] = clj_c_const(":test", 5);
	K[307] = clj_c_const("clojure.core/fn", 15);
	K[308] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([name & body]), :macro true, :doc \"Sets the :test of an existing var.\", :name set-test, :line 246, :column 1}", 176);
	K[309] = clj_c_const("fixtures", 8);
	K[310] = clj_c_const("{}", 2);
	K[311] = clj_c_const("{:ns clojure.test, :private true, :file \"<embedded>/clojure/test.clj\", :name fixtures, :line 254, :column 1}", 108);
	K[312] = clj_c_const("use-fixtures", 12);
	K[313] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :doc \"Wrap test runs in a fixture function to perform setup and teardown: (use-fixtures :each f) around every\\n  test var, (use-fixtures :once f) around the whole namespace run.\", :name use-fixtures, :line 256, :column 1}", 277);
	K[314] = clj_c_const(":default", 8);
	K[315] = clj_c_const("{}", 2);
	K[316] = clj_c_const("[:clojure.core/none {}]", 23);
	K[317] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :doc \"Wrap test runs in a fixture function to perform setup and teardown: (use-fixtures :each f) around every\\n  test var, (use-fixtures :once f) around the whole namespace run.\", :name use-fixtures, :line 256, :column 1}", 277);
	K[318] = clj_c_const("use-fixtures", 12);
	K[319] = clj_c_const(":each", 5);
	K[320] = clj_c_const("use-fixtures", 12);
	K[321] = clj_c_const(":once", 5);
	K[322] = clj_c_const("default-fixture", 15);
	K[323] = clj_c_const("clojure.test/default-fixture", 28);
	K[324] = clj_c_const("{:ns clojure.test, :private true, :file \"<embedded>/clojure/test.clj\", :arglists ([f]), :doc \"The default, empty, fixture function. Just calls its argument.\", :name default-fixture, :line 267, :column 1}", 203);
	K[325] = clj_c_const("compose-fixtures", 16);
	K[326] = clj_c_const("clojure.test/compose-fixtures", 29);
	K[327] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([f1 f2]), :doc \"Composes two fixture functions, creating a new fixture function that combines their behavior.\", :name compose-fixtures, :line 272, :column 1}", 224);
	K[328] = clj_c_const("join-fixtures", 13);
	K[329] = clj_c_const("clojure.test/join-fixtures", 26);
	K[330] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([fixtures]), :doc \"Composes a collection of fixtures, in order. Always returns a valid fixture function.\", :name join-fixtures, :line 277, :column 1}", 216);
	K[331] = clj_c_const("ns-key", 6);
	K[332] = clj_c_const("clojure.test/ns-key", 19);
	K[333] = clj_c_const("{:ns clojure.test, :private true, :file \"<embedded>/clojure/test.clj\", :arglists ([ns]), :name ns-key, :line 282, :column 1}", 124);
	K[334] = clj_c_const("test-var", 8);
	K[335] = clj_c_const("clojure.test/test-var", 21);
	K[336] = clj_c_const(":test", 5);
	K[337] = clj_c_const(":var", 4);
	K[338] = clj_c_const(":type", 5);
	K[339] = clj_c_const(":begin-test-var", 15);
	K[340] = clj_c_const(":expected", 9);
	K[341] = clj_c_const(":error", 6);
	K[342] = clj_c_const(":actual", 7);
	K[343] = clj_c_const(":message", 8);
	K[344] = clj_c_const("\"Uncaught exception, not in assertion.\"", 39);
	K[345] = clj_c_const(":end-test-var", 13);
	K[346] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([v]), :doc \"If v has a function in its :test metadata, calls that function, with *testing-vars* bound to (conj *testing-vars* v).\", :name test-var, :line 286, :column 1}", 236);
	K[347] = clj_c_const("test-vars", 9);
	K[348] = clj_c_const("clojure.test/test-vars", 22);
	K[349] = clj_c_const(":ns", 3);
	K[350] = clj_c_const(":once", 5);
	K[351] = clj_c_const(":each", 5);
	K[352] = clj_c_const(":test", 5);
	K[353] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([vars]), :doc \"Groups vars by their namespace and runs test-var on them with appropriate fixtures assuming they are\\n  present in the current namespace.\", :name test-vars, :line 299, :column 1}", 260);
	K[354] = clj_c_const("test-all-vars", 13);
	K[355] = clj_c_const("clojure.test/test-all-vars", 26);
	K[356] = clj_c_const(":line", 5);
	K[357] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([ns]), :doc \"Calls test-vars on every var interned in the namespace, in file order.\", :name test-all-vars, :line 313, :column 1}", 195);
	K[358] = clj_c_const("test-ns", 7);
	K[359] = clj_c_const("clojure.test/test-ns", 20);
	K[360] = clj_c_const(":ns", 3);
	K[361] = clj_c_const(":type", 5);
	K[362] = clj_c_const(":begin-test-ns", 14);
	K[363] = clj_c_const("test-ns-hook", 12);
	K[364] = clj_c_const(":end-test-ns", 12);
	K[365] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([ns]), :doc \"Runs the tests of one namespace (a symbol or namespace) under fresh report counters, which it returns.\\n  A test-ns-hook var replaces test-all-vars.\", :name test-ns, :line 318, :column 1}", 267);
	K[366] = clj_c_const("run-tests", 9);
	K[367] = clj_c_const("clojure.test/run-tests", 22);
	K[368] = clj_c_const(":type", 5);
	K[369] = clj_c_const(":summary", 8);
	K[370] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([] [& namespaces]), :doc \"Runs all tests in the given namespaces; prints results. Defaults to current namespace if none given.\\n  Returns a map summarizing test results.\", :name run-tests, :line 331, :column 1}", 277);
	K[371] = clj_c_const("run-all-tests", 13);
	K[372] = clj_c_const("clojure.test/run-all-tests", 26);
	K[373] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([] [re-or-pred]), :doc \"Runs all tests in all namespaces; prints results. Optional argument is a regex or a predicate\\n  matched against each namespace name.\", :name run-all-tests, :line 341, :column 1}", 269);
	K[374] = clj_c_const("run-test-var", 12);
	K[375] = clj_c_const("clojure.test/run-test-var", 25);
	K[376] = clj_c_const(":type", 5);
	K[377] = clj_c_const(":summary", 8);
	K[378] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([v]), :doc \"Runs the tests for a single var, with fixtures executed around the test, and summarizes the results.\", :name run-test-var, :line 349, :column 1}", 223);
	K[379] = clj_c_const("run-test", 8);
	K[380] = clj_c_const("clojure.test/run-test", 21);
	K[381] = clj_c_const("\"Unable to resolve \"", 20);
	K[382] = clj_c_const("\" to a test function.\"", 22);
	K[383] = clj_c_const("{}", 2);
	K[384] = clj_c_const(":test", 5);
	K[385] = clj_c_const("\" is not a test.\"", 17);
	K[386] = clj_c_const("clojure.test/run-test-var", 25);
	K[387] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([test-symbol]), :macro true, :doc \"Runs a single test. Because the intent is to run a single test, there is no check for the namespace test-ns-hook.\", :name run-test, :line 358, :column 1}", 255);
	K[388] = clj_c_const("successful?", 11);
	K[389] = clj_c_const("clojure.test/successful?", 24);
	K[390] = clj_c_const(":fail", 5);
	K[391] = clj_c_const(":error", 6);
	K[392] = clj_c_const("{:ns clojure.test, :file \"<embedded>/clojure/test.clj\", :arglists ([summary]), :doc \"Returns true if the given test summary indicates all tests were successful, false otherwise.\", :name successful?, :line 367, :column 1}", 220);
	OP[0] = clj_c_intrinsic("clojure.core/seq", 1);
	B[0] = clj_intrinsic_builtin(OP[0]);
	OP[1] = clj_c_intrinsic("clojure.core/seq?", 1);
	B[1] = clj_intrinsic_builtin(OP[1]);
	OP[2] = clj_c_intrinsic("clojure.core/next", 1);
	B[2] = clj_intrinsic_builtin(OP[2]);
	OP[3] = clj_c_intrinsic("clojure.core/first", 1);
	B[3] = clj_intrinsic_builtin(OP[3]);
	OP[4] = clj_c_intrinsic("clojure.core/get", 2);
	B[4] = clj_intrinsic_builtin(OP[4]);
	OP[5] = clj_c_intrinsic("clojure.core/=", 2);
	B[5] = clj_intrinsic_builtin(OP[5]);
	OP[6] = clj_c_intrinsic("clojure.core/nil?", 1);
	B[6] = clj_intrinsic_builtin(OP[6]);
	OP[7] = clj_c_intrinsic("clojure.core/symbol?", 1);
	B[7] = clj_intrinsic_builtin(OP[7]);
	OP[8] = clj_c_intrinsic("clojure.core/not", 1);
	B[8] = clj_intrinsic_builtin(OP[8]);
	OP[9] = clj_c_intrinsic("clojure.core/fn?", 1);
	B[9] = clj_intrinsic_builtin(OP[9]);
	OP[10] = clj_c_intrinsic("clojure.core/rest", 1);
	B[10] = clj_intrinsic_builtin(OP[10]);
	OP[11] = clj_c_intrinsic("clojure.core/sequential?", 1);
	B[11] = clj_intrinsic_builtin(OP[11]);
	OP[12] = clj_c_intrinsic("clojure.core/nth", 2);
	B[12] = clj_intrinsic_builtin(OP[12]);
	OP[13] = clj_c_intrinsic("clojure.core/with-meta", 2);
	B[13] = clj_intrinsic_builtin(OP[13]);
	OP[14] = clj_c_intrinsic("clojure.core/empty?", 1);
	B[14] = clj_intrinsic_builtin(OP[14]);
	OP[15] = clj_c_intrinsic("clojure.core/pos?", 1);
	B[15] = clj_intrinsic_builtin(OP[15]);
	OP[16] = clj_c_intrinsic("clojure.core/count", 1);
	B[16] = clj_intrinsic_builtin(OP[16]);
	OP[17] = clj_c_intrinsic("clojure.core/zero?", 1);
	B[17] = clj_intrinsic_builtin(OP[17]);
	OP[18] = clj_c_intrinsic("clojure.core/conj", 2);
	B[18] = clj_intrinsic_builtin(OP[18]);
	OP[19] = clj_c_intrinsic("clojure.core/nth", 3);
	B[19] = clj_intrinsic_builtin(OP[19]);
	OP[20] = clj_c_intrinsic("clojure.core/assoc", 3);
	B[20] = clj_intrinsic_builtin(OP[20]);
}

clj_value clj_compiled_lib_init_clojure_test(void) {
	clj_value r;
	(void)r;
	if (!pools_filled) unit_pools();
#ifdef CLJ_CLOSED
	clj_compiled_register_symbol("clojure_test_testing_vars_str_a1", clojure_test_testing_vars_str_a1);
	clj_compiled_register_symbol("clojure_test_testing_contexts_str_a0", clojure_test_testing_contexts_str_a0);
	clj_compiled_register_symbol("clojure_test_inc_report_counter_a1", clojure_test_inc_report_counter_a1);
	clj_compiled_register_symbol("clojure_test_print_trace_a1", clojure_test_print_trace_a1);
	clj_compiled_register_symbol("clojure_test_do_report_a1", clojure_test_do_report_a1);
	clj_compiled_register_symbol("clojure_test_function_QMARK__a1", clojure_test_function_QMARK__a1);
	clj_compiled_register_symbol("clojure_test_assert_predicate_a2", clojure_test_assert_predicate_a2);
	clj_compiled_register_symbol("clojure_test_assert_any_a2", clojure_test_assert_any_a2);
	clj_compiled_register_symbol("clojure_test_try_expr_a4", clojure_test_try_expr_a4);
	clj_compiled_register_symbol("clojure_test_is_a3", clojure_test_is_a3);
	clj_compiled_register_symbol("clojure_test_is_a4", clojure_test_is_a4);
	clj_compiled_register_symbol("clojure_test_default_fixture_a1", clojure_test_default_fixture_a1);
	clj_compiled_register_symbol("clojure_test_compose_fixtures_a2", clojure_test_compose_fixtures_a2);
	clj_compiled_register_symbol("clojure_test_join_fixtures_a1", clojure_test_join_fixtures_a1);
	clj_compiled_register_symbol("clojure_test_ns_key_a1", clojure_test_ns_key_a1);
	clj_compiled_register_symbol("clojure_test_test_var_a1", clojure_test_test_var_a1);
	clj_compiled_register_symbol("clojure_test_test_vars_a1", clojure_test_test_vars_a1);
	clj_compiled_register_symbol("clojure_test_test_all_vars_a1", clojure_test_test_all_vars_a1);
	clj_compiled_register_symbol("clojure_test_test_ns_a1", clojure_test_test_ns_a1);
	clj_compiled_register_symbol("clojure_test_run_tests_a0", clojure_test_run_tests_a0);
	clj_compiled_register_symbol("clojure_test_run_all_tests_a0", clojure_test_run_all_tests_a0);
	clj_compiled_register_symbol("clojure_test_run_all_tests_a1", clojure_test_run_all_tests_a1);
	clj_compiled_register_symbol("clojure_test_run_test_var_a1", clojure_test_run_test_var_a1);
	clj_compiled_register_symbol("clojure_test_run_test_a3", clojure_test_run_test_a3);
	clj_compiled_register_symbol("clojure_test_successful_QMARK__a1", clojure_test_successful_QMARK__a1);
#endif
	// form 1 at 3:1
	if (0) {
F1: ;
	if (!clj_load_form_failed(FILE_STR, 3, 1, CLJ_NIL)) goto fail;
	goto N1;
	}
	r = top_0();
	if (r == CLJ_THROWN) goto F1;
	clj_release(r);
	r = top_1();
	if (r == CLJ_THROWN) goto F1;
	clj_release(r);
	r = top_2();
	if (r == CLJ_THROWN) goto F1;
	clj_release(r);
	r = top_3();
	if (r == CLJ_THROWN) goto F1;
	clj_release(r);
N1: ;
	// form 2 at 6:1
	if (0) {
F2: ;
	if (!clj_load_form_failed(FILE_STR, 6, 1, K[4])) goto fail;
	goto N2;
	}
	r = top_4();
	if (r == CLJ_THROWN) goto F2;
	clj_release(r);
N2: ;
	// form 3 at 10:1
	if (0) {
F3: ;
	if (!clj_load_form_failed(FILE_STR, 10, 1, K[7])) goto fail;
	goto N3;
	}
	r = top_5();
	if (r == CLJ_THROWN) goto F3;
	clj_release(r);
N3: ;
	// form 4 at 12:1
	if (0) {
F4: ;
	if (!clj_load_form_failed(FILE_STR, 12, 1, K[9])) goto fail;
	goto N4;
	}
	r = top_6();
	if (r == CLJ_THROWN) goto F4;
	clj_release(r);
N4: ;
	// form 5 at 14:1
	if (0) {
F5: ;
	if (!clj_load_form_failed(FILE_STR, 14, 1, K[11])) goto fail;
	goto N5;
	}
	r = top_7();
	if (r == CLJ_THROWN) goto F5;
	clj_release(r);
N5: ;
	// form 6 at 16:1
	if (0) {
F6: ;
	if (!clj_load_form_failed(FILE_STR, 16, 1, K[14])) goto fail;
	goto N6;
	}
	r = top_8();
	if (r == CLJ_THROWN) goto F6;
	clj_release(r);
N6: ;
	// form 7 at 18:1
	if (0) {
F7: ;
	if (!clj_load_form_failed(FILE_STR, 18, 1, K[16])) goto fail;
	goto N7;
	}
	r = top_9();
	if (r == CLJ_THROWN) goto F7;
	clj_release(r);
N7: ;
	// form 8 at 20:1
	if (0) {
F8: ;
	if (!clj_load_form_failed(FILE_STR, 20, 1, K[18])) goto fail;
	goto N8;
	}
	r = top_10();
	if (r == CLJ_THROWN) goto F8;
	clj_release(r);
N8: ;
	// form 9 at 24:1
	if (0) {
F9: ;
	if (!clj_load_form_failed(FILE_STR, 24, 1, K[20])) goto fail;
	goto N9;
	}
	clj_c_stub_init(&S[0], K[21], 24, 1);
	r = top_11();
	if (r == CLJ_THROWN) goto F9;
	clj_release(r);
N9: ;
	// form 10 at 29:1
	if (0) {
F10: ;
	if (!clj_load_form_failed(FILE_STR, 29, 1, K[24])) goto fail;
	goto N10;
	}
	clj_c_stub_init(&S[1], K[25], 29, 1);
	clj_c_stub_init(&S[2], CLJ_NIL, 33, 19);
	r = top_12();
	if (r == CLJ_THROWN) goto F10;
	clj_release(r);
N10: ;
	// form 11 at 36:1
	if (0) {
F11: ;
	if (!clj_load_form_failed(FILE_STR, 36, 1, K[35])) goto fail;
	goto N11;
	}
	clj_c_stub_init(&S[3], K[36], 36, 1);
	r = top_13();
	if (r == CLJ_THROWN) goto F11;
	clj_release(r);
N11: ;
	// form 12 at 41:1
	if (0) {
F12: ;
	if (!clj_load_form_failed(FILE_STR, 41, 1, K[39])) goto fail;
	goto N12;
	}
	clj_c_stub_init(&S[4], K[40], 41, 1);
	r = top_14();
	if (r == CLJ_THROWN) goto F12;
	clj_release(r);
N12: ;
	// form 13 at 47:1
	if (0) {
F13: ;
	if (!clj_load_form_failed(FILE_STR, 47, 1, K[43])) goto fail;
	goto N13;
	}
	r = top_15();
	if (r == CLJ_THROWN) goto F13;
	clj_release(r);
N13: ;
	// form 14 at 51:1
	if (0) {
F14: ;
	if (!clj_load_form_failed(FILE_STR, 51, 1, K[50])) goto fail;
	goto N14;
	}
	clj_c_stub_init(&S[5], K[51], 51, 1);
	r = top_16();
	if (r == CLJ_THROWN) goto F14;
	clj_release(r);
N14: ;
	// form 15 at 56:1
	if (0) {
F15: ;
	if (!clj_load_form_failed(FILE_STR, 56, 1, K[61])) goto fail;
	goto N15;
	}
	clj_c_stub_init(&S[6], CLJ_NIL, 56, 1);
	r = top_17();
	if (r == CLJ_THROWN) goto F15;
	clj_release(r);
	r = top_18();
	if (r == CLJ_THROWN) goto F15;
	clj_release(r);
N15: ;
	// form 16 at 59:1
	if (0) {
F16: ;
	if (!clj_load_form_failed(FILE_STR, 59, 1, K[63])) goto fail;
	goto N16;
	}
	clj_c_stub_init(&S[7], CLJ_NIL, 59, 1);
	r = top_19();
	if (r == CLJ_THROWN) goto F16;
	clj_release(r);
	r = top_20();
	if (r == CLJ_THROWN) goto F16;
	clj_release(r);
N16: ;
	// form 17 at 62:1
	if (0) {
F17: ;
	if (!clj_load_form_failed(FILE_STR, 62, 1, K[65])) goto fail;
	goto N17;
	}
	clj_c_stub_init(&S[8], CLJ_NIL, 62, 1);
	r = top_21();
	if (r == CLJ_THROWN) goto F17;
	clj_release(r);
	r = top_22();
	if (r == CLJ_THROWN) goto F17;
	clj_release(r);
N17: ;
	// form 18 at 70:1
	if (0) {
F18: ;
	if (!clj_load_form_failed(FILE_STR, 70, 1, K[73])) goto fail;
	goto N18;
	}
	clj_c_stub_init(&S[9], CLJ_NIL, 70, 1);
	r = top_23();
	if (r == CLJ_THROWN) goto F18;
	clj_release(r);
	r = top_24();
	if (r == CLJ_THROWN) goto F18;
	clj_release(r);
N18: ;
	// form 19 at 79:1
	if (0) {
F19: ;
	if (!clj_load_form_failed(FILE_STR, 79, 1, K[81])) goto fail;
	goto N19;
	}
	clj_c_stub_init(&S[10], CLJ_NIL, 79, 1);
	r = top_25();
	if (r == CLJ_THROWN) goto F19;
	clj_release(r);
	r = top_26();
	if (r == CLJ_THROWN) goto F19;
	clj_release(r);
N19: ;
	// form 20 at 84:1
	if (0) {
F20: ;
	if (!clj_load_form_failed(FILE_STR, 84, 1, K[92])) goto fail;
	goto N20;
	}
	clj_c_stub_init(&S[11], CLJ_NIL, 84, 1);
	r = top_27();
	if (r == CLJ_THROWN) goto F20;
	clj_release(r);
	r = top_28();
	if (r == CLJ_THROWN) goto F20;
	clj_release(r);
N20: ;
	// form 21 at 87:1
	if (0) {
F21: ;
	if (!clj_load_form_failed(FILE_STR, 87, 1, K[96])) goto fail;
	goto N21;
	}
	clj_c_stub_init(&S[12], CLJ_NIL, 87, 1);
	r = top_29();
	if (r == CLJ_THROWN) goto F21;
	clj_release(r);
	r = top_30();
	if (r == CLJ_THROWN) goto F21;
	clj_release(r);
N21: ;
	// form 22 at 88:1
	if (0) {
F22: ;
	if (!clj_load_form_failed(FILE_STR, 88, 1, K[98])) goto fail;
	goto N22;
	}
	clj_c_stub_init(&S[13], CLJ_NIL, 88, 1);
	r = top_31();
	if (r == CLJ_THROWN) goto F22;
	clj_release(r);
	r = top_32();
	if (r == CLJ_THROWN) goto F22;
	clj_release(r);
N22: ;
	// form 23 at 89:1
	if (0) {
F23: ;
	if (!clj_load_form_failed(FILE_STR, 89, 1, K[100])) goto fail;
	goto N23;
	}
	clj_c_stub_init(&S[14], CLJ_NIL, 89, 1);
	r = top_33();
	if (r == CLJ_THROWN) goto F23;
	clj_release(r);
	r = top_34();
	if (r == CLJ_THROWN) goto F23;
	clj_release(r);
N23: ;
	// form 24 at 91:1
	if (0) {
F24: ;
	if (!clj_load_form_failed(FILE_STR, 91, 1, K[102])) goto fail;
	goto N24;
	}
	clj_c_stub_init(&S[15], K[103], 91, 1);
	r = top_35();
	if (r == CLJ_THROWN) goto F24;
	clj_release(r);
N24: ;
	// form 25 at 102:1
	if (0) {
F25: ;
	if (!clj_load_form_failed(FILE_STR, 102, 1, K[110])) goto fail;
	goto N25;
	}
	clj_c_stub_init(&S[16], K[111], 102, 1);
	r = top_36();
	if (r == CLJ_THROWN) goto F25;
	clj_release(r);
N25: ;
	// form 26 at 110:1
	if (0) {
F26: ;
	if (!clj_load_form_failed(FILE_STR, 110, 1, K[114])) goto fail;
	goto N26;
	}
	clj_c_stub_init(&S[17], K[115], 110, 1);
	r = top_37();
	if (r == CLJ_THROWN) goto F26;
	clj_release(r);
N26: ;
	// form 27 at 125:1
	if (0) {
F27: ;
	if (!clj_load_form_failed(FILE_STR, 125, 1, K[140])) goto fail;
	goto N27;
	}
	clj_c_stub_init(&S[18], K[141], 125, 1);
	r = top_38();
	if (r == CLJ_THROWN) goto F27;
	clj_release(r);
N27: ;
	// form 28 at 136:1
	if (0) {
F28: ;
	if (!clj_load_form_failed(FILE_STR, 136, 1, K[156])) goto fail;
	goto N28;
	}
	clj_c_stub_init(&S[19], CLJ_NIL, 138, 3);
	r = top_39();
	if (r == CLJ_THROWN) goto F28;
	clj_release(r);
N28: ;
	// form 29 at 144:1
	if (0) {
F29: ;
	if (!clj_load_form_failed(FILE_STR, 144, 1, K[163])) goto fail;
	goto N29;
	}
	clj_c_stub_init(&S[20], CLJ_NIL, 144, 1);
	r = top_40();
	if (r == CLJ_THROWN) goto F29;
	clj_release(r);
	r = top_41();
	if (r == CLJ_THROWN) goto F29;
	clj_release(r);
N29: ;
	// form 30 at 147:1
	if (0) {
F30: ;
	if (!clj_load_form_failed(FILE_STR, 147, 1, K[169])) goto fail;
	goto N30;
	}
	clj_c_stub_init(&S[21], CLJ_NIL, 147, 1);
	r = top_42();
	if (r == CLJ_THROWN) goto F30;
	clj_release(r);
	r = top_43();
	if (r == CLJ_THROWN) goto F30;
	clj_release(r);
N30: ;
	// form 31 at 152:1
	if (0) {
F31: ;
	if (!clj_load_form_failed(FILE_STR, 152, 1, K[171])) goto fail;
	goto N31;
	}
	clj_c_stub_init(&S[22], CLJ_NIL, 152, 1);
	r = top_44();
	if (r == CLJ_THROWN) goto F31;
	clj_release(r);
	r = top_45();
	if (r == CLJ_THROWN) goto F31;
	clj_release(r);
N31: ;
	// form 32 at 163:1
	if (0) {
F32: ;
	if (!clj_load_form_failed(FILE_STR, 163, 1, K[194])) goto fail;
	goto N32;
	}
	clj_c_stub_init(&S[23], CLJ_NIL, 163, 1);
	r = top_46();
	if (r == CLJ_THROWN) goto F32;
	clj_release(r);
	r = top_47();
	if (r == CLJ_THROWN) goto F32;
	clj_release(r);
N32: ;
	// form 33 at 174:1
	if (0) {
F33: ;
	if (!clj_load_form_failed(FILE_STR, 174, 1, K[209])) goto fail;
	goto N33;
	}
	clj_c_stub_init(&S[24], CLJ_NIL, 174, 1);
	r = top_48();
	if (r == CLJ_THROWN) goto F33;
	clj_release(r);
	r = top_49();
	if (r == CLJ_THROWN) goto F33;
	clj_release(r);
N33: ;
	// form 34 at 189:1
	if (0) {
F34: ;
	if (!clj_load_form_failed(FILE_STR, 189, 1, K[235])) goto fail;
	goto N34;
	}
	clj_c_stub_init(&S[25], K[236], 189, 1);
	r = top_50();
	if (r == CLJ_THROWN) goto F34;
	clj_release(r);
N34: ;
	// form 35 at 197:1
	if (0) {
F35: ;
	if (!clj_load_form_failed(FILE_STR, 197, 1, K[249])) goto fail;
	goto N35;
	}
	clj_c_stub_init(&S[26], K[250], 197, 1);
	r = top_51();
	if (r == CLJ_THROWN) goto F35;
	clj_release(r);
N35: ;
	// form 36 at 206:1
	if (0) {
F36: ;
	if (!clj_load_form_failed(FILE_STR, 206, 1, K[260])) goto fail;
	goto N36;
	}
	clj_c_stub_init(&S[27], K[261], 206, 1);
	r = top_52();
	if (r == CLJ_THROWN) goto F36;
	clj_release(r);
N36: ;
	// form 37 at 217:1
	if (0) {
F37: ;
	if (!clj_load_form_failed(FILE_STR, 217, 1, K[267])) goto fail;
	goto N37;
	}
	clj_c_stub_init(&S[28], K[268], 217, 1);
	r = top_53();
	if (r == CLJ_THROWN) goto F37;
	clj_release(r);
N37: ;
	// form 38 at 225:1
	if (0) {
F38: ;
	if (!clj_load_form_failed(FILE_STR, 225, 1, K[274])) goto fail;
	goto N38;
	}
	clj_c_stub_init(&S[29], K[275], 225, 1);
	r = top_54();
	if (r == CLJ_THROWN) goto F38;
	clj_release(r);
N38: ;
	// form 39 at 232:1
	if (0) {
F39: ;
	if (!clj_load_form_failed(FILE_STR, 232, 1, K[282])) goto fail;
	goto N39;
	}
	clj_c_stub_init(&S[30], K[283], 232, 1);
	r = top_55();
	if (r == CLJ_THROWN) goto F39;
	clj_release(r);
N39: ;
	// form 40 at 239:1
	if (0) {
F40: ;
	if (!clj_load_form_failed(FILE_STR, 239, 1, K[291])) goto fail;
	goto N40;
	}
	clj_c_stub_init(&S[31], K[292], 239, 1);
	r = top_56();
	if (r == CLJ_THROWN) goto F40;
	clj_release(r);
N40: ;
	// form 41 at 246:1
	if (0) {
F41: ;
	if (!clj_load_form_failed(FILE_STR, 246, 1, K[301])) goto fail;
	goto N41;
	}
	clj_c_stub_init(&S[32], K[302], 246, 1);
	r = top_57();
	if (r == CLJ_THROWN) goto F41;
	clj_release(r);
N41: ;
	// form 42 at 254:1
	if (0) {
F42: ;
	if (!clj_load_form_failed(FILE_STR, 254, 1, K[309])) goto fail;
	goto N42;
	}
	r = top_58();
	if (r == CLJ_THROWN) goto F42;
	clj_release(r);
N42: ;
	// form 43 at 256:1
	if (0) {
F43: ;
	if (!clj_load_form_failed(FILE_STR, 256, 1, K[312])) goto fail;
	goto N43;
	}
	clj_c_stub_init(&S[33], CLJ_NIL, 259, 3);
	r = top_59();
	if (r == CLJ_THROWN) goto F43;
	clj_release(r);
N43: ;
	// form 44 at 261:1
	if (0) {
F44: ;
	if (!clj_load_form_failed(FILE_STR, 261, 1, K[318])) goto fail;
	goto N44;
	}
	clj_c_stub_init(&S[34], CLJ_NIL, 261, 1);
	r = top_60();
	if (r == CLJ_THROWN) goto F44;
	clj_release(r);
	r = top_61();
	if (r == CLJ_THROWN) goto F44;
	clj_release(r);
N44: ;
	// form 45 at 264:1
	if (0) {
F45: ;
	if (!clj_load_form_failed(FILE_STR, 264, 1, K[320])) goto fail;
	goto N45;
	}
	clj_c_stub_init(&S[35], CLJ_NIL, 264, 1);
	r = top_62();
	if (r == CLJ_THROWN) goto F45;
	clj_release(r);
	r = top_63();
	if (r == CLJ_THROWN) goto F45;
	clj_release(r);
N45: ;
	// form 46 at 267:1
	if (0) {
F46: ;
	if (!clj_load_form_failed(FILE_STR, 267, 1, K[322])) goto fail;
	goto N46;
	}
	clj_c_stub_init(&S[36], K[323], 267, 1);
	r = top_64();
	if (r == CLJ_THROWN) goto F46;
	clj_release(r);
N46: ;
	// form 47 at 272:1
	if (0) {
F47: ;
	if (!clj_load_form_failed(FILE_STR, 272, 1, K[325])) goto fail;
	goto N47;
	}
	clj_c_stub_init(&S[37], K[326], 272, 1);
	clj_c_stub_init(&S[38], CLJ_NIL, 275, 3);
	clj_c_stub_init(&S[39], CLJ_NIL, 275, 15);
	r = top_65();
	if (r == CLJ_THROWN) goto F47;
	clj_release(r);
N47: ;
	// form 48 at 277:1
	if (0) {
F48: ;
	if (!clj_load_form_failed(FILE_STR, 277, 1, K[328])) goto fail;
	goto N48;
	}
	clj_c_stub_init(&S[40], K[329], 277, 1);
	r = top_66();
	if (r == CLJ_THROWN) goto F48;
	clj_release(r);
N48: ;
	// form 49 at 282:1
	if (0) {
F49: ;
	if (!clj_load_form_failed(FILE_STR, 282, 1, K[331])) goto fail;
	goto N49;
	}
	clj_c_stub_init(&S[41], K[332], 282, 1);
	r = top_67();
	if (r == CLJ_THROWN) goto F49;
	clj_release(r);
N49: ;
	// form 50 at 286:1
	if (0) {
F50: ;
	if (!clj_load_form_failed(FILE_STR, 286, 1, K[334])) goto fail;
	goto N50;
	}
	clj_c_stub_init(&S[42], K[335], 286, 1);
	r = top_68();
	if (r == CLJ_THROWN) goto F50;
	clj_release(r);
N50: ;
	// form 51 at 299:1
	if (0) {
F51: ;
	if (!clj_load_form_failed(FILE_STR, 299, 1, K[347])) goto fail;
	goto N51;
	}
	clj_c_stub_init(&S[43], K[348], 299, 1);
	clj_c_stub_init(&S[44], CLJ_NIL, 308, 8);
	clj_c_stub_init(&S[45], CLJ_NIL, 311, 31);
	r = top_69();
	if (r == CLJ_THROWN) goto F51;
	clj_release(r);
N51: ;
	// form 52 at 313:1
	if (0) {
F52: ;
	if (!clj_load_form_failed(FILE_STR, 313, 1, K[354])) goto fail;
	goto N52;
	}
	clj_c_stub_init(&S[46], K[355], 313, 1);
	clj_c_stub_init(&S[47], CLJ_NIL, 316, 23);
	r = top_70();
	if (r == CLJ_THROWN) goto F52;
	clj_release(r);
N52: ;
	// form 53 at 318:1
	if (0) {
F53: ;
	if (!clj_load_form_failed(FILE_STR, 318, 1, K[358])) goto fail;
	goto N53;
	}
	clj_c_stub_init(&S[48], K[359], 318, 1);
	r = top_71();
	if (r == CLJ_THROWN) goto F53;
	clj_release(r);
N53: ;
	// form 54 at 331:1
	if (0) {
F54: ;
	if (!clj_load_form_failed(FILE_STR, 331, 1, K[366])) goto fail;
	goto N54;
	}
	clj_c_stub_init(&S[49], K[367], 331, 1);
	r = top_72();
	if (r == CLJ_THROWN) goto F54;
	clj_release(r);
N54: ;
	// form 55 at 341:1
	if (0) {
F55: ;
	if (!clj_load_form_failed(FILE_STR, 341, 1, K[371])) goto fail;
	goto N55;
	}
	clj_c_stub_init(&S[50], K[372], 341, 1);
	clj_c_stub_init(&S[51], CLJ_NIL, 346, 41);
	clj_c_stub_init(&S[52], CLJ_NIL, 347, 31);
	r = top_73();
	if (r == CLJ_THROWN) goto F55;
	clj_release(r);
N55: ;
	// form 56 at 349:1
	if (0) {
F56: ;
	if (!clj_load_form_failed(FILE_STR, 349, 1, K[374])) goto fail;
	goto N56;
	}
	clj_c_stub_init(&S[53], K[375], 349, 1);
	r = top_74();
	if (r == CLJ_THROWN) goto F56;
	clj_release(r);
N56: ;
	// form 57 at 358:1
	if (0) {
F57: ;
	if (!clj_load_form_failed(FILE_STR, 358, 1, K[379])) goto fail;
	goto N57;
	}
	clj_c_stub_init(&S[54], K[380], 358, 1);
	r = top_75();
	if (r == CLJ_THROWN) goto F57;
	clj_release(r);
N57: ;
	// form 58 at 367:1
	if (0) {
F58: ;
	if (!clj_load_form_failed(FILE_STR, 367, 1, K[388])) goto fail;
	goto N58;
	}
	clj_c_stub_init(&S[55], K[389], 367, 1);
	r = top_76();
	if (r == CLJ_THROWN) goto F58;
	clj_release(r);
N58: ;
	return CLJ_NIL;
fail:
	return CLJ_THROWN;
}
#else
typedef int cljc_unit_disabled;
#endif

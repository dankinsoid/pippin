...
// One per running interpreted closure on the thread; natives are leaves and compiled fns are found on the real
// stack instead (trace.c), so `sp` orders a frame among them.
typedef struct {
	const clj_node *fn_node;
	const clj_node *call_site; // NULL for a host call or a call from a native
	const void     *sp;        // a stack address inside the frame that runs the body
} clj_shadow_frame;

...
| `objc.c`, `include/clj/objc.h`, `coro.c` (the pool drain) | the whole level-1 bridge: the Objective-C runtime (`objc_getClass`, `class_copyMethodList`, `method_copyArgumentType`), `objc_msgSend` through eight hand-written prototypes chosen by the type encoding, the ARC entry points `objc_retain`/`objc_release`/`objc_autorelease`/`objc_autoreleasePoolPush`/`Pop`, `NSString`/`NSNumber` and the `kCFBoolean*` singletons, and the `__x86_64__` reading of a `c` return as a BOOL | none — every entry point answers "The Objective-C bridge needs an Apple platform" | A listed gap, not a design: the bridge exists to call Apple frameworks. The kebab spelling and the analyzer's `.method` form are plain C and portable; a port would put a different host's dispatcher behind the same node. The eight prototypes are an arm64/x86_64 fact (separate integer and floating-point register files, unread extra argument registers), so another ABI re-derives the set. |

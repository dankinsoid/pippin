...
| `objc.c`, `include/clj/objc.h`, `coro.c` (the pool drain) | the whole level-1 bridge: the Objective-C runtime (`objc_getClass`, `class_copyMethodList`, `method_copyArgumentType`), `objc_msgSend` through hand-written prototypes chosen by the type encoding, the AAPCS64 classification of a struct argument or return, the ARC entry points `objc_retain`/`objc_release`/`objc_autorelease`/`objc_autoreleasePoolPush`/`Pop`, `NSString`/`NSNumber` and the `kCFBoolean*` singletons, the `__x86_64__` reading of a `c` return as a BOOL, and the `__x86_64__` refusal of every struct shape SysV classifies differently from arm64 | none — every entry point answers "The Objective-C bridge needs an Apple platform" | A listed gap, not a design: the bridge exists to call Apple frameworks. The kebab spelling and the analyzer's `.method` form are plain C and portable; a port would put a different host's dispatcher behind the same node. The prototypes are an arm64/x86_64 fact (separate integer and floating-point register files, unread extra argument registers), so another ABI re-derives the set. Struct passing is arm64's AAPCS64: an HFA of up to four same-typed floats or doubles in v0-v3, any other aggregate of at most 16 bytes in integer registers, a larger one by a pointer and through x8 for a return. On x86_64 SysV classifies by eightbyte instead ({float,float} share one xmm, an aggregate over 16 bytes rides the stack and its return needs `objc_msgSend_stret`, which arm64 does not have), so only all-integer aggregates of at most 16 bytes and one or two `double`s are accepted there and every other struct is refused rather than called through a wrong shape. |

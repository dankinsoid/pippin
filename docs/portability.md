...
| `sched.c` | `pthread_set_qos_class_self_np` on the carriers; the main carrier is a version-0 `CFRunLoopSource` signalled with `CFRunLoopWakeUp`; a far timer deadline is kept on a `DISPATCH_SOURCE_TYPE_TIMER` so the timer thread waits untimed (a timed wait wakes ~0.7 µs later on XNU) | the main carrier queue alone (a host can pump it by hand); `pthread_cond_timedwait` for every deadline | Linux: an `eventfd` in the host's `epoll`/GLib loop performs the same pump; QoS has no equivalent; measure whether futex timed waits carry the same wake penalty before porting the dispatch timer (a `timerfd` in the same thread's `poll` if they do). Every deadline then waits on the condition, so its clock must be monotonic — `pthread_condattr_setclock(CLOCK_MONOTONIC)` or the `timerfd`, never the `CLOCK_REALTIME` deadline the Apple build's sub-2 ms path uses; and the clock matching Darwin's `CLOCK_UPTIME_RAW` (stopped while the device sleeps, the semantics of NOTES.md "The timers' clock") is Linux's `CLOCK_MONOTONIC`, not `CLOCK_BOOTTIME`. |
